...
If you want the installed `undici` package to provide the globals, call [`install()`](/docs/docs/api/GlobalInstallation.md) so `fetch`, `Headers`, `Response`, `Request`, and `FormData` are installed together as a matching set.
