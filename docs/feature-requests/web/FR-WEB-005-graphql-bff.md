...
created: 2026-06-28
related_frs: [FR-INFRA-001, FR-WEB-001, FR-WEB-003, FR-WEB-004, FR-TRACK-002, FR-TRACK-003, FR-DEAL-003]
depends_on: [FR-INFRA-001, FR-WEB-001]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-WEB-21: BFF GraphQL là một service đứng SAU gateway (FR-INFRA-001 định tuyến /graphql tới đây); gateway đã verify JWT, BFF nhận claims qua context, KHÔNG tự verify lại token"
  ...
  - "DEC-WEB-25: BFF chỉ phục vụ đọc (Query) ở slice này; ghi (tạo wishlist/alert) vẫn qua REST của FR-TRACK-002/003 - không nhân đôi mutation; tránh hai đường ghi lệch nhau"
...
sub_tasks:
  ...
  - "1.0h: resolvers/wishlist.ts - resolve wishlists + items qua REST FR-TRACK-002 (ủy quyền kiểm chủ sở hữu)"
  - "1.0h: loaders/chart-loader.ts + resolvers/chart.ts - DataLoader gom nạp chart cho nhiều product (chống N+1) qua feed FR-DEAL-003"
...
5. Ở slice này BFF **MUST** chỉ phục vụ `Query` (đọc); KHÔNG định nghĩa `Mutation` ghi wishlist/alert (DEC-WEB-25). Mọi thao tác ghi vẫn qua REST của FR-TRACK-002/003 để tránh hai đường ghi lệch nhau.
6. Schema **MUST** phơi tối thiểu: `me` (thông tin user hiện tại), `wishlists` (danh sách wishlist của caller kèm item), và `productChart(productId, range)` (dữ liệu biểu đồ từ feed FR-DEAL-003) - đủ để dashboard nạp wishlist kèm biểu đồ trong một round-trip.
...
8. BFF **MUST** truyền `request-id` (do gateway gắn, FR-INFRA-001) xuống các lời gọi REST để giữ vết xuyên service (đồng nhất observability FR-INFRA-004).
9. `productChart` **MUST** trả đúng hình dạng feed FR-DEAL-003 (daily + annotations + maturity) - BFF không tự tính verdict/median, chỉ chuyển tiếp + gộp; client hiển thị giống FR-WEB-003.
...
**Vì sao BFF đứng sau gateway, không tự verify token (DEC-WEB-21)?** Gateway (FR-INFRA-001) đã là điểm thực thi JWT duy nhất cho toàn hệ thống. BFF tự verify lại là nhân đôi logic - hai chỗ kiểm token dễ lệch (một chỗ cập nhật JWKS, chỗ kia quên). BFF tin gateway và nhận claims qua context. Đây là mẫu BFF chuẩn: edge xác thực, dịch vụ phía sau nhận danh tính đã xác thực.

**Vì sao resolver ủy quyền REST, không chạm DB (DEC-WEB-22)?** Logic kiểm chủ sở hữu (chống IDOR) nằm ở track-svc (FR-TRACK-002/003). Nếu BFF chạm DB trực tiếp thì phải sao chép logic đó - một chỗ quên kiểm là lỗ IDOR rò rỉ wishlist/alert người khác. Ủy quyền REST giữ một nguồn logic và một chỗ kiểm quyền; BFF chỉ là lớp định hình + gộp dữ liệu cho client.
...
**Vì sao chỉ đọc, ghi vẫn qua REST (DEC-WEB-25)?** Hai đường ghi (REST + GraphQL mutation) dễ lệch validate (một đường cập nhật luật, đường kia quên) và lệch trạng thái. Slice này BFF chỉ tối ưu đọc (nơi GraphQL tỏa sáng: gộp nhiều nguồn trong một round-trip); ghi giữ một đường REST của FR-TRACK-002/003 để một nguồn validate.
...
type ChartData {                                # mirror feed FR-DEAL-003
  maturity: String!
  daily: [DailyPoint!]!
...
export interface GqlContext {
  ...
  requestId: string;       // do gateway gắn (FR-INFRA-001)
  rest: RestClient;
}
...
// Gom nhiều (productId, range) trong một request thành các lời gọi feed FR-DEAL-003,
...
export function makeChartLoader(rest: RestClient) {
  return new DataLoader<{ productId: string; range: string }, ChartData>(
    async (keys) => {
      ...
      return Promise.all(keys.map((k) => rest.getChart(k.productId, k.range))); // feed FR-DEAL-003
    },
...
6. Schema phơi `me`, `wishlists` (kèm items), `productChart(productId, range)`; `ChartData` mirror hình dạng feed FR-DEAL-003.
...
Xem §3. Thứ tự: `schema.graphql` (chỉ Query: me/wishlists/productChart) -> `context.ts` (nhận sub + request-id từ gateway, không verify) -> `rest-client.ts` (gọi track-svc + deal-svc, truyền request-id) -> `resolvers/wishlist.ts` + `resolvers/chart.ts` (ủy quyền REST) -> `loaders/chart-loader.ts` (DataLoader gộp) -> `security/limits.ts` (max depth + cost) -> `server.ts` (wire tất cả, đặt sau gateway) -> tests. BFF có thể chạy như route handler trong web app Next (`/api/graphql`) hoặc một service Node riêng - dù cách nào cũng nằm SAU gateway (FR-INFRA-001 định tuyến `/graphql`). Vì là SHOULD, có thể hoãn tới sau khi REST path (FR-WEB-001 lib/api.ts) đã phục vụ MVP; khi làm phải giữ đúng bốn ranh giới (không verify token, không chạm DB, có trần query, không mutation ghi).

---

## §7 - Phụ thuộc

- **FR-INFRA-001** - gateway định tuyến `/graphql` tới BFF và verify JWT trước; BFF nhận claims qua context, không verify lại (depends_on cứng).
- **FR-WEB-001** - web app tiêu thụ BFF; `lib/api.ts` mở rộng để gọi GraphQL endpoint cạnh REST (depends_on cứng).
- **FR-TRACK-002 / FR-TRACK-003** - nguồn wishlist/alert; resolver ủy quyền REST của chúng (giữ kiểm chủ sở hữu một chỗ).
- **FR-DEAL-003** - feed biểu đồ; `productChart`/`chart` resolver gọi feed này qua DataLoader; `ChartData` mirror hình dạng của nó.
- **FR-WEB-003 / FR-WEB-004 (đồng cấp)** - cùng tiêu thụ dữ liệu wishlist + biểu đồ; BFF là đường thay thế REST cho dashboard cần gộp nhiều nguồn.
...
- Subscription (WSS) cho cập nhật giá realtime trên dashboard - bám đường WSS của gateway (FR-INFRA-001) khi cần.
...
| BFF tự tính verdict/median | grep tính toán | lệch feed/thẻ | Chuyển tiếp feed FR-DEAL-003 (§1 #9) |
...
*Hết FR-WEB-005. Status: ready_to_implement (mục tiêu audit 10/10).*
