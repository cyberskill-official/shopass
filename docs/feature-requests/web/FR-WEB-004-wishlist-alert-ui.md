...
created: 2026-06-28
related_frs: [FR-WEB-001, FR-TRACK-002, FR-TRACK-003, FR-TRACK-004]
depends_on: [FR-WEB-001, FR-TRACK-002, FR-TRACK-003]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-WEB-16: UI wishlist/alert tiêu thụ API REST của FR-TRACK-002 (/v1/wishlists...) và FR-TRACK-003 (/v1/alerts...) qua lib/api.ts; không tự lưu trạng thái nguồn ở client, server là nguồn sự thật"
  - "DEC-WEB-17: form alert phản chiếu đúng ngữ nghĩa threshold theo rule_type của FR-TRACK-003 - price_below cần giá VND, drop_pct cần % (1..99), real_sale/bottom_predicted KHÔNG có ô threshold; validate phía client trước khi gửi để báo sớm, server vẫn validate lần cuối"
  - "DEC-WEB-18: bộ chọn channel chỉ phơi {push, email, sms} (push mặc định) khớp enum kênh FR-TRACK-003; không cho chọn kênh ngoài tập có dispatcher"
...
language: "TypeScript 5.x; Next.js 14 (App Router, client components form); gọi API FR-TRACK-002/003 qua lib/api.ts (FR-WEB-001)"
service: shopass/web/
new_files:
...
disallowed_tools:
  - cho chọn rule_type/channel ngoài enum của FR-TRACK-003 (vi phạm DEC-WEB-17/18, server trả 400 hoặc luật rác)
...
risk_if_skipped: "wishlist và alert là cách người dùng biến SănDeal từ công cụ tra cứu thành trợ lý chủ động: gom sản phẩm muốn mua, đặt giá mong muốn, và khai báo 'báo tôi khi nào'. Thiếu UI này thì backend FR-TRACK-002/003 không có mặt tiền - người dùng không tạo được luật cảnh báo, và toàn bộ chuỗi giá trị 'theo dõi giá rồi nhắc đúng lúc' (engine FR-TRACK-004 -> push FR-NOTIF-002) không có đầu vào từ người dùng. Nếu form cho chọn rule_type/channel ngoài enum thì server trả 400 (trải nghiệm hỏng) hoặc tệ hơn tạo luật engine không hiểu. Nếu render ô threshold sai ngữ nghĩa (vd cho nhập % cho price_below, hay cho nhập ngưỡng cho real_sale) thì người dùng tạo luật không bao giờ bắn hoặc bắn loạn. Nhập target_price dạng float gây sai số trên so sánh giá. Đây là màn hình chạm trực tiếp dữ liệu cá nhân (ý định mua sắm) nên phải trung thực về lỗi 403/404 mà không lộ tài nguyên người khác."
---

## §1 - Mô tả (BCP-14 normative)

UI quản lý wishlist và alert **MUST** là các màn hình trong khu vực đăng nhập, tiêu thụ API REST của FR-TRACK-002 (wishlist) và FR-TRACK-003 (alert_rule) qua `lib/api.ts`, phản chiếu đúng ngữ nghĩa `rule_type`/`threshold`/`channel` của backend, và chỉ thao tác trên tài nguyên của chính user. Hợp đồng:

1. UI **MUST** lấy và ghi dữ liệu qua API FR-TRACK-002/003 (`/v1/wishlists...`, `/v1/alerts...`) qua `apiFetch` (FR-WEB-001); server là nguồn sự thật, client KHÔNG tự giữ danh sách như nguồn chính (DEC-WEB-16).
2. Màn hình wishlist **MUST** cho: tạo wishlist (`POST /v1/wishlists {name}`), liệt kê wishlist của caller (`GET /v1/wishlists`), thêm item (`POST /v1/wishlists/{id}/items {product_id, target_price?}`), xóa item, xóa wishlist - khớp các route FR-TRACK-002.
...
4. Màn hình alert **MUST** cho tạo luật (`POST /v1/alerts {product_id, rule_type, threshold?, channel[]?}`), liệt kê luật của caller (`GET /v1/alerts`), bật/tắt (`PATCH /v1/alerts/{id} {active}`), xóa (`DELETE /v1/alerts/{id}`) - khớp các route FR-TRACK-003.
5. Bộ chọn `rule_type` **MUST** chỉ phơi enum `{price_below, drop_pct, real_sale, bottom_predicted}` của FR-TRACK-003 (DEC-WEB-17); không cho giá trị ngoài enum.
...
7. UI **MUST** validate phía client quan hệ `rule_type` <-> `threshold` trước khi gửi (báo lỗi sớm), nhưng KHÔNG coi đó là kiểm tra cuối: server (FR-TRACK-003 #5) vẫn validate lần cuối; UI hiển thị lỗi `400` từ server nếu có.
8. Bộ chọn `channel` **MUST** chỉ phơi `{push, email, sms}` với `push` mặc định (DEC-WEB-18), khớp enum kênh FR-TRACK-003; cho chọn nhiều kênh; KHÔNG cho kênh ngoài tập.
9. UI **MUST** chỉ thao tác trên tài nguyên của chính user (DEC-WEB-20): server đã chặn IDOR (FR-TRACK-002 #6, FR-TRACK-003), client không cố duyệt id người khác; lỗi `403`/`404` từ server hiển thị "không tìm thấy" trung lập, không lộ sự tồn tại tài nguyên người khác.
10. Các màn hình **MUST** nằm trong route group `(app)` (sau guard auth FR-WEB-001); request mang JWT do `apiFetch` gắn.
...
**Vì sao form phản chiếu đúng ngữ nghĩa threshold (DEC-WEB-17)?** Backend FR-TRACK-003 diễn giải `threshold` khác nhau theo `rule_type`: VND cho price_below, % cho drop_pct, không dùng cho real_sale/bottom_predicted. Nếu form cho nhập sai (vd % cho price_below) thì hoặc server trả 400 (trải nghiệm xấu), hoặc tạo luật vô nghĩa không bao giờ bắn. Ô threshold động theo loại giữ form khớp đúng ngữ nghĩa server.
...
**Vì sao channel chỉ allowlist {push,email,sms} (DEC-WEB-18)?** Mỗi kênh cần một dispatcher (FCM, email, SMS). Cho chọn kênh không có dispatcher tạo luật gửi vào hư không. Allowlist khớp đúng enum FR-TRACK-003; push mặc định khớp mô hình chi phí push > email > sms (§3.6).
...
export async function listWishlists(): Promise<Wishlist[]> {
  const res = await apiFetch("/v1/wishlists");           // GET của FR-TRACK-002
...
```ts
// web/lib/alerts/validate.ts - mirror DEC-TRACK-22 của FR-TRACK-003
...
1. `lib/wishlist/api.ts` và `lib/alerts/api.ts` gọi đúng route FR-TRACK-002/003 qua `apiFetch`; grep: client không giữ danh sách như nguồn sự thật (đọc từ API).
2. Tạo/liệt kê/thêm item/xóa wishlist khớp các route FR-TRACK-002; `target_price` gửi là số nguyên hoặc null.
3. `target_price` nhập là số nguyên VND; grep: không có parse float cho tiền; để trống gửi `null`.
4. Tạo/liệt kê/bật-tắt/xóa alert khớp các route FR-TRACK-003.
...
Xem §3. Thứ tự: `lib/wishlist/api.ts` + `lib/alerts/api.ts` (client gọi đúng route FR-TRACK-002/003 qua apiFetch) -> `lib/alerts/validate.ts` (mirror ngữ nghĩa threshold/channel, test thuần) -> `wishlist-panel.tsx` + `wishlist-item-row.tsx` (CRUD wishlist + sửa target_price) -> `alert-form.tsx` (rule_type + ô threshold động + channel) + `alert-list.tsx` (bật/tắt/xóa) -> `wishlist/page.tsx` + `alerts/page.tsx` (màn hình trong shell (app)) -> thêm điều hướng vào `app-shell.tsx` -> tests. Các form là client component (`"use client"`) cho tương tác; trang gọi API phía client sau khi shell (app) qua guard auth. Validate phía client báo sớm; server (FR-TRACK-003) vẫn là kiểm tra cuối.

---

## §7 - Phụ thuộc

- **FR-TRACK-002** - schema + API wishlist/wishlist_item; UI này là mặt tiền của các route `/v1/wishlists...` (depends_on cứng).
- **FR-TRACK-003** - schema + API alert_rule; UI phản chiếu đúng enum `rule_type`/`channel` và ngữ nghĩa `threshold` của nó (depends_on cứng).
- **FR-WEB-001** - scaffold + `lib/api.ts` (JWT) + shell `(app)` + guard; màn hình nằm trong khu vực bảo vệ (depends_on cứng).
- **FR-TRACK-004 (downstream)** - engine kích hoạt đọc các luật người dùng tạo qua UI này; UI là nơi luật được sinh ra.
...
// POST /v1/alerts -> 201 (FR-TRACK-003)
```
...
- Gợi ý target_price dựa median90/trailing_min (đọc feed FR-DEAL-003) khi đặt giá mong muốn - thêm khi tích hợp biểu đồ.
- Tạo nhanh alert ngay từ màn hình biểu đồ (FR-WEB-003) - liên kết hai màn hình ở slice sau.
- Hiển thị lịch sử bắn (alert history) inline - dùng `GET /v1/alerts/{id}/history` của FR-TRACK-003 khi cần.
...
|---|---|---|---|
| Chọn rule_type/channel ngoài enum | alert-form/channel test | server 400 hoặc luật rác | Chỉ phơi enum FR-TRACK-003 (DEC-WEB-17/18) |
...
| Bỏ qua validate client (gọi API thẳng) | server validate | luật xấu | Server FR-TRACK-003 validate cuối (§1 #7) |

---

## §11 - Ghi chú

- wishlist + alert biến SănDeal từ công cụ tra cứu thành trợ lý chủ động; đây là mặt tiền của backend FR-TRACK-002/003.
- Server là nguồn sự thật; UI đọc/ghi qua API và đồng bộ giữa web/extension/mobile, không giữ bản sao làm nguồn chính.
- Form alert phản chiếu đúng ngữ nghĩa threshold theo rule_type của FR-TRACK-003 - ô động tránh tạo luật vô nghĩa.
...
*Hết FR-WEB-004. Status: ready_to_implement (mục tiêu audit 10/10).*
