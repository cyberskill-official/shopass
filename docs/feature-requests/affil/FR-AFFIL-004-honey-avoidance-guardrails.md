...
created: 2026-06-28
related_frs: [FR-AFFIL-001, FR-AFFIL-002, FR-EXT-003, FR-TRUST-001, FR-TRUST-002, NFR-AFFIL-001]
depends_on: [FR-AFFIL-002, FR-EXT-003]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-AFFIL-17: mọi điều hướng affiliate phải bắt nguồn từ user gesture (bấm) + đi qua FR-AFFIL-002 với user_initiated=true + có disclosure hiển thị"
  ...
  - "DEC-AFFIL-20: không có chế độ 'tự gắn affiliate' ẩn; mọi affiliate đi qua đúng một cửa (FR-AFFIL-002) - guardrail khẳng định không có cửa thứ hai"
...
disallowed_tools:
  ...
  - tạo đường gắn affiliate thứ hai bỏ qua FR-AFFIL-002 (vi phạm DEC-AFFIL-20)
...
sub_tasks:
  ...
  - "0.5h: guardrail_assert.go - affil-svc khẳng định chỉ một route tạo link (FR-AFFIL-002) + mọi link kèm disclosure"
...
Hệ thống **MUST** có một bộ guardrail tự động (lint + assertion chạy như CI gate) khẳng định extension và affil-svc KHÔNG bao giờ thực hiện các hành vi bị cấm kiểu Honey, và mọi affiliate đi qua đúng một cửa user-initiated có disclosure (FR-AFFIL-002). Hợp đồng:
...
3. Mọi điều hướng affiliate **MUST** bắt nguồn từ một user gesture thật (sự kiện `isTrusted`, bấm) và đi qua FR-AFFIL-002 với `user_initiated=true` (DEC-AFFIL-17). Guardrail `user-gesture.ts` khẳng định không có đường mở affiliate link từ `alarm`/`setTimeout`/`setInterval`/khởi động nền.
...
5. Guardrail **MUST** khẳng định chỉ tồn tại đúng MỘT đường tạo affiliate link (FR-AFFIL-002) (DEC-AFFIL-20): không có hàm/endpoint thứ hai tự ghép affiliate URL hay tự ghi `affiliate_click` ngoài luồng đó. `guardrail_assert.go` kiểm affil-svc.
6. Guardrail **MUST** khẳng định mọi affiliate link trả ra đều kèm disclosure không rỗng (đồng nhất FR-AFFIL-002 #10): assertion fail nếu một đường nào trả `deep_link` mà không có `disclosure`.
...
8. **MUST** duy trì `CHROME-WEBSTORE-AFFILIATE-CHECKLIST.md` ánh xạ từng yêu cầu của chính sách Chrome Web Store (cập nhật 3/2025, thực thi 10/06/2025) tới guardrail/test tương ứng - mỗi mục policy có một kiểm tra chứng minh tuân thủ. Guardrail cũng **MUST** bao phủ auto-test mã giảm (FR-CART-005, nếu có): user-initiated + nhịp người, không tự chốt đơn, không tự áp affiliate khi thử mã (liên kết §3.5 thuật toán 4).
9. Guardrail extension **MUST** quét cả mã nguồn (static) lẫn khẳng định runtime: static bắt pattern bị cấm trong code; runtime assert chặn nếu một mở-link xảy ra mà không gắn user gesture (defense in depth với FR-EXT-003).
10. Guardrail **MUST** coi danh sách hành vi cấm là allowlist-of-paths cho affiliate: chỉ đường đã được phê duyệt (FR-AFFIL-002) hợp lệ; mọi đường mới chạm affiliate phải thêm vào kiểm tra và qua review (tinh thần allowlist của FR-EXT-003).
...
```ts
// extension/src/guardrails/user-gesture.ts
// openAffiliate CHỈ chạy khi có user gesture thật (ev.isTrusted) và qua FR-AFFIL-002.
...
- **FR-AFFIL-002** - cửa affiliate hợp lệ duy nhất (user-initiated + disclosure); guardrail khẳng định không có cửa thứ hai và mọi link kèm disclosure.
- **FR-EXT-003** - pipeline tối thiểu hóa (allowlist, local-first); guardrail dùng cùng tinh thần allowlist cho đường affiliate và bổ trợ defense-in-depth.
- **FR-TRUST-001** - extension open-source + reproducible build: guardrail là bằng chứng kiểm chứng được rằng mã công khai không cookie-stuffing.
- **FR-TRUST-002** - chính sách minh bạch/local-first; guardrail thực thi nó bằng CI gate.
- **FR-AFFIL-005 (downstream)** - cashback chỉ an toàn khi guardrail đảm bảo affiliate sạch (không gian lận attribution từ phía ta).
...
Đã chốt. Hoãn:
- Tự động quét bản build đã đóng gói (zip) trước khi upload Chrome Web Store, ngoài quét nguồn - thêm bước CI khi có pipeline release của FR-TRUST-001.
...
- Áp guardrail tương tự cho mobile app (FR-MOBILE) khi có universal checkout assistant - nhân rộng sau.
...
- Khẳng định đúng một cửa affiliate (FR-AFFIL-002) đóng lối sau cho hành vi Honey len vào qua một hàm "cho tiện".
...
*Hết FR-AFFIL-004. Status: ready_to_implement (mục tiêu audit 10/10).*
