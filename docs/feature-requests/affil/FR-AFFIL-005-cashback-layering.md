...
created: 2026-06-28
related_frs: [FR-AFFIL-001, FR-AFFIL-002, FR-AFFIL-003, FR-BILL-002, FR-TRUST-005]
depends_on: [FR-AFFIL-003, FR-BILL-002, FR-TRUST-005]
blocks: []
source_pages:
...
sub_tasks:
  ...
  - "1.0h: release.go - chuyển pending -> available khi qua cửa sổ điều tra (FR-TRUST-005 clear)"
  - "1.0h: payout.go - gom available -> payout_request khi >= ngưỡng, qua VietQR (FR-BILL-002)"
...
4. **MUST** giữ `cashback_entry` ở `pending` ít nhất qua cửa sổ điều tra (`available_at = confirmed_at + investigation_window`, mặc định 7-14 ngày) (DEC-AFFIL-51). Chỉ sang `available` khi cửa sổ đã qua VÀ FR-TRUST-005 không gắn cờ gian lận trên conversion/user.
5. **MUST** hỗ trợ clawback: nếu network thu hồi conversion (status đổi từ `confirmed` về `rejected`) hoặc FR-TRUST-005 xác nhận gian lận, cashback_entry sang `clawed_back`; nếu đã `available` nhưng chưa `paid` thì trừ khỏi số dư.
6. **MUST** gom các `available` entry của một user thành `payout_request` chỉ khi tổng đạt ngưỡng tối thiểu (ví dụ 50.000 VND) (DEC-AFFIL-53); payout qua VietQR/số dư nội bộ theo FR-BILL-002. KHÔNG tự động payout từng giao dịch nhỏ.
...
**Vì sao buộc qua FR-TRUST-005 (§1 #4)?** FR-TRUST-005 phát hiện gaming attribution + delay payout. Cashback release phải hỏi tín hiệu gian lận trước khi mở khóa, nếu không cashback thành kênh rút tiền cho fake-account farming (§5.3).
...
func (r *Releaser) ReleaseDue(ctx context.Context, now time.Time) (int, error) {
    ...
    n := 0
    for _, e := range rows {
        if r.fraud.IsFlagged(ctx, e.UserID, e.ConversionID) { // FR-TRUST-005
...
7. Sau `available_at` nhưng FR-TRUST-005 flag -> KHÔNG sang `available` (giữ/hủy để điều tra).
...
func TestRelease_FlaggedUserNotReleased(t *testing.T) {
    l, uid, cid := setupFlagged(t, uid) // FR-TRUST-005 gan co
    l.OnConversionConfirmed(ctx, Conversion{ID: cid, UserID: uid, Commission: 100_000, UserTier: "free", ConfirmedAt: t0})
...
Xem §3. Thứ tự: migration 0005 -> ledger.go (hook từ conversion.confirmed) -> release.go (cron gọi ReleaseDue mỗi giờ) -> payout.go (gom available >= ngưỡng -> VietQR qua FR-BILL-002) -> tests. Release chạy như job định kỳ; clawback chạy khi nhận postback rejected từ FR-AFFIL-003.

---

## §7 - Phụ thuộc

- **FR-AFFIL-003** - nguồn conversion confirmed/rejected (postback network).
- **FR-BILL-002** - kênh payout VietQR/số dư.
- **FR-TRUST-005** - tín hiệu gian lận + delay payout trước khi release.
- **FR-AFFIL-001** - bảng affiliate_conversion (FK).
...
| Farm fake-account rút cashback | FR-TRUST-005 flag | giữ pending | §1 #4 không release khi flagged |
...
- Delay payout + hỏi FR-TRUST-005 trước release là best practice chống farm (§5.3).
- Disclosure pending/có-thể-hủy giữ minh bạch hậu-Honey (§4.2).

---

*Hết FR-AFFIL-005. Status: ready_to_implement (mục tiêu audit 10/10).*
