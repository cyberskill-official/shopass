...
Tôi tái thẩm FR-PRICE-004 độc lập từ file `.md` hiện tại. Endpoint `GET /v1/compare?canonical_key=...` khớp shape §3.7 và đặc tả ở mức triển khai được: 12 mệnh đề §1 normative (11 MUST + 1 SHOULD), mỗi mệnh đề có AC §4 và test §5. Giá hiện tại lấy đúng nguồn qua `DISTINCT ON (product_id) ... ORDER BY product_id, ts DESC` trên `price_snapshot` (không dùng `price_daily`, giữ `ts` giây/phút), JOIN `tracked_product` theo `canonical_key` + `platform`. `is_cheapest` tính phía server, trả `ts` mỗi sàn cho độ tươi. Xử lý đúng việc bảng `platform` không có cột `name` (suy `platform_name` từ `code` qua `displayName`). Không có defect cần sửa. Score 10/10.
...
- ISS-002 (kiểm, đạt): bảng `platform` (FR-INFRA-002) chỉ có `id/code/country/base_url`, KHÔNG có `name`. FR né đúng: SELECT chỉ `pf.code`, suy tên qua `displayName(code)`. Không SELECT cột không tồn tại. Khớp DATA-MODEL.
...
- ISS-005 (kiểm, đạt): rủi ro over-merge nằm ở FR-PRICE-005 (matching), không ở endpoint này; §10 ghi nhận và trỏ DEC-PRICE-23. Ranh giới depends_on FR-PRICE-005 đúng.
...
*Hết audit độc lập FR-PRICE-004.*
