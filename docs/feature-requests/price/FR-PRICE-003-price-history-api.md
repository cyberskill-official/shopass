...
created: 2026-06-27
related_frs: [FR-PRICE-002, FR-WEB-003, FR-DEAL-003, FR-INFRA-001]
...
9. **MUST** trả mọi giá dưới dạng `BIGINT` VND (int64) trong payload - KHÔNG dùng float hay string (đồng nhất với DEC-PRICE-05 của FR-PRICE-002).
10. **MUST** xác thực qua JWT do API Gateway (FR-INFRA-001) gắn; handler giả định đã qua middleware, không tự parse token. Request thiếu auth bị gateway chặn trước khi tới handler.
...
Xem §3. Thứ tự: `history_query.go` (ParseRange + 2 truy vấn + ProductExists) -> `price_history.go` (DTO + handler) -> đăng ký route trong `router.go` sau JWT middleware của gateway (FR-INFRA-001) -> tests. Handler dùng `http.ServeMux` của Go 1.22 với pattern `GET /v1/products/{id}/price-history` và `req.PathValue("id")`. Không thêm caching ở slice này; cagg + đuôi một chunk đã đủ cho p95 <500ms.

---

## §7 - Phụ thuộc

- **FR-PRICE-002** - cung cấp `price_daily` (continuous aggregate) cho phần thân và `price_snapshot` (hypertable) cho đuôi raw. Đây là điều kiện cứng.
- **FR-INFRA-001 (gateway)** - gắn JWT auth trước handler; route nằm sau middleware này.
- **FR-WEB-003 (downstream)** - biểu đồ giá tiêu thụ response này; hình dạng JSON ở §3 là hợp đồng với nó.
- **FR-DEAL-003 (sibling)** - API dữ liệu biểu đồ song song (sale ảo); dùng chung quy ước range allowlist + giá int64 VND.
...
- API này là cầu nối duy nhất giữa kho time-series giá (FR-PRICE-002) và biểu đồ người dùng (FR-WEB-003).
...
- `close_p` là giá hiển thị; `min_p`/`max_p` chỉ vẽ dải biên - đồng nhất với cách FR-PRICE-002 dựng continuous aggregate.
...
*Hết FR-PRICE-003. Status: ready_to_implement (mục tiêu audit 10/10).*
