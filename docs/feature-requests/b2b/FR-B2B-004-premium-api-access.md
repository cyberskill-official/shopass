...
created: 2026-06-28
related_frs: [FR-INFRA-001, FR-B2B-001, FR-B2B-002]
depends_on: [FR-INFRA-001, FR-B2B-001]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-B2B-32: rate-limit thực thi per-key tại API Gateway (FR-INFRA-001); 429 + header Retry-After khi vượt; KHÔNG để rate-limit ở mỗi service riêng lẻ né được"
  - "DEC-B2B-33: API public CHỈ phục vụ dữ liệu đã ẩn danh từ market_trend_daily (qua FR-B2B-001) - KHÔNG bao giờ expose price_snapshot raw, price_daily raw, hay bất kỳ endpoint user-level nào ra API key"
...
language: "PostgreSQL 16 + Go 1.22 (b2b-svc) + cấu hình API Gateway (FR-INFRA-001)"
...
risk_if_skipped: "FR-B2B-004 mở dòng doanh thu API tiers (§6 mục 11) cho lập trình viên và doanh nghiệp tiêu thụ dữ liệu xu hướng giá theo chương trình. Đây là bề mặt công khai nhất của hệ thống nên rủi ro cao nhất: (1) nếu API public vô tình expose price_snapshot raw hay endpoint user-level thì toàn bộ moat dữ liệu độc quyền và k-anonymity của FR-B2B-001 bị mở toang ra Internet qua một API key - thảm họa PDPL (§5.5) và mất tài sản dữ liệu; (2) nếu rate-limit per-key không thực thi ở gateway thì một key có thể rút cạn dữ liệu hoặc làm quá tải hệ thống; (3) nếu lưu key cleartext hoặc thu hồi key không có hiệu lực ngay thì một key lộ trở thành cửa hậu lâu dài. API key auth + rate-limit + giới hạn endpoint chỉ-ẩn-danh là ba điều kiện bắt buộc trước khi mở API ra ngoài."
---
...
4. **MUST** thực thi rate-limit per-key tại API Gateway (FR-INFRA-001) theo `rate_per_min` của tier (DEC-B2B-32); vượt -> `429` + header `Retry-After`. **MUST NOT** đặt rate-limit chỉ ở service lẻ để có thể né.
5. **MUST** chỉ phục vụ dữ liệu đã ẩn danh từ `market_trend_daily` (ô `suppressed=false`, qua FR-B2B-001) (DEC-B2B-33). API public **MUST NOT** expose `price_snapshot` raw, `price_daily` raw, hay bất kỳ endpoint user-level nào (wishlist/alert/cart/auth).
...
```go
// services/b2b/internal/apikey/ratelimit.go
// allow thực thi token-bucket per-key; điểm này nằm ở lớp gateway (FR-INFRA-001).
...
Xem §3. Thứ tự: migration 0004 (api_key + api_usage) -> auth.go (xác thực key + cache thu hồi) -> ratelimit.go (token-bucket + endpoint gating) -> public_trend_handler.go (phục vụ market_trend_daily) -> usage.go -> tests. Rate-limit per-key cắm vào điểm thực thi của API Gateway (FR-INFRA-001) - FR này cung cấp logic + cấu hình, gateway là nơi chạy. Phát key (sinh secret, băm, trả cleartext một lần duy nhất cho khách) là quy trình quản trị; FR chỉ xác thực key đã phát.

---

## §7 - Phụ thuộc

- FR-INFRA-001 - API Gateway/BFF là điểm thực thi rate-limit per-key + định tuyến `/public/v1/*`.
- FR-B2B-001 - `market_trend_daily` + `QueryCells` (ô đã phát hành) là nguồn dữ liệu duy nhất API public phục vụ.
- FR-B2B-002 (cùng module) - chia sẻ khái niệm tier/tổ chức B2B; api_key có thể đồng bộ tier với subscription.
...
- Rate-limit ở gateway (không ở service lẻ) đảm bảo trần là trần thật; đúng vai trò FR-INFRA-001.
...
*Hết FR-B2B-004. Status: ready_to_implement (mục tiêu audit 10/10).*
