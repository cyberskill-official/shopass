...
created: 2026-06-28
related_frs: [FR-B2B-001, FR-B2B-002, FR-PRICE-001]
depends_on: [FR-B2B-001]
blocks: []
source_pages:
...
6. **MUST** đọc dải phân vị thị trường DUY NHẤT từ `market_trend_daily` qua repo của FR-B2B-001 (ô đã phát hành); giá của chính seller đọc từ `price_daily` của các `product_id` đã xác minh sở hữu.
...
Vì sao chỉ dùng dải phân vị từ market_trend_daily đã phát hành (DEC-B2B-23, §1 #6)? Dải median/p25/p75 đã qua cổng k-anonymity (FR-B2B-001) nên tự nó an toàn - không quy về cá thể. Nếu category quá hẹp khiến ô bị suppress thì cũng nghĩa là dải phân vị không đủ "đông" để an toàn; render vị thế lúc đó có thể vô tình lộ. Nên khi ô suppress, trả 422 thay vì cố dựng.
...
Xem §3. Thứ tự: migration 0003 (seller_owned_sku) -> ownership.go (cổng xác minh) -> position.go (percentile_rank từ dải phân vị) -> seller_handler.go -> tests. Handler chạy sau gateway (FR-INFRA-001). Xác minh quyền sở hữu shop (gắn `verified=true`) là quy trình ngoài phạm vi FR này (vd seller chứng minh qua kênh chính thức của sàn) - FR chỉ tiêu thụ cờ `verified`. Dải phân vị lấy qua `QueryCells` của FR-B2B-001.

---

## §7 - Phụ thuộc

- FR-B2B-001 - `market_trend_daily` + `QueryCells` (ô đã phát hành) là nguồn dải phân vị thị trường.
- FR-PRICE-001 - `tracked_product` (product_id, shop_id) để liên kết SKU seller.
- FR-B2B-002 (cùng module) - chia sẻ khái niệm tổ chức/subscription B2B.
- FR-INFRA-001 (gateway) - xác thực + rate-limit.
...
*Hết FR-B2B-003. Status: ready_to_implement (mục tiêu audit 10/10).*
