...
FR-B2B-002 đặc tả lớp báo cáo B2B + subscription gating ở mức triển khai được. 11 mệnh đề §1 normative, mỗi mệnh đề có AC và test. Quy tắc bảo mật cốt lõi - chỉ đọc `market_trend_daily` qua FR-B2B-001 (ô đã phát hành) - được giữ xuyên suốt builder. Gating server-side theo `b2b_subscription` với 402 (hết hạn) và 403 (vượt entitlement, rõ ràng không câm). Đạt 10/10.

## §2 - Findings (đã giải quyết)

### ISS-001 - Đường vòng đọc nguồn raw (đã chốt)
Cám dỗ "làm giàu" báo cáo bằng cách đọc price_snapshot/price_daily raw sẽ phá k-anonymity của FR-B2B-001. Giải: §1 #1 + DEC-B2B-10 ép builder chỉ gọi `QueryCells` (lọc suppressed); AC #11 + disallowed_tools.
