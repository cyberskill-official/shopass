...
created: 2026-06-28
related_frs: [FR-B2B-001, FR-BILL-001, FR-B2B-003, FR-B2B-004]
depends_on: [FR-B2B-001, FR-BILL-001]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-B2B-10: báo cáo B2B đọc DUY NHẤT market_trend_daily qua FR-B2B-001 (chỉ ô suppressed=false) - KHÔNG bao giờ chạm price_snapshot, price_daily raw, hay bảng user-level"
...
risk_if_skipped: "FR-B2B-002 là nơi dòng doanh thu B2B thực sự thu được tiền - báo cáo insights là sản phẩm khách hàng brand/seller trả tiền (margin cao, §1). Không có nó thì pipeline ẩn danh FR-B2B-001 chỉ là dữ liệu nằm im, không thành doanh thu. Hai rủi ro chí mạng: (1) nếu báo cáo đọc nguồn ngoài market_trend_daily thì có thể lọt giá SKU/shop đơn lẻ ra ngoài, phá k-anonymity đã dựng ở FR-B2B-001 và vi phạm PDPL (§5.5); (2) nếu gating sai - phục vụ báo cáo cho subscription hết hạn hoặc vượt entitlement mà cắt im lặng - thì vừa thất thu vừa làm khách hàng tier thấp lấy được giá trị của tier cao, hỏng mô hình định giá. Việc trả 403 rõ ràng thay vì cắt im lặng cũng là tín hiệu nâng cấp đúng lúc."
---
...
1. **MUST** đọc dữ liệu báo cáo DUY NHẤT từ `market_trend_daily` qua repo của FR-B2B-001, chỉ ô `suppressed=false` (DEC-B2B-10). **MUST NOT** chạm `price_snapshot`, `price_daily` raw, hay bất kỳ bảng user-level nào.
...
Vì sao gắn độ tươi (DEC-B2B-15, §1 #7)? Báo cáo dựa trên job đêm (FR-B2B-001) nên dữ liệu trễ tới một ngày. Khách hàng B2B ra quyết định giá phải biết số này tính tới khi nào. `source_computed_at` nói rõ thay vì để họ đoán.
...
Xem §3. Thứ tự: migration 0002 (b2b_subscription) -> entitlement.go (nạp + kẹp scope) -> builder.go (dựng từ ô đã phát hành) -> export.go -> report_handler.go -> tests. Handler chạy sau JWT/gateway (FR-INFRA-001) cho xác thực; resolve subscription của org từ claim. Báo cáo có thể cache theo `(scope hash, ngày)` vì market_trend_daily chỉ đổi sau job đêm.

---

## §7 - Phụ thuộc

- FR-B2B-001 - `market_trend_daily` + `QueryCells` (lọc suppressed) là nguồn dữ liệu duy nhất.
- FR-BILL-001 - vòng đời subscription/tier; `b2b_subscription` đồng bộ trạng thái thanh toán B2B với khung billing chung.
- FR-INFRA-001 (gateway) - xác thực + rate-limit trước handler.
...
- Lập lịch gửi báo cáo định kỳ qua email (digest tuần) - tái dùng FR-NOTIF-006 ở slice sau.
...
|---|---|---|---|
| Báo cáo đọc nguồn raw | review + test grep | rò rỉ giá SKU/shop, vỡ k-anonymity | Chỉ gọi QueryCells của FR-B2B-001 (DEC-B2B-10) |
...
- FR-B2B-002 là điểm thu tiền của dòng B2B: pipeline ẩn danh (FR-B2B-001) thành doanh thu qua báo cáo có gating.
...
*Hết FR-B2B-002. Status: ready_to_implement (mục tiêu audit 10/10).*
