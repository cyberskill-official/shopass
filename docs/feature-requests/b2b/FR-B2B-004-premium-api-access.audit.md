...
FR-B2B-004 đặc tả Premium API access ở mức triển khai được. 12 mệnh đề §1 normative, mỗi mệnh đề có AC và test. Ba điều kiện bảo mật của một API public được giữ chặt: key băm tại nghỉ + so khớp hằng-thời-gian, rate-limit per-key tại gateway (FR-INFRA-001), và phạm vi CHỈ market_trend_daily đã ẩn danh (không raw, không user-level). Thu hồi key có hiệu lực <= 60s. Đạt 10/10.
