...
created: 2026-06-28
related_frs: [FR-PRICE-002, FR-COMPLY-003, FR-B2B-002, FR-B2B-003, FR-B2B-004]
depends_on: [FR-PRICE-002, FR-COMPLY-003]
blocks: [FR-B2B-002, FR-B2B-003, FR-B2B-004]
source_pages:
...
Vì sao chạy sau khi price_daily refresh (§1 #11)? `price_daily` là continuous aggregate refresh tăng dần (FR-PRICE-002). Tính xu hướng trên một ngày chưa chốt sẽ cho số chập chờn. Chờ ngày đã ổn định để mỗi con số B2B là cuối cùng, không đổi về sau.
...
- FR-PRICE-002 - `price_daily` (continuous aggregate) phải tồn tại và đã refresh; là nguồn dữ liệu duy nhất của pipeline.
- FR-COMPLY-003 - quyền chủ thể dữ liệu (DSAR); khi user xóa dữ liệu, đường dẫn B2B đã ẩn danh + tổng hợp nên không chứa dữ liệu cá nhân của họ, nhưng pipeline phải nhất quán với chính sách xử lý dữ liệu PDPL.
- FR-B2B-002 / FR-B2B-003 / FR-B2B-004 (downstream) - đọc `market_trend_daily` qua `QueryCells`; không bao giờ đọc nguồn raw.
...
- Thêm chiều `region`/`country` cho ô khi mở SEA - gắn khi FR-COMPLY-007 chốt mô hình per-country (giữ k-anonymity per-country để không gộp chéo nước).
...
*Hết FR-B2B-001. Status: ready_to_implement (mục tiêu audit 10/10).*
