...
created: 2026-06-28
related_frs: [FR-AUTH-001, FR-BILL-002, FR-BILL-003, FR-BILL-004, FR-BILL-005, FR-INFRA-002]
depends_on: [FR-AUTH-001]
blocks: [FR-B2B-002, FR-BILL-002, FR-BILL-004, FR-BILL-005]
source_pages:
...
source_decisions:
  ...
  - "DEC-BILL-04: vòng đời status - {'active','past_due','canceled','expired'}; renews_at là mốc gia hạn kế (thanh toán thực do FR-BILL-002/003 đẩy)"
...
8. **MUST** expose hàm repo:
    ...
    - `SetRenewsAt(ctx, subID int64, t time.Time) error` - đẩy mốc gia hạn (do FR-BILL-002/003 gọi sau thanh toán).
...
**Vì sao renews_at tách khỏi thanh toán thực (DEC-BILL-04)?** `renews_at` là mốc dự kiến gia hạn kế. Thanh toán thực (thành công/thất bại) do FR-BILL-002/003 xử lý rồi đẩy `renews_at` tới và đặt status phù hợp. Tách "lịch gia hạn" (BILL-001) khỏi "thực thi thanh toán" (BILL-002/003) giữ schema này đơn giản và để phần tích hợp gateway ở đúng chỗ.

**Vì sao seed idempotent (§1 #10)?** `plan_catalog` là dữ liệu tham chiếu cố định (4 bậc). Seed qua `ON CONFLICT (tier) DO NOTHING` cho phép chạy migration/seed nhiều lần (mọi môi trường) mà không nhân đôi bậc giá - đồng nhất cách seed `platform` của FR-INFRA-002.
...
Xem §3. Thứ tự: migration `0001_plan_catalog.sql` (4 bậc + seed) -> `0002_subscription.sql` (bảng + partial unique active + CHECK) -> `types.go` + `repo.go` (`CreateSubscription`, `GetActive`, `SetRenewsAt`) -> `lifecycle.go` (máy trạng thái `CanTransition` + `UpdateStatus`) -> tests trên Postgres ephemeral (kiểm partial unique + CHECK thật, không mock). FR-BILL-002 gọi `SetRenewsAt`/`UpdateStatus` sau thanh toán; FR-BILL-005 đọc `GetActive` cho feature gating. Driver `pgx`.

---

## §7 - Phụ thuộc

- **FR-AUTH-001** - `app_user` phải tồn tại trước (FK `user_id`).
- **FR-INFRA-002** - bảng `app_user` cột lõi + quy ước đặt tên/migration.
- **FR-BILL-002 (downstream)** - payment gateway gắn thanh toán vào subscription, đẩy `renews_at`, đổi status.
- **FR-BILL-003 (downstream)** - reconciliation đối soát payment với subscription.
- **FR-BILL-004 (downstream)** - referral_code gắn vào user khi tạo subscription (attribution).
- **FR-BILL-005 (downstream)** - feature gating đọc `GetActive` để biết tier.
- Lib: `pgx`.
...
### Tạo subscription (nội bộ, do FR-BILL-002 gọi sau thanh toán đầu)
...
- Proration khi nâng/hạ bậc giữa kỳ - thêm logic ở FR-BILL-002 khi cần; FR này chỉ giữ một active.
- Trial period (dùng thử miễn phí có thời hạn) - thêm trạng thái `'trialing'` vào CHECK + máy trạng thái khi làm gamified upgrade (FR-BILL-005).
- Quyền lợi chi tiết per-tier (bảng `plan_feature`) - gắn vào FR-BILL-005 (feature gating).
...
| FK user không tồn tại | lỗi pgx | từ chối tạo | Tạo app_user trước (FR-AUTH-001) |
...
- `renews_at` là lịch gia hạn; thanh toán thực do FR-BILL-002/003 đẩy - tách lịch khỏi thực thi.
- Đây là nền cho FR-BILL-002 (thanh toán), FR-BILL-003 (đối soát), FR-BILL-005 (gating theo tier).

---

*Hết FR-BILL-001. Status: ready_to_implement (mục tiêu audit 10/10).*
