...
FR-BILL-003 đặc tả bảng `payment` + IPN webhook + reconciliation ở mức triển khai được. 12 mệnh đề §1 normative, mỗi mệnh đề có AC và test trong §5. Năm trụ chốt tiền được khóa: xác thực chữ ký IPN (khóa Vault), khớp số tiền IPN với payment.amount (lệch -> mismatch, không cấp Premium), idempotent theo order_ref+transaction_id, kích hoạt subscription chỉ khi paid, reconciliation định kỳ bù IPN mất. Lưu fee để tính doanh thu ròng. Đạt 10/10.
