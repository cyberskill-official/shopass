...
created: 2026-06-28
related_frs: [FR-BILL-001, FR-BILL-002, FR-INFRA-003, FR-INFRA-004, FR-COMPLY-005]
depends_on: [FR-BILL-002]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-BILL-11: payment lưu gateway, amount (BIGINT VND), fee (BIGINT VND), paid_at, status; gắn order_ref (FR-BILL-002) + subscription"
  - "DEC-BILL-12: IPN/webhook cổng xác thực chữ ký (khóa Vault FR-INFRA-003) trước khi tin; chữ ký sai -> bỏ qua, không đổi payment"
  ...
  - "DEC-BILL-15: payment 'paid' (khớp) -> kích hoạt/gia hạn subscription (FR-BILL-001 SetRenewsAt + status active); reconciliation định kỳ bù IPN bị mất"

language: "Go 1.22 (bill-svc); PostgreSQL 16; webhook nhận qua API Gateway (FR-INFRA-001)"
...
sub_tasks:
  ...
  - "1.0h: payment_repo.go - InsertPending (từ FR-BILL-002), MarkPaid, MarkFailed, MarkMismatch, ByOrderRef"
...
1. **MUST** định nghĩa bảng `payment (id, order_ref, subscription_id, gateway, amount, fee, status, transaction_id, paid_at, created_at)` với `order_ref TEXT NOT NULL UNIQUE` (idempotency từ FR-BILL-002) và `subscription_id` REFERENCES `subscription(id)`.
...
5. **MUST** xác thực chữ ký IPN bằng khóa của `{gateway}` đọc từ Vault (FR-INFRA-003) trước khi tin (DEC-BILL-12); chữ ký sai/thiếu -> bỏ qua (trả `200`/`400` theo spec cổng để cổng ngừng retry), KHÔNG đổi `payment`.
...
8. Khi IPN hợp lệ và khớp với `status='paid'` từ cổng, **MUST** đặt `payment.status='paid'`, `paid_at=now()`, lưu `transaction_id`, rồi kích hoạt subscription (DEC-BILL-15): gọi FR-BILL-001 `SetRenewsAt` (đẩy kỳ kế) + `UpdateStatus` về `active`.
...
CREATE TABLE payment (
  ...
  order_ref      TEXT        NOT NULL UNIQUE,        -- idempotency (FR-BILL-002)
...
func (h *Handler) HandleIPN(w http.ResponseWriter, req *http.Request) {
    ...
    case "paid":
        ...
        h.subs.Activate(req.Context(), p.SubscriptionID) // FR-BILL-001 SetRenewsAt + active (§1 #8)
...
Xem §3. Thứ tự: migration `0003_payment.sql` (order_ref UNIQUE + CHECK + index pending) -> `payment_repo.go` (`InsertPending` từ FR-BILL-002, `MarkPaid`/`MarkFailed`/`MarkMismatch`, `ByOrderRef`) -> `ipn.go` (verify chữ ký Vault -> idempotent check -> khớp amount -> MarkPaid + kích hoạt subscription FR-BILL-001) -> `reconcile.go` (job định kỳ đối soát pending) -> đăng ký `POST /v1/billing/ipn/{gateway}` (`http.ServeMux` `PathValue`) -> tests. Webhook KHÔNG dùng JWT user; xác thực bằng chữ ký HMAC, khóa từ FR-INFRA-003. `subs.Activate` gọi `SetRenewsAt` + `UpdateStatus(active)` của FR-BILL-001.

---

## §7 - Phụ thuộc

- **FR-BILL-002** - tạo `payment` `pending` + `order_ref`; IPN chốt trạng thái thanh toán đó.
- **FR-BILL-001** - `subscription` + `SetRenewsAt`/`UpdateStatus`; kích hoạt khi payment `paid`.
- **FR-INFRA-003 (Vault)** - khóa xác thực IPN mỗi cổng; không nằm trong DB/code.
- **FR-INFRA-001 (gateway)** - định tuyến webhook IPN; bỏ qua JWT user, dùng chữ ký.
- **FR-INFRA-004 (observability)** - alert khi `mismatch` / pending tồn lâu / IPN lỗi.
- **FR-BILL-005 (downstream)** - feature gating bật khi subscription active (sau payment paid).
- **FR-COMPLY-005** - audit no-cleartext: khóa IPN từ Vault.
...
*Hết FR-BILL-003. Status: ready_to_implement (mục tiêu audit 10/10).*
