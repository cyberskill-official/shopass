...
created: 2026-06-27
related_frs: [FR-EXT-002, FR-EXT-003, FR-EXT-004, FR-EXT-005, FR-EXT-006, FR-TRUST-001, NFR-EXT-001]
depends_on: []
blocks: [FR-EXT-002, FR-EXT-004, FR-EXT-006, FR-TRUST-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-EXT-05: tác vụ nặng/dài đẩy lên backend; extension chỉ là 'đầu đọc' nhẹ (heavy work off-device, FR-EXT-003/005)"
  - "DEC-EXT-06: TypeScript + bundler (esbuild/vite) build ra MV3 bundle; mã nguồn mở (FR-TRUST-001) nên ưu tiên build tái lập"
...
sub_tasks:
  ...
  - "0.5h: build.mjs + tsconfig.json - esbuild bundle ra dist/, build tái lập (FR-TRUST-001)"
...
risk_if_skipped: "Đây là nền của toàn bộ module EXT - mọi content script (Shopee/TikTok/Lazada), pipeline tối thiểu hóa, đồng bộ backend và UI consent đều nạp trên scaffold này. Làm sai vòng đời MV3 là lỗi phổ biến nhất khi viết extension: nếu giữ state trong biến global, service worker bị Chrome kill sau ~30s và toàn bộ trạng thái (hàng đợi, phiên đọc giỏ) biến mất giữa chừng -> đọc giỏ hàng lỗi không tái hiện được. Nếu dùng setInterval thay chrome.alarms, polling chết khi SW ngủ. Nếu khai <all_urls>, Chrome Web Store review từ chối và người dùng thấy quyền thừa (extension đọc cookie sàn vốn đã dễ bị nghi malware, §5.4). Scaffold đúng MV3 ngay từ đầu là rẻ; sửa sau khi đã chồng 5 FR lên trên là đắt."
---
...
6. `manifest.json` **MUST** khai báo `host_permissions` tường minh cho từng domain sàn được hỗ trợ (ví dụ `https://shopee.vn/*`; các sàn khác thêm khi FR tương ứng tới) - KHÔNG dùng `<all_urls>` hay wildcard rộng (DEC-EXT-04).
7. `permissions` trong manifest **MUST** là tập tối thiểu cần cho slice 1: `storage`, `alarms`. Quyền khác (ví dụ `offscreen`, `declarativeNetRequest`) chỉ thêm khi FR tương ứng (FR-EXT-004) tới.
...
10. Build **MUST** tái lập được (DEC-EXT-06): `build.mjs` (esbuild) sinh `dist/` xác định từ mã nguồn; phục vụ yêu cầu mã nguồn mở + reproducible build của FR-TRUST-001.
...
Xem §3. Thứ tự: `manifest.json` + `tsconfig.json` + `build.mjs` (khung build) -> `storage.ts` (điểm state) -> `lifecycle.ts` + `alarms.ts` + `messaging.ts` -> `service-worker.ts` (entrypoint nối listener top-level) -> tests. Bundler esbuild emit ESM module cho service worker (`type: "module"`). Tránh mọi top-level `await` trong entrypoint service worker (làm trễ đăng ký listener). Mọi FR-EXT sau nạp content script / offscreen / sync lên đúng scaffold này.

---

## §7 - Phụ thuộc

- **depends_on: []** - đây là FR gốc của module EXT, không phụ thuộc FR khác.
- **FR-EXT-002 (downstream)** - content script Shopee nạp trên scaffold + messaging này.
- **FR-EXT-003/004/005/006 (downstream)** - pipeline tối thiểu hóa, offscreen, đồng bộ backend, UI consent đều dựa scaffold.
- **FR-TRUST-001 (downstream)** - publish mã nguồn mở + reproducible build dùng `build.mjs` ở đây.
- **NFR-EXT-001** - ràng buộc MV3 (ephemeral, alarms >=30s, no global state) mà FR này hiện thực hóa.
...
Đã chốt. Hoãn:
- Có dùng `chrome.storage.session` cho state phiên-nhạy-cảm (RAM-only) hay không - quyết ở FR-EXT-003 khi định nghĩa dữ liệu tối thiểu hóa.
- Firefox MV3 (service worker khác Chrome) - hoãn tới khi có nhu cầu đa trình duyệt; slice 1 nhắm Chrome/Chromium.
- Cập nhật host_permissions cho tiktok.com / lazada.vn - thêm ở FR-EXT-007 / FR-EXT-008 (P2), không khai sớm quyền chưa dùng.
...
| Build không xác định | dist khác giữa 2 lần build | FR-TRUST-001 reproducible vỡ | Khóa version + esbuild xác định |
| Thừa permission trong manifest | manifest.test | quyền thừa, review chậm | Giữ tập tối thiểu, thêm theo FR |

---

## §11 - Ghi chú

- Đây là scaffold nền của toàn module EXT; ba ràng buộc MV3 (SW ephemeral / alarms >=30s / no global state) là bất biến cho mọi FR sau, được chốt cứng ở NFR-EXT-001.
...
- Việc nặng off-device là ranh giới kiến trúc xuyên suốt: extension đọc nhẹ, backend gánh nặng (FR-EXT-003/005, FR-SCRAPE-*).
- Reproducible build ở đây là tiền đề cho cam kết minh bạch FR-TRUST-001.

---

*Hết FR-EXT-001. Status: ready_to_implement (mục tiêu audit 10/10).*
