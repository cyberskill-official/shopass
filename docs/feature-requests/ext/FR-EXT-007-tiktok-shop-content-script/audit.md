...
- ISS-003 (kiểm, không phải lỗi): tái dùng normalize/health FR-EXT-002 (AC #8 grep import `../shared/`) giữ một điểm kiểm soát tối thiểu hóa duy nhất - đúng DEC-EXT-37, tránh nhân đôi bề mặt rò.
...
| #6 health signal khi hỏng | #7 | reportHealth -> FR-SCRAPE-006 |
| #7 tái dùng normalize/health FR-EXT-002 | #8 | grep import shared |
...
Đường né API ký (§3.2/§3.9) + SPA observer + tái dùng khung FR-EXT-002 đều có test; cam kết niềm tin khẳng định cho cả TikTok. Mọi mệnh đề có AC backing. Score = 10/10. Verdict: PASS.

---

*Hết audit độc lập FR-EXT-007.*
