...
Kiểm tra lại từ file hiện tại, không tin audit cũ. FR-SCRAPE-001 đặc tả orchestrator lõi ở mức triển khai được: 12 mệnh đề §1 normative (đủ MUST, có một SHOULD metric), mỗi mệnh đề testable có AC ở §4 và test ở §5. Schema `scrape_job` khớp DATA-MODEL (PK `product_id`, tier ENUM `scrape_tier` DEFAULT 'cold', `locked_until` cho lock, FK `tracked_product`/`platform`). Tiering hot/warm/cold là hàm thuần `NextRunAt`/`ReTier`; hàng đợi bền re-claim qua `locked_until`; concurrency cap per-platform; ghi qua `InsertSnapshot` của FR-PRICE-002 chứ không tự viết SQL. Score = 10/10.
...
§1 #8 + #11 + `pool.go::runOne` gọi `p.price.InsertSnapshot(ctx, snap)` (delta-only FR-PRICE-002), không có câu `INSERT INTO price_snapshot` nào trong orchestrator; idempotent re-claim dựa `ON CONFLICT DO NOTHING` của PRICE. Cross-ref đúng, không nhân bản logic delta.
...
| #11 idempotent re-claim | AC11 | ON CONFLICT (FR-PRICE-002) |
| #12 secrets qua Vault | - (§6/§10) | FR-INFRA-003 |
...
*Audit độc lập FR-SCRAPE-001 - hết.*
