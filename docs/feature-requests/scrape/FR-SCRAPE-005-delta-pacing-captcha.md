---
id: FR-SCRAPE-005
title: "Pacing ngẫu nhiên + jitter + CAPTCHA handling (slider/puzzle) + gọi delta-only InsertSnapshot của FR-PRICE-002"
module: SCRAPE
priority: MUST
status: done
verify: T
...
created: 2026-06-27
related_frs: [FR-SCRAPE-001, FR-SCRAPE-002, FR-SCRAPE-003, FR-SCRAPE-004, FR-SCRAPE-006, FR-PRICE-002]
depends_on: [FR-SCRAPE-002, FR-PRICE-002]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-SCRAPE-22: ghi giá qua InsertSnapshot delta-only của FR-PRICE-002 - không tự viết SQL; chỉ ghi khi giá đổi"
...
3. **MUST** phối hợp pacing với concurrency cap của FR-SCRAPE-001 (#6) - hai lớp độc lập: cap giới hạn số request song song, pacing giới hạn nhịp request tuần tự.
...
6. **MUST** ghi giá qua `price.Repo.InsertSnapshot` delta-only của FR-PRICE-002 (DEC-SCRAPE-22) - sink KHÔNG tự viết SQL `INSERT INTO price_snapshot`. Quy tắc "chỉ ghi khi giá đổi" thuộc về PRICE, không nhân bản ở đây.
...
8. **MUST** truyền tín hiệu re-tier cho orchestrator: sink trả `(written, flashSale)` để FR-SCRAPE-001 (#3) quyết định promote/demote tier - giá đổi hoặc flash -> hot.
...
12. **MUST** idempotent với retry: gọi `InsertSnapshot` cùng `(product_id, ts)` 2 lần (do retry) an toàn nhờ `ON CONFLICT DO NOTHING` của FR-PRICE-002.

---

## §2 - Vì sao thiết kế này (rationale cho người đọc)

**Vì sao pacing + jitter (DEC-SCRAPE-20)?** Bot lộ rõ nhất qua tính đều đặn: request cách nhau đúng 1000ms là chữ ký máy. Người dùng thật có nhịp ngẫu nhiên. Delay ngẫu nhiên `[min,max]` cộng jitter phá tính đều đặn, phối hợp với hành vi người-thật của farm (FR-SCRAPE-003) để giảm điểm nghi ngờ.

**Vì sao pacing tách khỏi concurrency cap (§1 #3)?** Cap (FR-SCRAPE-001) trả lời "bao nhiêu request song song"; pacing trả lời "cách nhau bao lâu". Một sàn có thể chịu 6 luồng song song nhưng không chịu 6 request/giây trên một luồng. Hai lớp giải hai bài toán khác nhau, cần cả hai.
...
**Vì sao gọi InsertSnapshot thay vì tự ghi (DEC-SCRAPE-22)?** Delta-only là một quyết định thiết kế của PRICE (FR-PRICE-002 #4): chỉ ghi khi `(price, list_price, stock, flash_sale)` đổi. Nếu sink tự `INSERT`, nó phải nhân bản hàm `changed()` và sẽ lệch khi PRICE đổi quy tắc. Một nguồn sự thật duy nhất cho quy tắc ghi - sink chỉ là người gọi.
...
11. Gọi `Write` cùng `(product_id, ts)` 2 lần an toàn (ON CONFLICT DO NOTHING của FR-PRICE-002), không nhân đôi dòng.
...
Xem §3. Thứ tự: limiter.go (pacing thuần, test trước) -> detect.go (nhận diện CAPTCHA từ fixture) -> solver.go (giải có ngân sách) -> sink.go (gọi InsertSnapshot) -> chèn vào `orchestrator/pool.go` (Wait trước Fetch, Write sau Fetch) -> tests. Sink phụ thuộc `price.Repo` của FR-PRICE-002; ngân sách CAPTCHA và pacing delays ở config per-platform. Tier hot có `min_delay` riêng nhỏ để không vỡ SLA quét-theo-phút.

---

## §7 - Phụ thuộc

- **FR-SCRAPE-002** - adapter cung cấp snapshot và response để Detect; pacing chèn trước Fetch.
- **FR-PRICE-002** - `Repo.InsertSnapshot` delta-only + `ON CONFLICT DO NOTHING`; sink chỉ gọi, không tự ghi.
- **FR-SCRAPE-001** - orchestrator nhận `(written, flashSale)` để re-tier; pacing phối hợp concurrency cap.
- **FR-SCRAPE-003** - farm-behavior là một đường giải CAPTCHA (slider) thay dịch vụ ngoài.
- **FR-SCRAPE-006 (giám sát)** - tỷ lệ CAPTCHA tăng đột biến là tín hiệu adapter-health.
...
snap, err := adapter.Fetch(ctx, job)                                   // FR-SCRAPE-002
...
written, flash, err := sink.Write(ctx, snap)                          // FR-PRICE-002 delta-only
...
- Đây là lớp khép kín giữa scraping (lấy dữ liệu) và PRICE (ghi dữ liệu); điểm nối cứng là `InsertSnapshot` delta-only của FR-PRICE-002.
- Pacing/jitter và hành vi người-thật (FR-SCRAPE-003) là hai lớp chống dấu vết bot bổ trợ: một về nhịp, một về tương tác.
...
- Sink trả `(written, flashSale)` khép vòng re-tier: biến động giá tự kéo SKU lên tần suất quét cao hơn (FR-SCRAPE-001).

---

*Hết FR-SCRAPE-005. Status: ready_to_implement (mục tiêu audit 10/10).*
