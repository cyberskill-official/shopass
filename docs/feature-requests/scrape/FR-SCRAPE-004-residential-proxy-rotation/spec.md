...
created: 2026-06-27
related_frs: [FR-SCRAPE-001, FR-SCRAPE-003, FR-SCRAPE-005, FR-SCRAPE-007, FR-SCRAPE-008, FR-INFRA-003]
...
source_decisions:
  ...
  - "DEC-SCRAPE-19: proxy theo nước (geo-targeting) phải khớp profile fingerprint của FR-SCRAPE-003 (cùng nước)"

language: "Go 1.22 (scrape-svc); proxy pool + cost accounting; creds từ Vault (FR-INFRA-003)"
...
sub_tasks:
  ...
  - "1.0h: session.go - gắn proxy session với fingerprint profile (cùng nước, FR-SCRAPE-003)"
...
6. **MUST** gắn proxy session với fingerprint profile cùng nước (DEC-SCRAPE-19): proxy VN chỉ ghép profile VN của FR-SCRAPE-003; ghép sai nước bị từ chối.
7. **MUST** cost-guard (DEC-SCRAPE-18): theo dõi GB tiêu thụ và chi phí ước tính per provider per ngày vào `proxy_usage`; khi chi phí ngày vượt ngân sách cấu hình, cost-guard **MUST**:
    ...
    - tạm dừng quét tier `cold` (FR-SCRAPE-001), giữ tier `hot` (flash sale) chạy.
...
11. **MUST** đọc credentials proxy từ Vault (FR-INFRA-003), KHÔNG hardcode/cleartext.
...
**Vì sao gắn proxy với fingerprint cùng nước (DEC-SCRAPE-19)?** Như đã nêu ở FR-SCRAPE-003: IP mang geolocation. Profile VN (timezone, ngôn ngữ VN) qua IP nước khác là mâu thuẫn. Buộc cặp (proxy <-> profile cùng nước) ngay tại tầng cấp session giữ câu chuyện nhất quán, không để adapter lỡ tay ghép sai.
...
func (p *Pool) Acquire(ctx context.Context, tier Tier, country string) (ProxySession, error) {
    ...
    creds := p.vault.ProxyCreds(tier)     // FR-INFRA-003, không cleartext
...
Xem §3. Thứ tự: migration 0002 (proxy_usage) -> tier.go (SelectTier thuần, test trước) -> costguard.go (Evaluate + repo SpentMicroUSD) -> pool.go (Acquire + rotate + MarkBanned) -> session.go (bind profile cùng nước) -> tests. Provider tiers và ngân sách/ngày ở config.go (scrape-svc, Go); creds đọc từ Vault (FR-INFRA-003). Cost-guard chạy đồng bộ trong đường `Acquire` để chặn trước khi tiêu, không phải cảnh báo sau.

---

## §7 - Phụ thuộc

- **FR-SCRAPE-003** - fingerprint profile ghép proxy cùng nước; farm dùng proxy khi render.
- **FR-SCRAPE-001** - orchestrator hỏi `CanProceed` trước khi cấp session; cost-guard chặn tier cold.
- **FR-SCRAPE-002 / 007 / 008** - adapter lấy proxy session để gọi endpoint / render.
- **FR-INFRA-003** - credentials proxy từ Vault.
...
- Cặp (proxy <-> fingerprint cùng nước) là một nửa của câu chuyện nhất quán geo; nửa kia là profile của FR-SCRAPE-003.
- Sổ chi phí số nguyên (micro-USD) cho phép báo cáo unit economics đáng tin để đối chiếu ngưỡng §4.1 hằng tháng.

---

*Hết FR-SCRAPE-004. Status: ready_to_implement (mục tiêu audit 10/10).*
