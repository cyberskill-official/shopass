...
created: 2026-06-27
related_frs: [FR-SCRAPE-001, FR-SCRAPE-003, FR-SCRAPE-005, FR-SCRAPE-006, FR-PRICE-001, FR-PRICE-002, FR-TRACK-001]
depends_on: [FR-SCRAPE-001]
blocks: [FR-SCRAPE-005, FR-SCRAPE-006, FR-TRACK-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-SCRAPE-08: rơi xuống Playwright farm (FR-SCRAPE-003) khi endpoint trả lỗi/HTML challenge thay vì JSON"
...
disallowed_tools:
  ...
  - lưu giá dạng float/numeric (vi phạm DEC-PRICE-05 của FR-PRICE-002)
...
risk_if_skipped: "Shopee là sàn lớn nhất VN và là bề mặt MVP (§3 backlog). Không có adapter này thì không có nguồn giá Shopee - sale ảo (FR-DEAL-001), biểu đồ (FR-DEAL-003), so sánh chéo (FR-PRICE-004) đều rỗng. Nếu gửi cookie người dùng kèm request backend là vi phạm nguyên tắc token-not-on-server (§3.8, §5.4) và biến SănDeal thành thứ bị nghi malware. Nếu không có fallback Playwright, ngày Shopee trả HTML challenge thay vì JSON là ngày dữ liệu Shopee đứng hình. Đây là adapter sàn đầu tiên và là khuôn mẫu cho TikTok/Lazada."
---

## §1 - Mô tả (BCP-14 normative)

Adapter Shopee **MUST** triển khai interface `PlatformAdapter` của FR-SCRAPE-001, lấy giá qua internal JSON endpoint của Shopee khi truy cập được, parse về `price.PriceSnapshot`, và rơi xuống Playwright farm khi gặp challenge. Hợp đồng:

1. **MUST** triển khai `Fetch(ctx, job ScrapeJob) (price.PriceSnapshot, error)` và `PlatformID() int16` trả mã của Shopee.
2. **MUST** gọi endpoint `/api/v4/pdp/get_pc?item_id={itemid}&shop_id={shopid}` làm đường lấy giá chính (DEC-SCRAPE-06); `item_id`/`shop_id` lấy từ `tracked_product.platform_item_id` (FR-PRICE-001).
3. **MUST** truy cập theo ngữ cảnh `is_login:false` (không phiên đăng nhập) - adapter backend KHÔNG dùng cookie phiên của bất kỳ người dùng nào (DEC-SCRAPE-09, §3.2). Yêu cầu đi qua proxy residential (FR-SCRAPE-004) và fingerprint của farm, tách hẳn khỏi extension.
...
5. **MUST** quy đổi giá KHÔNG dùng float trung gian: chia số nguyên `raw / 100000` (hoặc đọc trường đã là VND nếu Shopee trả vậy), tránh sai số float (đồng bộ DEC-PRICE-05 của FR-PRICE-002).
6. **MUST** phát hiện challenge/anti-bot: nếu phản hồi không phải JSON hợp lệ (HTML, trang verify, HTTP 4xx/5xx của WAF), adapter **MUST** rơi xuống Playwright farm (FR-SCRAPE-003) qua `DEC-SCRAPE-08` thay vì trả snapshot rỗng/sai.
7. **MUST** trả error (không panic) khi cả endpoint lẫn fallback đều thất bại, để orchestrator (FR-SCRAPE-001 #5) áp retry/backoff.
8. **SHOULD** dùng `/api/v4/recommend/recommend` như nguồn phụ để khám phá SKU liên quan/đối thủ (phục vụ canonical_key của FR-PRICE-005), nhưng đường giá chính là `pdp/get_pc`.
...
**Vì sao is_login:false và không cookie người dùng (DEC-SCRAPE-09)?** Đây là ranh giới đạo đức và pháp lý cốt lõi của SănDeal (§5.4). Scraping backend là hành vi của hệ thống, phải đứng bằng hạ tầng riêng (proxy + fingerprint farm), tuyệt đối không mượn phiên của người dùng. Extension đọc giỏ hàng client-side là chuyện khác (first-party, FR-EXT-002); hai đường này không được trộn.

**Vì sao quy đổi giá bằng số nguyên (§1 #5)?** Giá Shopee trong payload thường là số rất lớn (micro-đơn-vị). Chia bằng float đưa sai số vào tiền tệ, làm hỏng phép so `current_price >= median90 * 0.97` của sale ảo (FR-DEAL-001). Chia số nguyên giữ chính xác tuyệt đối, đồng bộ với BIGINT VND của FR-PRICE-002.
...
func (a *ShopeeAdapter) Fetch(ctx context.Context, job orchestrator.ScrapeJob) (price.PriceSnapshot, error) {
    ...
    // Challenge/HTML/lỗi WAF -> rơi xuống Playwright farm (FR-SCRAPE-003)
...
12. Tích hợp orchestrator: job Shopee qua pool -> adapter chạy -> `InsertSnapshot` delta-only được gọi (FR-SCRAPE-001 #8).
...
Xem §3. Thứ tự: endpoints.go (URL builder) -> parse.go (JSON -> snapshot, test trước với fixture) -> adapter.go (Fetch + fallback) -> registry.go (đăng ký vào orchestrator) -> tests. Fixture `pdp_get_pc.json` là JSON thật đã ẩn danh (xóa field shop/seller/review, chỉ giữ cấu trúc giá). Đường lấy giá chính là `pdp/get_pc`; `recommend` là tùy chọn phụ cho FR-PRICE-005.

---

## §7 - Phụ thuộc

- **FR-SCRAPE-001** - interface `PlatformAdapter`, orchestrator điều phối + gọi `InsertSnapshot`.
- **FR-SCRAPE-003 (fallback)** - Playwright farm `RenderPrice` khi gặp challenge.
- **FR-SCRAPE-004** - proxy residential cho `GetViaProxy`.
- **FR-PRICE-001** - `tracked_product.platform_item_id` cung cấp `item_id`/`shop_id`.
- **FR-PRICE-002 (downstream)** - snapshot ghi qua `InsertSnapshot` delta-only.
- **FR-PRICE-005 (tùy chọn)** - `recommend` hỗ trợ canonical_key matching.
...
- Khai thác `recommend` để dò biến thể (màu/size) cùng SKU - gắn khi FR-PRICE-005.
...
| JSON đổi schema (Shopee A/B) | `shopee_parse_fail_total` | parse hụt | FR-SCRAPE-006 giám sát + cập nhật struct |
...
| Fallback farm cũng fail | error trả về | quét hụt lần này | Orchestrator retry/backoff (FR-SCRAPE-001 #5) |

---

## §11 - Ghi chú

- Adapter Shopee là khuôn mẫu cho TikTok (FR-SCRAPE-007) và Lazada (FR-SCRAPE-008): cùng interface, khác cách lấy dữ liệu.
...
- Quy đổi giá bằng số nguyên đồng bộ BIGINT VND của FR-PRICE-002 - không sai số float trên tiền tệ.
...
*Hết FR-SCRAPE-002. Status: ready_to_implement (mục tiêu audit 10/10).*
