...
The build invariants in `docs/feature-requests/SHIP-GUIDE.md` still apply to all code produced here (stack choices, testing discipline, no secrets in code, parameterized SQL, consent-first extension rules). Where a card here conflicts with SHIP-GUIDE, SHIP-GUIDE wins and the conflict gets logged in the ledger.
