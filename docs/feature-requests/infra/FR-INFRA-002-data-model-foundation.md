...
created: 2026-06-27
related_frs: [FR-INFRA-001, FR-INFRA-005, FR-AUTH-001, FR-AUTH-003, FR-PRICE-001]
depends_on: []
blocks: [FR-AFFIL-001, FR-AUTH-001, FR-CART-001, FR-COMPLY-001, FR-INFRA-005, FR-NOTIF-001, FR-PRICE-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-INFRA-08: `app_user` định nghĩa cột lõi ở migration nền (id BIGSERIAL, email CITEXT unique, locale 'vi-VN'); FR-AUTH-001 mở rộng cột bảo mật"
...
8. FR-AUTH-001 **MUST** mở rộng `app_user` ở migration sau (thêm `pwd_hash`, `referral_code_id`, ...); FR này KHÔNG định nghĩa cột bảo mật để giữ ranh giới module.
...
Vì sao tách cột lõi `app_user` khỏi cột bảo mật (DEC-INFRA-08, §1 #8)? INFRA dựng khung dữ liệu; AUTH lo bảo mật. Để cột lõi (id, email, locale, status) ở migration nền cho mọi module gắn vào; để `pwd_hash`/`referral_code_id` cho FR-AUTH-001. Ranh giới này giữ migration nền không bị kẹt chờ quyết định bảo mật.

Vì sao quy ước đặt tên thành tài liệu (DEC-INFRA-09)? 16 module sẽ tạo hàng chục bảng. Không có quy ước chung thì mỗi module đặt kiểu khác (`userId` vs `user_id`, `createdAt` vs `created_at`), gây ma sát join và migration. Một tài liệu quy ước là hợp đồng để mọi FR sau tuân theo.

Vì sao `locale` mặc định `'vi-VN'` (§1 #6)? Thị trường gốc là Việt Nam; phần lớn người dùng là vi-VN. Đặt mặc định đúng giảm boilerplate và là điểm neo cho i18n và per-country gating (FR-INFRA-005) khi mở SEA.
...
-- timescaledb được bật khi triển khai PRICE (FR-PRICE-002); stub ở đây để thứ tự rõ ràng:
...
-- Cột bảo mật (pwd_hash, referral_code_id) do FR-AUTH-001 thêm ở migration sau.
```
...
Xem §3. Thứ tự cố định: `0001_extensions` (CITEXT trước) -> `0002_platform` -> `0003_app_user_core` -> seed. Runner bọc golang-migrate; nuốt `ErrNoChange` để `Up` idempotent. Seed chạy sau migration, idempotent qua `ON CONFLICT`. Tài liệu quy ước đặt cạnh migration để mọi FR sau tham chiếu. Test chạy trên DB tạm (testcontainers Postgres hoặc instance ephemeral) để kiểm cả CHECK và CITEXT thật, không mock.

---

## §7 - Phụ thuộc

- Không có FR phụ thuộc trước (đây là một trong các nền móng P0).
- FR-AUTH-001 (downstream) - thêm `pwd_hash`, `referral_code_id` vào `app_user` qua migration mới.
- FR-AUTH-003 / FR-PRICE-001 / FR-CART-001 / FR-AFFIL-001 / FR-BILL-001 (downstream) - FK tới `platform` và/hoặc `app_user`.
- FR-INFRA-005 (downstream) - đọc `platform.country` cho per-country gating.
...
- Soft-delete chuẩn hóa (cột `deleted_at`) cho mọi bảng - gắn vào FR-AUTH-005 (xóa tài khoản DSAR) và nhân rộng sau.
...
- Đây là nền dữ liệu: 9+ FR khác FK tới `platform` hoặc `app_user`, hoặc đọc `platform.country`.
...
- Tài liệu quy ước đặt tên là hợp đồng cho mọi FR sau, giảm ma sát join/migration khi 16 module cùng tạo bảng.

---

*Hết FR-INFRA-002. Status: ready_to_implement (mục tiêu audit 10/10).*
