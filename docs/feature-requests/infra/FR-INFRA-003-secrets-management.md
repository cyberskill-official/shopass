...
created: 2026-06-27
related_frs: [FR-INFRA-001, FR-INFRA-004, FR-AUTH-002, FR-SCRAPE-001, FR-SCRAPE-004, FR-COMPLY-005, FR-TRUST-001]
depends_on: []
blocks: [FR-COMPLY-005, FR-SCRAPE-001]
source_pages:
...
1. Mọi secret **MUST** sống trong Vault hoặc AWS Secrets Manager (DEC-INFRA-11): DB credentials, JWT signing key (cho FR-AUTH-002), proxy credentials theo nhà cung cấp (cho FR-SCRAPE-004), gateway tokens, payment gateway keys (cho FR-BILL-002). KHÔNG đặt trong file `.env` commit vào repo hay hằng số trong code.
...
11. Tài liệu path layout + least-privilege policy **MUST** tồn tại như nguồn sự thật để FR-AUTH-002, FR-SCRAPE-004, FR-BILL-002 biết đọc path nào.
12. Lớp secrets **MUST** là điểm tham chiếu duy nhất cho audit no-cleartext của FR-COMPLY-005 (chứng minh không có credential cleartext rời lớp này).
...
Vì sao là điểm tham chiếu cho audit PDPL (§1 #12)? FR-COMPLY-005 phải chứng minh no-cleartext credential. Nếu mọi secret đi qua lớp này, audit chỉ cần xác nhận không có credential nào rời lớp ra env/code/log - một điểm kiểm thay vì rà toàn codebase.
...
- Không có FR phụ thuộc trước (nền móng P0).
- FR-INFRA-001 (downstream) - gateway lấy JWKS/khoá qua provider, không nhúng env.
- FR-AUTH-002 (downstream) - đọc `auth/jwt-signing` để ký JWT.
- FR-SCRAPE-004 (downstream) - đọc `scrape/proxy/<vendor>` cho proxy rotation.
- FR-BILL-002 (downstream) - đọc payment keys.
- FR-COMPLY-005 (downstream) - dùng lớp này làm điểm chứng minh no-cleartext.
...
- Auto-rotation lịch trình cho JWT signing key (xoay định kỳ tự động) - phối với JWKS đa-`kid` ở FR-AUTH-002.
...
- Đây là điểm chứng minh no-cleartext cho audit PDPL (FR-COMPLY-005): một điểm kiểm thay vì rà toàn codebase.

---

*Hết FR-INFRA-003. Status: ready_to_implement (mục tiêu audit 10/10).*
