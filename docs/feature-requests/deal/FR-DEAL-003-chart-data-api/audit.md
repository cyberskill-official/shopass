...
Audit độc lập, suy lại từ file hiện tại. FR-DEAL-003 đặc tả `GET /v1/products/{id}/chart` trả feed biểu đồ giá có chú giải: thân daily đọc từ continuous aggregate `price_daily` (FR-PRICE-002), overlay median90 + trailing_min + verdict sale ảo (gọi FR-DEAL-001, không tự tính) + mốc ngày đôi, tôn trọng cổng độ chín FR-DEAL-002, p95 <500ms. Khớp §3.7 (shape API) và §3.8 (NFR biểu đồ <500ms). 13 mệnh đề §1 có AC §4 và test §5. Mọi giá là int64 VND trong JSON (không float/string). Phân biệt rõ với FR-PRICE-003 (chuỗi thô không chú giải). Đạt 10/10.
...
§1 #5 gọi đánh giá FR-DEAL-001 thay vì tính lại verdict ở DEAL-003 hay client; AC #5 yêu cầu khớp nhãn thẻ sản phẩm. Loại rủi ro dải verdict trên biểu đồ lệch nhãn thẻ. Cổng độ chín FR-DEAL-002 ép NEW -> verdict UNKNOWN (§1 #6, AC #6). Không cần sửa.
...
| #5 verdict FR-DEAL-001 | AC #5 | `h.deal.Verdict` |
| #6 maturity FR-DEAL-002 | AC #6 | `TestChart_MaturityFlag_Warming` |
