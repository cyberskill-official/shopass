...
Audit độc lập, suy lại từ file hiện tại. FR-DEAL-005 nâng cấp dự đoán đáy lên LightGBM cho SKU >=180 ngày: feature store thống nhất train/serve (1 dòng/(product_id, as_of_date)), nhãn `future_min_price_14d` dựng bằng forward-join CHỈ lúc train, cổng 180 ngày (dưới ngưỡng fallback Prophet FR-DEAL-004 rồi category prior FR-DEAL-002), giữ chung hợp đồng tín hiệu `p_bottom_14d`. Feature set khớp §3.5(2) (day_of_month, trailing_min_30/60/90, price_vs_median90, volatility_30d, category_seasonality, flash_sale_flag, platform_id; target future_min_price_14d; >=180d history). 14 mệnh đề §1 có AC §4 và test §5. Priority SHOULD hợp lý (nâng cấp, không chặn MVP). Đạt 10/10.
...
§1 #11 + §7 ghi rõ INSERT vào bảng `price_forecast` do FR-DEAL-004 sở hữu với `model_kind='lgbm'` (đã nằm trong CHECK của DEAL-004), KHÔNG tạo bảng thứ hai. Ánh xạ cột `expected_min_14d`->yhat, `as_of_date`->run_date, `horizon_days`->horizon_day. Migration §3 chỉ tạo `feature_store` (0002), không đụng price_forecast. Coherent.

### ISS-003 - Cổng 180 ngày + hợp đồng tín hiệu chung (đã xác nhận)
§1 #7 chỉ chạy LightGBM khi >=180 ngày, dưới ngưỡng fallback Prophet (DEC-DEAL-40); test `test_eligibility_180d_gate` (179->prophet, 180->lgbm). §1 #10 phát cùng schema tín hiệu FR-DEAL-004 (`p_bottom_14d`, `expected_min_14d`, `horizon_days=14`, chỉ khác model_kind) để FR-DEAL-006 bất khả tri theo mô hình; test `test_p_bottom_contract_matches_prophet`. Không cần sửa.
