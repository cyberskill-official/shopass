...
created: 2026-06-27
related_frs: [FR-PRICE-002, FR-PRICE-003, FR-DEAL-001, FR-DEAL-002, FR-WEB-003]
...
source_decisions:
  ...
  - "DEC-DEAL-21: overlay annotation phía server (median90, trailing_min, verdict sale ảo, mốc ngày đôi) để verdict đồng nhất với FR-DEAL-001 và client nhẹ"
  - "DEC-DEAL-22: DEAL-003 là feed chart-shaped (downsample daily + chú giải) - phân biệt rõ với FR-PRICE-003 là chuỗi giá thô (daily + raw tail, không chú giải)"
  - "DEC-DEAL-23: tôn trọng cổng độ chín FR-DEAL-002 - WARMING gắn cờ đang tích lũy, NEW (<14 ngày) trả dữ liệu nhưng verdict=UNKNOWN"
...
disallowed_tools:
  ...
  - tính verdict sale ảo ở client thay vì gọi FR-DEAL-001 (vi phạm DEC-DEAL-21, verdict lệch giữa thẻ và biểu đồ)
...
sub_tasks:
  ...
  - "1.0h: chart.go - đọc thân daily từ price_daily, kiểm tra tồn tại product (404), lấy độ chín FR-DEAL-002"
  - "1.5h: annotate.go - tính median90 + trailing_min từ chuỗi daily, sinh mốc ngày đôi trong range, gắn verdict từ FR-DEAL-001"
  - "0.5h: chart.go - ghép DTO {product_id, range, maturity, daily, annotations}, marshal JSON, đặt Content-Type"
  - "0.5h: router.go - đăng ký route sau JWT middleware của gateway (FR-INFRA-001)"
  - "1.0h: chart_test.go - 5 test (default 90d, annotation median/trailing, mốc ngày đôi, cờ WARMING, unknown product 404)"

risk_if_skipped: "Đây là feed duy nhất nuôi biểu đồ giá có chú giải (FR-WEB-003) - màn hình mà người dùng SănDeal mở để trả lời câu hỏi cốt lõi đây có phải sale thật không. Thiếu nó thì biểu đồ chỉ là đường giá trần trụi, không có dải verdict sale ảo, không có đường trailing_min, không có mốc ngày đôi, người dùng tự đoán. Nếu dựng sai bằng cách quét raw price_snapshot cho cả khoảng thì p95 vỡ NFR <500ms (§3.8), biểu đồ giật trên SKU lịch sử dài. Nếu tính verdict ở client thay vì gọi FR-DEAL-001 thì dải verdict trên biểu đồ lệch với nhãn trên thẻ sản phẩm, người dùng mất tin. Nếu bỏ qua cổng độ chín FR-DEAL-002 thì SKU mới <14 ngày bị dán nhãn SALE_XIN dựa trên 3 điểm dữ liệu - đúng kiểu kết luận ẩu mà SănDeal hứa loại bỏ."
---
...
5. **MUST** gắn verdict sale ảo bằng cách gọi đánh giá của **FR-DEAL-001** (`SALE_AO`, `SALE_XIN`, `TAM_DUOC`, hoặc `UNKNOWN`) - KHÔNG tự tính lại verdict ở DEAL-003 hay ở client (DEC-DEAL-21). Verdict trên biểu đồ phải khớp nhãn trên thẻ sản phẩm.
6. **MUST** lấy độ chín dữ liệu từ **FR-DEAL-002** và trả trong trường `maturity` (`MATURE`, `WARMING`, hoặc `NEW`) (DEC-DEAL-23). SKU `WARMING` vẫn trả biểu đồ kèm cờ `accumulating=true` (đang tích lũy); SKU `NEW` (<14 ngày) trả dữ liệu nhưng `verdict=UNKNOWN`.
...
11. **MUST** trả mọi giá (`min_p`, `max_p`, `close_p`, `median90`, `trailing_min`) dưới dạng `BIGINT` VND (int64) trong payload - KHÔNG dùng float hay string (đồng nhất với DEC-PRICE-05 của FR-PRICE-002).
12. **MUST** xác thực qua JWT do API Gateway (FR-INFRA-001) gắn; handler giả định đã qua middleware, không tự parse token. Request thiếu auth bị gateway chặn trước khi tới handler.
...
**Vì sao tách feed biểu đồ riêng thay vì dùng FR-PRICE-003?** FR-PRICE-003 trả chuỗi giá thô `{daily, tail}` - đường giá trần, ghép raw tail cho tươi tới phút, không chú giải. DEAL-003 trả `{daily, annotations}` - đã downsample về bucket ngày để vẽ và phủ thêm dải verdict, đường median90/trailing_min, mốc ngày đôi. Hai endpoint phục vụ hai nhu cầu khác nhau: PRICE-003 cho ai cần số giá nguyên bản (API tích hợp, B2B), DEAL-003 cho biểu đồ deal mà người dùng cuối nhìn để quyết "mua hay đợi". Trộn cả hai vào một endpoint làm hợp đồng rối và buộc client tự suy ra annotation.

**Vì sao tính chú giải phía server (DEC-DEAL-21)?** verdict sale ảo phải đồng nhất giữa thẻ sản phẩm và biểu đồ - nếu thẻ ghi SALE_XIN mà dải trên biểu đồ vẽ SALE_AO thì người dùng mất tin ngay. Gọi chung một nguồn (FR-DEAL-001) đảm bảo một verdict. median90 và trailing_min cũng nên tính một lần ở server: client chỉ vẽ, không phải tải cả chuỗi raw rồi tự tính lại - vừa lệch, vừa nặng.
...
**Vì sao tôn trọng cổng độ chín FR-DEAL-002 (DEC-DEAL-23)?** verdict trên 3 điểm dữ liệu là rác. SKU mới <14 ngày chưa đủ lịch sử để biết giá nền thật, nên trả `verdict=UNKNOWN` thay vì đoán liều. SKU WARMING (đủ vài tuần nhưng chưa đủ 90 ngày) vẫn vẽ được biểu đồ nhưng gắn cờ đang tích lũy để frontend hiển thị lưu ý. Đây là lời hứa cốt lõi của SănDeal: không kết luận ẩu.
...
5. `annotations.verdict` lấy từ FR-DEAL-001 (không tính lại); khớp nhãn thẻ sản phẩm.
6. `maturity` lấy từ FR-DEAL-002; `WARMING` kèm `annotations.accumulating=true`; `NEW` (<14 ngày) trả `verdict=UNKNOWN`.
...
Xem §3. Thứ tự: `annotate.go` (median90 + trailing_min + double_dates thuần, dễ unit-test) -> `chart.go` (DTO + handler: parse range, ProductExists, đọc price_daily, ghép verdict FR-DEAL-001 + maturity FR-DEAL-002) -> đăng ký route trong `router.go` sau JWT middleware của gateway (FR-INFRA-001) -> tests. Handler dùng `http.ServeMux` của Go 1.22 với pattern `GET /v1/products/{id}/chart` và `req.PathValue("id")`. Không thêm caching ở slice này; đọc price_daily + tính chú giải trong bộ nhớ đã đủ cho p95 <500ms.

---

## §7 - Phụ thuộc

- **FR-PRICE-002** - cung cấp continuous aggregate `price_daily` cho phần thân biểu đồ. Đây là điều kiện cứng (depends_on).
- **FR-DEAL-001 (sibling)** - nguồn verdict sale ảo (`SALE_AO`/`SALE_XIN`/`TAM_DUOC`); DEAL-003 gọi để gắn vào `annotations.verdict`, không tự tính.
- **FR-DEAL-002 (sibling)** - cổng độ chín dữ liệu; DEAL-003 lấy `maturity` và áp quy tắc WARMING/NEW.
- **FR-INFRA-001 (gateway)** - gắn JWT auth trước handler; route nằm sau middleware này.
- **FR-WEB-003 (downstream)** - biểu đồ giá có chú giải tiêu thụ response này; hình dạng JSON ở §3 là hợp đồng với nó.
...
| verdict client lệch verdict thẻ | so chéo nhãn | mất tin người dùng | Một nguồn verdict duy nhất FR-DEAL-001 (DEC-DEAL-21) |
| SKU NEW bị gắn verdict ẩu | maturity=NEW | kết luận sai trên ít dữ liệu | Ép `verdict=UNKNOWN` khi <14 ngày (DEC-DEAL-23) |
| FR-DEAL-002 chưa sẵn lúc gọi | maturity rỗng | thiếu cờ tích lũy | Mặc định an toàn: coi như WARMING tới khi đủ 90 ngày |
...
- **Phân biệt với FR-PRICE-003:** DEAL-003 và PRICE-003 đều phục vụ biểu đồ nhưng khác hợp đồng. PRICE-003 trả `{daily, tail}` - chuỗi giá thô, ghép raw tail cho tươi tới phút, KHÔNG chú giải; dành cho ai cần số giá nguyên bản. DEAL-003 trả `{daily, annotations}` - đã downsample về bucket ngày để vẽ và phủ thêm verdict sale ảo, đường median90/trailing_min, mốc ngày đôi; dành cho biểu đồ deal mà người dùng cuối đọc để quyết mua hay đợi. Một bên là feed thô, một bên là feed đã trang trí cho rendering.
...
- Cổng độ chín FR-DEAL-002 là rào chắn chống kết luận ẩu: NEW trả UNKNOWN, WARMING gắn cờ tích lũy - đúng lời hứa cốt lõi của SănDeal.
- Giá luôn là int64 VND suốt đường đi từ DB tới JSON, không có bước chuyển float nào để tránh sai số.

---

*Hết FR-DEAL-003. Status: ready_to_implement (mục tiêu audit 10/10).*
