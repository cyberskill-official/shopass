...
created: 2026-06-28
related_frs: [FR-AUTH-002, FR-NOTIF-002, FR-NOTIF-005, FR-MOBILE-002, FR-MOBILE-003]
depends_on: [FR-AUTH-002, FR-NOTIF-002]
blocks: [FR-MOBILE-002, FR-MOBILE-003]
source_pages:
...
source_decisions:
  - "DEC-MOBILE-01: chọn React Native (TypeScript) cho slice 1 - chia sẻ nhiều khái niệm với web app Next.js (FR-WEB) + hệ sinh thái push trưởng thành; Flutter là phương án thay thế đã cân nhắc nhưng không chọn ở slice này"
  - "DEC-MOBILE-02: auth dùng JWT access + refresh của FR-AUTH-002 (KHÔNG phát hành cơ chế auth riêng cho mobile); access token giữ trong bộ nhớ, refresh token lưu trong secure storage của HĐH (Keychain iOS / Keystore Android), KHÔNG lưu cleartext/AsyncStorage thường"
  - "DEC-MOBILE-03: đăng ký push qua FCM cho cả Android và iOS (FCM làm lớp trừu tượng trên APNs) -> backend đã có FCM dispatcher (FR-NOTIF-002); device token đăng về backend qua POST /v1/devices và gắn user_id qua bảng user_channel_token"
  ...
  - "DEC-MOBILE-05: mọi lời gọi backend đi qua API Gateway/BFF (FR-INFRA-001) bằng HTTPS với access token ở header Authorization; refresh tự động khi 401 do access token hết hạn"
...
disallowed_tools:
  ...
  - phát hành cơ chế auth riêng cho mobile thay vì JWT của FR-AUTH-002 (vi phạm DEC-MOBILE-02)
...
sub_tasks:
  ...
  - "2.0h: authClient.ts - login (gọi FR-AUTH-002), refresh khi 401, logout (xóa token + unregister device)"
...
4. **MUST** gọi backend qua API Gateway/BFF (FR-INFRA-001) bằng HTTPS với access token ở header `Authorization: Bearer ...` (DEC-MOBILE-05).
...
6. **MUST** đăng ký push qua FCM cho cả Android và iOS (FCM trừu tượng hóa APNs) (DEC-MOBILE-03); device token đăng về backend qua `POST /v1/devices` để gắn `user_id` vào `user_channel_token` (nguồn của FR-NOTIF-002/005).
...
10. **MUST** cung cấp điều hướng auth-gated: chưa đăng nhập -> màn Login; đã đăng nhập -> app shell (khung cho FR-MOBILE-002/003).
...
Vì sao React Native, không Flutter ở slice 1 (DEC-MOBILE-01)? Web app SănDeal là Next.js/TypeScript (FR-WEB). React Native dùng cùng ngôn ngữ + nhiều khái niệm UI, nên đội ngũ chuyển ngữ cảnh ít hơn và chia sẻ được logic (kiểu dữ liệu, client API). Flutter mạnh nhưng kéo theo Dart + một hệ sinh thái thứ hai. Slice 1 chọn React Native để ship nhanh; FR ghi rõ Flutter là phương án đã cân nhắc nếu sau này đổi.

Vì sao JWT chung, không auth riêng cho mobile (DEC-MOBILE-02, §1 #1)? Mỗi cơ chế xác thực là một bề mặt phải bảo mật + kiểm toán. Dùng lại JWT access/refresh của FR-AUTH-002 nghĩa là mobile thừa hưởng nguyên vòng đời token đã được thiết kế (hết hạn, refresh, thu hồi) thay vì phát minh lại. Một nguồn sự thật cho xác thực trên mọi client.
...
Vì sao FCM cho cả hai nền tảng (DEC-MOBILE-03, §1 #6)? Backend đã có FCM dispatcher (FR-NOTIF-002). FCM gửi được tới iOS qua APNs ngầm, nên một đường gửi phục vụ cả Android lẫn iOS - bớt một dispatcher phải vận hành ở giai đoạn đầu. FR-NOTIF-005 (APNs trực tiếp) dành cho khi cần kiểm soát sâu hơn ở iOS.
...
Xem §3. Thứ tự: scaffold RN -> tokenStore.ts (Keychain) -> authClient.ts (login/refresh/logout) -> httpClient.ts (auto-refresh) -> registerDevice.ts (FCM) -> RootNavigator.tsx -> tests. Backend cần endpoint `POST /v1/devices` + `DELETE /v1/devices` để gắn/gỡ `user_channel_token` (mở rộng nhẹ từ FR-NOTIF-002, vốn đã có bảng token). iOS cần cấu hình APNs auth key trong Firebase project để FCM gửi được tới iOS. Test chạy qua Jest với mock của Keychain/messaging/fetch.

---

## §7 - Phụ thuộc

- FR-AUTH-002 - JWT access + refresh + endpoint login/refresh; mobile dùng lại nguyên cơ chế.
- FR-NOTIF-002 - FCM dispatcher + bảng `user_channel_token`; mobile đăng device token để dispatcher gửi push.
- FR-NOTIF-005 (liên quan) - APNs dispatcher trực tiếp; mobile chuẩn bị nền iOS nếu sau này cần.
- FR-INFRA-001 (gateway) - mọi lời gọi backend qua gateway HTTPS + JWT.
...
- Hỗ trợ đăng nhập mạng xã hội trên mobile (Google/Zalo) - tái dùng FR-AUTH-004 qua deep-link OAuth ở slice sau.
...
| Auth riêng cho mobile | review | bề mặt xác thực thứ hai | Dùng lại JWT FR-AUTH-002 (DEC-MOBILE-02) |
...
- Một nguồn sự thật cho xác thực: JWT của FR-AUTH-002 dùng chung web + mobile, không phát minh lại.
- Một đường push (FCM) phục vụ cả Android + iOS ở giai đoạn đầu; FR-NOTIF-005 (APNs) cho khi cần kiểm soát sâu.
...
- FR này là khung; theo dõi giá + checkout (FR-MOBILE-002) và deep-link virality (FR-MOBILE-003) dựng lên trên.

---

*Hết FR-MOBILE-001. Status: ready_to_implement (mục tiêu audit 10/10).*
