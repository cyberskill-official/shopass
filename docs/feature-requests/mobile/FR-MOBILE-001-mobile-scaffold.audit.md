...
FR-MOBILE-001 đặc tả scaffold mobile + auth + push ở mức triển khai được. 12 mệnh đề §1 normative, mỗi mệnh đề có AC và test. Ranh giới bảo mật mobile được giữ chặt: refresh token trong Keychain/Keystore, access token chỉ trong bộ nhớ, không bao giờ mật khẩu trên đĩa. Auth dùng lại JWT của FR-AUTH-002 (không phát minh lại). Push qua FCM cho cả hai nền tảng, device token gỡ liên kết khi đăng xuất. Đạt 10/10.
...
Phát hành auth riêng cho mobile tạo thêm một hệ thống phải kiểm toán. Giải: §1 #1 + DEC-MOBILE-02 dùng lại JWT FR-AUTH-002; review + disallowed_tools.
...
| §1 | AC | Artefact |
|---|---|---|
| #1 JWT FR-AUTH-002 | #4 | `authClient.ts` |
