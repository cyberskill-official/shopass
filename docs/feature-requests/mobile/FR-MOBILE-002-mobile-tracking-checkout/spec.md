...
created: 2026-06-28
related_frs: [FR-MOBILE-001, FR-CART-003, FR-PRICE-003, FR-TRACK-002, FR-TRACK-003]
depends_on: [FR-MOBILE-001, FR-CART-003]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-MOBILE-10: màn theo dõi giá + wishlist + biểu đồ đọc qua API có sẵn (FR-PRICE-003 price-history, FR-TRACK-002 wishlist, FR-TRACK-003 alert_rule) - mobile là client mỏng, KHÔNG tính toán giá/sale phía client"
  ...
  - "DEC-MOBILE-12: optimizer voucher/giỏ chạy ở backend (FR-CART-003) - mobile gửi giỏ (đã tối thiểu hóa) + voucher khả dụng, nhận về tổ hợp tối ưu để HIỂN THỊ; mobile KHÔNG tự áp voucher hay tự thanh toán"
  ...
  - "DEC-MOBILE-14: push alert nhận qua FCM (FR-MOBILE-001) -> mở đúng màn sản phẩm (deep-link nội bộ); alert đã được engine backend (FR-TRACK-004) quyết định, mobile chỉ hiển thị"
...
language: "React Native 0.74 (TypeScript); biểu đồ qua victory-native; gọi backend qua httpClient của FR-MOBILE-001"
...
effort_hours: 10
sub_tasks:
  - "1.5h: trackClient.ts - gọi FR-TRACK-002 (wishlist) + FR-PRICE-003 (price-history) + FR-TRACK-003 (alert_rule)"
  ...
  - "1.5h: optimizerClient.ts - POST giỏ + voucher khả dụng tới FR-CART-003, nhận tổ hợp tối ưu (chỉ hiển thị)"
  - "1.0h: deep-link push -> mở màn sản phẩm đúng (tích hợp FCM message handler của FR-MOBILE-001)"
  - "1.0h: optimizerClient.test.ts + trackClient.test.ts - payload tối thiểu hóa; không auto-apply; đọc API đúng"

risk_if_skipped: "FR-MOBILE-002 là lớp giá trị người dùng của mobile app (§6 mục 6) - theo dõi giá, nhận alert, và universal checkout assistant là lý do người dùng mở app mỗi ngày. Không có nó thì scaffold FR-MOBILE-001 chỉ là vỏ rỗng. Rủi ro compliance là điểm dễ sai nhất: nếu checkout assistant tự động kích hoạt nền hoặc auto-apply voucher thì lặp lại đúng vết xe Honey (§4.2) mà SănDeal dựng cả định vị đạo đức để tránh - có thể bị sàn C&D hoặc store gỡ app. Nếu mobile tự tính sale ảo phía client thay vì đọc API thì logic giá phân mảnh giữa web/mobile/backend, dễ lệch và khó kiểm. Nếu gửi cookie/token sàn lên optimizer thì phá nguyên tắc tối thiểu hóa dữ liệu vốn là cam kết niềm tin cốt lõi. Checkout assistant phải tuyệt đối user-initiated + chỉ-hiển-thị, để người dùng tự áp mã trong app sàn."
---
...
1. **MUST** đọc danh sách theo dõi/wishlist từ FR-TRACK-002, lịch sử giá từ FR-PRICE-003, và quy tắc alert từ FR-TRACK-003 (DEC-MOBILE-10); mobile là client mỏng. **MUST NOT** tính toán sale ảo hay giá phía client.
2. **MUST** hiển thị biểu đồ lịch sử giá bằng dữ liệu trả về từ `GET /v1/products/{id}/price-history` (FR-PRICE-003); biểu đồ là tầng render, không tính toán giá.
3. **MUST** nhận push alert qua FCM (FR-MOBILE-001); chạm vào thông báo mở đúng màn sản phẩm liên quan qua deep-link nội bộ (DEC-MOBILE-14). Alert do engine backend (FR-TRACK-004) quyết định; mobile chỉ hiển thị.
...
5. **MUST** chạy tối ưu voucher/giỏ ở backend (FR-CART-003) (DEC-MOBILE-12): mobile gửi giỏ đã tối thiểu hóa + danh sách voucher khả dụng, nhận về tổ hợp tối ưu để hiển thị. Mobile **MUST NOT** tự áp voucher hay tự thanh toán.
...
8. **MUST** xử lý trạng thái rỗng/không đủ dữ liệu nhã nhặn: SKU chưa đủ lịch sử (FR-DEAL-002 trả UNKNOWN) hiển thị "chưa đủ dữ liệu" thay vì kết luận sai; optimizer không tìm được tổ hợp tốt hơn -> nói rõ "không có voucher áp dụng".
9. **MUST** dùng `httpClient` của FR-MOBILE-001 cho mọi lời gọi (kế thừa auth + auto-refresh + gateway).
...
11. **MUST** hiển thị disclosure rõ ràng khi gợi ý có yếu tố affiliate (nhất quán FR-AFFIL-004), để minh bạch với người dùng.

---

## §2 - Vì sao thiết kế này (rationale cho người đọc)

Vì sao mobile là client mỏng, đọc API (DEC-MOBILE-10, §1 #1)? Logic sale ảo (median90/p10/trailing_min) và tối ưu voucher (knapsack stacking) là lõi phức tạp, đã đặc tả ở FR-DEAL-001 và FR-CART-003 phía backend. Nếu mobile tự tính lại thì có hai (hoặc ba, cùng web) bản logic dễ lệch nhau - người dùng thấy "sale xịn" trên web nhưng "sale ảo" trên mobile cho cùng sản phẩm. Một nguồn sự thật ở backend; client chỉ hiển thị.
...
Vì sao chỉ hiển thị, người dùng tự áp mã (DEC-MOBILE-13, §1 #6)? Auto-apply/auto-click voucher trong app sàn là hành vi tự động hóa dễ bị coi là abuse và vi phạm policy sàn (tương tự ràng buộc trên web ở FR-CART-005). Mobile hiển thị "dùng mã X tiết kiệm Y" và để người dùng tự nhập trong app sàn - giữ con người trong vòng lặp, tránh tự động hóa rủi ro.

Vì sao optimizer ở backend, mobile gửi giỏ tối thiểu hóa (DEC-MOBILE-12/15, §1 #5, #7)? Tối ưu voucher cần dữ liệu catalog voucher + luật stacking per-country (FR-CART-003/004) sống ở backend. Mobile chỉ biết giỏ hiện tại. Gửi giỏ tối thiểu hóa (product_id/qty/giá, KHÔNG cookie/token) lên backend tính rồi nhận kết quả - vừa đúng nơi có dữ liệu, vừa giữ nguyên tắc tối thiểu hóa dữ liệu (không bao giờ gửi token sàn).

Vì sao xử lý trạng thái rỗng nhã nhặn (§1 #8)? Cold-start nghĩa là nhiều SKU chưa đủ 90 ngày lịch sử (FR-DEAL-002 trả UNKNOWN). Nếu mobile cố kết luận "sale xịn/ảo" trên dữ liệu thiếu thì sai và mất niềm tin. Hiển thị trung thực "chưa đủ dữ liệu" tốt hơn một kết luận bịa.
...
// gọi optimizer backend (FR-CART-003); chỉ HIỂN THỊ kết quả, KHÔNG tự áp.
...
9. Mọi lời gọi đi qua `httpClient` của FR-MOBILE-001 (kế thừa auth + auto-refresh).
...
Xem §3. Thứ tự: trackClient.ts (đọc wishlist/price-history/alert) -> trackScreen.tsx + priceChart.tsx -> optimizerClient.ts -> checkoutAssistant.tsx -> deep-link handler -> tests. Tất cả dựng trên `httpClient` của FR-MOBILE-001. Backend cần endpoint `POST /v1/cart/optimize` (mặt tiền của FR-CART-003) nhận giỏ tối thiểu hóa. Deep-link push tái dùng FCM message handler của FR-MOBILE-001, map `data.product_id` -> màn chi tiết. Test qua Jest + React Native Testing Library với mock http.

---

## §7 - Phụ thuộc

- FR-MOBILE-001 - scaffold + `httpClient` (auth/auto-refresh) + FCM handler; mọi thứ dựng lên trên.
- FR-CART-003 - optimizer voucher/giỏ ở backend; mobile gửi giỏ tối thiểu hóa, nhận tổ hợp tối ưu.
- FR-PRICE-003 - API lịch sử giá cho biểu đồ.
- FR-TRACK-002 / FR-TRACK-003 - wishlist + alert_rule.
- FR-TRACK-004 / FR-DEAL-002 (liên quan) - engine alert + xử lý cold-start UNKNOWN.
- FR-AFFIL-004 (liên quan) - chuẩn disclosure affiliate.
...
- So sánh giá chéo sàn ngay trong assistant (FR-PRICE-004) - thêm khi nhợp UX mobile.
...
| Thiếu disclosure affiliate | test disclosure banner | thiếu minh bạch | Banner khi has_affiliate (FR-AFFIL-004) |
...
- Dựng hoàn toàn trên httpClient của FR-MOBILE-001 nên kế thừa auth + auto-refresh + gateway sẵn.

---

*Hết FR-MOBILE-002. Status: ready_to_implement (mục tiêu audit 10/10).*
