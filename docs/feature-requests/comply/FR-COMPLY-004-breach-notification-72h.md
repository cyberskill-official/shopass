...
created: 2026-06-28
related_frs: [FR-COMPLY-001, FR-INFRA-004, FR-COMPLY-003, FR-NOTIF-001]
depends_on: [FR-COMPLY-001, FR-INFRA-004]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-COMPLY-18: tín hiệu phát hiện đến từ observability spine (FR-INFRA-004); breach record liên kết trace/alert nguồn"
...
7. **MUST** liên kết tín hiệu nguồn (DEC-COMPLY-18): `source_ref` trỏ tới alert/trace của observability spine (FR-INFRA-004) đã kích hoạt phát hiện, phục vụ điều tra.
8. **MUST** expose hàm:
...
**Vì sao liên kết observability spine (DEC-COMPLY-18)?** Phát hiện vi phạm thường bắt đầu từ một alert (truy cập bất thường, rò log). `source_ref` trỏ về alert/trace nguồn nối sự cố pháp lý với bằng chứng kỹ thuật, giúp điều tra nhanh và báo cáo có căn cứ. FR-INFRA-004 là nơi sinh tín hiệu đó.
...
CREATE TABLE breach_incident (
  ...
  source_ref            TEXT,                   -- alert/trace cua FR-INFRA-004
...
Xem §3. Thứ tự: migration 0006 -> types.go -> clock.go (đồng hồ thuần hàm) -> incident.go (state machine + ràng buộc Close) -> repo.go -> tests. `DeadlineFlag` và logic chuyển trạng thái tách thuần hàm để test không cần DB. Khi tích hợp, `Open` được gọi từ runbook ứng cứu sự cố (thủ công hoặc tự động từ alert FR-INFRA-004); `source_ref` mang ID alert/trace nguồn. Báo động `breach_overdue_total > 0` nối vào kênh on-call.

---

## §7 - Phụ thuộc

- **FR-INFRA-004** - observability spine sinh alert/trace; `source_ref` trỏ về đó.
- **FR-COMPLY-001 (liên quan)** - phân loại dữ liệu bị ảnh hưởng tham chiếu danh mục purpose.
- **FR-COMPLY-003 (liên quan)** - sự cố ảnh hưởng quyền chủ thể; thông báo chủ thể có thể trùng tập user của DSAR.
- **FR-NOTIF-001 (liên quan)** - kênh thông báo chủ thể dữ liệu có thể tái dùng hạ tầng notification.
- Lib: driver `pgx`.
...
- Tích hợp gửi thông báo chủ thể hàng loạt qua FR-NOTIF (khi danh sách ảnh hưởng lớn) - nối sau.
...
| Mất liên kết bằng chứng nguồn | `source_ref` §1 #7 | điều tra khó | Trỏ alert/trace FR-INFRA-004 |
...
- `source_ref` nối sự cố pháp lý với bằng chứng kỹ thuật từ observability spine (FR-INFRA-004).
- Khi danh sách chủ thể ảnh hưởng lớn, bước `notified_subjects` tái dùng hạ tầng FR-NOTIF (giai đoạn sau).

---

*Hết FR-COMPLY-004. Status: ready_to_implement (mục tiêu audit 10/10).*
