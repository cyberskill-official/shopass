...
created: 2026-06-28
related_frs: [FR-INFRA-002, FR-COMPLY-002, FR-COMPLY-003, FR-COMPLY-004, FR-COMPLY-005, FR-EXT-006, FR-AUTH-005]
depends_on: [FR-INFRA-002]
blocks: [FR-COMPLY-002, FR-COMPLY-003, FR-COMPLY-004, FR-COMPLY-007, FR-COMPLY-008, FR-EXT-006]
source_pages:
...
risk_if_skipped: "Không có khung consent đúng PDPL là rủi ro pháp lý hạng High (§9). Luật 91/2025 yêu cầu đồng thuận tự nguyện, cụ thể, đơn mục đích, tái lập được; im lặng không phải đồng thuận. Chế tài tới 5% doanh thu năm trước cho vi phạm xuyên biên giới, tới 3 tỷ VND cho vi phạm nghiêm trọng (§5.5). Mọi bề mặt xử lý dữ liệu cá nhân (extension đọc giỏ hàng, đăng ký, alert) phải tham chiếu bản ghi consent này làm cơ sở pháp lý. Đây là FR nền của toàn bộ module COMPLY: DPIA (FR-COMPLY-002), DSAR (FR-COMPLY-003), breach (FR-COMPLY-004), consent extension (FR-EXT-006) đều dựng trên nó."
---
...
6. **MUST** expose hàm service cho các service khác gọi như cổng pháp lý:
    ...
    - `History(ctx, userID int64, purpose string) ([]ConsentRecord, error)` - toàn bộ lịch sử cho DSAR (FR-COMPLY-003) và kiểm toán.
...
11. **MUST** bảo đảm `IsAllowed` là nguồn sự thật duy nhất cho cơ sở pháp lý: các service như EXT (FR-EXT-006), TRACK, NOTIF gọi `IsAllowed` trước khi xử lý dữ liệu của purpose tương ứng; thiếu consent -> từ chối xử lý.
...
- **FR-INFRA-002** - `app_user` phải tồn tại trước (FK `user_id`).
- **FR-EXT-006 (downstream)** - UI consent lúc cài extension gọi `Grant(cart_read)`; đọc giỏ hàng kiểm `IsAllowed(cart_read)`.
- **FR-COMPLY-003 (downstream)** - DSAR xuất `History` của user; xóa tài khoản tham chiếu trạng thái consent.
- **FR-AUTH-005 (downstream)** - vòng đời tài khoản (xóa) phối hợp thu hồi consent.
- **FR-COMPLY-002 (downstream)** - DPIA tham chiếu danh mục purpose làm cơ sở đánh giá tác động.
...
- Consent banner đa ngôn ngữ (EN/ID/TH) - gắn vào FR-COMPLY-007 khi mở SEA.
...
| Gộp nhiều purpose một nút | review §1 #3 | đồng thuận không cụ thể | Một purpose một bản ghi; tách nút trong FR-EXT-006 |
...
- Đây là FR nền pháp lý của SănDeal: mọi bề mặt xử lý dữ liệu cá nhân lấy cơ sở pháp lý từ `IsAllowed`.
...
- Khi mở SEA, adapter FR-COMPLY-007 tái dùng khung này, chỉ thay nội dung chính sách và bổ sung yêu cầu địa phương (Indonesia PDP, Thailand PDPA).

---

*Hết FR-COMPLY-001. Status: ready_to_implement (mục tiêu audit 10/10).*
