...
created: 2026-06-28
related_frs: [FR-INFRA-005, FR-CART-004, FR-COMPLY-007, FR-AFFIL-004]
depends_on: [FR-INFRA-005, FR-CART-004]
blocks: [FR-COMPLY-007]
source_pages:
...
source_decisions:
  ...
  - "DEC-COMPLY-26: tái dùng region/feature-flag hook của FR-INFRA-005; FR này cung cấp lớp luật, INFRA-005 cung cấp lớp định tuyến vùng"
...
6. **MUST** đảm bảo mã nghiệp vụ KHÔNG rải if-country (DEC-COMPLY-23): CART (FR-CART-004) lấy luật stacking qua `Value(country, voucher_stacking)`; AFFIL (FR-AFFIL-004) lấy kênh được phép qua `Value(country, affiliate_channel)`; quy ước này được audit (FR-COMPLY-005 có thể bổ sung rule).
7. **MUST** tái dùng region/feature-flag hook của FR-INFRA-005 (DEC-COMPLY-26): INFRA-005 quyết định "user thuộc nước nào"; FR này quyết định "nước đó được làm gì". Hai lớp tách bạch.
...
11. **MUST** đảm bảo `data_protection_regime` của một nước trỏ đúng chế độ để FR-COMPLY-007 chọn adapter (PDPL cho VN, PDP cho ID, PDPA cho TH).
...
**Vì sao tách INFRA-005 (vùng) khỏi COMPLY-006 (luật) (DEC-COMPLY-26)?** "User ở nước nào" là bài toán định tuyến vùng (INFRA-005: region config, feature flags). "Nước đó được làm gì" là bài toán luật (FR này). Trộn hai thứ làm cả hai khó bảo trì. Tách lớp: INFRA-005 trả `country`, COMPLY-006 trả `được phép gì` cho `country` đó.
...
**Vì sao data_protection_regime trỏ adapter (§1 #11)?** SEA có nhiều chế độ bảo vệ dữ liệu (PDPL VN, PDP Indonesia, PDPA Thailand). FR-COMPLY-007 cần biết nước này theo chế độ nào để chọn adapter tuân thủ tương ứng. `data_protection_regime` là điểm nối: gating khai báo chế độ, adapter thực thi chi tiết.
...
Xem §3. Thứ tự: migration 0007 (country_rule + seed) -> types.go (GateKey, RuleValue) -> rule.go (parse value theo gate) -> registry.go (Allow/Value deny-by-default + cache) -> repo.go (load + reload theo version) -> tests. Registry nạp ma trận hiệu lực vào bộ nhớ, reload khi version đổi (không cần triển khai lại mã nghiệp vụ). CART-004 và AFFIL-004 inject registry và gọi `Value`/`Allow`; không nơi nào hardcode luật theo nước. INFRA-005 cung cấp `country` của user; FR này nhận `country` đó và trả luật.

---

## §7 - Phụ thuộc

- **FR-INFRA-005** - region config xác định `country` của user (lớp định tuyến vùng).
- **FR-CART-004** - cart optimizer lấy luật `voucher_stacking` qua `Value`; thay if-country bằng tra ma trận.
- **FR-AFFIL-004 (liên quan)** - guardrails affiliate lấy kênh được phép qua `affiliate_channel`.
- **FR-COMPLY-007 (downstream)** - adapter SEA chọn theo `data_protection_regime`.
...
| Adapter SEA chọn sai regime | `data_protection_regime` + AC #6,#7,#8 | tuân thủ sai chế độ | Regime khai báo rõ; FR-COMPLY-007 đọc |
...
- `data_protection_regime` là điểm nối sang FR-COMPLY-007: gating khai báo chế độ (PDPL/PDP/PDPA), adapter thực thi chi tiết.

---

*Hết FR-COMPLY-006. Status: ready_to_implement (mục tiêu audit 10/10).*
