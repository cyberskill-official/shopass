...
Audit độc lập từ nội dung hiện tại. FR-COMPLY-002 đặc tả sổ DPIA/TIA: hạn nộp 60 ngày (filing_due_at = started_at + 60 days) + chu kỳ rà soát 6 tháng (review_due_at), trạng thái suy từ deadline không nhập tay, TIA bắt buộc khi cross_border. 12 mệnh đề §1 (11 MUST + 1 SHOULD gauge), testable; logic `Status` tách thuần hàm. Schema khớp DATA-MODEL.md (processing_activity CHECK cross_border->recipient_country, dpia UNIQUE(activity_id,version), tia). FR mở rộng dpia với mitigation_vi/filed_at/last_reviewed_at - nhất quán ghi chú "FR phát triển sâu hơn §3.4". PDPL P đạt. Đạt 10/10, PASS.
...
- ISS-002 (frontmatter A): id/module/folder khớp; key đủ; depends_on=[FR-COMPLY-001]. Pass.
