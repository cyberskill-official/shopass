...
created: 2026-06-28
related_frs: [FR-COMPLY-001, FR-COMPLY-006, FR-COMPLY-002, FR-COMPLY-003, FR-COMPLY-004]
depends_on: [FR-COMPLY-001, FR-COMPLY-006]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-COMPLY-27: bảo vệ dữ liệu SEA là lớp adapter theo regime trên khung lõi VN (FR-COMPLY-001/003/004), KHÔNG viết lại từ đầu mỗi nước"
  - "DEC-COMPLY-28: adapter chọn theo data_protection_regime của FR-COMPLY-006 (PDPL/PDP_ID/PDPA_TH); regime quyết định khác biệt theo nước"
...
disallowed_tools:
  ...
  - hardcode regime trong mã thay vì lấy từ FR-COMPLY-006 (vi phạm DEC-COMPLY-28)
...
sub_tasks:
  ...
  - "1.0h: registry.go - chọn adapter theo regime từ FR-COMPLY-006; chỉ nước đã mở"
...
5. **SHOULD** chọn adapter theo regime từ FR-COMPLY-006 (DEC-COMPLY-28): `registry.For(ctx, country)` đọc `data_protection_regime` của nước qua gating, trả adapter tương ứng. KHÔNG hardcode regime.
...
8. **SHOULD** expose `Profile(ctx, country) (RegimeProfile, error)` cho các FR khác (consent UI, DSAR SLA, breach window) đọc tham số đúng theo nước user.
...
**Vì sao chọn adapter theo regime của FR-COMPLY-006 (DEC-COMPLY-28)?** FR-COMPLY-006 đã khai báo `data_protection_regime` cho mỗi nước (PDPL/PDP_ID/PDPA_TH). Tái dùng điểm nối đó: `registry.For(country)` hỏi gating "nước này chế độ gì" rồi trả adapter tương ứng. Không hardcode regime trong adapter layer tránh hai nguồn sự thật mâu thuẫn.

**Vì sao chỉ kích hoạt cho nước đã mở (DEC-COMPLY-30)?** SEA sequencing có thứ tự (VN -> ID/TH -> PH/MY -> SG/TW). Một nước chưa mở thì chưa nghiên cứu xong luật và gating để deny-by-default. Adapter không nên chạy cho nước chưa sẵn sàng. `For` trả lỗi khi gating chưa mở nước đó - nhất quán với deny-by-default của FR-COMPLY-006.

**Vì sao khác biệt là dữ liệu, không nhánh điều kiện (§1 #11)?** Nếu khác biệt regime nằm trong `if regime == "PDPA_TH"` rải rác, thêm một nước SEA mới phải sửa khắp nơi. Đặt khác biệt vào `RegimeProfile` (các field tham số) làm việc thêm nước thành thêm một file profile khai báo - cùng triết lý khai báo của FR-COMPLY-006.
...
// services/comply/internal/regime/registry.go
// For chon adapter theo regime tu FR-COMPLY-006; chi nuoc da mo.
...
9. `Profile` đọc regime qua gating (FR-COMPLY-006), KHÔNG hardcode trong adapter.
...
Xem §3. Thứ tự: types.go (interface + RegimeProfile) -> vn_pdpl.go (baseline) -> id_pdp.go + th_pdpa.go (override) -> registry.go (chọn theo gating, chỉ nước đã mở) -> tests. Baseline là một hàm trả profile VN; mỗi adapter gọi `baseline()` rồi override field khác biệt - phần chung không lặp lại. `registry.For` đọc regime qua `gating.Value(country, GateDataRegime)` (FR-COMPLY-006), giữ một nguồn sự thật. Các FR consent UI / DSAR SLA / breach window đọc `Profile(country)` để áp tham số đúng nước.

---

## §7 - Phụ thuộc

- **FR-COMPLY-006** - cung cấp `data_protection_regime` mỗi nước + deny-by-default cho nước chưa mở; `For` đọc từ đây.
- **FR-COMPLY-001** - baseline consent PDPL; adapter điều chỉnh granularity/ngôn ngữ theo nước.
- **FR-COMPLY-002 (liên quan)** - DPIA filing window có thể khác theo regime (profile field).
- **FR-COMPLY-003 (liên quan)** - DSAR response SLA lấy từ profile theo nước.
- **FR-COMPLY-004 (liên quan)** - breach window lấy từ profile (baseline 72h).
- Lib: chỉ stdlib `time`.
...
| Hardcode regime trong adapter | review §1 #5 | hai nguồn sự thật | Đọc regime qua gating FR-COMPLY-006 |
...
- Adapter chọn theo `data_protection_regime` của FR-COMPLY-006; giữ một nguồn sự thật về "nước nào chế độ gì".
...
*Hết FR-COMPLY-007. Status: ready_to_implement (mục tiêu audit 10/10).*
