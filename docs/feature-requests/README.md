# SănDeal - Feature Requests (đọc đầu tiên)

Thư mục này là backlog Feature Request (FR) của SănDeal: 90 FR + 10 NFR đặc tả toàn bộ sản phẩm, viết theo workflow `feature-request` của CyberOS (engineering-spec@1). Trạng thái: v0.2.0 SHIP-READY - mọi FR đã qua audit độc lập 10/10, DAG phụ thuộc sạch, data model nhất quán. Đây là spec để xây, chưa có code; mục tiêu là một người hoặc một agent đọc xong là build được.
...
3. Đọc [`BACKLOG.md`](BACKLOG.md) để thấy bức tranh tổng: 4 phase, 16 module, 90 FR.
4. Mở một FR mẫu để xem một spec trông thế nào: [`price/FR-PRICE-002-price-snapshot-hypertable.md`](price/FR-PRICE-002-price-snapshot-hypertable.md).
...
3. Mở [`IMPLEMENTATION-ORDER.md`](IMPLEMENTATION-ORDER.md), lấy FR ở layer thấp nhất mà mọi `depends_on` đã `done`.
4. Đọc file FR đó, build theo `new_files` + `sub_tasks`, chạy test §5, đối chiếu acceptance criteria §4, lật status tới `done`.
...
| File | Là gì | Đọc khi |
|---|---|---|
| [`BACKLOG.md`](BACKLOG.md) | Index nguồn-sự-thật: 4 phase -> 16 module -> slice -> 90 FR, kèm priority/status/depends_on/effort + tổng quan + cổng thoát phase + risk register | Muốn bức tranh tổng, hoặc tra một FR thuộc đâu |
| [`SHIP-GUIDE.md`](SHIP-GUIDE.md) | Conventions build + 9 bất biến không thương lượng + stack theo module + quy ước viết | Trước khi build bất kỳ FR nào |
| [`IMPLEMENTATION-ORDER.md`](IMPLEMENTATION-ORDER.md) | Thứ tự build theo 8 layer topo (DAG acyclic + reciprocal) + critical path MVP | Chọn FR tiếp theo để build |
| [`DATA-MODEL.md`](DATA-MODEL.md) | Schema DB hợp nhất: ~50 bảng, mỗi bảng một owner FR, cột khóa + FK | Viết code đụng DB, kiểm tham chiếu bảng/cột |
| [`STATUS-REFERENCE.md`](STATUS-REFERENCE.md) | Enum 10 trạng thái + vòng đời FR + cách lật status | Khi đổi status một FR |
| `../../AGENTS.md` (gốc repo) | Giao thức memory CyberOS (BRAIN). KHÔNG phải conventions build | Môi trường có dùng memory CyberOS |
| File FR/NFR riêng | Đặc tả chi tiết từng yêu cầu | Khi build chính FR đó |
  ...
  docs/
    ...
    feature-requests/
      ...
      <module>/                      # 16 module (xem bảng dưới)
        FR-<MODULE>-NNN-<slug>.md     # đặc tả FR (engineering-spec@1)
...
## 16 module (90 FR qua 4 phase)

| Module | FR | Phase | Ngôn ngữ/stack | Lo việc gì |
|---|---:|---|---|---|
...
Phân bố phase: P0 = 5 FR (~33h), P1 = 46 FR (~303h), P2 = 25 FR (~177h), P3 = 14 FR (~114h). Tổng ~627h (~16 person-week thuần code).

## Đọc một FR thế nào

Mỗi FR là một file engineering-spec@1: frontmatter YAML + 11 mục thân. File mẫu chuẩn (gold standard): [`price/FR-PRICE-002-price-snapshot-hypertable.md`](price/FR-PRICE-002-price-snapshot-hypertable.md).

Frontmatter chính: `id`, `title`, `module`, `priority` (MUST/SHOULD/COULD/MAY), `status`, `phase`, `slice`, `depends_on` (FR phải xong trước), `blocks` (FR phụ thuộc vào cái này), `source_pages` (trỏ về tài liệu nguồn), `source_decisions` (DEC-<MODULE>-NN: quyết định thiết kế đã chốt), `language`, `service` (vị trí code), `new_files` (file cần tạo), `modified_files`, `allowed_tools`/`disallowed_tools`, `effort_hours`, `sub_tasks` (chia nhỏ + giờ), `risk_if_skipped`.
...
- §7 Phụ thuộc - FR + thư viện.
- §8 Payload ví dụ.
- §9 Câu hỏi mở.
- §10 Failure modes inventory - bảng lỗi + phát hiện + khắc phục.
- §11 Ghi chú.

Mỗi FR có file `.audit.md` đi kèm (`auditor: independent`, score 10/10) ghi lại verdict + traceability §1 -> AC -> artefact.

## Quy trình build cho agent (step-by-step)

1. Đọc SHIP-GUIDE.md (conventions + 9 bất biến). Đây là điều kiện tiên quyết.
2. Mở IMPLEMENTATION-ORDER.md. Lấy FR khả thi tiếp theo = FR ở layer thấp nhất chưa `done` mà mọi `depends_on` đã `done`. Trong cùng layer: `MUST` trước `SHOULD` trước `COULD`. Layer 0 là 4 root: FR-EXT-001, FR-INFRA-001, FR-INFRA-002, FR-INFRA-003.
3. Lật status FR từ `ready_to_implement` sang `implementing` (xem STATUS-REFERENCE.md), cập nhật ô status trên BACKLOG.
...
Critical path MVP (xây trước để chạy được vòng giá trị lõi): FR-INFRA-002 -> FR-PRICE-001 -> FR-PRICE-002 -> FR-DEAL-001/003 -> FR-WEB-003. Song song: FR-SCRAPE-001/002/005 (bắt đầu sớm để tích lũy 90 ngày dữ liệu cho sale ảo); FR-EXT-001/002/003.

## 9 bất biến KHÔNG thương lượng (tóm tắt - chi tiết ở SHIP-GUIDE.md)

Mọi FR phải giữ, dù FR không nhắc lại: (1) no-cleartext + token phiên sàn không rời client/không lưu server + argon2id + PDPL consent/DPIA/breach 72h; (2) affiliate chỉ user-initiated + disclosure, né Honey/cookie-stuffing, tuân Chrome policy 10/06/2025; (3) tiền BIGINT VND, không float; (4) giá ghi delta-only, price_snapshot là TimescaleDB hypertable, đọc biểu đồ từ price_daily; (5) extension MV3 (SW ephemeral, alarms >=30s, no global state, chỉ gửi productId/price/qty); (6) notification push>email>sms, FCM 600k/phút, flatten-the-curve, tách platform; (7) một table một owner FR, mở rộng bằng ALTER; (8) per-country gating (VN trước, MY/PH no-stack 2025); (9) scraping residential proxy + pacing, xu/voucher chỉ checklist + auto-test mã user-initiated.

## Đọc BACKLOG thế nào

BACKLOG tổ chức theo phase (P0 Nền tảng -> P1 MVP -> P2 Mở rộng -> P3 Tăng trưởng), trong mỗi phase theo module, trong mỗi module theo slice (đơn vị ship gọn; slice 1 luôn là bề mặt tối thiểu). Mỗi dòng là một FR với priority (BCP-14: MUST/SHOULD/COULD/MAY), status, depends_on, effort. §1 cho tổng quan, §7 cho thứ tự build, §8 cho tham chiếu chéo risk register.
...
- Status enum (10): `draft | ready_to_implement | implementing | ready_to_review | reviewing | ready_to_test | testing | done | on_hold | closed`. Mọi FR khởi tạo ở `ready_to_implement`.

## Sửa một FR an toàn

Nếu đổi `depends_on` của một FR: cập nhật cột Depends on trong BACKLOG.md, tính lại `blocks` (nghịch đảo của depends_on) cho mọi FR, regenerate IMPLEMENTATION-ORDER.md. Nếu đổi acceptance criteria đủ để dịch số AC: cập nhật bảng traceability trong file `.audit.md` tương ứng. Một table chỉ một owner FR - module khác cần bảng thì `depends_on` owner và tham chiếu, không re-create.

## Thuật ngữ

- FR (Feature Request): một yêu cầu nguyên tử, kiểm thử được; một file engineering-spec@1.
- NFR: ràng buộc phi chức năng (hiệu năng, bảo mật, chi phí, độ tin cậy) kèm SLO định lượng.
- audit: file `.audit.md` chấm điểm FR/NFR theo rubric; `auditor: independent` nghĩa là đã qua gate audit riêng (không tự-chấm).
...
Backlog dẫn xuất từ tài liệu nền tảng SănDeal v1.0 (16/06/2026), áp dụng workflow feature-request của CyberOS. Mỗi FR đã qua: tác giả -> audit độc lập (gate riêng, không tự-chấm) -> review chéo module -> kiểm tra DAG/data-model/typography bằng script. 90 FR + 10 NFR đều PASS 10/10. DAG acyclic, 0 lỗi reciprocity, 0 đụng độ schema. `source_pages` trong mỗi FR trỏ về mục tương ứng của tài liệu nguồn.

## Câu hỏi thường gặp (người mới)

- Code ở đâu? Chưa có. Đây là 90 spec để build; `services/`, `extension/`, `web/`, `mobile/` sẽ được agent tạo theo `new_files` của từng FR.
- Build cái gì trước? Layer 0 trong IMPLEMENTATION-ORDER: FR-EXT-001 + FR-INFRA-001/002/003.
- Một FR có "đủ" để build không? Có. Mỗi FR tự chứa (frontmatter + §1-§11) và đã qua audit độc lập 10/10, không cần hỏi lại tác giả.
