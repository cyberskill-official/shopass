...
created: 2026-06-28
related_frs: [FR-CART-001, FR-CART-002, FR-CART-004, FR-INFRA-001]
depends_on: [FR-CART-001, FR-CART-002]
blocks: [FR-CART-004, FR-MOBILE-002]
source_pages:
...
source_decisions:
  ...
  - "DEC-CART-14: validStack ủy quyền cho luật stacking per-country (FR-CART-004) qua interface StackRules; FR-CART-003 KHÔNG hardcode luật nước - chỉ gọi rules.ValidStack"
  ...
  - "DEC-CART-17: optimizer là hàm thuần (input: cart_items, vouchers, rules) - không tự đọc DB; caller (handler /v1/cart/optimize) nạp giỏ (FR-CART-002) + voucher còn hiệu lực (FR-CART-001 ListActive) rồi gọi"
...
disallowed_tools:
  - hardcode luật stacking per-country trong optimizer (vi phạm DEC-CART-14 - thuộc FR-CART-004 qua interface)
...
effort_hours: 12
sub_tasks:
  - "1.0h: types.go - CartItem, Voucher (từ FR-CART-001/002), OptimizeResult{discount, combo, breakdown}"
  - "1.0h: rules.go - interface StackRules{ValidStack(pv, fs, shopVouchers) bool}; FR-CART-004 hiện thực"
...
4. **MUST** kiểm `validStack(platformVoucher, freeship, shopVouchers)` qua interface `StackRules` do FR-CART-004 hiện thực (DEC-CART-14); FR-CART-003 KHÔNG hardcode luật stacking theo nước - chỉ gọi `rules.ValidStack`.
...
11. API `POST /v1/cart/optimize` **MUST** là caller: nạp giỏ (FR-CART-002) + voucher còn hiệu lực (FR-CART-001) cho `platform`, gọi `optimizeCart` thuần với `rules` của nước tương ứng (FR-CART-004), trả JSON `OptimizeResult`. Handler nằm sau JWT gateway (FR-INFRA-001).
...
**Vì sao luật stacking ủy quyền FR-CART-004 (DEC-CART-14)?** Luật xếp chồng đổi theo nước (VN cho stack 1 shop + 1 platform + freeship; MY/PH 2025 bỏ stack - §3.9, §5.7). Nếu hardcode luật VN vào optimizer thì mở nước mới phải sửa lõi thuật toán - dễ sai và vi phạm tách biệt. `optimizeCart` chỉ gọi `rules.ValidStack`; luật cụ thể nằm ở FR-CART-004 per-country. Lõi tính toán (chọn combo max) tách khỏi luật (combo nào hợp lệ).
...
### Interface luật stacking (rules.go) - FR-CART-004 hiện thực
...
4. `validStack` gọi `rules.ValidStack` (interface FR-CART-004); grep: optimizer KHÔNG hardcode luật nước (không có hằng VN/MY/PH).
...
Xem §3. Thứ tự: `types.go` (CartItem, Voucher, Vouchers, OptimizeResult, Breakdown) -> `rules.go` (interface StackRules, FR-CART-004 hiện thực) -> `discount.go` (`discountAmount`, `applyCap`, `freeshipValue` - int64, percent chia nguyên, test thuần) -> `optimize.go` (`filterByMinSpend`, `chooseBestShopVoucherPerShop`, vòng duyệt bám §3.5(3), `meetsMinSpend`, `breakdownOf`) -> `optimize.go` handler (`POST /v1/cart/optimize`: nạp giỏ FR-CART-002 + voucher FR-CART-001, gọi optimizer thuần với rules nước) -> tests. Optimizer là package thuần, test được với input cố định gồm cả ví dụ §3.5(3). Handler ghép I/O (DB) quanh hàm thuần. Luật `VNStackRules` dùng trong test là stub đơn giản; hiện thực đầy đủ per-country ở FR-CART-004.

---

## §7 - Phụ thuộc

- **FR-CART-001** - `voucher_catalog` + `ListActive` cấp tập voucher còn hiệu lực (shop/platform/freeship, cap, min_spend, stack_group) cho optimizer (depends_on cứng).
- **FR-CART-002** - `cart_snapshot` + `cart_item` cấp giỏ (qty, unit_price, shop_id) để tối ưu (depends_on cứng).
- **FR-CART-004 (downstream)** - hiện thực `StackRules.ValidStack` per-country (VN stack vs MY/PH no-stack); optimizer gọi interface này, không hardcode luật.
- **FR-INFRA-001 (gateway)** - JWT auth trước handler `/v1/cart/optimize`.
...
- Voucher điều kiện bậc thang trong optimizer - chờ FR-CART-001 mô hình hóa bậc thang.
- Tối ưu khi số voucher platform/freeship lớn (cận biên hiệu năng) - đo trước, thêm cắt tỉa nếu cần.
- Đa tiền tệ - giữ int64 minor unit; luật stacking đã tách per-country (FR-CART-004).
...
- Luật stacking ủy quyền FR-CART-004 qua interface - lõi tính toán (chọn combo max) tách khỏi luật (combo nào hợp lệ theo nước).
...
*Hết FR-CART-003. Status: ready_to_implement (mục tiêu audit 10/10).*
