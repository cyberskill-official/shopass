...
created: 2026-06-27
related_frs: [FR-INFRA-002, FR-AUTH-002, FR-AUTH-003, FR-AUTH-005, FR-COMPLY-005, FR-BILL-004]
depends_on: [FR-INFRA-002]
blocks: [FR-AUTH-002, FR-AUTH-003, FR-AUTH-005, FR-BILL-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-AUTH-02: app_user mở rộng từ cột lõi của FR-INFRA-002 qua migration mới; thêm pwd_hash, referral_code_id"
...
1. Migration `0004_app_user_secrets` **MUST** thêm vào `app_user` (đã có cột lõi từ FR-INFRA-002): `pwd_hash TEXT` và `referral_code_id BIGINT` (DEC-AUTH-02). KHÔNG định nghĩa lại cột lõi.
...
6. Đăng ký **MUST** chuẩn hóa email trước khi lưu (trim khoảng trắng) (DEC-AUTH-03); nhờ CITEXT unique (FR-INFRA-002), `Chi@Mail.com` và `chi@mail.com` xung đột - từ chối tạo trùng.
...
11. `app_user.status` (cột lõi) **MUST** mặc định `'active'` khi đăng ký; FR-AUTH-005 quản vòng đời status về sau.
...
Vì sao chuẩn hóa email + CITEXT (DEC-AUTH-03, §1 #6)? Người dùng gõ email tùy hứng hoa thường và có thể thừa khoảng trắng. Trim + CITEXT unique chặn một người tạo nhiều tài khoản qua `Chi@x.com` / `chi@x.com` / ` chi@x.com `, vốn làm rối billing và referral (FR-BILL-004).
...
-- FK referral_code_id -> referral_code(id) thêm khi FR-BILL-004 tạo bảng referral_code.
```
...
Xem §3. Migration 0004 thêm cột bảo mật vào `app_user` (cột lõi do FR-INFRA-002). `password.go` dùng `golang.org/x/crypto/argon2` với định dạng PHC tự mã hóa/giải mã tham số. `Verify` so sánh hằng thời gian (`subtle.ConstantTimeCompare`) chống timing attack. Tham số mặc định lấy từ cấu hình (cho phép nâng theo phần cứng) - có thể đọc qua FR-INFRA-003 nếu cần điều chỉnh tập trung. FK `referral_code_id -> referral_code(id)` hoãn tới khi FR-BILL-004 tạo bảng đó.

---

## §7 - Phụ thuộc

- FR-INFRA-002 - `app_user` cột lõi (id, email CITEXT unique, locale, status) phải tồn tại trước.
- FR-AUTH-002 (downstream) - phát hành JWT sau khi xác thực mật khẩu qua `Verify`.
- FR-AUTH-003 / FR-AUTH-005 (downstream) - liên kết sàn và vòng đời tài khoản gắn vào `app_user`.
- FR-BILL-004 (downstream) - tạo bảng `referral_code`; khi đó thêm FK cho `referral_code_id`.
- FR-COMPLY-005 (đồng hàng) - audit no-cleartext xác nhận chỉ có `pwd_hash` argon2id, không cleartext.
...
Đã chốt. Hoãn:
- Pepper toàn cục (khóa bí mật trộn vào mọi hash, lưu trong Vault) ngoài salt per-hash - tăng phòng thủ khi DB lộ; gắn FR-INFRA-003.
...
- Đây là điểm chứng minh no-cleartext cho FR-COMPLY-005: chỉ `pwd_hash` argon2id rời lớp này, không có mật khẩu thô.

---

*Hết FR-AUTH-001. Status: ready_to_implement (mục tiêu audit 10/10).*
