...
Audit độc lập từ nội dung hiện tại. FR-AUTH-001 đặc tả schema bảo mật `app_user` + đăng ký: argon2id định dạng PHC (chuỗi bắt đầu `$argon2id$`, lưu time/memory/parallelism + salt + hash), salt ngẫu nhiên mỗi lần, ConstantTimeCompare, email CITEXT chuẩn hóa, no-cleartext. 12 mệnh đề §1 (10 MUST + 2 SHOULD tham số/log), testable. Migration 0004 thêm pwd_hash/referral_code_id khớp DATA-MODEL.md (mở rộng app_user). Bất biến bảo mật D đạt: argon2id PHC, NEVER cleartext. Đạt 10/10, PASS.

## §2 - Findings

- ISS-001 (frontmatter A): id/module/folder khớp; key bắt buộc đủ; depends_on=[FR-INFRA-002], blocks 4 FR. Pass.
