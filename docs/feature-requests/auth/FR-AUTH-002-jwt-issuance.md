...
created: 2026-06-27
related_frs: [FR-AUTH-001, FR-INFRA-001, FR-INFRA-003, FR-AUTH-004, FR-AUTH-005, FR-WEB-001]
depends_on: [FR-AUTH-001, FR-INFRA-001]
blocks: [FR-AUTH-004, FR-EXT-005, FR-MOBILE-001, FR-WEB-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-AUTH-10: JWKS hỗ trợ nhiều kid (xoay khóa ký không downtime); khóa ký lấy qua FR-INFRA-003"
...
sub_tasks:
  ...
  - "1.0h: login.go - verify mật khẩu (FR-AUTH-001) -> cấp TokenPair; refresh -> cấp cặp mới"
...
1. Đăng nhập thành công (mật khẩu verify qua FR-AUTH-001) **MUST** trả một `TokenPair`: access JWT ngắn hạn + refresh token dài hạn (DEC-AUTH-06).
2. Access token **MUST** là JWT ký RS256 (DEC-AUTH-07). Khóa ký private lấy qua FR-INFRA-003 (`auth/jwt-signing`), KHÔNG nhúng trong code/env, KHÔNG chia sẻ với service khác.
3. Access JWT **MUST** mang claims: `user_id`, `locale`, `tier`, cùng claim chuẩn `exp`, `iat`, `iss`, `aud` (DEC-AUTH-08). `aud` đặt đúng để gateway (FR-INFRA-001 #2) verify khớp.
...
8. AUTH **MUST** hỗ trợ thu hồi: token bị `revoked_at` không cấp được cặp mới (dùng cho đăng xuất và FR-AUTH-005 suspend/delete).
...
Vì sao RS256 + JWKS, không HS256 (DEC-AUTH-07)? HS256 dùng một secret đối xứng: ai verify được cũng ký được. Nếu gateway giữ secret đó để verify, một lần lộ là giả mạo mọi phiên. RS256 tách private (chỉ AUTH ký) khỏi public (gateway verify qua JWKS). Gateway không bao giờ chạm khóa ký, giữ ranh giới phát-hành vs verify (FR-INFRA-001) sạch.
...
Vì sao nhiều kid trong JWKS (DEC-AUTH-10)? Xoay khóa ký định kỳ là thực hành tốt. Nếu JWKS chỉ có một khóa, lúc xoay sẽ có khoảng token cũ (ký khóa cũ) và token mới (ký khóa mới) cùng tồn tại. Giữ nhiều `kid` cho cả hai verify được trong giai đoạn chuyển, xoay không downtime - khớp với FR-INFRA-001 #12.
...
func (s *Service) issueAccess(u AppUser) (string, error) {
    ...
    return tok.SignedString(s.signingKey) // signingKey lấy qua FR-INFRA-003
}
```
...
Xem §3. `token.go` ký RS256 với private key lấy qua FR-INFRA-003; header JWT mang `kid` hiện hành. `jwks.go` build JWKS từ public key(s); giữ nhiều `kid` để xoay không downtime, khớp gateway refresh JWKS (FR-INFRA-001 #12). `refresh.go` lưu hash refresh + `family_id` cho rotation và theft detection. `login.go` nối: verify mật khẩu (FR-AUTH-001) -> `IssueTokenPair`. Gateway chỉ verify access JWT bằng JWKS, không gọi AUTH mỗi request - đó là lý do access TTL phải ngắn.

---

## §7 - Phụ thuộc

- FR-AUTH-001 - `Verify` mật khẩu trước khi cấp token; `app_user` cung cấp `locale`/`tier`.
- FR-INFRA-001 - gateway verify access JWT bằng JWKS này; `aud` phải khớp.
- FR-INFRA-003 - khóa ký RS256 lấy qua provider secret (`auth/jwt-signing`).
- FR-AUTH-005 (downstream) - suspend/delete tài khoản dùng thu hồi refresh token.
- FR-WEB-001 / FR-EXT-005 / FR-MOBILE-001 (downstream) - client dùng cặp token để gọi API.
...
- Đăng nhập bằng số điện thoại + OTP (ngoài email + mật khẩu) - gắn FR-NOTIF (SMS) khi mở.
...
- RS256 + JWKS tách phát-hành (AUTH ký bằng private) khỏi verify (gateway dùng public), giữ ranh giới FR-INFRA-001 sạch và không khóa ký nào rời AUTH.
...
*Hết FR-AUTH-002. Status: ready_to_implement (mục tiêu audit 10/10).*
