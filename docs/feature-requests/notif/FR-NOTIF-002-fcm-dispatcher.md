...
created: 2026-06-28
related_frs: [FR-NOTIF-001, FR-NOTIF-003, FR-NOTIF-004, FR-NOTIF-005, FR-INFRA-002]
depends_on: [FR-NOTIF-001]
blocks: [FR-MOBILE-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-NOTIF-14: dispatcher là consumer downstream của fan-out (FR-NOTIF-003); chỉ nhặt dòng notification channel='push' status='queued'; gửi xong cập nhật status=sent|failed + sent_at"
...
disallowed_tools:
  ...
  - tự routing/chọn kênh hay render lại nội dung (thuộc FR-NOTIF-001; dispatcher chỉ gửi push đã định kênh)
...
2. **MUST** lấy địa chỉ đích (FCM registration token) từ `user_channel_token` của FR-NOTIF-001: bản ghi `channel='push'`, `verified=true`, và `platform IN ('android','web')` (FCM chỉ phụ trách Android + Web; token `platform='ios'` thuộc FR-NOTIF-005/APNs và **MUST NOT** bị FCM nhặt). Token chưa verified hoặc đã bị vô hiệu **MUST NOT** được gửi tới.
3. **MUST** dựng JSON message HTTP v1 đúng cấu trúc: `message.token`, `message.notification.{title,body}` (đã render bởi FR-NOTIF-001), tùy chọn `message.data` (key-value string) cho deep-link, và override theo nền tảng (`webpush`, `android`) khi cần.
...
6. **MUST** phân loại phản hồi FCM thành ba nhóm và hành xử đúng:
    ...
    - Lỗi tạm thời (429 `RESOURCE_EXHAUSTED`, 500/503 `UNAVAILABLE`, timeout) -> retry theo backoff; vượt số lần thử tối đa thì `MarkFailed` để dead-letter (FR-NOTIF-003).
    - Lỗi token vĩnh viễn (404 `UNREGISTERED`, 400 `INVALID_ARGUMENT` cho token sai định dạng) -> token chết.
7. **MUST** xử lý `UNREGISTERED` / token không hợp lệ bằng cách set `user_channel_token.verified=false` cho `(user_id, 'push')` tương ứng (DEC-NOTIF-13) và `MarkFailed` dòng `notification`. Lần sau routing (FR-NOTIF-001) sẽ không coi token đó là khả dụng nữa - ngừng gửi vào token chết.
...
12. **MUST** giữ ranh giới trách nhiệm: dispatcher chỉ gửi push cho dòng đã được FR-NOTIF-001 chọn kênh và render. Nó **MUST NOT** tự chọn kênh, **MUST NOT** đổi nội dung, và **MUST NOT** gọi email/SMS provider.
...
Không cần migration mới - tái dùng `notification` và `user_channel_token` của FR-NOTIF-001. Chỉ bổ sung hàm repo.
...
// InvalidateToken: tắt token chết (UNREGISTERED) -> routing FR-NOTIF-001 thôi chọn push.
...
Xem §3. Thứ tự: `repo.go` (ClaimPushBatch + MarkSent/MarkFailed + InvalidateToken trên schema sẵn của FR-NOTIF-001) -> `message.go` (dựng JSON HTTP v1) -> `client.go` (Send + classify) -> `backoff.go` + `quota.go` (retry + van quota) -> `dispatcher.go` (vòng RunOnce: claim -> Allow -> Send -> mark) -> tests. Dispatcher chạy thành pool worker, mỗi worker `ClaimPushBatch` rồi gửi; `FOR UPDATE SKIP LOCKED` giữ an toàn khi scale ngang. Fan-out (FR-NOTIF-003) là thứ đẩy dòng vào `status='queued'` và lo rải đỉnh; dispatcher chỉ nhặt và gửi.

---

## §7 - Phụ thuộc

- **FR-NOTIF-001** - bảng `notification` (đọc `channel='push'`, cập nhật `status`/`sent_at`) và `user_channel_token` (đọc token `verified=true`, set `verified=false` khi token chết). Dispatcher KHÔNG routing/render - hai việc đó thuộc FR-NOTIF-001.
- **FR-NOTIF-003 (upstream)** - fan-out chuyển dòng `notification` push sang `status='queued'` và rải đỉnh (flatten-the-curve); dispatcher là consumer downstream nhặt các dòng đó. Dead-letter các dòng `status='failed'` cũng do fan-out lo.
...
Đã chốt. Hoãn:
- APNs (iOS) là dispatcher riêng (FR-NOTIF-005) - cùng vòng đời `notification` nhưng provider khác; không gộp vào FR này.
...
- Đo deliverability thật (push tới mà người dùng có thấy không) - cần tín hiệu client-side, để FR analytics sau.
...
- Ranh giới cứng với FR-NOTIF-001: dispatcher không routing, không render lại nội dung; chỉ gửi push cho dòng đã định kênh. Email/SMS có dispatcher riêng (FR-NOTIF-006/007).
...
*Hết FR-NOTIF-002. Status: ready_to_implement (mục tiêu audit 10/10).*
