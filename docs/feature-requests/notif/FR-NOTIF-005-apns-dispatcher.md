...
created: 2026-06-28
related_frs: [FR-NOTIF-001, FR-NOTIF-002, FR-NOTIF-003, FR-NOTIF-004, FR-INFRA-002]
...
source_decisions:
  ...
  - "DEC-NOTIF-54: dispatcher là consumer downstream của fan-out (FR-NOTIF-003); chỉ nhặt dòng notification channel='push' status='queued' cho thiết bị iOS (platform='ios'); gửi xong cập nhật status=sent|failed + sent_at. KHÔNG routing/render (việc của FR-NOTIF-001)"
...
disallowed_tools:
  ...
  - tự routing/chọn kênh hay render lại nội dung (thuộc FR-NOTIF-001; dispatcher chỉ gửi push iOS đã định kênh)
...
sub_tasks:
  ...
  - "0.5h: payload.go - dựng aps payload JSON (alert title/body đã render bởi FR-NOTIF-001, sound, badge, custom keys deep-link)"
  ...
  - "0.25h: repo.go - ClaimIOSPushBatch (SELECT ... FOR UPDATE SKIP LOCKED channel='push' status='queued' platform='ios'); tái dùng MarkSent/MarkFailed/InvalidateToken của FR-NOTIF-002"
...
4. **MUST** lấy `device_token` đích từ `user_channel_token` của FR-NOTIF-001: bản ghi `channel='push'`, `verified=true`, và thuộc thiết bị iOS. Token chưa verified hoặc đã bị vô hiệu **MUST NOT** được gửi tới.
...
6. **MUST** dựng aps payload JSON đúng cấu trúc: `aps.alert.{title,body}` (đã render bởi FR-NOTIF-001), tùy chọn `aps.sound`, `aps.badge`, và custom key ngoài `aps` (vd `product_id`, `deeplink`) cho điều hướng. Payload **MUST** giữ dưới giới hạn 4KB của APNs cho alert push.
...
8. **MUST** xử lý HTTP `410` (`Unregistered` - apns-id/device token không còn active) bằng cách set `user_channel_token.verified=false` cho `(user_id, 'push')` tương ứng (DEC-NOTIF-52) và `MarkFailed` dòng `notification`. Lần sau routing (FR-NOTIF-001) sẽ không coi token đó là khả dụng nữa - ngừng gửi vào token chết. HTTP `400 BadDeviceToken` xử lý cùng nhánh token chết.
9. **MUST** xử lý HTTP `500`/`503` (lỗi server APNs) bằng exponential backoff có jitter và **MUST** tôn trọng header `Retry-After` khi APNs trả về (DEC-NOTIF-53). Message bị 500/503 **MUST NOT** bị drop; nó được trả lại hàng đợi (giữ `status='queued'`) để thử lại sau, không mất thông báo. Vượt số lần thử tối đa thì `MarkFailed` để dead-letter (FR-NOTIF-003).
...
**Vì sao 410 set verified=false thay vì xóa (DEC-NOTIF-52)?** APNs trả `410 Unregistered` khi device token không còn active - máy đã gỡ app, hoặc iOS xoay vòng token. Gửi tiếp vào đó là đốt kết nối và công sức vào hư không, lại càng dễ nghẽn lúc cao điểm. Đặt `verified=false` (thay vì xóa hàng) giữ ranh giới sạch với FR-NOTIF-001 giống hệt cách FR-NOTIF-002 xử lý `UNREGISTERED` của FCM: routing chỉ coi bản ghi `verified=true` là khả dụng, nên token chết tự rớt khỏi vòng chọn kênh mà vẫn còn vết để chẩn đoán. Khi thiết bị đăng ký lại, token mới ghi đè và `verified` bật lại. Một cơ chế, hai nhà cung cấp - dễ suy luận và đối xứng với anh em FCM.

**Vì sao 500/503 phải backoff + tôn trọng Retry-After, không drop (DEC-NOTIF-53)?** `500 InternalServerError` và `503 ServiceUnavailable` là lỗi phía server APNs, thường thoáng qua. Đúng lúc đỉnh 00:00 là lúc mọi hệ thống chịu tải nặng nhất, kể cả APNs. Đây là tín hiệu "thử lại sau", không phải "bỏ đi". Drop thẳng nghĩa là người dùng iOS mất đúng cảnh báo họ chờ cả ngày chỉ vì một trục trặc server tạm thời. Backoff có jitter tránh cả đàn worker thử lại đồng pha (thundering herd) sau một đợt lỗi hàng loạt; tôn trọng `Retry-After` là làm theo đúng nhịp APNs yêu cầu thay vì đoán. Cùng triết lý retry như FR-NOTIF-002 áp cho 429/500/503 của FCM.

**Vì sao consumer của fan-out chứ không tự lấy thẳng từ DB (DEC-NOTIF-54)?** Fan-out (FR-NOTIF-003) là nơi áp flatten-the-curve và rải tải theo thời gian; dispatcher APNs chỉ là cái miệng cuối nhặt dòng `status='queued'` đã được fan-out chuẩn bị, lọc đúng thiết bị iOS. Tách như vậy cho phép nhiều dispatcher APNs chạy song song mà không phải tự lo việc rải đỉnh, và giữ logic gửi (pool kết nối, backoff, token JWT) gọn trong một chỗ. `FOR UPDATE SKIP LOCKED` làm việc nhặt an toàn khi scale ngang worker. FR-NOTIF-003 route các dòng `channel='push'` của thiết bị iOS sang dispatcher này, còn dòng push của Android/Web sang FR-NOTIF-002.

**Vì sao ranh giới cứng với FR-NOTIF-001 (§1 #12)?** FR-NOTIF-001 đã quyết kênh (push là rẻ nhất khả dụng) và render nội dung an toàn rồi. Nếu dispatcher lại đụng vào chọn kênh hay nội dung thì hai chỗ cùng sửa một thứ, khó test và dễ lệch. Dispatcher APNs dừng đúng ở "gửi token iOS này nội dung này qua APNs HTTP/2, ghi kết quả". Trách nhiệm đơn, test gọn, và đối xứng hoàn toàn với FR-NOTIF-002 ở phía FCM.
...
Không cần migration mới - tái dùng `notification` và `user_channel_token` của FR-NOTIF-001, và tái dùng `MarkSent`/`MarkFailed`/`InvalidateToken` đã thêm ở FR-NOTIF-002. Chỉ bổ sung hàm nhặt việc lọc thiết bị iOS.
...
// chạy song song không gửi trùng. MarkSent/MarkFailed/InvalidateToken tái dùng từ FR-NOTIF-002.
...
Xem §3. Thứ tự: `repo.go` (ClaimIOSPushBatch trên schema sẵn của FR-NOTIF-001, tái dùng MarkSent/MarkFailed/InvalidateToken của FR-NOTIF-002) -> `jwt.go` (ký JWT provider token ES256 từ .p8, cache <60 phút) -> `payload.go` (dựng aps JSON dưới 4KB) -> `client.go` (Send + classify) -> `pool.go` (nhiều kết nối HTTP/2 song song + round-robin) + `backoff.go` (retry 500/503) -> `dispatcher.go` (vòng RunOnce: claim -> Pick kết nối -> Send song song -> mark) -> tests. Dispatcher chạy thành pool worker, mỗi worker `ClaimIOSPushBatch` rồi gửi qua `Pool.Pick()`; `FOR UPDATE SKIP LOCKED` giữ an toàn khi scale ngang. Fan-out (FR-NOTIF-003) đẩy dòng push iOS vào `status='queued'` và lo rải đỉnh; dispatcher chỉ nhặt và gửi. Khóa `.p8`, `team_id`, `key_id`, `bundle_id` nạp từ cấu hình/secret store, không hard-code.

---

## §7 - Phụ thuộc

- **FR-NOTIF-001** - bảng `notification` (đọc `channel='push'`, cập nhật `status`/`sent_at`) và `user_channel_token` (đọc `device_token` `verified=true` của thiết bị iOS, set `verified=false` khi token chết). Dispatcher KHÔNG routing/render - hai việc đó thuộc FR-NOTIF-001.
- **FR-NOTIF-003 (upstream)** - fan-out chuyển dòng `notification` push sang `status='queued'`, rải đỉnh (flatten-the-curve), và route dòng push của thiết bị iOS sang dispatcher này; dispatcher là consumer downstream. Dead-letter các dòng `status='failed'` cũng do fan-out lo.
- **FR-NOTIF-002 (anh em)** - dispatcher FCM cho Android/Web là anh em của FR này: cùng vòng đời `notification`, cùng pattern claim/mark, cùng triết lý backoff và đánh dấu token chết (`UNREGISTERED` của FCM tương ứng `Unregistered` 410 của APNs); khác nhà cung cấp và cách auth (OAuth2 service-account vs JWT provider token .p8). `MarkSent`/`MarkFailed`/`InvalidateToken` được FR-NOTIF-002 thêm vào `repo.go` và FR này tái dùng.
- **FR-INFRA-002** - `app_user` cho FK `user_id` (gián tiếp qua schema FR-NOTIF-001).
...
- Dispatcher APNs là cái miệng cuối của kênh push cho iOS - anh em của FR-NOTIF-002 (FCM Android/Web); cùng vòng đời `notification`, cùng pattern claim/mark, khác nhà cung cấp và cách auth.
...
- 410 set `verified=false` thay vì xóa hàng: routing FR-NOTIF-001 tự thôi chọn push, vẫn còn vết chẩn đoán; thiết bị đăng ký lại sẽ ghi đè token và bật lại verified - đối xứng hoàn toàn với cách FR-NOTIF-002 xử lý `UNREGISTERED` của FCM.
...
- Ranh giới cứng với FR-NOTIF-001: dispatcher không routing, không render lại nội dung; chỉ gửi push iOS cho dòng đã định kênh. FCM (Android/Web), email, SMS có dispatcher riêng.

---

*Hết FR-NOTIF-005. Status: ready_to_implement (mục tiêu audit 10/10).*
