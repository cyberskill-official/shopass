...
created: 2026-06-28
related_frs: [FR-NOTIF-001, FR-NOTIF-003, FR-NOTIF-002, FR-NOTIF-005, FR-INFRA-002]
...
source_decisions:
  ...
  - "DEC-NOTIF-64: dispatcher là consumer downstream của fan-out (FR-NOTIF-003); chỉ nhặt dòng notification channel='email' status='queued'; nội dung Title/Body do template engine FR-NOTIF-001 render sẵn, dispatcher chỉ truyền đi; gửi xong cập nhật status=sent|failed + sent_at"
...
disallowed_tools:
  ...
  - tự routing/chọn kênh hay render lại Title/Body (thuộc FR-NOTIF-001; dispatcher chỉ truyền nội dung đã render)
...
3. **MUST** lấy địa chỉ đích (email) từ `user_channel_token` của FR-NOTIF-001: bản ghi `channel='email'`, `verified=true`. Địa chỉ chưa verified hoặc đã bị vô hiệu (đã hard bounce/complaint) **MUST NOT** được gửi tới.
4. **MUST** truyền nguyên Title/Body do template engine FR-NOTIF-001 render sẵn (DEC-NOTIF-64): `EmailMessage.Subject` lấy từ `Rendered.Title`, `EmailMessage.HTMLBody`/`TextBody` lấy từ `Rendered.Body`. Dispatcher **MUST NOT** chèn thêm hay render lại nội dung - escape an toàn đã làm ở FR-NOTIF-001.
...
7. **MUST** phân loại phản hồi provider thành bốn nhóm và hành xử đúng:
    ...
    - Lỗi tạm thời (throttling, `5xx`, timeout) -> retry theo backoff; vượt số lần thử tối đa thì `MarkFailed` để dead-letter (FR-NOTIF-003).
...
12. **MUST** giữ ranh giới trách nhiệm: dispatcher chỉ gửi email cho dòng đã được FR-NOTIF-001 chọn kênh và render. Nó **MUST NOT** tự chọn kênh, **MUST NOT** đổi nội dung, và **MUST NOT** gọi push/SMS provider.
...
Không cần migration mới - tái dùng `notification` và `user_channel_token` của FR-NOTIF-001. Chỉ bổ sung hàm repo (`MarkSent`/`MarkFailed` đã có từ FR-NOTIF-002, tái dùng).
...
// InvalidateEmail: tắt địa chỉ chết (hard bounce/complaint) -> routing FR-NOTIF-001 thôi chọn email.
...
// EmailMessage: nội dung đã render bởi FR-NOTIF-001, dispatcher chỉ truyền (§1 #4).
type EmailMessage struct {
    To       string // địa chỉ verified từ user_channel_token
    Subject  string // = Rendered.Title
    HTMLBody string // = Rendered.Body (đã escape ở FR-NOTIF-001)
...
Xem §3. Thứ tự: `repo.go` (ClaimEmailBatch + InvalidateEmailByAddress trên schema sẵn của FR-NOTIF-001; MarkSent/MarkFailed tái dùng từ FR-NOTIF-002) -> `provider.go` (interface EmailProvider + EmailMessage + SendOutcome) -> `ses.go` (SESProvider + classifySESError) -> `backoff.go` (retry) -> `bounce.go` (HandleSNS hard bounce + complaint) -> `dispatcher.go` (vòng RunOnce: claim -> Provider.Send -> mark) -> tests. Dispatcher chạy thành pool worker, mỗi worker `ClaimEmailBatch` rồi gửi qua `EmailProvider` đã tiêm; `FOR UPDATE SKIP LOCKED` giữ an toàn khi scale ngang. Provider tiêm qua constructor (`NewDispatcher(repo, provider)`) nên thay SES bằng SendGrid/Postmark chỉ là đổi tham số, không đụng dispatcher. Fan-out (FR-NOTIF-003) là thứ đẩy dòng vào `status='queued'` và lo rải đỉnh; dispatcher chỉ nhặt và gửi. SNS endpoint là một HTTP handler riêng (đăng ký SES bắn vào), gọi `HandleSNS`.

---

## §7 - Phụ thuộc

- **FR-NOTIF-001** - bảng `notification` (đọc `channel='email'`, cập nhật `status`/`sent_at`) và `user_channel_token` (đọc địa chỉ `verified=true`, set `verified=false` khi hard bounce/complaint). Title/Body do template engine FR-NOTIF-001 render sẵn; dispatcher chỉ truyền, KHÔNG render lại.
- **FR-NOTIF-003 (upstream)** - fan-out chuyển dòng `notification` channel='email' sang `status='queued'` và rải đỉnh (flatten-the-curve); dispatcher là consumer downstream nhặt các dòng đó. Dead-letter các dòng `status='failed'` cũng do fan-out lo.
- **FR-NOTIF-002 (anh em)** - dispatcher FCM cùng cấu trúc claim/mark/status-update; FR này gương theo để hai kênh nhất quán (push và email).
...
out, err := provider.Send(ctx, email.EmailMessage{
    ...
    Subject:  "Giá đã giảm về mức bạn chờ",   // = Rendered.Title (FR-NOTIF-001)
...
- DKIM/SPF/DMARC ký domain - cấu hình DNS phía hạ tầng (FR-INFRA), không thuộc lõi dispatcher; bật trước khi gửi production để vào inbox thay vì spam.
...
- Đo open/click rate (tracking pixel + link wrap) - cần cho tối ưu nội dung; để FR analytics sau, tránh đụng privacy sớm.
...
| Dispatcher render lại nội dung | code review | hỏng escape của FR-NOTIF-001 | Chỉ truyền Title/Body đã render (§1 #4) |
...
- Bounce/complaint từ SES SNS set `verified=false` thay vì xóa hàng: routing FR-NOTIF-001 tự thôi chọn email, vẫn còn vết chẩn đoán; phân biệt hard bounce (vĩnh viễn, vô hiệu) với soft bounce (tạm thời, retry).
...
- Ranh giới cứng với FR-NOTIF-001: dispatcher không routing, không render lại Title/Body; chỉ gửi email cho dòng đã định kênh. Push/SMS có dispatcher riêng (FR-NOTIF-002/007).
- Cấu trúc claim/mark/status-update gương đúng dispatcher FCM (FR-NOTIF-002) để hai kênh push và email nhất quán: cùng `FOR UPDATE SKIP LOCKED`, cùng `MarkSent`/`MarkFailed`, cùng backoff có jitter chống thundering herd lúc 00:00.

---

*Hết FR-NOTIF-006. Status: ready_to_implement (mục tiêu audit 10/10).*
