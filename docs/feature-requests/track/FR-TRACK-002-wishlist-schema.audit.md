...
Audit độc lập, tái diễn từ file FR-TRACK-002 hiện tại. FR đặc tả schema + API CRUD wishlist / wishlist_item với target_price BIGINT VND nullable và kiểm chủ sở hữu mọi route. 12 mệnh đề §1 BCP-14 (11 MUST + 1 SHOULD), testable. Schema khớp DATA-MODEL: wishlist_item có FK wishlist_id ON DELETE CASCADE + product_id -> tracked_product, target_price BIGINT CHECK > 0, UNIQUE(wishlist_id, product_id). Quyết định bảo mật 404-không-403 chống IDOR trên khóa BIGSERIAL tuần tự. Đạt 10/10.
