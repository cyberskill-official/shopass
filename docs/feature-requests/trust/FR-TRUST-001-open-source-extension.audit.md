...
Audit độc lập từ nội dung hiện tại (không tin .audit cũ). FR-TRUST-001 đặc tả extension mã nguồn mở + reproducible build + disclosure. Trục: reproducible build so SHA-256 bản ship với build-lại-từ-source (build-deterministic.mjs + verify-reproducible.sh, SOURCE_DATE_EPOCH + zip xác định). Disclosure neo vào allowlist FR-EXT-003 qua disclosure-consistency.test.ts (không lệch hành vi thật). CI publish gate biến ba bất biến (tái lập + có tag + disclosure khớp) thành điều kiện ship. 12 mệnh đề §1 (10 MUST + 1 MUST NOT obfuscation + 1 SHOULD hash). Không bảng DB (FR doc/build/test). Nền cho TRUST-003. Đạt 10/10, PASS.

## §2 - Findings

- ISS-001 (frontmatter A): id/module/folder khớp; key đủ; depends_on=[FR-EXT-001]. related_frs có NFR-TRUST-001 (hợp lệ, related có thể trỏ NFR). Pass.
