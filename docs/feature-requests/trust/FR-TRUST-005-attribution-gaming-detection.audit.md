...
Audit độc lập từ nội dung hiện tại. FR-TRUST-005 đặc tả delay payout + phát hiện gaming attribution. Trục: cửa sổ delay (hold-then-release, eligible_at = confirmed_at + delay_window) + ba điều kiện giải ngân (hết delay + không hold_reason + network-confirm). Ba dấu hiệu gaming: last-click thao túng, self-referral qua SameCluster (FR-TRUST-004), cookie-stuffing. Nguyên tắc: chỉ delay/giữ + điều tra, KHÔNG tự từ chối; denied chỉ sau confirmed_fraud. Job release idempotent (lock + status). 12 mệnh đề §1 (10 MUST + 2 MUST NOT/MUST tiền). Schema khớp DATA-MODEL.md. Tiền payout_amount BIGINT VND (D đạt). Đạt 10/10, PASS.

## §2 - Findings

- ISS-001 (frontmatter A): id/module/folder khớp; phase P3; key đủ; depends_on=[FR-AFFIL-001, FR-AFFIL-003, FR-BILL-002, FR-TRUST-004], blocks=[FR-AFFIL-005]. Pass.
