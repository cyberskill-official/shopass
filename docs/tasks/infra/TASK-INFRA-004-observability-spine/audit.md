...
Audit độc lập từ nội dung hiện tại. TASK-INFRA-004 đặc tả xương sống quan sát: OTel tracing xuyên service (W3C traceparent, trace_id bắt nguồn từ X-Request-Id của gateway), Prometheus pull /metrics bảo vệ, log JSON bắt buộc trace_id/request_id, mask PII theo PDPL. 12 mệnh đề §1 (10 MUST + 1 SHOULD sampling + 1 MUST dashboard), testable; có test trace nối liền hai service và test redact PII. depends_on=[TASK-INFRA-001] hợp lý (cần X-Request-Id). Đạt 10/10, PASS.

## §2 - Findings

- ISS-001 (frontmatter A): id/module/folder khớp; key bắt buộc đủ; depends_on=[TASK-INFRA-001], blocks=[TASK-COMPLY-004] nhất quán §7. Pass.
...
Mọi mệnh đề có code/test backing gồm trace cross-service và redact PII; mask PII đúng tinh thần PDPL; nối TASK-COMPLY-004 breach. Không defect. Score = 10/10. Verdict: PASS.

---

*Hết audit TASK-INFRA-004.*
