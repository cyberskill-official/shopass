...
created: 2026-06-27
related_frs: [TASK-INFRA-002, TASK-CART-004, TASK-AFFIL-004, TASK-COMPLY-006, TASK-COMPLY-007]
depends_on: [TASK-INFRA-002]
blocks: [TASK-CART-004, TASK-COMPLY-006]
source_pages:
...
source_decisions:
  ...
  - "DEC-INFRA-25: country của một thao tác suy ra từ platform.country (TASK-INFRA-002) hoặc locale người dùng; không đoán từ IP một mình"
...
risk_if_skipped: "Luật voucher, affiliate và dữ liệu khác nhau theo nước là ràng buộc cứng (§2). MY/PH đã bỏ stacking voucher 2025 - nếu optimizer (TASK-CART-003) áp luật VN cho MY/PH thì gợi ý sai luật, làm hỏng giỏ của người dùng và có thể vi phạm điều khoản sàn. Affiliate channel 'browser extension' bị cấm ở một số nước (ví dụ Shopee Indonesia) - bật nhầm là vi phạm ToS, bị reject affiliate. Data residency khác theo nước (Indonesia PDP, Thailand PDPA) - sai là vi phạm luật. Không tập trung gating thì các luật này rải khắp code, dễ sai và khó kiểm. Đây là nền cho mở SEA an toàn."
---
...
8. Country của một thao tác **MUST** suy ra từ `platform.country` (TASK-INFRA-002) hoặc locale người dùng, KHÔNG đoán chỉ từ IP (DEC-INFRA-25). IP chỉ là tín hiệu phụ.
9. `affiliate_channels_allowed` **MUST** dùng enum kênh rõ ràng (ví dụ `web`, `extension`, `coupon`, `app`); TASK-AFFIL-004 đọc đây để quyết định kênh nào hợp lệ ở nước hiện tại.
...
12. Lớp region **MUST** là điểm tham chiếu duy nhất cho TASK-CART-004 (stacking), TASK-AFFIL-004 (channel), TASK-COMPLY-006/007 (residency) - một nguồn sự thật per-country.
...
- TASK-INFRA-002 - `platform.country` là nguồn suy ra country của thao tác.
- TASK-CART-004 (downstream) - đọc `VoucherStackingAllowed` cho engine stacking per-country.
- TASK-AFFIL-004 (downstream) - đọc `AffiliateChannelsAllowed` để chặn kênh bị cấm theo nước.
- TASK-COMPLY-006 / TASK-COMPLY-007 (downstream) - đọc `DataResidencyRegion` cho gating dữ liệu SEA.
...
if !pol.VoucherStackingAllowed {
    // MY/PH: chỉ chọn max(platform, freeship) thay vì stack (TASK-CART-004)
}
```
...
Đã chốt. Hoãn:
- Bản đồ luật chi tiết hơn (cookie window per-network, ngưỡng giao dịch MOIT) - mở rộng CountryPolicy khi TASK-COMPLY-008 cần.
...
*Hết TASK-INFRA-005. Status: ready_to_implement (mục tiêu audit 10/10).*
