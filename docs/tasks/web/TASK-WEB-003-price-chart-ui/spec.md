---
id: TASK-WEB-003
title: "UI biểu đồ lịch sử giá - render p95 <500ms, tiêu thụ feed TASK-DEAL-003 (price_daily + overlay median90/trailing_min/verdict sale ảo/mốc ngày đôi), trả lời câu hỏi lõi sale thật hay ảo"
module: WEB
priority: MUST
status: done
verify: T
...
created: 2026-06-28
related_frs: [TASK-WEB-001, TASK-DEAL-003, TASK-DEAL-001, TASK-PRICE-002]
depends_on: [TASK-WEB-001, TASK-DEAL-003]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-WEB-11: biểu đồ tiêu thụ DUY NHẤT feed TASK-DEAL-003 (GET /v1/products/{id}/chart) - không tự gọi raw price_snapshot, không tự tính verdict/median; client chỉ vẽ"
...
language: "TypeScript 5.x; Next.js 14 (App Router, client component cho chart); thư viện chart (Recharts/visx); gọi feed TASK-DEAL-003 qua lib/api.ts (TASK-WEB-001)"
service: shopass/web/
new_files:
...
disallowed_tools:
  - gọi raw price_snapshot / tự dựng biểu đồ từ dữ liệu thô thay vì dùng feed TASK-DEAL-003 (vi phạm DEC-WEB-11, vỡ p95 <500ms)
...
effort_hours: 6
sub_tasks:
  - "0.5h: lib/chart/types.ts - mirror DTO feed TASK-DEAL-003 (daily[], annotations{median90,trailing_min,verdict,accumulating,double_dates}, maturity)"
...
5. UI **MUST** hiển thị nhãn verdict lấy thẳng từ `annotations.verdict` (`SALE_AO`, `SALE_XIN`, `TAM_DUOC`, hoặc `UNKNOWN`) - KHÔNG suy diễn lại ở client (DEC-WEB-13). Nhãn này phải đồng nhất với nhãn trên thẻ sản phẩm (cùng nguồn TASK-DEAL-001 qua feed).
...
10. Màn hình **MUST** nằm trong route group `(app)` (sau guard auth của TASK-WEB-001); request feed mang JWT do `lib/api.ts` gắn.
...
```ts
// web/lib/chart/types.ts - mirror DTO của TASK-DEAL-003
...
export async function fetchChart(productId: number, range: Range): Promise<ChartResponse> {
  ...
  const res = await apiFetch(`/v1/products/${productId}/chart?range=${range}`); // feed TASK-DEAL-003
...
Xem §3. Thứ tự: `lib/chart/types.ts` (mirror DTO feed, allowlist range) -> `lib/chart/fetch-chart.ts` (gọi feed qua apiFetch, validate range) -> `verdict-badge.tsx` + `range-selector.tsx` + `maturity-notice.tsx` (mảnh hiển thị nhỏ, test riêng) -> `price-chart.tsx` (vẽ thân daily + đường tham chiếu + mốc ngày đôi, ghép các mảnh) -> `products/[id]/chart/page.tsx` (client component trong shell (app)) -> tests. Chart là client component (`"use client"`) vì cần tương tác range; trang gọi `fetchChart` phía client sau khi shell (app) qua guard auth (TASK-WEB-001). Dùng thư viện chart nhẹ (Recharts/visx); không tự viết engine vẽ.

---

## §7 - Phụ thuộc

- **TASK-DEAL-003** - feed `GET /v1/products/{id}/chart` là nguồn dữ liệu DUY NHẤT (daily + annotations + maturity). Hình dạng JSON của nó là hợp đồng với UI này (depends_on cứng).
- **TASK-WEB-001** - scaffold + `lib/api.ts` (gắn JWT) + shell `(app)` + guard auth; màn hình chart nằm trong khu vực bảo vệ (depends_on cứng).
- **TASK-DEAL-001 (qua feed)** - nguồn verdict sale ảo; UI hiển thị `annotations.verdict`, không tính lại - đảm bảo nhãn khớp thẻ sản phẩm.
- **TASK-PRICE-002 (gián tiếp)** - `price_daily` nuôi feed; UI không chạm trực tiếp.
- **TASK-WEB-002 (upstream funnel)** - landing SEO dẫn CTA tới màn hình này; đây là đầu ra trải nghiệm của funnel.
...
### Feed nhận từ TASK-DEAL-003 (rút gọn)
...
Đã chốt. Hoãn:
- So sánh giá chéo 3 sàn trên cùng biểu đồ (overlay nhiều sàn) - chờ TASK-PRICE-004; slice này một SKU một sàn.
- Tô màu vùng theo verdict per đoạn (band per khoảng) khi feed trả dải verdict - bám DEC-DEAL của TASK-DEAL-003.
...
|---|---|---|---|
| Gọi raw price_snapshot tự dựng | grep + p95 metric | vỡ <500ms, biểu đồ giật | Chỉ tiêu thụ feed TASK-DEAL-003 (DEC-WEB-11) |
...
- Đây là màn hình trả lời câu hỏi lõi "sale thật hay ảo" - minh chứng trực quan cho lời hứa sản phẩm, và là đầu ra của funnel SEO (TASK-WEB-002).
- Client chỉ vẽ; mọi tính toán (downsample, median90, trailing_min, verdict) nằm ở feed TASK-DEAL-003 để giữ p95 <500ms và một nguồn verdict.
...
*Hết TASK-WEB-003. Status: ready_to_implement (mục tiêu audit 10/10).*
