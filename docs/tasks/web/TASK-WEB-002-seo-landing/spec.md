...
created: 2026-06-28
related_frs: [TASK-WEB-001, TASK-WEB-003, TASK-DEAL-001, TASK-DEAL-003]
...
6. Trang landing **MUST** công khai: thuộc group `(marketing)` ngoài `(app)`, KHÔNG qua middleware auth (TASK-WEB-001 #7), KHÔNG gọi API người dùng, KHÔNG yêu cầu đăng nhập (DEC-WEB-10).
...
10. Nội dung keyword **MUST** trung thực với định vị SănDeal: không hứa hẹn quá mức, gắn với năng lực thật (phát hiện sale ảo TASK-DEAL-001, biểu đồ giá TASK-WEB-003); tránh nội dung spam/cloaking (rủi ro phạt SEO + lệch thông điệp niềm tin).
...
export default function KeywordPage({ params }: { params: { keyword: string } }) {
  ...
  return (
    <article lang="vi-VN">
      <h1>{p.keyword}</h1>
      {/* nội dung định hướng tìm kiếm, gắn năng lực thật (TASK-DEAL-001 / TASK-WEB-003) */}
...
6. Trang landing thuộc group `(marketing)`, KHÔNG khớp matcher middleware auth của TASK-WEB-001; grep: không gọi `apiFetch` người dùng trên trang keyword.
...
Xem §3. Thứ tự: `lib/seo/keywords.ts` (nguồn sự thật cụm keyword) -> `lib/seo/jsonld.ts` (sinh structured data theo intent) -> `app/(marketing)/[keyword]/page.tsx` (`generateStaticParams` + `generateMetadata` + render SSG + JSON-LD + CTA) -> `app/(marketing)/layout.tsx` (layout công khai, không auth) -> `app/sitemap.ts` + `app/robots.ts` (sinh tự động từ keywords) -> tests. Trang marketing nằm ngoài matcher middleware của TASK-WEB-001 nên không bị guard. ISR (`export const revalidate = ...`) bật cho trang phụ thuộc thời gian như "lịch sale" nếu nội dung cần làm tươi định kỳ; các trang còn lại SSG thuần.

---

## §7 - Phụ thuộc

- **TASK-WEB-001** - cung cấp scaffold Next.js + route group; landing dùng group `(marketing)` công khai, nằm ngoài guard `(app)` (depends_on cứng).
- **TASK-DEAL-001 (nội dung tham chiếu)** - năng lực phát hiện sale ảo là nội dung thật cho trang "sale thật hay sale ảo"; landing không gọi API nhưng mô tả đúng năng lực.
- **TASK-WEB-003 (nội dung tham chiếu)** - biểu đồ giá là minh chứng cho thông điệp; CTA dẫn người dùng tới trải nghiệm này sau đăng ký.
...
- Nội dung động "lịch sale sắp tới" lấy từ dữ liệu mốc ngày đôi (TASK-DEAL-003 double_dates) - cân nhắc ISR khi có nguồn.
- Trang blog/nội dung dài cho long-tail keyword - giai đoạn SEO sâu hơn.
- Đa ngôn ngữ trang landing khi mở SEA (hreflang) - bám i18n nền của TASK-WEB-001.
...
*Hết TASK-WEB-002. Status: ready_to_implement (mục tiêu audit 10/10).*
