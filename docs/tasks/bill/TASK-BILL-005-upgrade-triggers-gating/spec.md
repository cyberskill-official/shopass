...
created: 2026-06-28
related_frs: [TASK-BILL-001, TASK-BILL-003, TASK-INFRA-001, TASK-TRACK-002, TASK-DEAL-004]
...
source_decisions:
  - "DEC-BILL-21: feature gating thực thi ở BACKEND theo subscription tier (TASK-BILL-001 GetActive); KHÔNG tin cờ tier do client gửi"
...
sub_tasks:
  ...
  - "1.5h: gate.go - Allow(ctx, userID, feature_key) -> đọc tier (TASK-BILL-001) + plan_feature -> so giới hạn; fail-safe free"
...
1. **MUST** định nghĩa bảng `plan_feature (id, tier, feature_key, limit_value)`: ánh xạ mỗi `tier` (TASK-BILL-001) tới giới hạn của từng `feature_key` (DEC-BILL-22). `limit_value = -1` nghĩa không giới hạn (unlimited); `0` nghĩa không có quyền.
2. **MUST** thực thi gating ở BACKEND (DEC-BILL-21): hàm `Allow(ctx, userID, feature_key)` đọc tier hiện tại qua TASK-BILL-001 `GetActive` rồi tra `plan_feature`. KHÔNG tin bất kỳ cờ tier/quyền nào do client gửi.
3. **MUST** giữ tính năng miễn phí lõi KHÔNG bị khóa cứng (DEC-BILL-23): theo dõi giá, phát hiện sale ảo (TASK-DEAL-001), biểu đồ giá (TASK-DEAL-003) phải khả dụng cho tier `free`. Gating chỉ áp GIỚI HẠN (số lượng/tần suất) và tính năng NÂNG CAO, không chặn giá trị lõi.
...
**Vì sao gating ở backend, không tin client (DEC-BILL-21)?** Nếu client tự quyết "user này là Premium" qua một cờ, ai cũng sửa được cờ đó để mở Premium miễn phí - thất thoát doanh thu. Quyền lợi phải được quyết ở nơi không sửa được từ ngoài: backend đọc subscription thật (TASK-BILL-001) rồi tra quyền. Client chỉ hiển thị theo những gì backend cho phép, không tự cấp quyền.
...
**Vì sao tách gating khỏi thanh toán (§1 #11)?** `Allow` chỉ trả lời "user hiện có quyền không" dựa subscription hiện tại. Nó không nên tự thu tiền hay kích hoạt Premium - đó là việc của checkout (TASK-BILL-002) và IPN (TASK-BILL-003). Tách bạch giữ gating đơn giản, dễ test, và tránh lẫn "kiểm quyền" với "đổi quyền".
...
Xem §3. Thứ tự: migration `0005_plan_feature.sql` (ánh xạ tier->feature->limit + seed; tính năng lõi free unlimited) -> `repo.go` (`LimitFor`, `CountUsage`) -> `gate.go` (`Allow` đọc tier TASK-BILL-001 + tra plan_feature + fail-safe free) -> `triggers.go` (`EvaluateTriggers` trả tín hiệu CTA) -> tests. `Allow` dùng làm middleware/kiểm trong các handler có giới hạn (ví dụ TASK-TRACK-002 wishlist gọi `Allow` trước khi thêm). Quyền lợi đổi bằng cập nhật `plan_feature`, không sửa code. Nâng cấp thật đi qua checkout (TASK-BILL-002), không phải `Allow`.

---

## §7 - Phụ thuộc

- **TASK-BILL-001** - `subscription` + `GetActive` + `plan_catalog` (tier); gating đọc tier hiện tại từ đây.
- **TASK-BILL-003 (liên quan)** - subscription chuyển `active` sau payment paid; gating phản ánh tier mới ngay.
- **TASK-INFRA-001 (gateway)** - gắn JWT + `user_id` cho các handler dùng `Allow`.
- **TASK-TRACK-002** - wishlist là feature có giới hạn (`wishlist_items`); gọi `Allow` trước khi thêm item.
- **TASK-DEAL-004** - dự đoán đáy nâng cao (`bottom_predict`) là tính năng Premium; gating kiểm tier.
- Lib: `pgx`.
...
### Kiểm gating khi thêm wishlist item (nội bộ, TASK-TRACK-002 gọi)
...
- Gamification sâu hơn (điểm, streak, mở khóa tạm tính năng Premium) - thêm lớp trên trigger; task này là nền gating + tín hiệu.
- Giới hạn theo tần suất thời gian (ví dụ X alert/ngày cho free) - thêm feature_key dạng rate + cửa sổ; hiện limit là đếm tổng.
- Hiển thị CTA cụ thể (copy, vị trí) - thuộc TASK-WEB/TASK-MOBILE; backend chỉ trả tín hiệu trung lập.
...
- Tách gating khỏi thanh toán: `Allow` kiểm quyền, checkout (TASK-BILL-002) đổi quyền.

---

*Hết TASK-BILL-005. Status: ready_to_implement (mục tiêu audit 10/10).*
