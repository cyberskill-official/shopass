...
TASK-BILL-005 đặc tả feature gating + upgrade trigger ở mức triển khai được. 12 mệnh đề §1 normative, mỗi mệnh đề có AC và test trong §5. Năm trụ được khóa: gating ở backend (không tin client), tính năng lõi không khóa cứng (free-tier mạnh), quyền lợi ở `plan_feature` (không hardcode), fail-safe về free khi lỗi, trigger là tín hiệu không ép (không dark pattern). Tách gating khỏi thanh toán. Đạt 10/10.

## §2 - Findings (đã giải quyết)

### ISS-001 - Tin cờ tier client
Client tự xưng Premium mở quyền miễn phí. Giải: §1 #2 + DEC-BILL-21 - `Allow` đọc subscription thật (TASK-BILL-001), bỏ qua cờ client; AC #7 + `TestAllow_IgnoresClientTierFlag`.
