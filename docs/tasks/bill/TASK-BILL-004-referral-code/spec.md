...
created: 2026-06-28
related_frs: [TASK-BILL-001, TASK-AUTH-001, TASK-TRUST-004, TASK-INFRA-002, TASK-MOBILE-003]
depends_on: [TASK-BILL-001]
blocks: [TASK-MOBILE-003, TASK-TRUST-004]
source_pages:
...
source_decisions:
  ...
  - "DEC-BILL-17: attribution gắn người được giới thiệu qua app_user.referral_code_id (TASK-AUTH-001) tại lúc đăng ký; bất biến sau khi gắn"
  - "DEC-BILL-18: CẤM tự giới thiệu (user dùng mã của chính mình) và CẤM gắn lại (đã có referrer thì không đổi)"
  - "DEC-BILL-19: referral KHÔNG tự trả thưởng ngay; phát một sự kiện cho TASK-TRUST-004 (anti-fraud) + delay reward tới khi vượt cửa kiểm tra (best practice §5.3)"
...
4. **MUST** gắn attribution lúc đăng ký (DEC-BILL-17): khi người mới đăng ký kèm một mã giới thiệu hợp lệ, đặt `app_user.referral_code_id` (TASK-AUTH-001) trỏ tới `referral_code` của người giới thiệu. Gắn xong là bất biến.
...
**Vì sao không tự trả thưởng ngay (DEC-BILL-19)?** Tài liệu nguồn (§5.3) nêu rõ delay payout là best practice ngành chống farming. Trả thưởng tức thì cho phép kẻ tạo hàng loạt tài khoản giả rút thưởng trước khi bị phát hiện. Phát một sự kiện cho anti-fraud (TASK-TRUST-004) và chỉ cấp thưởng sau khi qua velocity/đồ thị quan hệ chuyển rủi ro từ "trả rồi mới phát hiện" sang "kiểm rồi mới trả".
...
**Vì sao mã lỗi không chặn đăng ký (§1 #11)?** Đăng ký là hành động cốt lõi; nó không nên thất bại vì một mã giới thiệu gõ sai. Tách bạch: đăng ký luôn thành công nếu thông tin tài khoản hợp lệ (TASK-AUTH-001); attribution là phần thêm, mã lỗi chỉ làm bỏ qua gắn, không cản người dùng vào hệ thống.
...
-- app_user.referral_code_id (TASK-AUTH-001) trỏ tới referral_code(id) của người giới thiệu;
...
Xem §3. Thứ tự: migration `0004_referral_code.sql` (code UNIQUE + user_id UNIQUE) -> `code.go` (sinh mã từ alphabet loại ký tự nhầm) -> `repo.go` (`CreateCodeForUser`, `FindByCode`, `IncrementUses`, `SetReferrer`) -> `attribute.go` (gắn referrer + áp ràng buộc + phát sự kiện) -> tests. `Attribute` được gọi từ luồng đăng ký TASK-AUTH-001 khi có mã giới thiệu, nhưng lỗi của nó không lăn ngược đăng ký (§1 #11). Sự kiện `referral.attributed` lên bus (cùng hạ tầng fan-out TASK-NOTIF-003 hoặc bus nội bộ) cho TASK-TRUST-004 tiêu thụ. FK `app_user.referral_code_id -> referral_code(id)` thêm khi cả hai bảng tồn tại.

---

## §7 - Phụ thuộc

- **TASK-BILL-001** - module BILL + migration nền; referral sống cùng bill-svc.
- **TASK-AUTH-001** - `app_user.referral_code_id` (cột để gắn attribution); luồng đăng ký gọi `Attribute`.
- **TASK-INFRA-002** - `app_user` + quy ước migration.
- **TASK-TRUST-004 (downstream)** - tiêu thụ sự kiện `referral.attributed`, áp velocity/đồ thị quan hệ, quyết định trả thưởng (delay payout).
- **TASK-MOBILE-003 (downstream, P3)** - share-on-sale + referral dùng mã này cho virality.
...
### Đăng ký kèm mã giới thiệu (luồng TASK-AUTH-001 gọi Attribute)
...
Đã chốt. Hoãn:
- Cơ chế thưởng cụ thể (giảm giá Premium, ngày dùng thử) khi referral qua cửa anti-fraud - định ở TASK-TRUST-004 + TASK-BILL-005; task này chỉ attribution + sự kiện.
...
- Giới hạn số referral/người/ngày - đặt ở TASK-TRUST-004 (velocity), không ở schema.
...
- Không tự trả thưởng: phát sự kiện cho anti-fraud (TASK-TRUST-004) + delay payout chuyển từ "trả rồi phát hiện" sang "kiểm rồi trả".
...
- Đây là nguồn dữ liệu cho TASK-TRUST-004 (anti-fraud) và virality TASK-MOBILE-003.

---

*Hết TASK-BILL-004. Status: ready_to_implement (mục tiêu audit 10/10).*
