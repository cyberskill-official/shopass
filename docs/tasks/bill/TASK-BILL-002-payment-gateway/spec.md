...
created: 2026-06-28
related_frs: [TASK-BILL-001, TASK-BILL-003, TASK-INFRA-003, TASK-INFRA-001, TASK-COMPLY-005]
depends_on: [TASK-BILL-001]
blocks: [TASK-AFFIL-005, TASK-BILL-003, TASK-TRUST-005]
source_pages:
...
source_decisions:
  ...
  - "DEC-BILL-09: secret/khóa ký mỗi cổng đọc từ Vault (TASK-INFRA-003); KHÔNG cleartext; SănDeal KHÔNG lưu thông tin thẻ (PCI scope tối thiểu)"
...
2. **MUST** phục vụ `POST /v1/billing/checkout {plan_tier, gateway}`: tra `plan_catalog` (TASK-BILL-001) lấy `price` (BIGINT VND), sinh một `order_ref` idempotency, gọi adapter, trả `PaymentResult` (pay_url hoặc qr_payload tùy cổng).
...
4. **MUST** ký mọi request gửi cổng theo đúng spec ký của cổng đó (HMAC/checksum), với khóa bí mật đọc từ Vault (TASK-INFRA-003) (DEC-BILL-09); khóa KHÔNG nằm trong DB/code cleartext.
...
9. **MUST** ghi một bản ghi `payment` trạng thái khởi tạo (`pending`) gắn `order_ref` + `subscription`/user trước khi trả về (để TASK-BILL-003 đối soát khi IPN/webhook về). Tiền `amount` BIGINT VND.
10. **MUST** lấy `user_id` từ JWT (TASK-INFRA-001); plan_tier không tồn tại trong `plan_catalog` -> `400`; `gateway` ngoài tập -> `400`.
...
**Vì sao khóa ký từ Vault (DEC-BILL-09, §1 #4)?** Khóa ký là thứ chứng minh request đến từ SănDeal. Lộ khóa là lộ khả năng giả mạo yêu cầu thanh toán/hoàn tiền. Đọc khóa từ Vault (không để trong DB/code cleartext) tuân no-cleartext của PDPL (§5.5) và thu hẹp bề mặt lộ - đồng nhất cách TASK-AFFIL-003 xử lý postback secret.
...
func SignMoMo(ctx context.Context, secrets SecretReader, raw string) (string, error) {
    key, err := secrets.Get(ctx, "bill/momo/secret_key") // Vault (TASK-INFRA-003)
    if err != nil {
        return "", err
    }
...
func (h *Handler) HandleCheckout(w http.ResponseWriter, req *http.Request) {
    userID := auth.UserID(req.Context()) // TASK-INFRA-001
...
Xem §3. Thứ tự: `gateway.go` (interface + registry) -> `sign.go` (ký per-cổng, khóa Vault TASK-INFRA-003) -> bốn adapter `momo.go`/`zalopay.go`/`vnpay.go`/`vietqr.go` (tạo yêu cầu + parse phản hồi) -> `checkout.go` (handler: tra plan, sinh order_ref idempotent, gọi gateway, ghi payment pending) -> đăng ký route sau JWT middleware (TASK-INFRA-001) -> tests. Số tiền luôn lấy từ `plan_catalog` (TASK-BILL-001), không từ client. HTTP client tới cổng có timeout; lỗi giữ `payment` pending. Trạng thái thanh toán cuối do TASK-BILL-003 chốt qua IPN/webhook.

---

## §7 - Phụ thuộc

- **TASK-BILL-001** - `plan_catalog` (giá BIGINT theo tier) + `subscription`; checkout lấy giá từ đây.
- **TASK-INFRA-003 (Vault)** - đọc khóa ký mỗi cổng; khóa không nằm trong DB/code.
- **TASK-INFRA-001 (gateway)** - gắn JWT + `user_id`; định tuyến `POST /v1/billing/checkout`.
- **TASK-BILL-003 (downstream)** - nhận IPN/webhook cổng, đối soát `payment`, đẩy `subscription` sang active + `renews_at`.
- **TASK-COMPLY-005** - audit no-cleartext: khóa ký từ Vault, không lưu thẻ.
...
Đã chốt. Hoãn:
- Phí chính xác từng cổng theo hợp đồng merchant (§10 tài liệu nguồn - cần xác minh) - lưu vào cấu hình khi có hợp đồng; task này ưu tiên QR theo khoảng ~1,5-2,5%.
...
- Hoàn tiền (refund) qua API cổng - thêm khi có quy trình hủy/DSAR; task này chỉ tạo thanh toán.
...
| Trạng thái thật chưa chốt | payment pending | - | TASK-BILL-003 chốt qua IPN |

---

## §11 - Ghi chú

- TASK-BILL-002 là cửa thu tiền của Premium: không có nó, TASK-BILL-001 không thành doanh thu.
...
- Lỗi gọi cổng giữ payment pending - nguồn sự thật về thanh toán là cổng (qua IPN TASK-BILL-003), không phải kết quả lời gọi.
- Số tiền luôn từ plan_catalog, không từ client - chống thao túng giá.

---

*Hết TASK-BILL-002. Status: ready_to_implement (mục tiêu audit 10/10).*
