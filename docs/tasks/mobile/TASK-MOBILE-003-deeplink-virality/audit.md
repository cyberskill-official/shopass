...
TASK-MOBILE-003 đặc tả deep-link + share-on-sale virality + referral ở mức triển khai được. 11 mệnh đề §1 normative, mỗi mệnh đề có AC và test. Ranh giới chống-spam + chống-gian-lận được giữ chặt: share user-initiated (không tự đăng nền), referral_code lấy từ TASK-BILL-004 (không tự sinh), self-referral chặn hai lớp, link công khai chỉ mang product_id + ref ngắn (không token/PII). Trả thưởng + anti-fraud do backend gác cổng. Đạt 10/10.
...
Bỏ qua hook anti-fraud (TASK-TRUST-004), mở cửa farming. Giải: §1 #2/#8 + DEC-MOBILE-21/24 mã từ TASK-BILL-004, thưởng do backend + delay; test `mobile không tự sinh mã`.
...
| #2 mã từ TASK-BILL-004 | #2 | `myReferralCode` + test |
| #3 user-initiated | #4 | `ShareSheet.tsx` + review |
| #4 route + ref pending | #5 | `linkHandler.ts` + test |
| #5 attribution lúc đăng ký | #6 | `authClient.ts` + TASK-BILL-004 |
