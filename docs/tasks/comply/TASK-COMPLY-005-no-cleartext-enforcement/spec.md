...
created: 2026-06-28
related_frs: [TASK-INFRA-003, TASK-AUTH-001, TASK-AUTH-003, TASK-EXT-003, TASK-TRUST-003]
depends_on: [TASK-INFRA-003]
blocks: [TASK-TRUST-003]
source_pages:
...
source_decisions:
  ...
  - "DEC-COMPLY-21: mật khẩu chỉ tồn tại dạng argon2id PHC (tái dùng TASK-AUTH-001); secrets ứng dụng chỉ trong Vault (tái dùng TASK-INFRA-003)"
...
risk_if_skipped: "Đây là điểm chứng minh kỹ thuật cho lời hứa niềm tin cốt lõi của SănDeal (hậu-Honey): KHÔNG gửi cookie/mật khẩu, token không rời client (§5.4). ~45% người tiêu dùng VN lo ngại lừa đảo/lộ dữ liệu (Ken Research) - nếu lộ ra backend có lưu token phiên sàn thì sụp đổ niềm tin và vi phạm PDPL hạng High (§9). TASK-AUTH-001 đặt argon2id, TASK-INFRA-003 đặt Vault, nhưng không có cưỡng chế tự động thì quy tắc bị phá âm thầm qua một commit sơ ý. CI gate + payload guard biến lời hứa thành ràng buộc máy kiểm. TASK-TRUST-003 (audit độc lập) dựa trên cưỡng chế này."
---
...
6. **MUST** tái dùng argon2id PHC của TASK-AUTH-001 cho mật khẩu (DEC-COMPLY-21): quy tắc quét xác nhận không có đường ghi mật khẩu nào ngoài hàm hash chuẩn.
7. **MUST** tái dùng Vault/Secrets Manager của TASK-INFRA-003 cho secrets (DEC-COMPLY-21): quy tắc quét cấm secret literal trong mã và file `.env` commit.
...
**Vì sao đây là nền cho TASK-TRUST-003 (§7)?** Audit bảo mật độc lập (TASK-TRUST-003) cần chứng minh "không gửi cookie/mật khẩu". Bộ quét + payload guard ở đây cung cấp bằng chứng tự động, lặp lại được mà auditor có thể chạy. Cưỡng chế nội bộ làm audit độc lập đáng tin hơn.
...
var bannedRules = []Rule{
    {"cleartext_password",
        ...
        "Mat khau phai la pwd_hash argon2id (TASK-AUTH-001)"},
    ...
    {"hardcoded_secret",
        ...
        "Secret phai doc tu Vault (TASK-INFRA-003)"},
    {"weak_password_hash",
...
Xem §3. Thứ tự: rules.go (tập quy tắc) -> scan.go (walk + match + allowlist + sort) -> scan_test.go -> payload_guard.go -> payload_guard_test.go -> no_cleartext_gate.sh -> nối vào ci.yml. Gate chạy ở mọi PR, chặn merge nếu đỏ. `GuardPayload` được gọi tại biên BFF/EXT-ingest (TASK-EXT-003) trên mọi payload nhận từ extension trước khi xử lý. Quy tắc nhắm danh sách token SÀN cụ thể để hạn chế false-positive; allowlist dòng cho ngoại lệ hiếm và hợp lệ.

---

## §7 - Phụ thuộc

- **TASK-INFRA-003** - Vault/Secrets Manager; quy tắc `hardcoded_secret` cưỡng chế đọc secret từ đây.
- **TASK-AUTH-001** - argon2id PHC; quy tắc `cleartext_password`/`weak_password_hash` cưỡng chế.
- **TASK-AUTH-003 (liên quan)** - liên kết tài khoản sàn KHÔNG lưu token; quy tắc `platform_session_token` bảo vệ.
- **TASK-EXT-003 (liên quan)** - tối thiểu hóa dữ liệu client; `GuardPayload` là lớp biên backend tương ứng.
- **TASK-TRUST-003 (downstream)** - audit độc lập dùng bộ quét + guard làm bằng chứng tự động.
...
| Secret hardcode trong mã | `Scan` rule hardcoded_secret + AC #3 | rò khóa | Ép đọc từ Vault (TASK-INFRA-003) |
...
- Tái dùng argon2id (TASK-AUTH-001) và Vault (TASK-INFRA-003): task này cưỡng chế hai quy tắc đó tự động, không phát minh lại.
- Allowlist có lý do bắt buộc cân bằng giữa gate cứng và false-positive; mọi ngoại lệ có dấu vết và bị review.
- TASK-TRUST-003 (audit độc lập) dựa trên bộ quét + guard này làm bằng chứng tự động lặp lại được.

---

*Hết TASK-COMPLY-005. Status: ready_to_implement (mục tiêu audit 10/10).*
