...
Audit độc lập từ nội dung hiện tại. TASK-COMPLY-008 đặc tả theo dõi nghĩa vụ TMĐT VN: đăng ký MOIT (NĐ 52/2013), trách nhiệm sàn + ngưỡng 100.000 giao dịch/năm foreign platform (NĐ 85/2021), disclosure livestream + affiliate (dự thảo Luật TMĐT 2025). Cờ must_register suy tự động từ bộ đếm; ngưỡng + checklist cấu hình versioned một chỗ; giá trị dự thảo 2025 đánh dấu "chờ luật chốt". 12 mệnh đề §1 (priority SHOULD, cấp P3, SHOULD/MAY đúng). Schema khớp DATA-MODEL.md (ecommerce_obligation source_law CHECK ND_52_2013|ND_85_2021|DRAFT_2025, yearly_transaction_count, compliance_threshold). PDPL/TMĐT P đạt. Đạt 10/10, PASS.
...
- ISS-002 (frontmatter A): id/module/folder khớp; priority SHOULD + phase P3 hợp lệ; depends_on=[TASK-COMPLY-001]. Pass.
- ISS-003 (contract D): 3 bảng DDL khớp DATA-MODEL owner; ecommerce_obligation status CHECK (task mở rộng tập trạng thái sâu hơn DATA-MODEL "status TEXT" - nhất quán); compliance_threshold UNIQUE(key,version). Pass.
