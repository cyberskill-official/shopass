...
created: 2026-06-28
related_frs: [TASK-COMPLY-001, TASK-COMPLY-003, TASK-COMPLY-004, TASK-COMPLY-007, TASK-B2B-001]
...
1. **MUST** định nghĩa bảng `processing_activity (id, name, purpose_key, data_categories, started_at, cross_border, recipient_country, created_at)` - mỗi hoạt động xử lý đăng ký một dòng. `purpose_key` liên kết tập purpose của TASK-COMPLY-001.
...
**Vì sao gắn `purpose_key` với TASK-COMPLY-001 (§1 #1)?** DPIA và consent là hai mặt của cùng một hoạt động xử lý: consent là cơ sở pháp lý, DPIA là đánh giá rủi ro. Liên kết qua `purpose_key` giữ hai sổ đồng bộ - mỗi mục đích xử lý vừa có consent vừa có DPIA.
...
- **TASK-COMPLY-001** - tập purpose (`purpose_key`) là cơ sở liên kết hoạt động xử lý với consent.
- **TASK-COMPLY-007 (downstream)** - adapter SEA dùng cross_border + TIA khi dữ liệu rời VN sang ID/TH.
- **TASK-B2B-001 (liên quan)** - pipeline dữ liệu B2B ẩn danh là một `processing_activity` cần DPIA riêng.
- **TASK-COMPLY-003 (liên quan)** - DSAR có thể tham chiếu danh mục `data_categories` của hoạt động.
- Lib: driver `pgx`.
...
- Liên kết `purpose_key` giữ sổ DPIA đồng bộ với sổ consent (TASK-COMPLY-001): mỗi mục đích xử lý có cả hai.
- Khi mở SEA (TASK-COMPLY-007), mỗi luồng chuyển dữ liệu là một activity cross_border kèm TIA ghi rõ nước nhận và cơ chế bảo vệ.
- DPIA versioned cho phép chứng minh lịch sử rà soát từng chu kỳ 6 tháng trước cơ quan.

---

*Hết TASK-COMPLY-002. Status: ready_to_implement (mục tiêu audit 10/10).*
