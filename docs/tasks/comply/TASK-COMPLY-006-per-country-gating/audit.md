...
Audit độc lập từ nội dung hiện tại. TASK-COMPLY-006 đặc tả khung per-country gating: ma trận luật khai báo (country_rule versioned) thay if-country rải rác, cổng Allow/Value deny-by-default, ba GateKey (voucher_stacking, affiliate_channel, data_protection_regime). 12 mệnh đề §1 (11 MUST + 1 SHOULD metric), testable. Seed khớp source §5.7 (MY/PH bỏ stacking 2025). Tách lớp với INFRA-005 (vùng) + nối COMPLY-007 (regime). country_rule khớp DATA-MODEL.md (country CHECK len=2, gate_key, value JSONB, UNIQUE(country, gate_key, version)). Đạt 10/10, PASS.

## §2 - Findings

- ISS-001 (frontmatter A): id/module/folder khớp; phase P3 hợp lệ; key đủ; depends_on=[TASK-INFRA-005, TASK-CART-004], blocks=[TASK-COMPLY-007]. Pass.
