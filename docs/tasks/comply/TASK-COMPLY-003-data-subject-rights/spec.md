...
created: 2026-06-28
related_frs: [TASK-COMPLY-001, TASK-AUTH-005, TASK-AUTH-001, TASK-B2B-001]
depends_on: [TASK-COMPLY-001]
blocks: [TASK-AUTH-005, TASK-B2B-001]
source_pages:
...
risk_if_skipped: "Luật 91/2025 trao chủ thể dữ liệu quyền truy cập, sửa, xóa, di chuyển dữ liệu của mình. Không có quy trình DSAR -> không đáp ứng yêu cầu hợp pháp của user và vi phạm hạng High (§9), chế tài tới 3 tỷ VND cho vi phạm nghiêm trọng (§5.5). Xóa tài khoản (TASK-AUTH-005) phụ thuộc trực tiếp vào erase ở đây. Nếu xóa sai cách (xóa luôn chứng cứ consent, hoặc để rò dữ liệu sang user khác khi xuất) thì tự tạo vi phạm mới. DSAR là quyền cốt lõi của PDPL và là bề mặt cơ quan quản lý kiểm tra đầu tiên."
---
...
type ExportBundle struct {
    ...
    ConsentHistory []ConsentView    `json:"consent_history"` // gom tu TASK-COMPLY-001
...
12. `rectify` cập nhật email trùng email user khác -> lỗi ràng buộc unique (tôn trọng TASK-AUTH-001), DSAR không hoàn tất.
...
- **TASK-COMPLY-001** - `Export` gom `HistoryAll` (lịch sử consent); `Erase` giữ `consent_record`.
- **TASK-AUTH-001** - `app_user` (anonymize PII); `rectify` tôn trọng email unique.
- **TASK-AUTH-005 (downstream)** - xóa tài khoản gọi `Erase` ở đây.
- **TASK-BILL-003 (liên quan)** - bản ghi payment là dữ liệu ràng buộc kế toán -> soft-anonymize.
- **TASK-TRACK-002 (liên quan)** - wishlist là dữ liệu thuần -> hard-delete.
...
- Xóa tài khoản (TASK-AUTH-005) là người tiêu dùng chính của `Erase`; hai task phải nhất quán về phạm vi xóa.
- SLA suy từ `requested_at` giữ phản hồi DSAR thành quy trình có deadline, không để treo vô thời hạn.

---

*Hết TASK-COMPLY-003. Status: ready_to_implement (mục tiêu audit 10/10).*
