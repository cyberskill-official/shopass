...
Audit độc lập, tái diễn từ file TASK-TRACK-001 hiện tại (không dựa vào audit đồng-tác-giả cũ). task đặc tả `POST /v1/track`: parse item_url theo sàn -> Upsert tracked_product (TASK-PRICE-001) -> link user (user_tracked_product) -> enqueue scrape job priming. 12 mệnh đề §1 BCP-14 (11 MUST + 1 SHOULD), mỗi mệnh đề testable. Schema user_tracked_product khớp DATA-MODEL (PK user_id+product_id, cả hai FK). Ranh giới sạch: không INSERT thẳng tracked_product, không tự gửi scrape. Đạt 10/10.
...
- Typography prose plain ASCII + tiếng Việt có dấu; không tự cấm; sentinel "Hết TASK-TRACK-001" có mặt.
...
| #5 Upsert không nhân bản | #7 | price.Upsert (TASK-PRICE-001) |
