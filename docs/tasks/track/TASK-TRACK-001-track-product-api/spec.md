...
created: 2026-06-28
related_frs: [TASK-PRICE-001, TASK-SCRAPE-002, TASK-TRACK-002, TASK-TRACK-003, TASK-INFRA-001]
depends_on: [TASK-PRICE-001, TASK-SCRAPE-002]
blocks: [TASK-TRACK-002, TASK-TRACK-003]
source_pages:
...
source_decisions:
  ...
  - "DEC-TRACK-02: tạo tracked_product qua Upsert idempotent của TASK-PRICE-001 - một SKU track nhiều lần (kể cả nhiều user) chỉ một dòng"
  ...
  - "DEC-TRACK-04: gắn user_id (từ JWT của gateway TASK-INFRA-001) vào tracked_product qua bảng nối user_tracked_product - một user track cùng SKU chỉ một lần"
...
disallowed_tools:
  ...
  - tạo tracked_product trực tiếp thay vì gọi Upsert của TASK-PRICE-001 (vi phạm DEC-TRACK-02, nhân bản SKU)
...
sub_tasks:
  ...
  - "0.5h: router.go - đăng ký route sau JWT middleware (TASK-INFRA-001)"
...
risk_if_skipped: "POST /v1/track là cửa duy nhất để người dùng đưa một sản phẩm vào hệ thống theo dõi - không có nó thì wishlist (TASK-TRACK-002), alert (TASK-TRACK-003) và biểu đồ giá không có thực thể sản phẩm nào của người dùng để gắn vào, toàn bộ vòng giá trị theo dõi giá đứng. Nếu tin client gửi sẵn platform_item_id thay vì parse từ item_url thì hệ thống nhận id rác hoặc id giả mạo trỏ tới SKU người khác, làm hỏng quan hệ user-sản phẩm. Nếu tạo tracked_product thẳng thay vì qua Upsert idempotent của TASK-PRICE-001 thì mỗi lần track nhân bản một dòng, registry phình và price_snapshot trỏ id rác. Nếu bỏ bước enqueue scrape job priming thì SKU mới track không có snapshot đầu tiên, người dùng vào xem thấy biểu đồ rỗng đúng lúc họ vừa quan tâm - phá trải nghiệm cold-start (§5.1)."
---

## §1 - Mô tả (BCP-14 normative)

Service TRACK **MUST** expose endpoint REST `POST /v1/track` nhận `{platform, item_url}`, parse `item_url` theo từng sàn ra định danh SKU, upsert vào `tracked_product` (qua TASK-PRICE-001), gắn người dùng vào SKU đó, rồi enqueue một scrape job priming để có snapshot giá đầu tiên. Hợp đồng:
...
4. **MUST** map `platform` code sang `platform_id` (SMALLINT) qua bảng `platform` (TASK-INFRA-002) trước khi upsert; code hợp lệ nhưng chưa seed trong `platform` trả `400`.
5. **MUST** tạo hoặc lấy `tracked_product` qua `Upsert` idempotent của TASK-PRICE-001 (DEC-TRACK-02): cùng `(platform_id, platform_item_id)` chỉ một dòng dù nhiều user track. Handler **MUST NOT** ghi `tracked_product` bằng INSERT trần.
...
7. **MUST** lấy `user_id` từ JWT do API Gateway (TASK-INFRA-001) gắn; handler KHÔNG tự parse token. Request thiếu auth bị gateway chặn trước.
8. **MUST** enqueue một scrape job priming sau khi có `tracked_product.id` (DEC-TRACK-03): job ưu tiên để scraper (TASK-SCRAPE-002) lấy snapshot giá đầu tiên ngay, không chờ vòng quét định kỳ. Enqueue đặt sau khi link user thành công.
...
**Vì sao upsert qua TASK-PRICE-001 thay vì INSERT (DEC-TRACK-02)?** Nhiều user có thể cùng theo dõi một SKU hot. SKU là thực thể dùng chung, không thuộc riêng user nào. `Upsert` idempotent của TASK-PRICE-001 đảm bảo đúng một dòng `tracked_product` cho mỗi SKU; quan hệ "ai theo dõi" nằm ở bảng nối riêng. Tách vậy giữ registry sản phẩm sạch và cho phép chia sẻ chi phí scraping (một SKU quét một lần phục vụ mọi người theo dõi).
...
func (h *Handler) HandleTrack(w http.ResponseWriter, req *http.Request) {
    userID := auth.UserID(req.Context()) // do gateway gắn (TASK-INFRA-001)
    ...
    tp, err := h.price.Upsert(req.Context(), price.TrackedProduct{ // TASK-PRICE-001
...
Xem §3. Thứ tự: `url_parser.go` (parse 3 sàn) -> migration `0001_user_tracked_product.sql` -> `repo.go` (`LinkUserProduct` ON CONFLICT DO NOTHING) -> `track.go` (handler ghép parse + Upsert price-svc + link + enqueue) -> đăng ký route trong `router.go` sau JWT middleware (TASK-INFRA-001) -> tests. Handler dùng `http.ServeMux` Go 1.22. `EnqueuePriming` đẩy vào cùng hàng đợi scrape của TASK-SCRAPE-001/002 với cờ ưu tiên. `Upsert` gọi sang package `price` (cùng monorepo) hoặc qua client nội bộ - không tự ghi `tracked_product`.

---

## §7 - Phụ thuộc

- **TASK-PRICE-001** - cung cấp `Upsert(tracked_product)` idempotent và bảng `tracked_product`; là điều kiện cứng cho việc tạo/lấy SKU.
- **TASK-SCRAPE-002** - tiêu thụ scrape job priming, lấy snapshot giá đầu tiên của SKU mới track.
- **TASK-INFRA-001 (gateway)** - gắn JWT auth và `user_id` vào context trước handler.
- **TASK-INFRA-002** - bảng `platform` để map code sàn -> `platform_id`; bảng `app_user` cho FK `user_id`.
- **TASK-TRACK-002 / TASK-TRACK-003 (downstream)** - wishlist và alert_rule gắn vào `tracked_product` đã track ở đây.
...
Đã chốt. Hoãn:
- Resolve short link (vd `s.shopee.vn/...`) bằng một HEAD request theo redirect trước khi parse - thêm khi gặp nhiều short link trong thực tế; không cản task này.
- Giới hạn số SKU theo dõi theo tier (free vs Premium) - gắn vào TASK-BILL khi có gating.
- Track theo `canonical_key` để tự theo dõi cùng món trên cả 3 sàn một lần - chờ TASK-PRICE-005 so khớp; mở rộng sau.
- Bỏ theo dõi (`DELETE /v1/track/{product_id}`) - thêm khi làm UI quản lý wishlist (TASK-WEB-004).
...
| platform chưa seed | IDByCode false | 400 | Seed platform (TASK-INFRA-002) chạy trước |
...
*Hết TASK-TRACK-001. Status: ready_to_implement (mục tiêu audit 10/10).*
