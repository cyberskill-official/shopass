...
created: 2026-06-28
related_frs: [TASK-TRACK-001, TASK-TRACK-004, TASK-DEAL-006, TASK-WEB-004, TASK-INFRA-002]
depends_on: [TASK-TRACK-001]
blocks: [TASK-DEAL-006, TASK-TRACK-004, TASK-WEB-004]
source_pages:
...
source_decisions:
  ...
  - "DEC-TRACK-23: bảng alert là sổ ghi mỗi lần luật bắn (alert_rule_id, fired_at, payload JSONB, status); TASK-TRACK-004 ghi, task này chỉ định nghĩa schema + đọc lịch sử"
  - "DEC-TRACK-24: active BOOLEAN bật/tắt luật không cần xóa; engine TASK-TRACK-004 chỉ đánh giá luật active=true; mọi CRUD kiểm chủ sở hữu (chống IDOR)"
...
11. **MUST** tạo index hỗ trợ engine TASK-TRACK-004 truy vấn luật cần đánh giá: `idx_ar_eval ON alert_rule (product_id, rule_type) WHERE active = true` (chỉ index luật đang bật, partial index nhẹ).
...
**Vì sao rule_type là enum CHECK (DEC-TRACK-20)?** Bốn loại cảnh báo phủ đúng nhu cầu: "về giá X" (price_below), "giảm Y%" (drop_pct), "sale thật chứ không ảo" (real_sale - dùng TASK-DEAL-001), "sắp chạm đáy" (bottom_predicted - dùng TASK-DEAL-006). Engine đánh giá (TASK-TRACK-004) viết một nhánh xử lý cho mỗi loại. Cho `rule_type` tùy ý nghĩa là engine gặp loại nó không có nhánh -> hoặc bỏ sót cảnh báo, hoặc panic. CHECK ở DB là rào cuối cùng đảm bảo chỉ có bốn loại đã biết.
...
**Vì sao tách bảng alert khỏi alert_rule (DEC-TRACK-23)?** `alert_rule` là cấu hình (ý định, sống lâu); `alert` là sự kiện (một lần bắn, bất biến). Trộn lại làm rối. Tách ra cho ta sổ lịch sử "luật này đã bắn khi nào, gửi gì" để hiển thị cho user và để TASK-TRACK-004 dedup (không bắn lại cùng cảnh báo). task này dựng schema + đọc; engine TASK-TRACK-004 ghi.

**Vì sao partial index chỉ trên active=true (§1 #11)?** Engine TASK-TRACK-004 quét "mọi luật đang bật của một SKU vừa đổi giá". Luật `active=false` không bao giờ được đánh giá, nên không cần nằm trong index. Partial index `WHERE active = true` nhỏ hơn và nhanh hơn full index, đúng hình dạng truy vấn của engine.
...
-- Engine TASK-TRACK-004 chỉ quét luật đang bật của một SKU -> partial index
...
CREATE TABLE alert (
  ...
  payload       JSONB,
  status        TEXT        NOT NULL DEFAULT 'pending' -- pending|sent|failed (TASK-TRACK-004/NOTIF cập nhật)
);
...
10. `PATCH /v1/alerts/{id} {active:false}` tắt luật; engine TASK-TRACK-004 sau đó không đánh giá luật này (không còn trong partial index).
...
func TestToggleActive_RemovesFromEvalIndex(t *testing.T) {
    ...
    require.False(t, ruleActive(t, h, rid)) // engine TASK-TRACK-004 sẽ bỏ qua
}
```
...
Xem §3. Thứ tự: migration `0003_alert_rule.sql` (2 bảng + CHECK rule_type + partial index) -> `alert_validate.go` (ValidateRule theo loại) -> `alert_rule_repo.go` (5 hàm scope user, mỗi route {id} qua kiểm chủ sở hữu) -> `alert_rule.go` (handler) -> đăng ký route trong `router.go` sau JWT middleware (TASK-INFRA-001) -> tests. Validate ở tầng API là tuyến đầu (thông báo lỗi rõ); CHECK ở DB là tuyến cuối (an toàn dữ liệu). Lưu ý alias: một số task DEAL gọi cờ này là `enabled`; schema chuẩn dùng `active` theo §3.4 tài liệu nguồn - engine TASK-TRACK-004 đọc đúng tên `active`.

---

## §7 - Phụ thuộc

- **TASK-TRACK-001** - SKU phải đã track (FK `product_id` tới `tracked_product`).
- **TASK-INFRA-002** - bảng `app_user` cho FK `user_id`.
- **TASK-INFRA-001 (gateway)** - gắn JWT auth và `user_id`.
- **TASK-TRACK-004 (downstream)** - engine đọc `alert_rule` (qua `idx_ar_eval`) và ghi dòng `alert`; tên cột `active`/`channel`/`threshold` là hợp đồng với nó.
- **TASK-DEAL-006 (downstream)** - batch đáy giá khớp `alert_rule` type `bottom_predicted`.
- **TASK-WEB-004 (downstream)** - UI quản lý alert tiêu thụ các route này.
...
Đã chốt. Hoãn:
- Mốc tham chiếu cho `drop_pct` (so với giá hôm qua, median 7 ngày, hay giá lúc tạo luật) - chốt ở TASK-TRACK-004 khi định nghĩa cách đánh giá; task này chỉ lưu phần trăm.
- Lịch quiet-hours (không gửi cảnh báo ban đêm trừ khi user đồng ý) - gắn vào TASK-NOTIF khi có tùy chọn người dùng.
- Giới hạn số luật theo tier (free vs Premium) - gắn vào TASK-BILL.
- Luật trên `canonical_key` (cảnh báo khi bất kỳ sàn nào về giá X) - chờ TASK-PRICE-005; mở rộng sau.
...
| product_id chưa track | FK 23503 | 400 product not tracked | Buộc qua TASK-TRACK-001 |
...
- Tách `alert` (sự kiện bất biến) khỏi `alert_rule` (cấu hình sống lâu) cho ta sổ lịch sử và nền dedup cho TASK-TRACK-004.
- Partial index `WHERE active=true` khớp đúng truy vấn của engine (chỉ luật đang bật), nhẹ hơn full index.
- Kiểm chủ sở hữu trả 404 (không 403) đóng lỗ IDOR trên khóa BIGSERIAL tuần tự, đồng nhất với TASK-TRACK-002.
- Khi mở SEA, `threshold` của price_below vẫn int64 theo minor unit từng nước; enum rule_type và validate không đổi.

---

*Hết TASK-TRACK-003. Status: ready_to_implement (mục tiêu audit 10/10).*
