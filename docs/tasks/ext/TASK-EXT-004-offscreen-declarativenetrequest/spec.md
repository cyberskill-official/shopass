...
created: 2026-06-27
related_frs: [TASK-EXT-001, TASK-EXT-002, TASK-EXT-003, TASK-EXT-005, NFR-EXT-001]
...
source_decisions:
  ...
  - "DEC-EXT-22: offscreen chỉ parse HTML THÔ đã có sẵn (string) hoặc thao tác clipboard do người dùng kích hoạt; KHÔNG tự fetch trang sàn từ offscreen; dữ liệu ra vẫn đi qua pipeline tối thiểu hóa (TASK-EXT-003)"
...
6. Tài liệu offscreen **MUST** chỉ parse HTML THÔ đã có sẵn (truyền vào dạng string qua message) hoặc thao tác clipboard do người dùng kích hoạt; **MUST NOT** tự `fetch` trang sàn từ offscreen (DEC-EXT-22). Việc đọc trang sàn là của content script (TASK-EXT-002).
7. Dữ liệu kết quả từ offscreen **MUST** đi tiếp qua pipeline tối thiểu hóa (TASK-EXT-003) trước khi rời client - offscreen không phải đường tắt bỏ qua allowlist.
8. `manifest.json` **MUST** thêm vào `permissions`: `"offscreen"`, `"declarativeNetRequest"` - chỉ khi task này hiện thực hóa, đúng nguyên tắc quyền tối thiểu của TASK-EXT-001.
...
7. Kết quả parse offscreen đi qua `minimize()` (TASK-EXT-003) trước khi rời client (test đường dẫn).
...
Xem §3. Thứ tự: cập nhật `manifest.json` (permissions offscreen + DNR + rule_resources) + `types.ts` (ParseDomRequest/Result) -> `offscreen.html` + `offscreen.ts` (tài liệu nhận HTML thô, parse, trả) -> `manager.ts` (kiểm hasDocument -> create reason -> sendWithTimeout -> close) -> `rules.json` + `dnr.ts` (rule static tối thiểu + sanity check) -> nối kết quả parse vào `minimize()` (TASK-EXT-003) -> tests. Offscreen chỉ là công cụ nhất thời cho DOM/clipboard; content script vẫn là đường đọc trang sàn duy nhất. DNR rule khai static để Web Store và audit kiểm được.

---

## §7 - Phụ thuộc

- **TASK-EXT-001** - scaffold MV3 + service worker + manifest + messaging làm khung cho offscreen/DNR.
- **TASK-EXT-002** - content script lấy HTML thô trang sàn rồi giao cho offscreen parse (offscreen không tự đọc).
- **TASK-EXT-003 (downstream)** - kết quả parse offscreen đi qua pipeline tối thiểu hóa trước khi rời client.
- **TASK-EXT-005 (downstream)** - lớp đồng bộ gửi payload đã sạch; DNR/offscreen không phải đường tắt ra mạng.
...
Đã chốt. Hoãn:
- Có cần offscreen cho audio/iframe ngoài DOM_SCRAPING/CLIPBOARD hay không - chỉ thêm reason khi có task tiêu thụ.
...
| Kết quả parse bỏ qua minimize | đường-dẫn test | rò trường thừa | Mọi kết quả qua TASK-EXT-003 (§1 #7) |
...
- declarativeNetRequest thay webRequest blocking là bắt buộc ở MV3; DNR rule khai static + tối thiểu để Web Store và audit (TASK-TRUST-003) kiểm được.
- Offscreen KHÔNG tự đọc trang sàn - content script (TASK-EXT-002) là đường đọc duy nhất; offscreen chỉ parse HTML thô đã có.
- Mọi dữ liệu ra từ offscreen vẫn qua pipeline tối thiểu hóa (TASK-EXT-003) - không có đường tắt bỏ qua allowlist.
- Khả năng can thiệp request (DNR) là quyền lực nhạy cảm hậu-Honey; giữ nó tối thiểu và minh bạch là mắt xích niềm tin (§5.4).

---

*Hết TASK-EXT-004. Status: ready_to_implement (mục tiêu audit 10/10).*
