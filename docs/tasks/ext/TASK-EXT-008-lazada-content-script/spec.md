---
id: TASK-EXT-008
title: "Content script Lazada - Akamai-aware, đọc DOM giỏ đã render; KHÔNG né/giả Akamai sensor từ client; tái dùng khung reader/normalize/health của TASK-EXT-002"
module: EXT
priority: MUST
status: done
verify: T
...
created: 2026-06-27
related_frs: [TASK-EXT-002, TASK-EXT-003, TASK-EXT-006, TASK-SCRAPE-008, TASK-SCRAPE-006, TASK-TRUST-002, NFR-EXT-001]
...
source_decisions:
  ...
  - "DEC-EXT-43: tái dùng khung reader + normalize + health signal của TASK-EXT-002; chỉ thay lớp selector đặc thù Lazada (DOM riêng, đổi theo A/B)"
  - "DEC-EXT-44: giữ nguyên cam kết niềm tin TASK-EXT-002: KHÔNG mật khẩu, token/cookie KHÔNG rời client; chỉ trích productId/giá/qty đã render; chỉ đọc, KHÔNG sửa giỏ/đặt hàng"
...
disallowed_tools:
  ...
  - đọc/sao chép cookie/session token vào biến hay payload (vi phạm DEC-EXT-44 - phá cam kết niềm tin TASK-EXT-002)
  - thu thập mật khẩu hoặc tự sửa giỏ/đặt hàng (vi phạm DEC-EXT-44 - chỉ đọc)
  - phân nhánh khung reader riêng thay vì tái dùng normalize/health của TASK-EXT-002 (vi phạm DEC-EXT-43)
...
sub_tasks:
  ...
  - "2.0h: cart-reader.ts - đọc DOM giỏ đã render (KHÔNG API qua Akamai); orchestrate qua khung TASK-EXT-002; health khi hỏng"
...
6. Khi mọi selector hỏng, content script **MUST** phát health signal về service worker để TASK-SCRAPE-006 ghi nhận - KHÔNG nuốt lỗi im lặng (tái dùng cơ chế TASK-EXT-002).
7. Content script **MUST** tái dùng khung reader + `normalize.ts` + health signal của TASK-EXT-002 (DEC-EXT-43); chỉ thay lớp selector đặc thù Lazada. **MUST NOT** phân nhánh một pipeline tối thiểu hóa riêng.
8. Content script **MUST** giữ nguyên cam kết niềm tin TASK-EXT-002 (DEC-EXT-44): KHÔNG đọc `document.cookie`/token, KHÔNG thu mật khẩu, token/cookie KHÔNG rời client; chỉ trích `productId`/giá/qty đã render.
9. Content script **MUST** chỉ đọc: KHÔNG tự sửa giỏ, KHÔNG đặt hàng, KHÔNG áp voucher tự động (DEC-EXT-44).
10. Kết quả **MUST** gửi cho service worker dạng `CartReadMessage` typed với `platform: "lazada"`, chứa CHỈ `CartItem[]` + `VoucherItem[]` tối thiểu - đi tiếp qua pipeline TASK-EXT-003.
...
**Vì sao tái dùng khung TASK-EXT-002 (DEC-EXT-43)?** Giống TikTok (TASK-EXT-007), phân nhánh pipeline riêng cho Lazada nhân đôi bug và mở bề mặt mới có thể lọt credential. Khung reader + normalize + health của TASK-EXT-002 đã được test cho cam kết "không rò cookie"; tái dùng giữ một điểm kiểm soát tối thiểu hóa duy nhất. Chỉ lớp selector là per-sàn - Lazada có DOM riêng nhưng quy trình đọc/chuẩn hóa/báo hỏng là chung.

**Vì sao giữ nguyên cam kết niềm tin (DEC-EXT-44)?** Ranh giới "không cookie, không mật khẩu, chỉ đọc" là định vị hậu-Honey của toàn extension (§5.4), không phụ thuộc sàn. Lazada không phải ngoại lệ. Test no-secret-leak tương tự TASK-EXT-002/007 khẳng định ranh giới cho cả Lazada.
...
export async function readLazadaCart(): Promise<CartReadMessage> {
  ...
  if (raw === null) {
    reportHealth({ platform: "lazada", broke: "cart", source: "dom" }); // TASK-SCRAPE-006
    raw = [];
  }
...
7. Reader tái dùng `normalize.ts` + health của TASK-EXT-002 (grep import từ `content/shared/`), không phân nhánh riêng.
...
test("reader tái dùng normalize/health TASK-EXT-002", async () => {
...
- **TASK-EXT-003** - pipeline tối thiểu hóa nhận `CartReadMessage(platform:lazada)`, lọc lần hai.
- **TASK-EXT-006** - consent "read_cart" gate trước khi đọc.
- **TASK-SCRAPE-006** - DOM-change monitoring nhận health signal khi parser Lazada hỏng.
- **TASK-SCRAPE-008 (song song, P2)** - adapter Lazada backend (Akamai, residential bắt buộc); reader này là phía client.
...
### Health signal khi parser Lazada hỏng (-> TASK-SCRAPE-006)
...
- Phân biệt LazMall vs marketplace seller trong dữ liệu giỏ - hoãn tới TASK-CART-001 (voucher_catalog) nếu cần.
- Xử lý Lazada đa quốc gia SEA (lazada.co.th/.com.my) - thêm khi mở per-country (TASK-COMPLY-006); task này nhắm lazada.vn.
...
| Phân nhánh pipeline riêng | grep import shared | nhân đôi bug + bề mặt rò | Tái dùng normalize/health TASK-EXT-002 (DEC-EXT-43) |
...
- Tái dùng khung reader/normalize/health của TASK-EXT-002 giữ một điểm kiểm soát tối thiểu hóa duy nhất; chỉ selector là per-sàn (giống TikTok TASK-EXT-007).
- Cam kết niềm tin (không cookie/mật khẩu, chỉ đọc) không phụ thuộc sàn - test no-secret-leak khẳng định cho cả Lazada.

---

*Hết TASK-EXT-008. Status: ready_to_implement (mục tiêu audit 10/10).*
