...
created: 2026-06-27
related_frs: [TASK-EXT-001, TASK-EXT-002, TASK-EXT-005, TASK-AUTH-003, TASK-TRUST-002, TASK-TRUST-003, TASK-COMPLY-005, NFR-EXT-001]
depends_on: [TASK-EXT-002]
blocks: [TASK-AFFIL-004, TASK-CART-002, TASK-EXT-005, TASK-TRUST-002, TASK-TRUST-003]
source_pages:
...
source_decisions:
  ...
  - "DEC-EXT-15: pipeline là lần lọc THỨ HAI (defense in depth) sau khi content script (TASK-EXT-002) đã tối thiểu hóa - không tin tầng trên, tự lọc lại"
  - "DEC-EXT-16: local-first - chuẩn hóa/khử trùng diễn ra trên client; chỉ payload đã làm sạch mới đưa cho lớp đồng bộ (TASK-EXT-005)"
...
modified_files:
  - extension/src/background/service-worker.ts   # nối onMessage(CART_READ) -> minimize -> queue (TASK-EXT-005)
...
sub_tasks:
  ...
  - "0.5h: nối service-worker onMessage(CART_READ) -> minimize -> đẩy queue cho TASK-EXT-005"
...
risk_if_skipped: "Đây là van an toàn cuối cùng trước khi dữ liệu rời máy người dùng - cam kết PDPL + niềm tin hậu-Honey của SănDeal phụ thuộc vào nó. Tài liệu nguồn (§3.2/§5.4) nêu rõ chỉ gửi productId/giá/số lượng và KHÔNG gửi cookie. Nếu pipeline dùng denylist (gửi mọi thứ trừ vài trường), một trường mới do tầng trên vô tình thêm sẽ lọt ra backend - đúng kiểu rò rỉ âm thầm. Allowlist (chỉ cho qua trường liệt kê) là an toàn theo mặc định. Đây phải là lần lọc thứ hai độc lập: nếu content script (TASK-EXT-002) có bug để lọt cookie, pipeline vẫn chặn (defense in depth). Không có van này, một thay đổi vô hại ở tầng đọc có thể biến extension thành công cụ rò rỉ dữ liệu cá nhân - phá hủy toàn bộ định vị và mời chế tài PDPL tới 5% doanh thu."
---
...
3. Pipeline **MUST** là lần lọc THỨ HAI độc lập (DEC-EXT-15, defense in depth): không tin payload từ content script (TASK-EXT-002) đã sạch - tự lọc lại từ đầu. Nếu tầng trên có bug để lọt credential, pipeline vẫn chặn.
...
7. Toàn bộ chuẩn hóa/khử trùng **MUST** diễn ra trên client (local-first, DEC-EXT-16, TASK-TRUST-002); backend chỉ nhận kết quả đã làm sạch. KHÔNG gửi dữ liệu thô để "backend lọc giúp".
...
10. Pipeline **MUST** ghi metric đếm: số payload đi tiếp, số trường bị loại bởi allowlist, số payload bị từ chối bởi schema/redact - để TASK-TRUST-003 (audit) và giám sát có bằng chứng định lượng pipeline đang lọc.
11. Service worker **MUST** nối `onMessage(CART_READ)` -> `minimize()` -> đẩy `OutboundPayload` vào hàng đợi đồng bộ (TASK-EXT-005). Không có đường tắt nào gửi `CartReadMessage` thẳng ra mạng bỏ qua pipeline.
12. Pipeline **MUST** không phụ thuộc trạng thái đăng nhập/định danh: nó chỉ biến đổi dữ liệu, không gắn token. Việc gắn danh tính/JWT cho request là của lớp đồng bộ TASK-EXT-005 (và token đó là JWT của SănDeal, KHÔNG phải token sàn).
...
**Vì sao lọc lần hai độc lập (DEC-EXT-15)?** Content script (TASK-EXT-002) đã tối thiểu hóa, nhưng đó là cùng một codebase có thể có bug. Defense in depth: pipeline coi như input có thể bẩn và lọc lại từ đầu. Nếu một thay đổi ở tầng đọc vô tình để lọt trường nhạy cảm, van thứ hai này vẫn chặn trước khi dữ liệu rời máy.
...
**Vì sao tách việc gắn token sang TASK-EXT-005 (§1 #12)?** Pipeline chỉ làm sạch dữ liệu, không biết gì về danh tính. Việc đính JWT (của SănDeal, không phải sàn) cho request là trách nhiệm của lớp đồng bộ. Tách bạch giữ pipeline đơn giản, dễ audit, và tránh lẫn token SănDeal với token sàn.
...
8. Không tồn tại đường gửi `CartReadMessage` ra mạng bỏ qua `minimize` (grep: mọi outbound đi qua pipeline -> queue TASK-EXT-005).
...
Xem §3. Thứ tự: `schema.ts` (OutboundPayload + validatePayload) -> `allowlist.ts` (pick theo danh sách trường) -> `redact.ts` (quét giá trị credential/PII) -> `minimize.ts` (allowlist -> redact -> validate, fail-closed) -> nối `service-worker.ts` `onMessage(CART_READ) -> minimize -> queue` -> tests. Lớp đồng bộ (TASK-EXT-005) nhận `OutboundPayload` đã sạch từ hàng đợi và đính JWT SănDeal. Mọi mở rộng trường trong tương lai phải sửa cả `schema.ts` lẫn `allowlist.ts` và đi qua review - đó là điểm kiểm soát duy nhất.

---

## §7 - Phụ thuộc

- **TASK-EXT-002** - content script Shopee cung cấp `CartReadMessage` (đã tối thiểu hóa lần một).
- **TASK-EXT-001** - service worker + messaging + storage làm khung nối pipeline.
- **TASK-EXT-005 (downstream)** - lớp đồng bộ nhận `OutboundPayload` sạch, đính JWT, gửi backend.
- **TASK-TRUST-002 (downstream)** - chính sách local-first + tối thiểu hóa được hiện thực ở đây.
- **TASK-TRUST-003 (downstream)** - audit độc lập dùng metric + test pipeline làm bằng chứng không gửi cookie/PII.
- **TASK-COMPLY-005** - cưỡng chế no-cleartext + token-not-on-server; pipeline là điểm thực thi phía client.
- **TASK-CART-002 / TASK-AFFIL-004 (downstream, P2)** - cart_snapshot + guardrails affiliate dựa dữ liệu sạch từ pipeline.
...
Đã chốt. Hoãn:
- Có hash `productId` thêm một lớp trước khi gửi hay không - không cần (productId là công khai); xét lại nếu B2B (TASK-B2B-001) yêu cầu ẩn danh mạnh hơn.
- Nén/gộp payload nhiều lần đọc thành batch - tối ưu ở TASK-EXT-005 (đồng bộ), không thuộc pipeline lọc.
- Mở rộng allowlist cho trường mới (ví dụ rating) - chỉ khi có task tiêu thụ; thêm phải qua review.
...
- Metric đếm passed/dropped/rejected cung cấp bằng chứng định lượng cho audit độc lập TASK-TRUST-003.
- Token gắn cho request là JWT của SănDeal (TASK-EXT-005), KHÔNG phải token sàn - pipeline không chạm danh tính.

---

*Hết TASK-EXT-003. Status: ready_to_implement (mục tiêu audit 10/10).*
