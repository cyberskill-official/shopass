...
created: 2026-06-27
related_frs: [TASK-EXT-001, TASK-EXT-002, TASK-EXT-003, TASK-AUTH-002, TASK-AUTH-003, TASK-TRUST-002, NFR-EXT-001]
depends_on: [TASK-EXT-003, TASK-AUTH-002]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-EXT-23: request lên backend đính JWT của SănDeal (cấp bởi TASK-AUTH-002), KHÔNG BAO GIỜ token/cookie sàn; ext gắn Authorization: Bearer <jwt SănDeal> qua auth bridge"
  - "DEC-EXT-24: chỉ OutboundPayload đã qua pipeline tối thiểu hóa (TASK-EXT-003) mới được đưa vào hàng đợi gửi; không có đường gửi CartReadMessage thô"
  ...
  - "DEC-EXT-27: refresh JWT khi hết hạn qua refresh token (TASK-AUTH-002); token SănDeal lưu chrome.storage.session (RAM-only), KHÔNG storage.local bền quá mức cần"
...
disallowed_tools:
  ...
  - gửi CartReadMessage thô bỏ qua pipeline TASK-EXT-003 (vi phạm DEC-EXT-24)
...
effort_hours: 6
sub_tasks:
  - "1.0h: auth-bridge.ts - lấy/refresh JWT SănDeal (TASK-AUTH-002), gắn Authorization Bearer; token ở storage.session"
...
9. Lớp đồng bộ **MUST** giữ vai "đầu đọc nhẹ": không chạy xử lý nặng/dài (DEC-EXT-05); flush hàng đợi theo nhịp alarm (TASK-EXT-001) hoặc khi có sự kiện, không vòng lặp dài làm SW vượt 5 phút.
...
11. Lớp đồng bộ **MUST** ghi metric: số payload gửi thành công, số retry, số fail-closed do thiếu JWT, số lần mở/đóng WSS - làm bằng chứng cho audit (TASK-TRUST-003) và giám sát.
...
**Vì sao JWT SănDeal, không token sàn (DEC-EXT-23)?** Đây là ranh giới niềm tin cứng nhất của extension. Backend SănDeal chỉ cần biết "request này thuộc người dùng SănDeal nào" - đó là việc của JWT SănDeal (TASK-AUTH-002). Token/cookie sàn KHÔNG BAO GIỜ được rời client (cam kết §5.4); chúng ở lại tab để content script mượn phiên qua trình duyệt (TASK-EXT-002), không bao giờ đi lên server. Lẫn hai loại token là lỗi biến extension thành kênh chiếm tài khoản sàn - đúng thảm họa cần né.

**Vì sao hàng đợi bền trong chrome.storage (DEC-EXT-25)?** SW bị kill bất cứ lúc nào (NFR-EXT-001). Nếu hàng đợi nằm trong biến global, một lần SW ngủ giữa lúc gửi là mất dữ liệu giỏ đã đọc. Hàng đợi bền trong storage sống qua chu kỳ kill: SW thức dậy, đọc lại hàng đợi, gửi tiếp. Đây là hệ quả trực tiếp của quy ước "state luôn ở storage" từ TASK-EXT-001.
...
**Vì sao JWT ở storage.session, không storage.local (DEC-EXT-27)?** Token nhạy cảm không nên bền lâu hơn cần. `storage.session` là RAM-only, mất khi trình duyệt đóng - giảm bề mặt rò token nếu máy bị truy cập. Refresh token (theo TASK-AUTH-002) khôi phục phiên khi cần, nên không cần giữ JWT bền quá mức.
...
export async function authedFetch(url: string, env: SyncEnvelope): Promise<Response> {
  const jwt = await getJwt();                 // TASK-AUTH-002; storage.session
...
Xem §3. Thứ tự: `auth-bridge.ts` (lấy/refresh JWT SănDeal, đính Bearer, fail-closed thiếu JWT) -> `queue.ts` (hàng đợi bền trong chrome.storage) -> `sender.ts` (flush có retry/backoff, ack 2xx, 401->refresh) -> `ws-client.ts` (mở/đóng theo nhu cầu) -> nối `service-worker.ts` (pipeline-done -> enqueue; alarm tick -> flush; mở WSS khi cần realtime) -> tests. JWT ở storage.session; token sàn KHÔNG BAO GIỜ rời client (vẫn ở tab cho TASK-EXT-002 mượn phiên). Body chỉ OutboundPayload đã sạch (TASK-EXT-003), danh tính ở header.

---

## §7 - Phụ thuộc

- **TASK-EXT-003** - cung cấp `OutboundPayload` đã sạch; lớp đồng bộ chỉ gửi cái này, không gửi thô.
- **TASK-AUTH-002** - cấp JWT + refresh token SănDeal mà auth bridge đính vào request.
- **TASK-EXT-001** - service worker + chrome.storage + alarm làm khung hàng đợi + flush; messaging nối pipeline.
- **TASK-AUTH-003** - liên kết platform_account ẩn danh (ext_user_ref), KHÔNG token sàn - đồng bộ với ranh giới ở đây.
- **TASK-TRUST-002 (downstream)** - chính sách local-first; lớp đồng bộ là điểm dữ liệu sạch rời máy.
- **NFR-EXT-001** - hàng đợi bền + WSS theo nhu cầu để không phá ràng buộc SW ephemeral.
- **TASK-CART-002 / TASK-AFFIL-004 (downstream, P2)** - tiêu thụ dữ liệu giỏ đồng bộ về backend.
...
- Kênh realtime cụ thể (WSS topic cho alert giá) - định nghĩa chi tiết khi TASK-TRACK-004 (engine alert) tới.
- Chính sách hàng đợi pending lúc đăng xuất (giữ chờ vs xóa) - chốt cùng TASK-COMPLY-003 (DSAR/xóa dữ liệu).
...
- Ranh giới cứng nhất: request lên backend đính JWT SănDeal, token/cookie sàn KHÔNG BAO GIỜ rời client (ở lại tab cho TASK-EXT-002 mượn phiên qua trình duyệt).
...
- Body chỉ OutboundPayload đã sạch, danh tính ở header Authorization - tách bạch để audit (TASK-TRUST-003) kiểm body thấy không credential.

---

*Hết TASK-EXT-005. Status: ready_to_implement (mục tiêu audit 10/10).*
