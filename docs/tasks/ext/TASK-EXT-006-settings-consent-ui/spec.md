---
id: TASK-EXT-006
title: "UI settings + consent PDPL lúc cài - disclosure dữ liệu rõ ràng, opt-in từng loại dữ liệu (im lặng != đồng thuận), gắn consent record TASK-COMPLY-001"
module: EXT
priority: MUST
status: done
verify: T
...
created: 2026-06-27
related_frs: [TASK-EXT-001, TASK-EXT-002, TASK-EXT-003, TASK-COMPLY-001, TASK-COMPLY-003, TASK-TRUST-001, TASK-TRUST-002, NFR-EXT-001]
depends_on: [TASK-EXT-001, TASK-COMPLY-001]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-EXT-31: ghi consent record (phiên bản chính sách, thời điểm, mục đã đồng ý) gửi cho TASK-COMPLY-001 - đồng thuận phải TÁI LẬP ĐƯỢC (reproducible: chứng minh được đã đồng ý gì, khi nào)"
  - "DEC-EXT-32: disclosure nêu rõ KHÔNG thu thập mật khẩu, token/cookie sàn KHÔNG rời máy, chỉ gửi productId/giá/số lượng (khớp TASK-EXT-003) - minh bạch hậu-Honey §5.4"
  ...
  - "DEC-EXT-34: cung cấp lối quản lý consent + xóa dữ liệu (liên kết DSAR TASK-COMPLY-003) trong settings; người dùng xem/đổi/thu hồi bất cứ lúc nào"
...
sub_tasks:
  ...
  - "1.0h: settings.html + settings.ts - options page xem/đổi/thu hồi consent + lối xóa dữ liệu (DSAR TASK-COMPLY-003)"
  - "1.0h: consent-store.ts - lưu ConsentRecord (version chính sách, ts, mục đã đồng ý) chrome.storage + gửi TASK-COMPLY-001"
  - "1.0h: consent-gate.ts - hàm gate kiểm consent trước mọi đọc/gửi; rút consent có hiệu lực ngay"
  - "0.5h: nối service-worker onInstalled mở onboarding; gate chặn đường dữ liệu TASK-EXT-002/003/005"
...
4. Trước mọi đọc/gửi của một mục, `consent-gate.ts` **MUST** kiểm consent của mục đó; nếu chưa đồng ý, đường dữ liệu **MUST** bị chặn (DEC-EXT-33). Content script (TASK-EXT-002) đọc giỏ chỉ chạy khi consent "đọc giỏ" bật; đồng bộ (TASK-EXT-005) chỉ chạy khi consent "đồng bộ" bật.
5. `consent-store.ts` **MUST** ghi `ConsentRecord` tái lập được (DEC-EXT-31): phiên bản chính sách, thời điểm (epoch), danh sách mục đã đồng ý. Record **MUST** gửi cho TASK-COMPLY-001 (khung consent trung tâm) để chứng minh đã đồng ý gì, khi nào.
6. Disclosure **MUST** nêu rõ, bằng ngôn ngữ người dùng hiểu (tiếng Việt): KHÔNG thu thập mật khẩu; token/cookie sàn KHÔNG rời máy client; chỉ gửi `productId`/giá/số lượng về backend (khớp TASK-EXT-003) (DEC-EXT-32).
...
8. `settings.ts` (options page) **MUST** cho người dùng xem trạng thái consent hiện tại, đổi từng mục, và truy lối xóa dữ liệu (liên kết DSAR TASK-COMPLY-003) (DEC-EXT-34) bất cứ lúc nào.
...
### consent-store.ts (ghi record bền + gửi TASK-COMPLY-001)
...
export async function setConsent(granted: ConsentPurpose[]): Promise<void> {
  ...
  await reportConsentToCompliance(rec);   // TASK-COMPLY-001: tái lập được
}
```
...
5. `setConsent` ghi `ConsentRecord` có `policyVersion` + `decidedAt` + `granted`, và gọi `reportConsentToCompliance` (TASK-COMPLY-001).
...
11. `settings.ts` có lối xóa dữ liệu liên kết DSAR (TASK-COMPLY-003) (test có entry point).
...
Xem §3. Thứ tự: `consent-store.ts` (ConsentRecord bền + gửi TASK-COMPLY-001, mặc định granted rỗng) -> `consent-gate.ts` (ensureConsent chặn trước đọc/gửi) -> `onboarding.html` + `onboarding.ts` (consent lúc cài, mặc định tắt, disclosure rõ) -> `settings.html` + `settings.ts` (xem/đổi/thu hồi + lối DSAR) -> nối `service-worker.ts` (onInstalled mở onboarding; gate chặn TASK-EXT-002/003/005) -> tests. Content script đọc giỏ (TASK-EXT-002) và đồng bộ (TASK-EXT-005) đều gọi `ensureConsent` trước. Consent state là nguồn sự thật bền trong chrome.storage; mọi đổi consent ghi record mới gửi compliance.

---

## §7 - Phụ thuộc

- **TASK-EXT-001** - scaffold MV3 + chrome.storage + onInstalled + options page làm khung UI consent.
- **TASK-COMPLY-001** - khung consent PDPL trung tâm nhận ConsentRecord; task này là bề mặt thu consent của extension.
- **TASK-EXT-002** - content script đọc giỏ chỉ chạy khi consent "read_cart"/"read_voucher" bật (gate).
- **TASK-EXT-003 / TASK-EXT-005** - tối thiểu hóa + đồng bộ chỉ chạy khi consent "sync_backend" bật.
- **TASK-COMPLY-003** - DSAR (xem/xóa dữ liệu); settings cung cấp lối vào.
- **TASK-TRUST-001 / TASK-TRUST-002** - open-source + minh bạch local-first; disclosure ở đây khớp cam kết đó.
...
Đã chốt. Hoãn:
- Đa ngôn ngữ disclosure cho SEA (ID/TH...) - thêm khi mở per-country (TASK-COMPLY-006/007), slice 1 tiếng Việt.
- Consent cho mục đích phụ (ví dụ phân tích ẩn danh B2B) - chỉ thêm khi TASK-B2B-001 tới, opt-in riêng.
- Nhắc lại consent định kỳ (re-consent theo thời gian) - chốt cùng TASK-COMPLY-002 (DPIA cập nhật mỗi 6 tháng).
...
| Không ghi consent record | consent-record test | không chứng minh được tuân thủ | Record tái lập được -> TASK-COMPLY-001 (DEC-EXT-31) |
...
- Consent gate biến đồng thuận thành ràng buộc thực thi: mỗi đường dữ liệu (TASK-EXT-002/005) hỏi gate trước; chưa opt-in thì chặn - không phải trang trí.
- ConsentRecord tái lập được (version + ts + mục) là bằng chứng tuân thủ gửi TASK-COMPLY-001 - trả lời được "đồng ý gì, khi nào".
- Disclosure nêu thẳng ranh giới kỹ thuật (không mật khẩu, token không rời máy, chỉ productId/giá/số lượng) và để TASK-EXT-003 + TASK-TRUST-003 chứng minh - lời hứa kiểm chứng được.
- Mặc định tắt + granular + rút-được-ngay là ba trụ giữ đồng thuận hợp lệ; mọi đổi consent ghi record mới.

---

*Hết TASK-EXT-006. Status: ready_to_implement (mục tiêu audit 10/10).*
