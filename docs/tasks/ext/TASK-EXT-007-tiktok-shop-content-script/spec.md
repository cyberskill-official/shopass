---
id: TASK-EXT-007
title: "Content script TikTok Shop - đọc DOM giỏ trong webview/SPA, tránh API ký msToken/_signature/X-Bogus + app attestation; tái dùng khung reader/normalize/health của TASK-EXT-002"
module: EXT
priority: MUST
status: done
verify: T
...
created: 2026-06-27
related_frs: [TASK-EXT-002, TASK-EXT-003, TASK-EXT-006, TASK-SCRAPE-007, TASK-SCRAPE-006, TASK-TRUST-002, NFR-EXT-001]
depends_on: [TASK-EXT-002]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-EXT-37: tái dùng khung reader + normalize + health signal của TASK-EXT-002; chỉ thay lớp selector/đường đọc đặc thù TikTok (SPA route đổi -> quan sát DOM thay đổi, không dựa internal JSON)"
  ...
  - "DEC-EXT-39: giữ nguyên cam kết niềm tin TASK-EXT-002: KHÔNG mật khẩu, token/cookie KHÔNG rời client; chỉ trích productId/giá/qty đã render; chỉ đọc, KHÔNG sửa giỏ/đặt hàng"
...
disallowed_tools:
  ...
  - đọc/sao chép cookie/session token vào biến hay payload (vi phạm DEC-EXT-39 - phá cam kết niềm tin TASK-EXT-002)
  - thu thập mật khẩu hoặc tự sửa giỏ/đặt hàng (vi phạm DEC-EXT-39 - chỉ đọc)
  - phân nhánh khung reader riêng thay vì tái dùng normalize/health của TASK-EXT-002 (vi phạm DEC-EXT-37)
...
sub_tasks:
  ...
  - "2.0h: cart-reader.ts - đọc DOM giỏ (KHÔNG gọi API ký); orchestrate qua khung TASK-EXT-002; health signal khi hỏng"
...
6. Khi mọi selector hỏng, content script **MUST** phát health signal về service worker để TASK-SCRAPE-006 (DOM-change monitoring) ghi nhận - KHÔNG nuốt lỗi im lặng (tái dùng cơ chế TASK-EXT-002).
7. Content script **MUST** tái dùng khung reader + `normalize.ts` + health signal của TASK-EXT-002 (DEC-EXT-37); chỉ thay lớp selector/đường đọc đặc thù TikTok. **MUST NOT** phân nhánh một pipeline tối thiểu hóa riêng.
8. Content script **MUST** giữ nguyên cam kết niềm tin TASK-EXT-002 (DEC-EXT-39): KHÔNG đọc `document.cookie`/token, KHÔNG thu mật khẩu, token/cookie KHÔNG rời client; chỉ trích `productId`/giá/qty đã render.
9. Content script **MUST** chỉ đọc: KHÔNG tự sửa giỏ, KHÔNG đặt hàng, KHÔNG áp voucher tự động (DEC-EXT-39).
10. Kết quả **MUST** gửi cho service worker dạng `CartReadMessage` typed với `platform: "tiktok"`, chứa CHỈ `CartItem[]` + `VoucherItem[]` tối thiểu - đi tiếp qua pipeline TASK-EXT-003.
11. Content script **MUST** chỉ chạy khi consent "read_cart" (TASK-EXT-006) bật; gọi `ensureConsent` trước khi đọc.
...
**Vì sao tái dùng khung TASK-EXT-002 (DEC-EXT-37)?** TASK-EXT-002 đã định nghĩa khung reader + normalize (tối thiểu hóa) + health signal và đã được test cho cam kết "không rò cookie". Phân nhánh một pipeline riêng cho TikTok nghĩa là nhân đôi bug và mở một bề mặt mới có thể lọt credential. Tái dùng giữ một điểm kiểm soát tối thiểu hóa duy nhất; chỉ lớp selector/đường đọc là per-sàn.

**Vì sao giữ nguyên cam kết niềm tin (DEC-EXT-39)?** Ranh giới "không cookie, không mật khẩu, chỉ đọc" không phụ thuộc sàn - nó là định vị hậu-Honey của toàn extension (§5.4). TikTok không phải ngoại lệ. Test no-secret-leak tương tự TASK-EXT-002 khẳng định ranh giới này cho cả TikTok, không chỉ tin quy ước.

**Vì sao chờ render thay vì báo hỏng nhầm (§1 #12)?** Trong SPA, DOM trống ngay sau khi đổi route là bình thường (chưa vẽ xong), không phải lỗi parser. Nếu reader báo health "hỏng" mỗi lần gặp DOM trống tạm thời, TASK-SCRAPE-006 ngập tín hiệu nhiễu. Chờ observer báo render xong rồi mới đọc/đánh giá hỏng phân biệt đúng "chưa xong" với "thật sự hỏng".
...
### cart-reader.ts (đọc DOM, KHÔNG API ký; tái dùng normalize + health TASK-EXT-002)

```ts
// extension/src/content/tiktok/cart-reader.ts
import { normalizeCart } from "../shared/normalize";     // tái dùng TASK-EXT-002
import { reportHealth } from "../shared/health";         // tái dùng TASK-EXT-002

export async function readTiktokCart(): Promise<CartReadMessage> {
  if (!(await ensureConsent("read_cart"))) {             // TASK-EXT-006 gate
    return { type: "CART_READ", platform: "tiktok", items: [], vouchers: [] };
  }
  let raw = readCartFromDom();                            // CHỈ DOM; KHÔNG msToken/_signature/X-Bogus
  if (raw === null) {
    reportHealth({ platform: "tiktok", broke: "cart", source: "dom" }); // TASK-SCRAPE-006
    raw = [];
  }
...
8. Reader tái dùng `normalize.ts` + health của TASK-EXT-002 (grep import từ `content/shared/`), không phân nhánh riêng.
...
test("reader tái dùng normalize/health TASK-EXT-002, không phân nhánh", async () => {
...
Xem §3. Thứ tự: cập nhật `manifest.json` (content_scripts tiktok + host_permissions) -> `dom-selectors.ts` (bảng selector dự phòng TikTok) -> `spa-observer.ts` (MutationObserver/route change báo render xong) -> `cart-reader.ts` (đọc DOM, tái dùng normalize + health TASK-EXT-002, gate consent) -> `index.ts` (entry, gửi CartReadMessage platform tiktok) -> tests. KHÔNG chạm msToken/_signature/X-Bogus. Khung reader/normalize/health dùng chung với Shopee (TASK-EXT-002) và Lazada (TASK-EXT-008); chỉ lớp selector + observer SPA là đặc thù TikTok. Kết quả qua pipeline TASK-EXT-003.

---

## §7 - Phụ thuộc

- **TASK-EXT-002** - cung cấp khung reader + normalize + health signal tái dùng; ranh giới niềm tin chung.
- **TASK-EXT-003** - pipeline tối thiểu hóa nhận `CartReadMessage(platform:tiktok)`, lọc lần hai.
- **TASK-EXT-006** - consent "read_cart" gate trước khi đọc.
- **TASK-SCRAPE-006** - DOM-change monitoring nhận health signal khi parser TikTok hỏng.
- **TASK-SCRAPE-007 (song song, P2)** - adapter TikTok backend (ưu tiên DOM-render, né API ký); reader này là phía client.
- **TASK-TRUST-002** - local-first + tối thiểu hóa; reader giữ cam kết.
...
### Health signal khi parser TikTok hỏng (-> TASK-SCRAPE-006)
...
- Xử lý TikTok in-app webview (ngoài trình duyệt desktop) - hoãn tới mobile (TASK-MOBILE-002); task này nhắm web desktop.
...
| Parse hỏng nuốt im lặng | thiếu health signal | mất tính năng âm thầm | Bắt buộc health -> TASK-SCRAPE-006 (§1 #6) |
| Phân nhánh pipeline riêng | grep import shared | nhân đôi bug + bề mặt rò | Tái dùng normalize/health TASK-EXT-002 (DEC-EXT-37) |
...
- Tái dùng khung reader/normalize/health của TASK-EXT-002 giữ một điểm kiểm soát tối thiểu hóa duy nhất; chỉ selector + observer là per-sàn.
- Cam kết niềm tin (không cookie/mật khẩu, chỉ đọc) không phụ thuộc sàn - test no-secret-leak khẳng định cho cả TikTok.

---

*Hết TASK-EXT-007. Status: ready_to_implement (mục tiêu audit 10/10).*
