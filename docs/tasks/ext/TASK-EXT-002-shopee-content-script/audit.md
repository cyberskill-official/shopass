...
TASK-EXT-002 đặc tả content script Shopee đọc giỏ/voucher theo nguyên tắc session piggyback của §3.2. Tôi đối chiếu lại từng mệnh đề: nguyên tắc lõi ("content script chạy trong tab đăng nhập, mượn cookie first-party, KHÔNG thu mật khẩu, token KHÔNG rời client, chỉ gửi productId/giá/qua tối thiểu hóa") được hiện thực đúng - `fetch` dùng `credentials:'include'` để trình duyệt tự gắn cookie mà code KHÔNG đọc giá trị cookie; cấm `document.cookie` được test grep khẳng định, không chỉ quy ước. Ưu tiên internal JSON `/api/v4/cart/get` rồi fallback DOM nhiều selector (A/B resilient) + health signal khớp §3.2 mục Shopee. Kết luận độc lập: 10/10.

## §2 - Findings

Frontmatter đầy đủ và hợp lệ (id khớp tên file, module EXT, priority MUST, status ready_to_implement; depends_on TASK-EXT-001 đúng, blocks gồm TASK-EXT-007/008 hợp lý; new_files/sub_tasks/risk_if_skipped không rỗng). §1 có 12 mệnh đề; §4 có 11 AC; §5 có 3 test file. Quét typography: mọi `->` chỉ trong code block ts; prose ASCII thuần, không em-dash/curly quote.
...
- Không có bảng CSDL phát lệnh ở task này (cart_snapshot/cart_item thuộc TASK-CART-002); CartItem/VoucherItem là kiểu message client, không cần đối chiếu DATA-MODEL. Không phát hiện defect cần sửa trong lượt này.
...
| #7 health signal khi hỏng hẳn | #8 | reportHealth -> TASK-SCRAPE-006 |
...
| #12 local-first, chỉ tối thiểu đi tiếp | #3, #8 | normalize.ts + TASK-EXT-003 |
...
*Hết audit độc lập TASK-EXT-002.*
