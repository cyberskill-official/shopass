---
id: TASK-CART-004
title: "Engine luật stacking per-country - VN cho stack 1 shop + 1 platform + freeship (vd 110k); MY/PH 2025 bỏ stacking, freeship gộp nhóm platform (vd 80k); hiện thực StackRules đọc CountryPolicy (TASK-INFRA-005)"
module: CART
priority: MUST
status: done
verify: T
...
created: 2026-06-28
related_frs: [TASK-CART-003, TASK-INFRA-005, TASK-CART-001, TASK-COMPLY-006]
depends_on: [TASK-CART-003, TASK-INFRA-005]
blocks: [TASK-COMPLY-006]
source_pages:
...
source_decisions:
  - "DEC-CART-19: luật stacking đọc từ CountryPolicy của TASK-INFRA-005 (voucher_stacking_allowed + cách gộp nhóm) - KHÔNG hardcode if-country trong optimizer (DEC-CART-14); engine là hiện thực StackRules"
  ...
  - "DEC-CART-22: gộp nhóm thể hiện qua stack_group (TASK-CART-001 DEC-CART-04) - hai voucher cùng stack_group loại trừ lẫn nhau; MY/PH gán freeship cùng stack_group với platform; VN để khác nhóm (stack được)"
...
language: "Go 1.22 (cart-svc); hiện thực interface StackRules (TASK-CART-003) đọc CountryPolicy (TASK-INFRA-005)"
...
risk_if_skipped: "Luật stacking per-country là điều kiện bắt buộc để mở SEA mà không sai con số tiền cho người dùng: tài liệu nêu rõ MY & PH đã bỏ stacking voucher 2025 (§5.7, §3.9), trong khi VN vẫn cho stack. Nếu áp luật VN cho MY/PH thì optimizer tính ra tổng giảm 110k trong khi luật MY/PH thực tế chỉ cho 80k - hiển thị con số cao hơn thực tế, người dùng thất vọng lúc thanh toán và mất niềm tin (đụng định vị minh bạch). Ngược lại áp no-stack cho VN thì bỏ lỡ giá trị, người dùng VN thấy SănDeal kém hơn đối thủ. Đây là task gating per-country mà §2 tài liệu nhấn mạnh là bắt buộc. Nếu hardcode if-country vào optimizer lõi thì mỗi nước mới phải sửa lõi thuật toán (TASK-CART-003) thay vì thêm một policy - vi phạm tách biệt và dễ sai. Mặc định cho stack khi nước chưa cấu hình là nguy hiểm nhất: hiển thị giảm cao hơn luật thực tế ở nước chưa kiểm - phải mặc định no-stack (an toàn)."
---

## §1 - Mô tả (BCP-14 normative)

Service CART **MUST** hiện thực engine luật stacking per-country (interface `StackRules` của TASK-CART-003) đọc `CountryPolicy` của TASK-INFRA-005: VN cho stack 1 shop + 1 platform + freeship; MY/PH (2025) bỏ stacking đa nhóm và gộp freeship vào nhóm platform; nước chưa cấu hình mặc định no-stack. Hợp đồng:

1. **MUST** hiện thực `StackRules.ValidStack(pv, fs, shopVouchers)` (interface TASK-CART-003) cho từng chính sách nước, đọc cấu hình từ `CountryPolicy` (TASK-INFRA-005) - KHÔNG hardcode `if country == ...` trong optimizer lõi (DEC-CART-19).
...
4. Việc gộp nhóm **MUST** thể hiện qua `stack_group` (DEC-CART-22, TASK-CART-001 DEC-CART-04): voucher cùng `stack_group` loại trừ lẫn nhau. Với MY/PH, freeship mang cùng `stack_group` với platform; với VN, freeship khác nhóm (stack được). `ValidStack` từ chối tổ hợp có hai voucher cùng `stack_group`.
5. Engine **MUST** đọc cờ từ `CountryPolicy`: tối thiểu `voucher_stacking_allowed` (TASK-INFRA-005 DEC-INFRA-22) và quy tắc gộp freeship-platform; KHÔNG nhúng luật cứng tách rời cấu hình nước.
6. `factory` **MUST** chọn `StackRules` đúng theo country của thao tác (suy từ `platform.country`, TASK-INFRA-005 DEC-INFRA-25): VN -> VN rules; MY/PH -> MY/PH rules; nước khác -> theo `CountryPolicy` của nước đó.
7. Nước chưa cấu hình `CountryPolicy` **MUST** mặc định policy hạn chế nhất (no-stack) (DEC-CART-23, TASK-INFRA-005 DEC-INFRA-23) - tối đa một voucher mỗi loại, không stack đa nhóm; KHÔNG mặc định cho stack.
...
9. Engine **MUST NOT** tự tính giá trị giảm (đó là việc của TASK-CART-003 `discount.go`); engine CHỈ quyết tổ hợp nào hợp lệ (`ValidStack` trả bool). Tách quyết-định-hợp-lệ khỏi tính-giá-trị.
...
```go
// services/cart/internal/optimizer/stacking/policy_rules.go
// PolicyStackRules đọc CountryPolicy (TASK-INFRA-005) thay vì hardcode luật (DEC-CART-19).
...
```go
// services/cart/internal/optimizer/stacking/factory.go
import "shopass/region" // CountryPolicy (TASK-INFRA-005)
...
8. Ví dụ §3.5(3) đầu-cuối: với optimizer (TASK-CART-003) + VN rules, combo tối ưu ra 110k; với MY/PH rules, combo tối ưu ra 80k (platform+freeship bị loại, chọn max).
...
Xem §3. Thứ tự: `policy_rules.go` (`PolicyStackRules` đọc cờ stacking + gộp nhóm) -> `vn_rules.go` + `mypph_rules.go` (cấu hình policy cho VN và MY/PH) -> `factory.go` (`RulesForCountry` chọn theo country, mặc định no-stack) -> cập nhật handler `optimize.go` (suy country từ `platform.country` rồi chọn rules) -> tests. Engine hiện thực interface `StackRules` của TASK-CART-003 - optimizer lõi gọi `ValidStack`, không biết luật cụ thể. `CountryPolicy` đến từ TASK-INFRA-005 (region package); country suy từ `platform.country` (DEC-INFRA-25). Test vàng là hai ví dụ §3.5(3): VN ra 110k, MY/PH ra 80k - chạy qua optimizer thật (TASK-CART-003) với rules tương ứng.

---

## §7 - Phụ thuộc

- **TASK-CART-003** - định nghĩa interface `StackRules` mà engine này hiện thực; optimizer lõi gọi `ValidStack` (depends_on cứng).
- **TASK-INFRA-005** - cung cấp `CountryPolicy` (`voucher_stacking_allowed`, quy tắc gộp) + cách suy country từ `platform.country`; engine đọc policy thay vì hardcode (depends_on cứng).
- **TASK-CART-001** - `stack_group` trên voucher là cơ chế thể hiện gộp nhóm/loại trừ mà engine đọc.
- **TASK-COMPLY-006 (đồng hướng, P3)** - khung per-country gating tổng quát; luật stacking là một mặt của gating này.
...
### CountryPolicy đọc từ TASK-INFRA-005 (minh họa)

```yaml
# region/config/countries.yaml (TASK-INFRA-005)
...
Đã chốt. Hoãn:
- Luật stacking các nước SEA còn lại (ID/TH/SG/TW) - thêm policy khi mở từng nước (TASK-COMPLY-007), mặc định no-stack tới khi xác minh.
- Cashback gộp nhóm (MY/PH gộp cả cashback nhóm platform) - mô hình khi có task cashback (TASK-AFFIL-005).
- Luật giới hạn số voucher shop trên một đơn nếu sàn thay đổi - thêm cờ vào CountryPolicy.
- Theo dõi thay đổi luật theo thời gian (versioned policy) - bám config versioned của TASK-INFRA-005.
...
- Engine đọc CountryPolicy (TASK-INFRA-005) thay vì hardcode - thêm nước là thêm cấu hình, không sửa lõi optimizer.
...
- Engine chỉ quyết tổ hợp hợp lệ (bool); tính giá trị giảm là việc của TASK-CART-003 - hai trách nhiệm tách riêng.
- Test vàng là hai con số §3.5(3): VN 110k, MY/PH 80k qua optimizer thật - bằng chứng luật per-country đúng.

---

*Hết TASK-CART-004. Status: ready_to_implement (mục tiêu audit 10/10).*
