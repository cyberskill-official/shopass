...
created: 2026-06-28
related_frs: [TASK-EXT-002, TASK-CART-005, TASK-NOTIF-001, NFR-AFFIL-001]
...
source_decisions:
  ...
  - "DEC-CART-31: chỉ ĐỌC trạng thái nhiệm vụ xu hiển thị (đã làm/chưa làm) qua khung reader TASK-EXT-002; KHÔNG mô phỏng click, KHÔNG gọi API hoàn thành nhiệm vụ"
  ...
  - "DEC-CART-33: dùng user_activity_coin_task (§3.4) lưu trạng thái nhắc per user/platform; reminder qua TASK-NOTIF-001 (nhắc nhẹ, không spam)"
  - "DEC-CART-34: KHÔNG thu thập credential khi đọc trạng thái xu (đồng nhất ranh giới TASK-EXT-002/003); chỉ trạng thái done/chưa-done của nhiệm vụ hiển thị"
...
2. `coin-task-reader.ts` **MUST** chỉ ĐỌC trạng thái nhiệm vụ xu hiển thị qua khung reader TASK-EXT-002 (DEC-CART-31): KHÔNG mô phỏng click (`element.click()`, dispatch sự kiện chuột), KHÔNG gọi API hoàn thành nhiệm vụ.
...
5. Reminder **MUST** gửi qua TASK-NOTIF-001 dạng nhắc nhẹ (DEC-CART-33), KHÔNG spam - tối đa nhắc hợp lý mỗi ngày, tôn trọng tùy chọn người dùng.
6. Quá trình đọc trạng thái xu **MUST NOT** thu thập credential (DEC-CART-34): không đọc cookie/token (đồng nhất ranh giới TASK-EXT-002/003); chỉ lấy trạng thái done/chưa-done của nhiệm vụ hiển thị.
...
10. Toàn bộ đường đọc **MUST** local-first (TASK-TRUST-002): trạng thái đọc trên máy client; chỉ trạng thái tối thiểu (done/chưa) đồng bộ nếu cần nhắc.
...
**Vì sao user tự bấm trên sàn (DEC-CART-32)?** SănDeal hiện checklist "hôm nay bạn chưa điểm danh, chưa xem live" và nút dẫn tới trang nhiệm vụ; người dùng tự bấm điểm danh trên sàn. Quyền và hành động ở người dùng - vừa tránh ban, vừa minh bạch (không giả vờ tự động giúp). Đây là cùng triết lý với auto-test mã user-initiated (TASK-CART-005).

**Vì sao không spam reminder (DEC-CART-33)?** Nhắc quá nhiều phản tác dụng - người dùng tắt thông báo. Nhắc nhẹ (một lần mỗi ngày cho nhiệm vụ chưa làm, tôn trọng tùy chọn) giữ giá trị mà không phiền. Reminder qua TASK-NOTIF-001 với tần suất hợp lý.

**Vì sao không thu thập credential (DEC-CART-34)?** Đồng nhất ranh giới niềm tin lõi (TASK-EXT-002/003): đọc trạng thái nhiệm vụ hiển thị không cần và không được chạm cookie/token. Chỉ lấy done/chưa-done. Giữ cam kết tối thiểu hóa ngay cả ở tính năng phụ này.
...
export function readCoinTasks(): CoinTask[] {
  const rows = queryCoinTaskRows(); // đọc DOM khu nhiệm vụ xu (khung reader TASK-EXT-002)
  return rows.map((el) => ({
...
8. Reminder gửi qua TASK-NOTIF-001 nhắc nhẹ (không spam); tôn trọng tùy chọn.
...
Xem §3. Thứ tự: `types.ts` (CoinTask, CoinChecklistMessage) -> `coin-task-reader.ts` (CHỈ đọc trạng thái hiển thị, không click/API) -> `0012_coin_task.up/down.sql` (bảng user_activity_coin_task) -> `coin/types.go` + `repo.go` (lưu/đọc scope user) -> `coin-checklist.ts` (UI hiện checklist + link dẫn user tự bấm, ghi chú minh bạch) -> tests. Migration 0012 nối tiếp 0011 (TASK-CART-002). Reader tái dùng khung TASK-EXT-002 nhưng tuyệt đối chỉ-đọc (không có ngoại lệ áp-gỡ như TASK-CART-005 vì xu không có thao tác đo). Reminder nối TASK-NOTIF-001 nhắc nhẹ. Test then chốt là grep khẳng định không auto-click/dispatch/API-hoàn-thành - ranh giới chống ban High phải kiểm chứng được.

---

## §7 - Phụ thuộc

- **TASK-EXT-002** - khung content script + reader (chỉ-đọc); `coin-task-reader` đọc trạng thái nhiệm vụ xu hiển thị, tuyệt đối không click (depends_on cứng).
- **TASK-NOTIF-001 (downstream)** - template + routing để gửi reminder nhắc nhẹ; checklist không tự gửi, dùng kênh notif.
- **TASK-CART-005 (sibling)** - auto-test mã cùng triết lý user-initiated/không-tự-động; checklist xu còn nghiêm hơn (không có cả thao tác đo, chỉ nhắc).
...
extension đọc trạng thái nhiệm vụ xu hiển thị (CHỈ đọc)
  ...
  -> reminder nhắc nhẹ qua TASK-NOTIF-001 (không spam)
...
- Mở rộng đọc nhiệm vụ xu TikTok Shop/Lazada (khung TASK-EXT-007/008) - per-sàn sau, vẫn chỉ-đọc.
- Gamification điểm SănDeal nội bộ cho việc hoàn thành (§6 #10) - tách task riêng, không auto-click sàn.
...
- Không thu thập credential khi đọc trạng thái - đồng nhất cam kết tối thiểu hóa lõi (TASK-EXT-002/003).
- Reminder nhắc nhẹ qua TASK-NOTIF-001, không spam; tôn trọng tùy chọn người dùng.
...
*Hết TASK-CART-006. Status: ready_to_implement (mục tiêu audit 10/10).*
