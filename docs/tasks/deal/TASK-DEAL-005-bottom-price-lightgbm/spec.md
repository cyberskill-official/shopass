---
id: TASK-DEAL-005
title: "Dự đoán đáy giá bằng LightGBM - feature store thống nhất train/serve, nhãn future_min_price_14d dựng bằng forward-join, cổng >=180 ngày lịch sử (dưới ngưỡng fallback Prophet TASK-DEAL-004), giữ chung hợp đồng tín hiệu P(bottom within 14d)"
module: DEAL
priority: SHOULD
status: done
verify: T
...
created: 2026-06-27
related_frs: [TASK-DEAL-004, TASK-DEAL-002, TASK-DEAL-006, TASK-PRICE-002]
depends_on: [TASK-DEAL-004]
blocks: []
source_pages:
  - "docs/TÀI LIỆU NỀN TẢNG SẢN PHẨM SănDeal §3.5 (LightGBM target future_min_price_14d, feature set, >=180d history)"
source_decisions:
  - "DEC-DEAL-40: dùng LightGBM khi SKU có >=180 ngày lịch sử; dưới ngưỡng fallback về Prophet baseline TASK-DEAL-004 / category prior TASK-DEAL-002"
  ...
  - "DEC-DEAL-44: giữ chung hợp đồng tín hiệu P(bottom within 14d) với TASK-DEAL-004 để TASK-DEAL-006 cảnh báo bất khả tri theo mô hình"
...
modified_files:
  - services/ml/bottom/serve.py                 # route SKU >=180 ngày sang LightGBM, dưới ngưỡng giữ Prophet (TASK-DEAL-004)
allowed_tools:
...
disallowed_tools:
  ...
  - đổi hình dạng tín hiệu P(bottom within 14d) khác TASK-DEAL-004 (vi phạm DEC-DEAL-44, phá TASK-DEAL-006)
...
sub_tasks:
  ...
  - "1.0h: serve.py - cổng >=180 ngày: route LightGBM, dưới ngưỡng gọi Prophet TASK-DEAL-004; ghi price_forecast model_kind='lgbm'"
...
risk_if_skipped: "Prophet baseline (TASK-DEAL-004) chỉ mô hình hóa xu hướng và mùa vụ một chiều, không bắt được tương tác giữa các đặc trưng (ví dụ double-date trùng cửa sổ payday cộng hưởng làm đáy sâu hơn tổng từng yếu tố). Với SKU đã có lịch sử dài (>=180 ngày), dữ liệu đủ để một mô hình gradient boosting học các tương tác đó và dự đoán đáy 14 ngày tới chính xác hơn rõ rệt, nâng chất lượng cảnh báo của TASK-DEAL-006. Thiếu feature store thì train và serve dễ tính đặc trưng lệch nhau (train/serve skew), mô hình trông tốt lúc offline nhưng sai lúc online - lỗi âm thầm và rất khó truy. Thiếu kỷ luật forward-join chỉ lúc train thì nhãn rò tương lai vào feature, mô hình học gian và sụp khi chạy thật. Thiếu cổng >=180 ngày thì LightGBM bị chạy trên SKU dữ liệu mỏng, thua cả Prophet. Thiếu việc giữ chung hợp đồng P(bottom within 14d) thì TASK-DEAL-006 phải biết mình đang gọi mô hình nào - phá thế bất khả tri theo mô hình và làm tầng cảnh báo dễ vỡ khi ta đổi mô hình."
---

## §1 - Mô tả (BCP-14 normative)

Service ML (ml-svc) **MUST** nâng cấp bộ dự đoán đáy giá từ Prophet baseline (TASK-DEAL-004) lên LightGBM cho các SKU có lịch sử dài, qua một feature store thống nhất giữa train và serve, một nhãn `future_min_price_14d` dựng bằng forward-join chỉ lúc train, và một cổng đủ điều kiện `>=180` ngày. Hợp đồng:
...
2. **MUST** tính mọi đặc trưng từ `price_daily`/`price_snapshot` (TASK-PRICE-002) cộng `tracked_product.category_id` và `platform_id`, tất cả tính tới mốc `as_of_date` và KHÔNG dùng bất kỳ điểm dữ liệu nào sau `as_of_date` (đặc trưng chỉ nhìn về quá khứ).
...
7. **MUST** đặt cổng đủ điều kiện: chỉ train và serve LightGBM cho SKU có `>=180` ngày lịch sử; SKU dưới ngưỡng **MUST** fallback về Prophet baseline (TASK-DEAL-004), và nếu cả Prophet cũng chưa đủ thì về category prior (TASK-DEAL-002) (DEC-DEAL-40).
...
10. **MUST** phát ra cùng một hợp đồng tín hiệu như TASK-DEAL-004: cùng tên trường `p_bottom_14d` (thực `[0,1]`), cùng `expected_min_14d`, cùng `horizon_days = 14`, chỉ khác `model_kind = 'lgbm'`. TASK-DEAL-006 tiêu thụ tín hiệu này mà KHÔNG cần biết mô hình nào sinh ra (bất khả tri theo mô hình, DEC-DEAL-44).
11. **MUST** ghi dự đoán vào bảng chia sẻ `price_forecast` (cùng bảng TASK-DEAL-004 ghi) với `model_kind = 'lgbm'`, `p_bottom_14d`, `expected_min_14d`, `as_of_date`, `horizon_days = 14`.
...
**Vì sao giữ chung hợp đồng P(bottom within 14d) với TASK-DEAL-004 (DEC-DEAL-44)?** TASK-DEAL-006 (cảnh báo) chỉ nên quan tâm "xác suất giờ là đáy", không nên quan tâm mô hình nào tính ra nó. Nếu LightGBM và Prophet phát hai hình dạng tín hiệu khác nhau thì tầng cảnh báo phải rẽ nhánh theo mô hình, rồi vỡ mỗi lần ta đổi mô hình hay thêm mô hình thứ ba. Giữ chung đúng một hợp đồng (`p_bottom_14d`, `expected_min_14d`, `horizon_days`, chỉ khác `model_kind`) làm tầng trên bất khả tri theo mô hình - ta thay engine bên dưới mà không động tới TASK-DEAL-006.
...
7. SKU `<180` ngày -> fallback Prophet (TASK-DEAL-004), KHÔNG chạy LightGBM; SKU `>=180` ngày -> chạy LightGBM.
...
10. Tín hiệu phát ra cùng schema TASK-DEAL-004: `p_bottom_14d`, `expected_min_14d`, `horizon_days=14`, chỉ khác `model_kind='lgbm'`.
...
def test_eligibility_180d_gate():
    assert route_model(days_history=179) == "prophet"   # fallback TASK-DEAL-004
...
def test_p_bottom_contract_matches_prophet(one_feat_row):
    ...
    assert 0.0 <= p <= 1.0                                # cùng miền tín hiệu như TASK-DEAL-004
```
...
Xem §3. Thứ tự: migration `0002_feature_store.sql` (bảng + index) -> `feature_store.py` (`build_features` + `FEATURE_COLS`) -> `labels.py` (`future_min_price_14d`, chỉ pipeline train) -> `lgbm_model.py` (`train`/`predict`/`p_bottom`) -> sửa `serve.py` (cổng `route_model` theo số ngày: `>=180` LightGBM, dưới ngưỡng gọi Prophet TASK-DEAL-004) -> tests. Pipeline train chạy theo lô: dựng `feature_store` cho dải `as_of_date` lịch sử, điền nhãn bằng forward-join, fit `LGBMRegressor`, lưu model artifact. Đường serve chỉ đọc một hàng đặc trưng tại `as_of = today` (cột nhãn trống), gọi `predict` + `p_bottom`, ghi `price_forecast`. Một hàm `build_features` duy nhất dùng cho cả hai để chặn skew.

---

## §7 - Phụ thuộc

- **TASK-DEAL-004** - Prophet baseline; TASK-DEAL-005 fallback về nó cho SKU `<180` ngày và dùng chung bảng `price_forecast` (TASK-DEAL-004 sở hữu DDL) + hợp đồng tín hiệu. Ánh xạ cột khi ghi: `expected_min_14d` của task này ghi vào cột `yhat`; `as_of_date` ghi vào cột `run_date`; `horizon_days = 14` ứng với cột `horizon_day` (1..14); `model_kind = 'lgbm'` đã nằm trong CHECK của bảng. Không tạo bảng `price_forecast` thứ hai - chỉ INSERT vào bảng do TASK-DEAL-004 định nghĩa.
- **TASK-DEAL-002** - category prior là fallback cuối khi cả Prophet chưa đủ; `category_seasonality` mượn ý tưởng gộp theo category.
- **TASK-PRICE-002** - `price_daily` (continuous aggregate) và `price_snapshot` là nguồn cho mọi đặc trưng và nhãn.
- **TASK-DEAL-006 (downstream)** - tiêu thụ tín hiệu `p_bottom_14d` chia sẻ, bất khả tri theo `model_kind`.
...
- Hiệu chỉnh xác suất (calibration) cho `p_bottom_14d` để con số đọc đúng là xác suất - tách sang task riêng khi có dữ liệu kết quả.
...
| Hợp đồng tín hiệu lệch TASK-DEAL-004 | test p_bottom_contract | TASK-DEAL-006 phải rẽ nhánh theo mô hình | Cùng `p_bottom_14d`/`expected_min_14d`/`horizon_days` (§1 #10) |
...
- LightGBM là nâng cấp theo SKU, không thay thế toàn cục: nó chỉ nhận các SKU `>=180` ngày, phần còn lại vẫn do Prophet (TASK-DEAL-004) và category prior (TASK-DEAL-002) lo - một cầu thang chất lượng theo độ dày dữ liệu.
...
- Giữ chung hợp đồng `p_bottom_14d` với TASK-DEAL-004 là điều khiến TASK-DEAL-006 không phải biết mô hình nào - đổi engine bên dưới không động tới tầng cảnh báo.
...
*Hết TASK-DEAL-005. Status: ready_to_implement (mục tiêu audit 10/10).*
