...
Audit độc lập, suy lại từ file hiện tại. TASK-DEAL-002 đặc tả chính sách cold-start cho sale ảo + biểu đồ: ba trạng thái trưởng thành NEW (<14 ngày) / WARMING (14-90) / MATURE (>=90), cổng `IsFeatureReady` 90 ngày trước khi công khai tính năng cho 1 SKU (đúng §5.1), và `category_prior` materialized view làm fallback chỉ gộp từ SKU MATURE. Mệnh đề khớp §3.5 (SKU <14 ngày trả UNKNOWN) và §5.1 (baseline ~90 ngày). 13 mệnh đề §1 có AC §4 và test §5. Sàn `min_sample_count` (30) chặn prior mỏng. Đạt 10/10.

## §2 - Findings (đã kiểm trong lượt này)

### ISS-001 - Ngưỡng trưởng thành nhất quán với TASK-DEAL-001 (đã xác nhận)
NEW <14 ngày trả UNKNOWN khớp đúng biên 14 của TASK-DEAL-001 (`len(hist)<14 -> UNKNOWN`); cổng MATURE 90 ngày khớp cửa sổ 90 ngày của sale ảo. Hằng số có tên `warmingDays=14`, `matureDays=90` (không số ma thuật). Không cần sửa.
...
View tính từ `price_daily` (TASK-PRICE-002) + `tracked_product` (TASK-PRICE-001); `idx_category_prior_cat` unique tạo cùng migration để `REFRESH ... CONCURRENTLY` hợp lệ. Failure mode "thiếu unique index" ghi rõ ở §10. Không cần sửa.
...
| #3 NEW -> UNKNOWN | AC #3 | handoff TASK-DEAL-001, AC #3 |
...
*Hết audit độc lập TASK-DEAL-002.*
