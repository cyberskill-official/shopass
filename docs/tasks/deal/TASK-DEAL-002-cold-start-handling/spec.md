...
created: 2026-06-27
related_frs: [TASK-DEAL-001, TASK-DEAL-004, TASK-PRICE-002, TASK-PRICE-001]
depends_on: [TASK-DEAL-001]
blocks: [TASK-DEAL-004]
source_pages:
...
source_decisions:
  ...
  - "DEC-DEAL-13: SKU <14 ngày luôn trả verdict UNKNOWN (nhất quán với TASK-DEAL-001), không đoán mò"
...
sub_tasks:
  ...
  - "0.5h: detect.go - chèn cổng maturity trước khi surface verdict tự tin (handoff TASK-DEAL-001)"
...
3. **MUST** trả verdict `UNKNOWN` cho mọi SKU ở trạng thái `NEW` (`< 14` ngày), nhất quán với TASK-DEAL-001 (DEC-DEAL-13). Không đoán mò khi dữ liệu chưa đủ.
...
3. SKU `NEW` (`d<14`) -> verdict `UNKNOWN` (handoff TASK-DEAL-001), không có ngoại lệ.
...
Xem §3. Thứ tự: migration `0001_category_prior.sql` (materialized view + unique index) -> `maturity.go` (State + Maturity + IsFeatureReady) -> `priors.go` (PriorFor + sàn min_sample_count) -> chèn cổng vào `detect.go` (handoff TASK-DEAL-001) -> tests. Materialized view tính từ `price_daily` (TASK-PRICE-002) và `tracked_product` (TASK-PRICE-001); refresh hằng ngày qua scheduler của deal-svc (`REFRESH MATERIALIZED VIEW CONCURRENTLY category_prior`, cần unique index nên đã tạo `idx_category_prior_cat`). Driver dùng `pgx`. Cổng `IsFeatureReady` đặt ngay đầu nhánh surface verdict công khai trong `detect.go`: nếu `false`, trả nhánh "đang tích lũy" thay vì verdict.

---

## §7 - Phụ thuộc

- **TASK-DEAL-001** - bộ phát hiện sale ảo cung cấp verdict enum (gồm `UNKNOWN`); task này nối vào cổng maturity trước khi surface verdict tự tin.
- **TASK-PRICE-001** - `tracked_product.first_seen` và `category_id` là input tính `daysOfHistory` và khóa gộp prior.
- **TASK-PRICE-002** - `price_daily` (continuous aggregate) và `price_snapshot` là nguồn cho `category_prior`.
- **TASK-DEAL-004 (downstream)** - Prophet baseline dùng category priors làm điểm khởi đầu cho SKU cold-start.
...
- Prior theo trục thời gian (mùa double-date, payday) ngoài median tĩnh - tách sang TASK-DEAL-004 nơi Prophet xử lý mùa vụ.
...
- Cold-start là bài toán con-gà-quả-trứng cốt lõi của SănDeal (§5.1): tính năng cần lịch sử, lịch sử cần thời gian. task này biến nó thành chính sách rõ ràng thay vì hành vi ngầm.
...
- Ưu tiên backfill sớm cho top SKU (TASK-SCRAPE-002) để chúng qua cổng 90 ngày càng nhanh càng tốt - đây là đòn bẩy rút ngắn thời gian ra mắt tính năng.
- category_prior là cầu nối tới TASK-DEAL-004: cùng một prior dùng cho fallback verdict cũng là điểm khởi đầu cho Prophet baseline trên SKU cold-start.

---

*Hết TASK-DEAL-002. Status: ready_to_implement (mục tiêu audit 10/10).*
