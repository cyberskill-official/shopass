...
created: 2026-06-27
related_frs: [TASK-DEAL-004, TASK-DEAL-005, TASK-TRACK-003, TASK-NOTIF-003, TASK-DEAL-002]
depends_on: [TASK-DEAL-004, TASK-TRACK-003]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-DEAL-52: model-agnostic - đọc cột p_bottom_14d chung của bảng price_forecast do Prophet (TASK-DEAL-004) hoặc LightGBM (TASK-DEAL-005) sinh, batch không biết và không quan tâm model nào"
  ...
  - "DEC-DEAL-54: chỉ chấm điểm SKU đã qua maturity gate MATURE (TASK-DEAL-002 IsFeatureReady) và chỉ khớp alert_rule type 'bottom_predicted' đang bật (TASK-TRACK-003) rồi enqueue vào notification fan-out (TASK-NOTIF-003)"

language: "Go 1.22 (deal-svc batch orchestrator); reads price_forecast (Postgres, do Python sinh), enqueues to notification fan-out (TASK-NOTIF-003)"
...
sub_tasks:
  ...
  - "0.5h: enqueue vào notification fan-out (TASK-NOTIF-003) + ghi bottom_alert_log"
...
2. **MUST** chỉ chấm điểm SKU đã đạt `MATURE` qua maturity gate `IsFeatureReady` của TASK-DEAL-002 (DEC-DEAL-54): query `price_forecast` JOIN điều kiện trưởng thành, loại SKU `NEW`/`WARMING` khỏi vòng chấm điểm.
3. **MUST** đọc cột `p_bottom_14d` chung của `price_forecast` (DEC-DEAL-52): batch KHÔNG biết và KHÔNG quan tâm dòng đó do Prophet (TASK-DEAL-004) hay LightGBM (TASK-DEAL-005) sinh; hai model hoán đổi được qua cùng một hợp đồng bảng.
...
6. **MUST** với mỗi SKU vượt ngưỡng, join sang `alert_rule` type `'bottom_predicted'` đang bật (`active = true`) để lấy danh sách user đang theo dõi SKU đó muốn nhận cảnh báo đáy (TASK-TRACK-003); SKU không có luật khớp thì bỏ qua, không enqueue gì.
...
9. **MUST** enqueue mỗi cảnh báo đã qua dedupe vào notification fan-out (TASK-NOTIF-003) với payload gồm `user_id`, `product_id`, `p_bottom_14d`, và lý do `reason = "bottom_predicted"`; fan-out lo chuyện chọn kênh và rate-limit phía người dùng.
...
**Vì sao chỉ chấm SKU MATURE (DEC-DEAL-54)?** Dự báo đáy trên SKU chưa đủ 90 ngày lịch sử là dự báo trên cát: cửa sổ quá ngắn để mô hình mùa vụ có nghĩa. TASK-DEAL-002 đã chốt cổng `IsFeatureReady` cho mọi tính năng công khai; cảnh báo đáy đi qua cùng cổng đó để không bao giờ bắn dựa trên dự báo từ dữ liệu mỏng. Giữ nhất quán một định nghĩa "đủ chín" toàn module.
...
Xem §3. Thứ tự: migration `0002_bottom_alert_log.sql` (bảng + UNIQUE + index cooldown) -> `dedupe.go` (shouldSkip + enqueueAndLog) -> `alert_match.go` (matchBottomRules) -> `nightly_score.go` (RunNightlyScore) -> đăng ký cron trong `main.go` -> tests. Cron dùng scheduler sẵn của deal-svc (cùng cơ chế refresh `category_prior` của TASK-DEAL-002), đặt khung `02:00 Asia/Ho_Chi_Minh` và bọc advisory lock `pg_try_advisory_lock` để một instance chạy tại một thời điểm. Driver dùng `pgx`. Notification fan-out là interface `notif.Enqueuer` (TASK-NOTIF-003) inject vào `Batch`, nên test thay bằng fake đếm mục, không cần hệ thống thật. Bảng `price_forecast` do tầng Python (TASK-DEAL-004/005) sinh và là hợp đồng đọc-chỉ của batch này.

---

## §7 - Phụ thuộc

- **TASK-DEAL-004** (depends_on) - Prophet baseline sinh `price_forecast.p_bottom_14d`; là nguồn dự báo chính cho batch.
- **TASK-DEAL-005** (related) - LightGBM thay thế/song song, ghi cùng cột `p_bottom_14d` - đây là điều làm batch model-agnostic.
- **TASK-TRACK-003** (depends_on) - định nghĩa `alert_rule` type `'bottom_predicted'`; batch join vào để biết user nào muốn cảnh báo đáy cho SKU nào.
- **TASK-NOTIF-003** (consumer) - notification fan-out nhận mục enqueue, lo chọn kênh và rate-limit phía người dùng.
- **TASK-DEAL-002** (related) - maturity gate `IsFeatureReady`/`MATURE` quyết định SKU nào đủ chín để chấm điểm.
...
- Có nên kèm "khoảng cách tới đáy ước lượng" (giá đáy dự báo) vào payload ngoài xác suất - chờ TASK-DEAL-004/005 expose thêm cột rồi mở rộng hợp đồng.
- Gộp nhiều SKU vượt ngưỡng của cùng một user thành một thông báo tóm tắt (digest) thay vì từng cái - cân nhắc ở tầng fan-out (TASK-NOTIF-003), không ở batch này.
- Ngưỡng `0.7` có nên khác theo ngành hàng (thời trang biến động khác điện máy) - giữ một hằng toàn cục theo §3.5 ở v1, để ML TASK-DEAL-004/005 hấp thụ khác biệt ngành.
...
- TASK-DEAL-006 là tầng serving của dự đoán đáy: nó biến một cột xác suất trong bảng thành cảnh báo hành động được, đúng như §3.5 mô tả "batch nightly score -> alert".
...
- Maturity gate dùng lại đúng `IsFeatureReady` của TASK-DEAL-002, giữ một định nghĩa "đủ chín" thống nhất toàn module DEAL.
...
- Batch đọc-chỉ với `price_forecast` (Python sinh) và `alert_rule` (TASK-TRACK-003), chỉ ghi vào `bottom_alert_log` và hàng đợi fan-out - ranh giới quyền rõ ràng.

---

*Hết TASK-DEAL-006. Status: ready_to_implement (mục tiêu audit 10/10).*
