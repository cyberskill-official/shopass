...
§1 #11 + `detect.go` dùng nhân chéo số nguyên thay cho nhân float (`*1.15` -> `*115` so `*100`). Khép rủi ro sai số float đúng tại biên phần trăm; test `TestDetect_BoundaryThresholds` đóng các biên `list_bang_115`, `current_bang_97`, `current_duoi_97`. Khớp DEC-PRICE-05 của TASK-PRICE-002.
...
Mỗi mệnh đề normative ánh xạ 1:1 với một dòng §3.5(1) và có test backing; không mệnh đề mồ côi. Ba hằng ngưỡng load-bearing tái tạo đúng; integer-safe math khép rủi ro float; cold-start trung thực bàn giao TASK-DEAL-002. Đã sửa surgical 9 mũi tên comment sang ASCII. Score = 10/10. Verdict: PASS.

---

*Hết audit độc lập TASK-DEAL-001.*
