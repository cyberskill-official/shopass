...
created: 2026-06-27
related_frs: [TASK-PRICE-002, TASK-DEAL-002, TASK-DEAL-003, TASK-TRACK-003]
depends_on: [TASK-PRICE-002]
blocks: [TASK-DEAL-002]
source_pages:
...
source_decisions:
  ...
  - "DEC-DEAL-02: dưới 14 ngày dữ liệu trả UNKNOWN (cold-start), bàn giao cho TASK-DEAL-002 - KHÔNG đoán bừa khi thiếu lịch sử"
  - "DEC-DEAL-03: dùng integer-safe math trên BIGINT VND (so current*100 >= median90*97) để tránh sai số float, đồng bộ TASK-PRICE-002 DEC-PRICE-05"
  - "DEC-DEAL-04: verdict là enum đóng SALE_AO / SALE_XIN / TAM_DUOC / UNKNOWN - không trả chuỗi tự do"
  - "DEC-DEAL-05: cửa sổ lịch sử cố định 90 ngày, đọc từ price_snapshot (TASK-PRICE-002)"
...
sub_tasks:
  ...
  - "0.5h: chốt cold-start len(hist) < 14 -> UNKNOWN, bàn giao TASK-DEAL-002"
...
2. **MUST** nếu `len(hist) < 14` thì trả `UNKNOWN` ngay (cold-start) và KHÔNG tính tiếp - đây là điểm bàn giao cho TASK-DEAL-002 (DEC-DEAL-02).
...
11. **MUST** thực hiện mọi phép so ngưỡng bằng integer-safe math trên `int64` (ví dụ `current*100 >= median90*97`, `list*100 > median90*115`, `current*100 <= trailingMin*102`), KHÔNG dùng float (DEC-DEAL-03), nhất quán với BIGINT VND của TASK-PRICE-002.
...
13. **MUST** trả kiểu `Verdict` là enum đóng gồm đúng bốn giá trị `SALE_AO`, `SALE_XIN`, `TAM_DUOC`, `UNKNOWN` (DEC-DEAL-04). Verdict này là đầu vào cho luật cảnh báo sale thật của TASK-TRACK-003.
...
Xem §3. Thứ tự: `types.go` (enum Verdict) -> `stats.go` (percentile + minInt64) -> `detect.go` (DetectFakeSale) -> `detect_test.go`. Sau khi gói `fakesale` xanh, nối vào `service.go`: nạp `QueryRange(productID, now-90d, now)` từ price repo (TASK-PRICE-002), rút cột `price` thành `[]int64`, gọi `DetectFakeSale`, phát OTel counter `fake_sale_verdict_total{verdict}`. Gói `fakesale` không phụ thuộc DB - chỉ là hàm thuần trên chuỗi - nên test chạy nhanh, không cần TimescaleDB.

---

## §7 - Phụ thuộc

- **TASK-PRICE-002** - nguồn 90 ngày lịch sử giá (`price_snapshot.price`, BIGINT VND) qua `QueryRange`. Hằng integer-safe math đồng bộ DEC-PRICE-05.
- **TASK-DEAL-002 (downstream, blocks)** - mở rộng nhánh cold-start: khi `DetectFakeSale` trả `UNKNOWN`, TASK-DEAL-002 dùng nguồn thay thế.
- **TASK-TRACK-003 (consumer)** - luật cảnh báo `real_sale` tiêu thụ verdict (`SALE_XIN` kích cảnh báo sale thật).
- **TASK-DEAL-003 (downstream)** - hiển thị nhãn verdict kèm biểu đồ giá.
...
- => verdict = `SALE_XIN`. TASK-TRACK-003 bắn cảnh báo sale thật.
...
Đã chốt. Hoãn:
- Ngưỡng theo từng ngành hàng (thời trang biến động khác điện máy) - hoãn sang TASK-DEAL-004/005 (ML), giai đoạn này dùng ba hằng cố định toàn cục.
...
| `len(hist) < 14` | kiểm tra đầu hàm | thiếu cơ sở thống kê | Trả UNKNOWN, bàn giao TASK-DEAL-002 (theo thiết kế) |
...
| Ngưỡng cần tinh chỉnh theo ngành hàng | phản hồi người dùng | phân loại lệch một số ngành | Hoãn sang ML TASK-DEAL-004/005, giữ ba hằng cố định ở v1 |
...
- Integer-safe math trên BIGINT VND đảm bảo verdict tất định, đồng bộ DEC-PRICE-05 của TASK-PRICE-002.
- Trung vị ít nhạy với ngoại lai hơn trung bình, nên median90 là mốc tham chiếu ổn định cho chuỗi giá có nhiễu.
- UNKNOWN không phải lỗi mà là câu trả lời trung thực khi thiếu dữ liệu, đồng thời là điểm bàn giao sạch cho TASK-DEAL-002.
- Gói `fakesale` cố ý không chạm DB: hàm thuần trên `[]int64`, dễ test và tái dùng cho mô phỏng/backtest sau này.

---

*Hết TASK-DEAL-001. Status: ready_to_implement (mục tiêu audit 10/10).*
