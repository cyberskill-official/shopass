...
Tôi tái thẩm TASK-PRICE-001 độc lập từ file `.md` hiện tại, không tin audit cũ. Bảng `tracked_product` được đặc tả ở mức triển khai được: 12 mệnh đề §1 normative (11 MUST + 1 SHOULD), mỗi mệnh đề ánh xạ tới AC §4 và test §5. DDL §3 khớp DATA-MODEL và §3.4 nguồn từng cột: `id BIGSERIAL PK`, `platform_id SMALLINT REFERENCES platform(id)`, `platform_item_id TEXT NOT NULL`, `canonical_key TEXT` nullable, `first_seen TIMESTAMPTZ DEFAULT now()`, `UNIQUE(platform_id, platform_item_id)`, `idx_tp_canonical`. Ranh giới với TASK-PRICE-005 sạch: task này để `canonical_key = NULL`, không chạy matching. Không có defect cần sửa. Score 10/10.
...
- ISS-003 (kiểm, đạt): `canonical_key` để NULL khi insert; TASK-PRICE-005 mới UPDATE. AC #4 + test `TestCanonicalKey_NullOnInsert` chốt ranh giới.
...
Mọi mệnh đề normative có DDL/code/test backing; không mệnh đề mồ côi. Schema khớp chính xác DATA-MODEL và §3.4. Ranh giới với TASK-PRICE-002 (đích FK) và TASK-PRICE-005 (điền canonical_key) rạch ròi. Frontmatter đủ khóa, `new_files`/`sub_tasks`/`risk_if_skipped` không rỗng, không "TBD"/"TODO". Score = 10/10. Verdict: PASS.

---

*Hết audit độc lập TASK-PRICE-001.*
