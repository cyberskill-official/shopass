...
created: 2026-06-27
related_frs: [TASK-PRICE-002, TASK-PRICE-005, TASK-INFRA-002, TASK-SCRAPE-002, TASK-TRACK-001]
depends_on: [TASK-INFRA-002]
blocks: [TASK-PRICE-002, TASK-PRICE-005, TASK-SCRAPE-001, TASK-TRACK-001]
source_pages:
...
source_decisions:
  ...
  - "DEC-PRICE-07: canonical_key tách khỏi thuật toán matching - ở task này chỉ là cột TEXT nullable + index; giá trị do TASK-PRICE-005 điền sau"
...
disallowed_tools:
  - điền canonical_key trong task này (thuộc TASK-PRICE-005; insert phải để canonical_key NULL)
  - tạo nhiều dòng tracked_product cho cùng (platform_id, platform_item_id) (vi phạm DEC-PRICE-06/08)
  - dùng SERIAL/INT cho id (phải BIGSERIAL/BIGINT theo quy ước TASK-INFRA-002)
...
risk_if_skipped: "tracked_product là registry chuẩn hóa mà toàn bộ module PRICE neo vào. Không có nó thì price_snapshot (TASK-PRICE-002) không có đích FK product_id, thuật toán so khớp chéo sàn (TASK-PRICE-005) không có cột canonical_key để điền, và API theo dõi giá (TASK-TRACK-001) không có thực thể sản phẩm để gắn người dùng. Thiếu UNIQUE(platform_id, platform_item_id) thì mỗi lần quét lại tạo một dòng mới, registry phình và price_snapshot trỏ tới nhiều id rác cho cùng một SKU. Thiếu index idx_tp_canonical thì JOIN gom SKU cùng sản phẩm trên nhiều sàn phải quét toàn bảng, vỡ moat so sánh chéo sàn (§5.6)."
---

## §1 - Mô tả (BCP-14 normative)

Service PRICE **MUST** định nghĩa registry chuẩn hóa `tracked_product`: đúng một dòng cho mỗi cặp (platform_id, platform_item_id), có cột `canonical_key` làm khóa gom chéo sàn (điền sau bởi TASK-PRICE-005), và là đích FK cho `price_snapshot`. Hợp đồng:
...
2. **MUST** ràng buộc `platform_id` REFERENCES `platform(id)` (FK tới bảng nền của TASK-INFRA-002). Một SKU luôn thuộc đúng một sàn đã đăng ký.
...
4. **MUST** để `canonical_key` là `TEXT` nullable (DEC-PRICE-07): task này chỉ tạo cột; TASK-PRICE-005 mới chạy thuật toán so khớp và UPDATE giá trị. Insert ở task này **MUST** để `canonical_key = NULL`.
...
**Vì sao tách canonical_key khỏi thuật toán matching (DEC-PRICE-07)?** So khớp chéo sàn (cùng một chiếc tai nghe bán trên Shopee, TikTok, Lazada) là bài toán khó: tên khác nhau, ảnh khác nhau, không có mã chung. Nhồi thuật toán đó vào task schema này làm phình phạm vi và chặn các task phụ thuộc. task này chỉ dựng cột `canonical_key` (nullable) và index; TASK-PRICE-005 chạy so khớp và điền giá trị sau. Schema sẵn sàng trước, thuật toán đến sau mà không phải đổi bảng.
...
**Vì sao canonical_key để NULL khi insert (§1 #4)?** Lúc scraper phát hiện SKU, ta chưa biết nó là biến thể của sản phẩm nào. NULL nghĩa là "chưa so khớp". TASK-PRICE-005 quét các dòng NULL và gán nhóm. Điền sẵn một giá trị đoán mò ở task này sẽ tạo nhóm sai mà sau khó dọn.
...
CREATE TABLE tracked_product (
  ...
  title            TEXT,
  category_id      BIGINT,
  canonical_key    TEXT,                                  -- NULL cho tới khi TASK-PRICE-005 so khớp
...
Xem §3. Thứ tự: migration `0001_tracked_product.sql` (bảng + UNIQUE + index) -> `product.go` (struct) -> `product_repo.go` (Upsert + 3 hàm tra cứu) -> tests. Migration chạy qua golang-migrate (cùng runner của TASK-INFRA-002). Index `idx_tp_canonical` tạo cùng migration, trên bảng còn rỗng nên không khóa lâu. Driver dùng `pgx`. Hàm `Upsert` dùng `RETURNING` để lấy lại `id` và `first_seen` do DB sinh, tránh một round-trip đọc thêm.

---

## §7 - Phụ thuộc

- **TASK-INFRA-002** - bảng `platform` và framework migration phải có trước (FK `platform_id`, runner golang-migrate).
- **TASK-PRICE-002 (downstream)** - `price_snapshot.product_id` REFERENCES `tracked_product(id)`; cần bảng này tồn tại trước.
- **TASK-PRICE-005 (downstream)** - thuật toán so khớp chéo sàn UPDATE `canonical_key` cho các dòng NULL; đọc qua index `idx_tp_canonical`.
- **TASK-SCRAPE-002 (upstream)** - scraper gọi `Upsert` khi phát hiện hoặc làm mới một SKU.
- **TASK-TRACK-001 (downstream)** - API theo dõi giá gắn người dùng vào `tracked_product.id`.
- Lib: driver `pgx`.
...
tp, err := productRepo.Upsert(ctx, price.TrackedProduct{
    PlatformID:     1,                       // shopee (theo platform seed TASK-INFRA-002)
    PlatformItemID: "20114455667",
    ShopID:         ptr("88123"),
    Title:          ptr("Tai nghe Bluetooth ABC Pro"),
    CategoryID:     ptr(int64(7021)),
    // CanonicalKey để trống - TASK-PRICE-005 điền sau
})
// tp.ID dùng làm product_id khi ghi price_snapshot (TASK-PRICE-002)
```
...
Đã chốt. Hoãn:
- Định dạng nội bộ của `canonical_key` (hash chuẩn hóa hay id cụm) - thuộc TASK-PRICE-005, không ảnh hưởng schema ở đây.
- Lưu lịch sử đổi `title`/`category_id` (audit metadata) - chỉ thêm nếu cần truy vết, slice sau.
- Đánh dấu SKU "đã ngừng bán" (cột `delisted_at`) - bổ sung khi scraper báo 404; không cản task này.
- Soft-delete vs hard-delete dòng tracked_product - giữ đơn giản tới khi có yêu cầu xóa dữ liệu (TASK-COMPLY).
...
|---|---|---|---|
| platform_id không có trong platform | lỗi FK pgx | từ chối ghi | Seed platform (TASK-INFRA-002) chạy trước; scraper map đúng id |
...
| Insert kèm canonical_key (vi phạm ranh giới) | code review + test NullOnInsert | nhóm chéo sàn sai | Upsert không nhận canonical_key; chỉ TASK-PRICE-005 UPDATE |
...
- Tách `canonical_key` (cột + index ở task này) khỏi thuật toán so khớp (TASK-PRICE-005) cho phép schema sẵn sàng trước, không chặn các task phụ thuộc chờ thuật toán khó.
...
*Hết TASK-PRICE-001. Status: ready_to_implement (mục tiêu audit 10/10).*
