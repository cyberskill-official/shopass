...
- ISS-006 (kiểm, đạt): tiền tệ không xuất hiện ở task này (chỉ matching), không có rủi ro float trên giá.
- Typography: prose sạch ASCII; glyph mũi tên chỉ trong khối code và khối `text`/`sql` minh họa của §8 (ngoài phạm vi rule typography, đồng nhất house style của exemplar TASK-PRICE-002). Đạt rule O.
...
*Hết audit độc lập TASK-PRICE-005.*
