...
created: 2026-06-27
related_frs: [TASK-PRICE-001, TASK-PRICE-004, TASK-PRICE-002, TASK-SCRAPE-002]
depends_on: [TASK-PRICE-001]
blocks: [TASK-PRICE-004]
source_pages:
...
risk_if_skipped: "canonical_key là moat đa sàn của SănDeal (§5.6). Không có thuật toán matching thì so sánh giá chéo 3 sàn (TASK-PRICE-004, GET /v1/compare) không có hàng để JOIN - mỗi listing Shopee/TikTok/Lazada là một dòng rời, không ai biết chúng là cùng một sản phẩm vật lý. Khoảng trống so sánh giá chéo sàn mà BeeCost bỏ ngỏ sẽ vẫn bỏ ngỏ. Nếu làm ẩu (auto-merge dưới ngưỡng), hệ thống gộp nhầm hai SKU khác nhau và hiển thị giá so sánh sai - phá vỡ niềm tin, đúng cái bài học hậu-Honey mà SănDeal phải tránh. Thiếu fold dấu thì 'điện thoại' và 'dien thoai' thành hai nhóm, dedup vô dụng cho thị trường VN."
---

## §1 - Mô tả (BCP-14 normative)

Service PRICE **MUST** tính `canonical_key` cho mỗi `tracked_product` để gộp các listing cùng một sản phẩm vật lý trên Shopee/TikTok/Lazada thành một nhóm so sánh giá, ghi key ngược vào cột `tracked_product.canonical_key` (cột + index `idx_tp_canonical` do TASK-PRICE-001 định nghĩa). Pipeline gồm chuẩn hóa title, dựng key xác định, fuzzy match và hàng đợi duyệt tay. Hợp đồng:
...
- **TASK-PRICE-001** - `tracked_product(title, canonical_key)` và `idx_tp_canonical` phải tồn tại trước (ghi key ngược).
- **TASK-PRICE-004 (downstream)** - `GET /v1/compare?canonical_key=...` JOIN theo `canonical_key` mà task này sinh ra.
- **TASK-SCRAPE-002 (tùy chọn)** - `recommend` của Shopee cấp SKU liên quan, làm giàu ứng viên matching.
- **TASK-PRICE-002 (liên quan)** - sau khi gộp, mỗi `canonical_key` đọc `price_snapshot` của các `product_id` thành viên để dựng dòng so sánh.
...
```sql
-- TASK-PRICE-004 dùng canonical_key để gom giá 3 sàn
...
- Gộp theo biến thể (variant) trong cùng listing (màu/dung lượng) - slice sau khi mô hình variant của TASK-PRICE-001 ổn định.
...
- Vùng confidence lưng chừng (0,60-0,82) luôn vào người duyệt, không bao giờ tự gộp - đây là van an toàn chính của task.
...
*Hết TASK-PRICE-005. Status: ready_to_implement (mục tiêu audit 10/10).*
