...
Audit độc lập, tái diễn từ file TASK-NOTIF-003 hiện tại. task đặc tả fan-out pipeline: producer -> Kafka/Redis Streams -> fan-out worker -> per-channel dispatcher, at-least-once + idempotent, backoff + jitter + DLQ. 12 mệnh đề §1 BCP-14 (11 MUST + 1 SHOULD), testable. Kiểm khớp rubric chuyên biệt: định tuyến theo channel in {push,email,sms} - KHÔNG có giá trị apns riêng (§1 #4); push iOS phân theo user_channel_token.platform='ios' (TASK-NOTIF-005), KHÔNG phải channel riêng; fan-out KHÔNG gọi nhà cung cấp. Đạt 10/10.
