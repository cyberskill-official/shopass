...
created: 2026-06-28
related_frs: [TASK-NOTIF-001, TASK-NOTIF-002, TASK-NOTIF-004, TASK-NOTIF-005, TASK-NOTIF-006, TASK-NOTIF-007]
depends_on: [TASK-NOTIF-001]
blocks: [TASK-NOTIF-004, TASK-NOTIF-005, TASK-NOTIF-006, TASK-NOTIF-007]
source_pages:
...
source_decisions:
  - "DEC-NOTIF-20: pipeline là producer -> hàng đợi bền (Kafka hoặc Redis Streams) -> fan-out worker -> per-channel dispatcher; fan-out CHỈ định tuyến/điều phối, KHÔNG tự gọi nhà cung cấp (gọi thật ở TASK-NOTIF-002/005/006/007)"
  ...
  - "DEC-NOTIF-23: fan-out định tuyến theo notification.channel (đã được TASK-NOTIF-001 routing chốt) tới đúng handler kênh; dispatcher trả Permanent vs Transient để pipeline quyết DLQ ngay hay retry"
  ...
  - "DEC-NOTIF-25: TASK-NOTIF-004 (scheduler chống dồn 00:00) đẩy việc VÀO pipeline này; fan-out là tầng tiêu thụ, scheduler là tầng điều tiết đầu vào"
...
disallowed_tools:
  - tự gọi FCM/APNs/SES/SendGrid/SMS trong package fanout (vi phạm DEC-NOTIF-20; gọi thật là việc của TASK-NOTIF-002/005/006/007)
...
1. **MUST** dựng đường ống `producer -> hàng đợi bền (Kafka hoặc Redis Streams) -> fan-out worker -> per-channel dispatcher` (DEC-NOTIF-20). Producer chỉ đẩy `notification_id` (con trỏ tới dòng đã ghi bởi TASK-NOTIF-001), KHÔNG nhồi toàn bộ nội dung vào message - nguồn sự thật là bảng `notification`.
...
4. **MUST** định tuyến theo `notification.channel` (đã được TASK-NOTIF-001 routing chốt thành đúng một kênh trong enum `{push, email, sms}`) tới đúng `ChannelDispatcher` đã đăng ký (DEC-NOTIF-23): `push` tới dispatcher push (TASK-NOTIF-002 cho Android/Web, TASK-NOTIF-005 cho iOS - phân theo `user_channel_token.platform`, KHÔNG phải theo một giá trị channel riêng); `email -> TASK-NOTIF-006`; `sms -> TASK-NOTIF-007`. `channel` chỉ nhận `{push, email, sms}` theo CHECK của TASK-NOTIF-001 - KHÔNG có giá trị `apns` riêng. Fan-out KHÔNG chứa mã gọi FCM/APNs/SES/SendGrid/nhà-cung-cấp-SMS.
...
8. **MUST** cập nhật trạng thái cuối: dispatch thành công -> `UPDATE notification SET status='sent', sent_at=now()`; thất bại tạm -> giữ `queued` cho lần retry (đặt lại `lease_until`); thất bại vĩnh viễn hoặc cạn retry -> `status='failed'` + DLQ. Vòng đời trạng thái khớp TASK-NOTIF-001: `pending -> queued -> sent | failed`.
...
10. **MUST** dùng `idx_notif_dispatch` (đã tạo ở TASK-NOTIF-001, partial trên `status IN ('pending','queued')`) làm nguồn lấy việc cho producer/scheduler; thêm cột `attempts`, `lease_until`, `last_error` vào `notification` qua migration của task này.
...
```sql
-- services/notif/migrations/0003_notification_lease.sql
-- Bổ sung cột phục vụ claim/lease + retry cho fan-out (TASK-NOTIF-003).
...
-- đã tạo ở TASK-NOTIF-001; phục vụ producer/scheduler lấy việc và re-claim lease hết hạn.
...
// ChannelDispatcher là per-channel gửi thật, hiện thực ở TASK-NOTIF-002/005/006/007.
...
type ChannelDispatcher interface {
    ...
    Channel() string // "push" | "email" | "sms" (enum TASK-NOTIF-001; iOS push phân theo platform, không phải channel riêng)
}
...
func (w *Worker) handle(ctx context.Context, id int64) error {
    ...
    // 2) Định tuyến theo channel đã chốt bởi routing TASK-NOTIF-001.
    d, ok := w.router.Route(n.Channel)
    if !ok {
        return w.dead(ctx, n, "permanent", "không có dispatcher cho kênh "+n.Channel)
    }

    // 3) Gửi (gọi thật nằm trong dispatcher TASK-NOTIF-002/005/006/007).
...
11. Định tuyến đúng kênh: `notification.channel='push'` -> dispatcher push (TASK-NOTIF-002 cho Android/Web, TASK-NOTIF-005 cho iOS - phân theo `user_channel_token.platform`); `email` -> email (TASK-NOTIF-006); `sms` -> sms (TASK-NOTIF-007). `channel` không nhận giá trị `apns` (CHECK TASK-NOTIF-001). Không có lời gọi nhà cung cấp nào trong package `fanout`.
...
Xem §3. Thứ tự: migration `0003_notification_lease.sql` (cột claim/lease) + `0004_notification_dlq.sql` (bảng DLQ) -> `repo.go` bổ sung `ClaimPending`/`MarkSent`/`Requeue` -> `backoff.go` (thuần, test trước) -> `dispatch.go` (interface + Router) -> `dlq.go` (Publish) -> `worker.go` (vòng claim -> dispatch -> ack/retry/DLQ) -> `producer.go` (Enqueue) -> tests. Dispatcher thật (push/APNs/email/sms) đến ở TASK-NOTIF-002/005/006/007; ở task này dùng dispatcher giả (stub) để kiểm thử pipeline độc lập. Chọn Kafka khi triển khai đa-node có sẵn Kafka; Redis Streams cho khởi đầu nhẹ - cả hai cùng cung cấp claim/ack + consumer group, interface producer ẩn lựa chọn này.

---

## §7 - Phụ thuộc

- **TASK-NOTIF-001 (upstream)** - định nghĩa bảng `notification`, routing chốt `channel`, và `idx_notif_dispatch`; fan-out tiêu thụ các dòng `pending` này.
- **TASK-NOTIF-002 (downstream)** - dispatcher push (FCM) hiện thực `ChannelDispatcher` cho `channel='push'`.
- **TASK-NOTIF-005 (downstream)** - dispatcher APNs cho push iOS: nhặt dòng `channel='push'` lọc thêm `user_channel_token.platform='ios'` (KHÔNG phải một giá trị `channel='apns'` riêng).
- **TASK-NOTIF-006 (downstream)** - dispatcher email (SES/SendGrid) cho `channel='email'`.
- **TASK-NOTIF-007 (downstream)** - dispatcher SMS cho `channel='sms'`.
- **TASK-NOTIF-004 (upstream điều tiết)** - scheduler chống dồn 00:00 đẩy việc vào pipeline này (rải `scheduled_at`); fan-out là tầng tiêu thụ.
...
- Circuit breaker per-channel (khi một nhà cung cấp sập diện rộng, tạm ngắt để khỏi đốt retry) - phối hợp với dispatcher TASK-NOTIF-002/006 sau.
- Ưu tiên hàng đợi (priority lane) cho OTP/high-value tách khỏi thông báo giá thường - gắn khi BILL/Premium (TASK-BILL-001) lên.
...
| Fan-out tự gọi nhà cung cấp | code review | vượt rate-limit, lệch ranh giới | Chỉ định tuyến; gọi thật ở TASK-NOTIF-002/005/006/007 (§1 #4) |
...
- Fan-out là xương sống vận chuyển của module thông báo: nó nối engine alert (qua dòng `notification` pending của TASK-NOTIF-001) tới người dùng cuối, nhưng KHÔNG tự gọi nhà cung cấp.
...
- Backoff full jitter là tuyến phòng thủ chống thundering herd lúc đỉnh 00:00, song song với pacing/jitter của scraping farm (TASK-SCRAPE-001/005) ở phía thu thập.
...
- Ranh giới cứng: fan-out giữ phần chung (claim, retry, backoff, DLQ, định tuyến); mỗi `ChannelDispatcher` (TASK-NOTIF-002/005/006/007) là một implementation cắm vào theo `channel`, thêm kênh không đụng xương sống.
- TASK-NOTIF-004 (scheduler chống dồn 00:00) là tầng điều tiết đầu vào; fan-out là tầng tiêu thụ - hai mảnh khớp nhau ở hàng đợi.

---

*Hết TASK-NOTIF-003. Status: ready_to_implement (mục tiêu audit 10/10).*
