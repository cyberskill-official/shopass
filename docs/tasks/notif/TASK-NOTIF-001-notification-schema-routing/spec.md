...
created: 2026-06-28
related_frs: [TASK-INFRA-002, TASK-NOTIF-002, TASK-NOTIF-003, TASK-TRACK-004, TASK-DEAL-006]
depends_on: [TASK-INFRA-002]
blocks: [TASK-NOTIF-002, TASK-NOTIF-003, TASK-TRACK-004]
source_pages:
...
source_decisions:
  ...
  - "DEC-NOTIF-04: request từ producer (TASK-TRACK-004 / TASK-DEAL-006) gồm user_id + channel[] mong muốn + template + data; routing giao nhau channel[] với khả năng nhận của user rồi quyết kênh thực"
  - "DEC-NOTIF-05: status lifecycle pending -> queued -> sent | failed; TASK-NOTIF-003 (fan-out) và các dispatcher cập nhật; task này định nghĩa schema + routing + render, KHÔNG tự gọi nhà cung cấp"
...
disallowed_tools:
  - tự gọi FCM/APNs/SES/SMS trong task này (vi phạm DEC-NOTIF-05; gửi thuộc TASK-NOTIF-002..007 qua fan-out)
...
risk_if_skipped: "TASK-NOTIF-001 là nền của toàn bộ module thông báo - bảng notification, template engine và routing kênh mà mọi dispatcher (TASK-NOTIF-002..007) và fan-out (TASK-NOTIF-003) phụ thuộc. Không có nó thì engine alert (TASK-TRACK-004) và batch đáy giá (TASK-DEAL-006) có cảnh báo cần gửi nhưng không có nơi ghi và không có logic chọn kênh - chuỗi thông báo đứt ở khúc cuối. Nếu routing không theo cost model (push > email > sms §3.6) mà chọn SMS bừa thì chi phí thông báo bùng nổ: SMS tới VN ~200-500 VND/tin nội địa và ~$0,1552 qua Twilio, đắt gấp hàng nghìn lần push gần-miễn-phí - một sai lầm routing đủ phá unit economics. Nếu template engine không escape payload thì giá/tên sản phẩm do scraper lấy về có thể chứa ký tự phá nội dung hoặc chèn markup, làm thông báo vỡ hoặc thành kênh injection. Tự gọi nhà cung cấp ở đây thay vì qua fan-out làm mất rate-limit và flatten-the-curve, ăn 429 lúc đỉnh 00:00."
---

## §1 - Mô tả (BCP-14 normative)

Service NOTIF **MUST** định nghĩa bảng `notification`, một template engine render an toàn, và một bộ routing chọn kênh theo cost model (push > email > sms). task này là điểm vào của fan-out; nó KHÔNG tự gọi nhà cung cấp. Hợp đồng:
...
3. **MUST** định nghĩa bảng `user_channel_token (user_id BIGINT, channel TEXT, platform TEXT, address TEXT, verified BOOLEAN DEFAULT false, updated_at TIMESTAMPTZ, PRIMARY KEY (user_id, channel, platform))` lưu khả năng nhận của user: token FCM/APNs cho `push`, email cho `email`, số điện thoại cho `sms`. Cột `platform` thuộc `{ios, android, web, email, sms}` tách kênh `push` thành hai nhánh gửi: TASK-NOTIF-002 (FCM) nhặt `platform IN ('android','web')`, TASK-NOTIF-005 (APNs) nhặt `platform='ios'`; nhờ `platform` trong khóa chính, một user có thể có đồng thời token iOS + Android + Web và nhận push trên từng thiết bị. Chỉ bản ghi `verified=true` (push token còn hạn, email/phone đã xác minh) mới được routing coi là khả dụng.
4. **MUST** cung cấp template engine với registry các template chốt: `price_below`, `drop_pct`, `real_sale`, `bottom_predicted` (khớp `rule_type` của TASK-TRACK-003). Mỗi template có `title` + `body` theo từng kênh.
...
9. **MUST** expose `InsertNotification(ctx, n Notification) (int64, error)` ghi một dòng `status='pending'` sau khi routing đã quyết kênh và render đã thành công; trả `id`. Đồng thời giữ ranh giới với tầng gửi (DEC-NOTIF-05): task này KHÔNG gọi FCM/APNs/SES/SendGrid/SMS. Sau khi có dòng `notification` pending, fan-out (TASK-NOTIF-003) và các dispatcher (TASK-NOTIF-002/005/006/007) lo việc gửi và cập nhật `status` -> `sent`/`failed` + `sent_at`.
...
**Vì sao task này không tự gửi (DEC-NOTIF-05, §1 #9)?** Gửi ở quy mô là module riêng (TASK-NOTIF-002..007) với fan-out, rate-limit FCM, flatten-the-curve, dead-letter. Trộn routing/render với gọi nhà cung cấp làm hai thứ khó test chung và dễ vượt rate-limit. task này dừng ở "quyết kênh + dựng nội dung + ghi pending"; phần gửi là việc của các task sau, tách trách nhiệm sạch.
...
-- Fan-out (TASK-NOTIF-003) lấy việc theo trạng thái + lịch -> partial index
...
-- platform tach push thanh FCM (android/web) vs APNs (ios): TASK-NOTIF-002 nhat platform IN ('android','web'),
-- TASK-NOTIF-005 nhat platform='ios'. Email/sms dat platform='email'/'sms' cho dong nhat khoa.
```
...
// templates: registry chốt khớp rule_type của TASK-TRACK-003.
...
Xem §3. Thứ tự: migration `0001_notification.sql` + `0002_user_channel_token.sql` -> `template.go` (registry + render escape) -> `routing.go` (ResolveChannel cost model) -> `repo.go` (InsertNotification + GetUserChannels) -> tests. Producer (TASK-TRACK-004 / TASK-DEAL-006) gọi `GetUserChannels` -> `ResolveChannel` -> `Render` -> `InsertNotification` (pending). Không có lời gọi nhà cung cấp nào trong package `notif` ở task này. Format VND nhóm hàng nghìn bằng dấu chấm ở bước render; dữ liệu gốc giữ int64.

---

## §7 - Phụ thuộc

- **TASK-INFRA-002** - bảng `app_user` cho FK `user_id`.
- **TASK-TRACK-004 (upstream)** - engine alert enqueue request (user_id + channel[] + template + data); task này routing + render + ghi pending.
- **TASK-DEAL-006 (upstream)** - batch đáy giá cũng đẩy request qua điểm vào này.
- **TASK-NOTIF-003 (downstream)** - fan-out lấy dòng `notification` pending (qua `idx_notif_dispatch`) để gửi.
- **TASK-NOTIF-002/005/006/007 (downstream)** - dispatcher từng kênh cập nhật `status` + `sent_at`.
...
### Producer đẩy request (nội bộ, từ TASK-TRACK-004)
...
- Quiet-hours (không push ban đêm trừ khi user đồng ý) - phối hợp với TASK-NOTIF-004 scheduler.
...
| task này tự gọi nhà cung cấp | code review | vượt rate-limit | Chỉ ghi pending (DEC-NOTIF-05) |
...
- TASK-NOTIF-001 là nền module thông báo: bảng `notification`, template engine, routing - mọi dispatcher và fan-out neo vào.
...
- Ranh giới cứng: task này dừng ở "quyết kênh + dựng nội dung + ghi pending"; gửi là việc của TASK-NOTIF-002..007 qua fan-out.
- Tiền luôn int64 VND trong payload; format hiển thị làm ở render, không có bước float.

---

*Hết TASK-NOTIF-001. Status: ready_to_implement (mục tiêu audit 10/10).*
