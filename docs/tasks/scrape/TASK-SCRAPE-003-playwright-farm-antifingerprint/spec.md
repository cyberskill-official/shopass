...
created: 2026-06-27
related_frs: [TASK-SCRAPE-001, TASK-SCRAPE-002, TASK-SCRAPE-004, TASK-SCRAPE-005, TASK-SCRAPE-007, TASK-SCRAPE-008, TASK-PRICE-002]
depends_on: [TASK-SCRAPE-001]
blocks: [TASK-SCRAPE-004, TASK-SCRAPE-007, TASK-SCRAPE-008]
source_pages:
...
source_decisions:
  ...
  - "DEC-SCRAPE-12: một fingerprint profile gắn liền một proxy session (TASK-SCRAPE-004) - không trộn fingerprint nước này với IP nước khác"
  - "DEC-SCRAPE-13: hành vi giống người (mouse move, scroll, delay đọc trang) trước khi trích dữ liệu, không request thô tức thì"
  - "DEC-SCRAPE-14: farm expose RenderPrice(ctx, job) cho adapter fallback (TASK-SCRAPE-002/007/008)"
...
risk_if_skipped: "Anti-bot 3 sàn dựa trên fingerprinting (§3.9): Shopee hash thiết bị từ Canvas/WebGL/hành vi; Akamai của Lazada đọc TLS/HTTP. Playwright mặc định lộ headless tức thì (navigator.webdriver, Canvas readback đặc trưng, JA3 automation) -> bị ban ngay, đốt proxy. Không có farm patch thì fallback của TASK-SCRAPE-002 vô dụng, TikTok (TASK-SCRAPE-007) và Lazada (TASK-SCRAPE-008) không quét được. Trộn fingerprint nước này với IP nước khác tạo mâu thuẫn locale/timezone/IP - cờ đỏ phát hiện bot. Đây là lớp chống ban tốn công nhất và là điều kiện để 2/3 sàn có dữ liệu."
---
...
5. **MUST** gắn một fingerprint profile với một proxy session (DEC-SCRAPE-12): profile có timezone/locale VN phải đi cùng IP residential VN của TASK-SCRAPE-004; KHÔNG trộn timezone nước này với IP nước khác.
...
7. **MUST** expose `RenderPrice(ctx, job ScrapeJob) (PriceSnapshot, error)` (DEC-SCRAPE-14): mở trang sản phẩm, chạy behavior, trích giá từ DOM render, trả `PriceSnapshot` cho adapter gọi fallback (TASK-SCRAPE-002/007/008).
8. **MUST** phát hiện khi bị challenge (CAPTCHA/slider/verify page) và trả error phân loại `ErrChallenged` để TASK-SCRAPE-005 xử lý CAPTCHA hoặc orchestrator backoff, KHÔNG trả snapshot rỗng coi như giá hợp lệ.
...
**Vì sao hành vi giống người (DEC-SCRAPE-13)?** Hệ thống hành vi (behavioral) đo nhịp tương tác. Tải trang rồi đọc giá trong 50ms không phải hành vi người. Mouse move, scroll, dwell time đưa farm vào phân bố hành vi người-thật, phối hợp với pacing/jitter của TASK-SCRAPE-005.

**Vì sao tách ErrChallenged (§1 #8)?** Khi bị challenge, trả "không có giá" làm bẩn dữ liệu và làm orchestrator coi như parse fail. Phân loại challenge rõ ràng để TASK-SCRAPE-005 quyết định giải CAPTCHA hay lùi, và để metric `farm_detected_total` cảnh báo sớm khi tỷ lệ challenge tăng.
...
- **TASK-SCRAPE-001** - orchestrator gọi adapter; adapter gọi farm khi fallback; kiểu `ScrapeJob`.
- **TASK-SCRAPE-002 / 007 / 008** - các adapter gọi `RenderPrice` của farm khi gặp challenge / sàn buộc DOM-render.
- **TASK-SCRAPE-004** - proxy session residential gắn theo profile (cùng nước).
- **TASK-SCRAPE-005 (downstream)** - `ChallengedError` chuyển sang xử lý CAPTCHA + pacing.
- **TASK-PRICE-002 (downstream)** - snapshot từ farm ghi qua `InsertSnapshot` delta-only.
...
```typescript
// trong ShopeeAdapter khi nhận HTML challenge (TASK-SCRAPE-002 #6)
...
| Challenge coi như "không giá" | `ChallengedError` | dữ liệu bẩn | Phân loại + chuyển TASK-SCRAPE-005 (§1 #8) |
...
| Selector DOM đổi (A/B) | render fail tăng | parse hụt | TASK-SCRAPE-006 giám sát + cập nhật selectors |
...
- Farm là đường đắt; adapter ưu tiên JSON (TASK-SCRAPE-002) và chỉ gọi farm khi buộc, để giữ unit economics (§4.1).
- Behavior người-thật ở đây phối hợp với pacing/jitter của TASK-SCRAPE-005 - hai lớp chống hành vi-bot bổ trợ nhau.

---

*Hết TASK-SCRAPE-003. Status: ready_to_implement (mục tiêu audit 10/10).*
