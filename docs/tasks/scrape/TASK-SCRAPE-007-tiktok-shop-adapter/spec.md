...
source_decisions:
  - "DEC-SCRAPE-28: TikTok Shop ưu tiên đọc DOM-render qua farm Playwright (TASK-SCRAPE-003), KHÔNG cố gọi/ký API nội bộ"
  - "DEC-SCRAPE-29: không tái tạo msToken/_signature/X-Bogus (app attestation mạnh, leo thang đối đầu + giòn)"
  - "DEC-SCRAPE-30: dùng proxy tier enterprise (TASK-SCRAPE-004 DiffByteDance) vì rủi ro ban High"
...
sub_tasks:
  ...
  - "2.0h: adapter.ts - render qua farm (TASK-SCRAPE-003), chạy behavior, gọi extract -> PriceSnapshot"
...
1. **MUST** triển khai interface adapter của TASK-SCRAPE-001 và lấy giá qua farm `RenderPrice` của TASK-SCRAPE-003 (DEC-SCRAPE-28) - TikTok buộc DOM-render, không có đường JSON-không-login như Shopee.
...
3. **MUST** dùng proxy tier `enterprise` (DEC-SCRAPE-30): khi cấp proxy cho job TikTok, dùng `SelectTier(DiffByteDance)` của TASK-SCRAPE-004 (= enterprise) vì rủi ro ban High.
...
5. **MUST** map về `price.PriceSnapshot` với `price`/`list_price` BIGINT VND số nguyên (đồng bộ DEC-PRICE-05 của TASK-PRICE-002), `flash_sale` true khi trang báo đang flash/live-deal.
6. **MUST** chạy hành vi giống người của farm (TASK-SCRAPE-003 #6) trước khi trích - TikTok có hệ hành vi mạnh, đọc tức thì là cờ đỏ.
7. **MUST** phát hiện challenge/verify và ném `ChallengedError` (như TASK-SCRAPE-003 #8) để TASK-SCRAPE-005 / orchestrator xử lý, KHÔNG trả snapshot rỗng.
8. **MUST** báo outcome (success/parse_fail/challenge) cho monitor của TASK-SCRAPE-006 - TikTok là webview/SPA dễ đổi cấu trúc, cần giám sát drift sát.
...
**Vì sao proxy enterprise (DEC-SCRAPE-30)?** TikTok là rủi ro ban High (§3.9) - cao nhất trong ba sàn. Proxy budget/datacenter bị chặn ngay. `SelectTier(DiffByteDance)` của TASK-SCRAPE-004 trả enterprise đúng cho trường hợp này: trả tiền cao để đổi lấy độ tin cậy, vì TikTok không tha IP chất lượng thấp.
...
**Vì sao giám sát drift sát (§1 #8)?** TikTok là SPA, đổi cấu trúc thường xuyên hơn trang truyền thống. Báo outcome đều cho monitor (TASK-SCRAPE-006) để bắt sớm khi TikTok đổi state shape/selectors - adapter SPA hỏng âm thầm là rủi ro lớn hơn adapter JSON-API.

**Vì sao vẫn dùng farm thay adapter riêng (§1 #1)?** Farm (TASK-SCRAPE-003) đã giải bài fingerprint + TLS + behavior. TikTok adapter chỉ thêm phần "đọc giá ở đâu trên trang TikTok". Tái dùng farm tránh nhân bản lớp chống ban; adapter chỉ là selectors + extract logic riêng của TikTok.
...
Xem §3. Thứ tự: selectors.ts -> extract.ts (JSON-ưu-tiên + DOM-fallback, test trước với fixture) -> adapter.ts (render qua farm + chờ hydrate + challenge) -> registry.go (đăng ký, gắn proxy enterprise) -> tests. Adapter chạy trên farm Playwright của TASK-SCRAPE-003 (tái dùng fingerprint/TLS/behavior). Proxy lấy qua `SelectTier(DiffByteDance)` của TASK-SCRAPE-004. Outcome báo về monitor TASK-SCRAPE-006.

---

## §7 - Phụ thuộc

- **TASK-SCRAPE-003** - farm Playwright (`RenderPrice`, fingerprint, behavior, ChallengedError).
- **TASK-SCRAPE-004** - proxy tier enterprise cho TikTok (DiffByteDance).
...
- **TASK-SCRAPE-006** - monitor nhận outcome (SPA drift cần giám sát sát).
...
| Challenge coi như "không giá" | ChallengedError | dữ liệu bẩn | Ném lỗi, chuyển TASK-SCRAPE-005 (§1 #7) |
...
- TikTok là SPA dễ đổi cấu trúc -> giám sát drift (TASK-SCRAPE-006) quan trọng hơn so với adapter JSON-API; báo outcome đều.

---

*Hết TASK-SCRAPE-007. Status: ready_to_implement (mục tiêu audit 10/10).*
