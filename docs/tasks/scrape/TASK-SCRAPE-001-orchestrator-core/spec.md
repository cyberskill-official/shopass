...
created: 2026-06-27
related_frs: [TASK-SCRAPE-002, TASK-SCRAPE-003, TASK-SCRAPE-004, TASK-SCRAPE-005, TASK-SCRAPE-006, TASK-PRICE-002, TASK-TRACK-001]
depends_on: [TASK-INFRA-003, TASK-PRICE-001]
blocks: [TASK-SCRAPE-002, TASK-SCRAPE-003]
source_pages:
...
source_decisions:
  ...
  - "DEC-SCRAPE-05: orchestrator chỉ điều phối; nó KHÔNG ghi DB trực tiếp mà gọi InsertSnapshot delta-only của TASK-PRICE-002"
...
7. **MUST** định nghĩa interface adapter chung (DEC-SCRAPE-04): `PlatformAdapter.Fetch(ctx, job ScrapeJob) (price.PriceSnapshot, error)`. Mỗi sàn là một implementation (TASK-SCRAPE-002 Shopee, TASK-SCRAPE-007 TikTok, TASK-SCRAPE-008 Lazada) đăng ký theo `platform_id`.
8. **MUST** sau khi adapter trả snapshot, orchestrator gọi `priceRepo.InsertSnapshot` delta-only của TASK-PRICE-002 (DEC-SCRAPE-05) - orchestrator KHÔNG tự viết SQL vào `price_snapshot`.
...
11. **MUST** idempotent khi re-claim: xử lý cùng một job 2 lần (do timeout re-claim) KHÔNG được tạo dữ liệu sai - delta-only của TASK-PRICE-002 (`ON CONFLICT DO NOTHING`) đảm bảo điều này.
12. **MUST** đọc cấu hình bí mật (proxy creds, API base) qua TASK-INFRA-003 (Vault), KHÔNG hardcode trong mã hay env cleartext.
...
**Vì sao giới hạn concurrency per-platform (DEC-SCRAPE-03)?** Anti-bot của Shopee/TikTok/Lazada (§3.9) phản ứng với mật độ request từ một nguồn. Bắn song song không giới hạn là cách nhanh nhất để bị ban. Cap concurrency per-platform giữ áp lực dưới ngưỡng phát hiện, phối hợp với pacing/jitter của TASK-SCRAPE-005.
...
**Vì sao orchestrator không ghi DB (DEC-SCRAPE-05)?** Delta-only là logic của PRICE (TASK-PRICE-002): chỉ ghi khi giá đổi. Nếu orchestrator tự `INSERT`, nó sẽ phải nhân bản logic so sánh và dễ lệch. Gọi `InsertSnapshot` giữ một nguồn sự thật duy nhất cho quy tắc ghi.
...
11. Re-claim cùng job 2 lần không tạo dữ liệu sai (nhờ `ON CONFLICT DO NOTHING` của TASK-PRICE-002).
...
Xem §3. Thứ tự: migration 0001 (scrape_job) -> tier.go (thuần, test trước) -> queue.go (Redis Streams) -> pool.go (worker, gọi adapter + InsertSnapshot) -> scheduler.go (vòng lặp quét job đến hạn) -> tests. Adapter Shopee thật đến ở TASK-SCRAPE-002; ở task này dùng adapter giả (stub) để kiểm thử orchestrator độc lập. Concurrency cap và proxy đọc từ cấu hình Vault (TASK-INFRA-003).

---

## §7 - Phụ thuộc

- **TASK-INFRA-003** - secrets (proxy creds, API base) qua Vault; orchestrator không hardcode.
- **TASK-PRICE-001** - `scrape_job.product_id` REFERENCES `tracked_product(id)` (FK cứng); bảng này phải tồn tại trước khi migration `0001_scrape_job.sql` chạy. `platform(id)` của TASK-INFRA-002 cũng phải có (FK `platform_id`).
- **TASK-PRICE-002 (downstream của ghi)** - orchestrator gọi `InsertSnapshot` delta-only.
- **TASK-SCRAPE-002 / 007 / 008** - các adapter sàn cắm vào interface `PlatformAdapter`.
- **TASK-SCRAPE-003 / 004 / 005** - farm Playwright, proxy rotation, pacing chạy bên trong adapter/pool.
- **TASK-TRACK-001** - tạo `tracked_product` (và một `scrape_job` tương ứng) khi user theo dõi SKU.
...
### Tạo job khi user theo dõi sản phẩm (nội bộ, gọi từ TASK-TRACK-001)
...
- Tier "burst" riêng cho double-date (1.1, 2.2 ... 12.12) tạm gộp vào `hot` qua re-tier; tách thành lịch sự kiện ở giai đoạn ML (TASK-DEAL-004).
- Ưu tiên job (priority queue) cho SKU Premium-user - gắn khi BILL (TASK-BILL-001) lên.
...
| Job mồ côi re-claim -> xử lý 2 lần | property test | ghi trùng | ON CONFLICT DO NOTHING (TASK-PRICE-002) |
| Backoff không jitter -> đồng pha retry | metric spike | tự dồn tải / ban | jitter trong backoff (§1 #5) |
| Proxy creds rò qua env cleartext | audit secrets | lộ bí mật | Đọc từ Vault (TASK-INFRA-003) |
| Dead-letter phình (sàn đổi DOM hàng loạt) | DLQ size | quét hỏng diện rộng | Alert qua TASK-SCRAPE-006, sửa adapter |
...
- Tiering là đòn bẩy unit economics phía băng thông (§4.1), song song với delta-only phía storage (TASK-PRICE-002).
- Concurrency cap per-platform là tuyến phòng thủ anti-ban đầu tiên (§3.9), trước pacing/jitter (TASK-SCRAPE-005) và proxy rotation (TASK-SCRAPE-004).
...
*Hết TASK-SCRAPE-001. Status: ready_to_implement (mục tiêu audit 10/10).*
