...
created: 2026-06-27
related_frs: [TASK-SCRAPE-001, TASK-SCRAPE-002, TASK-SCRAPE-003, TASK-SCRAPE-004, TASK-SCRAPE-005, TASK-SCRAPE-006, TASK-PRICE-002]
...
source_decisions:
  - "DEC-SCRAPE-32: Lazada ưu tiên đọc DOM-render qua farm Playwright (TASK-SCRAPE-003); API Lazada thường ký"
  - "DEC-SCRAPE-33: Akamai đọc fingerprint ở tầng TLS/HTTP2 -> bắt buộc TLS match của farm (TASK-SCRAPE-003 #4) + residential"
  - "DEC-SCRAPE-34: residential bắt buộc (DiffAkamai -> enterprise của TASK-SCRAPE-004); datacenter vô dụng với Akamai"
...
disallowed_tools:
  ...
  - lưu giá float (vi phạm DEC-PRICE-05 của TASK-PRICE-002)
...
1. **MUST** triển khai interface adapter của TASK-SCRAPE-001 và lấy giá qua farm `RenderPrice` của TASK-SCRAPE-003 (DEC-SCRAPE-32) - Lazada API thường ký, ưu tiên đọc DOM đã render.
2. **MUST** dựa vào TLS/HTTP2 fingerprint match của farm (TASK-SCRAPE-003 #4) để qua Akamai (DEC-SCRAPE-33): Akamai đọc JA3/JA4 + HTTP/2 SETTINGS ở tầng bắt tay, trước khi JS chạy; adapter KHÔNG được dùng client thô bỏ qua lớp này.
3. **MUST** dùng proxy residential tier `enterprise` (DEC-SCRAPE-34): job Lazada dùng `SelectTier(DiffAkamai)` của TASK-SCRAPE-004 (= enterprise); datacenter bị từ chối (Akamai chặn chắc).
...
5. **MUST** map về `price.PriceSnapshot` với `price`/`list_price` BIGINT VND số nguyên (đồng bộ DEC-PRICE-05 của TASK-PRICE-002), `flash_sale` true khi trang báo đang flash sale.
6. **MUST** chạy hành vi giống người của farm (TASK-SCRAPE-003 #6) trước khi trích.
7. **MUST** phát hiện Akamai challenge (sensor/verify page) và ném `ChallengedError` (TASK-SCRAPE-003 #8) để TASK-SCRAPE-005 / orchestrator xử lý, KHÔNG trả snapshot rỗng.
8. **MUST** báo outcome (success/parse_fail/challenge) cho monitor của TASK-SCRAPE-006.
...
**Vì sao TLS/HTTP2 match là then chốt với Akamai (DEC-SCRAPE-33)?** Đây là điểm khác biệt cốt lõi của Lazada so với Shopee. Akamai chấm điểm ở tầng bắt tay TLS (JA3/JA4) và HTTP/2 SETTINGS - trước khi một dòng JS nào chạy. Client Node/Go mặc định có chữ ký TLS khác Chrome thật, bị Akamai nhận diện ngay dù JS patch hoàn hảo. Adapter phải đi qua farm có TLS match (TASK-SCRAPE-003 #4), không được dùng client thô.

**Vì sao residential enterprise bắt buộc (DEC-SCRAPE-34)?** Tài liệu nói thẳng: datacenter vô dụng với Akamai (§3.3). Akamai chặn dải IP datacenter theo lô. `SelectTier(DiffAkamai)` của TASK-SCRAPE-004 trả enterprise - residential chất lượng cao nhất - vì Lazada Medium-High không qua được bằng IP rẻ.
...
**Vì sao vẫn tái dùng farm (§1 #1, #2)?** Farm (TASK-SCRAPE-003) đã giải fingerprint + TLS + behavior - đúng những gì Akamai kiểm. Lazada adapter chỉ thêm "đọc giá ở đâu trên trang Lazada". Tái dùng farm tránh nhân bản lớp chống Akamai; adapter chỉ là selectors + extract riêng.
...
export class LazadaAdapter implements PlatformAdapter {
  ...
  async fetch(ctx: BrowserContext, job: ScrapeJob): Promise<PriceSnapshot> {
    // ctx do farm tạo với TLS match (TASK-SCRAPE-003 #4) + proxy residential enterprise
    const page = await ctx.newPage();
    try {
      await page.goto(job.url, { waitUntil: 'domcontentloaded' });
      if (await detectAkamaiChallenge(page)) throw new ChallengedError('akamai sensor/verify');
      await humanize(page);                                  // TASK-SCRAPE-003 behavior
...
10. Adapter báo outcome (success/parse_fail/challenge) cho monitor TASK-SCRAPE-006.
...
Xem §3. Thứ tự: selectors.ts -> extract.ts (JSON-ưu-tiên + DOM-fallback, test trước với fixture) -> adapter.ts (render qua farm TLS-match + chờ render + Akamai challenge) -> registry.go (đăng ký, gắn proxy enterprise) -> tests. Adapter chạy trên farm Playwright của TASK-SCRAPE-003 (đặc biệt cần TLS/HTTP2 match cho Akamai). Proxy lấy qua `SelectTier(DiffAkamai)` của TASK-SCRAPE-004. Outcome báo về monitor TASK-SCRAPE-006.

---

## §7 - Phụ thuộc

- **TASK-SCRAPE-003** - farm Playwright với TLS/HTTP2 match (then chốt cho Akamai), behavior, ChallengedError.
- **TASK-SCRAPE-004** - proxy residential tier enterprise (DiffAkamai); datacenter bị từ chối.
- **TASK-SCRAPE-001** - interface adapter; orchestrator điều phối + gọi sink.
- **TASK-SCRAPE-005** - pacing + xử lý ChallengedError; ghi qua delta-only.
- **TASK-SCRAPE-006** - monitor nhận outcome.
- **TASK-PRICE-002 / TASK-PRICE-004 (downstream)** - snapshot ghi qua InsertSnapshot; Lazada là chân thứ ba của so sánh chéo.
...
| Akamai challenge coi như "không giá" | ChallengedError | dữ liệu bẩn | Ném lỗi, chuyển TASK-SCRAPE-005 (§1 #7) |
...
- Lazada là chân thứ ba của moat so sánh chéo 3 sàn (TASK-PRICE-004) - thiếu nó thì so sánh giá yếu hẳn.
- Khác biệt cốt lõi của Lazada là Akamai: kiểm fingerprint ở tầng TLS/HTTP2 trước khi JS chạy - TLS match của farm (TASK-SCRAPE-003 #4) là điều kiện sống còn, không phải tùy chọn.
- Residential enterprise bắt buộc vì datacenter vô dụng với Akamai (§3.3) - đây không phải lựa chọn tiết kiệm.
- Cùng khuôn với TikTok (TASK-SCRAPE-007): đọc DOM-render, JSON nhúng ưu tiên, chờ render, né API ký; khác ở lớp Akamai TLS.
- Giá VND số nguyên đồng bộ TASK-PRICE-002 - sai số float làm hỏng so sánh chéo sàn vốn là điểm mạnh của Lazada trong moat.

---

*Hết TASK-SCRAPE-008. Status: ready_to_implement (mục tiêu audit 10/10).*
