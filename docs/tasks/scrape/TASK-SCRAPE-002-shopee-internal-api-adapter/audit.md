...
Re-derive từ file hiện tại. TASK-SCRAPE-002 đặc tả adapter Shopee internal-API ở mức triển khai được: 12 mệnh đề §1 normative (đủ MUST), mỗi cái testable có AC §4 và test §5. Đường giá chính là `/api/v4/pdp/get_pc` truy cập `is_login:false` (khớp §3.2 nguồn), `recommend` là nguồn phụ; fallback Playwright khi gặp HTML challenge (anti-bot Shopee Medium-High §3.9). Quy đổi giá micro-VND bằng phép chia số nguyên, đồng bộ BIGINT VND của TASK-PRICE-002. Ranh giới token-not-on-server (không cookie người dùng ở backend) được khóa bằng test. Score = 10/10.
...
Kiểm chéo nguồn: endpoint `/api/v4/pdp/get_pc` và `/api/v4/recommend/recommend`, một số truy cập được khi `is_login:false` (§3.2 dòng 69); anti-bot Shopee Medium-High với slider/puzzle CAPTCHA (§3.9). task mô tả đúng. Không khiếm khuyết.
...
Adapter chỉ trả `PriceSnapshot`; ghi delta-only do orchestrator gọi `InsertSnapshot` (TASK-SCRAPE-001 #8), AC12 kiểm tích hợp. Adapter không tự ghi SQL. Đúng.
...
*Audit độc lập TASK-SCRAPE-002 - hết.*
