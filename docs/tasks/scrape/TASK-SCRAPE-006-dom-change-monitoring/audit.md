...
Kiểm chéo: §3.2 nói DOM giỏ hàng Shopee thay đổi theo A/B test; §5.2 là rủi ro phụ thuộc nền tảng existential (sàn đổi DOM/API). task §1 + §11 neo đúng vào hai mục này; DEC-SCRAPE-24..27 hợp lý. Không khiếm khuyết.

### ISS-003 - Cross-ref orchestrator + observability (xác nhận)
§1 #4 alert qua TASK-INFRA-004; §1 #6 `ShouldThrottle` cho orchestrator TASK-SCRAPE-001 nhân `next_run_at` khi broken (AC9); §1 #9 phân biệt parse_fail vs challenge (challenge đến từ TASK-SCRAPE-005), AC10 `TestMonitor_ParseFailNotChallenge`. Bảng §10 non-trivial (10 dòng, đủ phát hiện + khắc phục). task này không sở hữu bảng DB nên không có ràng buộc schema/tiền tệ - phù hợp.
...
*Audit độc lập TASK-SCRAPE-006 - hết.*
