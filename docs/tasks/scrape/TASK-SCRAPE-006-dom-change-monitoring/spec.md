...
created: 2026-06-27
related_frs: [TASK-SCRAPE-001, TASK-SCRAPE-002, TASK-SCRAPE-003, TASK-SCRAPE-005, TASK-SCRAPE-007, TASK-SCRAPE-008, TASK-INFRA-004]
depends_on: [TASK-SCRAPE-002]
blocks: []
source_pages:
...
language: "Go 1.22 (scrape-svc); rolling-window stats + alert qua observability (TASK-INFRA-004)"
...
risk_if_skipped: "Rủi ro phụ thuộc nền tảng là existential (§5.2): sàn đổi DOM/API bất cứ lúc nào, Shopee còn A/B test DOM liên tục (§3.2). Không giám sát thì adapter hỏng âm thầm - dữ liệu giá ngừng cập nhật mà không ai biết cho tới khi user phàn nàn, sale ảo (TASK-DEAL-001) và biểu đồ (TASK-DEAL-003) chạy trên dữ liệu chết. Tệ hơn: parse hỏng hàng loạt vẫn quét full tần suất sẽ đốt proxy vào các request vô dụng (vỡ §4.1). Đây là hệ thần kinh cảm giác của farm - phát hiện sớm khi sàn đổi để con người sửa adapter trước khi dữ liệu thối."
---
...
**Vì sao phân biệt parse_fail với challenge/network (§1 #9)?** Ba loại lỗi cần ba phản ứng khác nhau. `challenge` cao -> việc của TASK-SCRAPE-005 (CAPTCHA) và TASK-SCRAPE-004 (proxy). `network_err` cao -> hạ tầng. Chỉ `parse_fail` cao mới nghĩa "sàn đổi DOM, cần người sửa adapter". Gộp chung làm alert sai địa chỉ, người trực sửa nhầm chỗ.
...
Xem §3. Thứ tự: window.go (ring buffer thread-safe, test trước) -> state.go (Next thuần với hysteresis) -> monitor.go (ghép window + state + baseline động) -> alert.go (dedup + cooldown) -> tích hợp pool.go (báo outcome + đọc ShouldThrottle) -> tests. Alert đi qua observability của TASK-INFRA-004. Orchestrator (TASK-SCRAPE-001) đọc `ShouldThrottle` để nhân `next_run_at` khi target broken.

---

## §7 - Phụ thuộc

- **TASK-SCRAPE-002 / 007 / 008** - adapter báo outcome (success/parse_fail) vào monitor.
- **TASK-SCRAPE-001** - orchestrator đọc `ShouldThrottle` để hạ tải target broken.
- **TASK-SCRAPE-005** - `challenge` outcome đến từ CAPTCHA detect; monitor phân biệt với parse_fail.
- **TASK-INFRA-004** - alert đi qua observability spine (Prometheus/Grafana/log).
...
- Rủi ro phụ thuộc nền tảng là existential (§5.2); giám sát DOM + đa sàn là hai mitigation chính, task này lo phần giám sát.
...
*Hết TASK-SCRAPE-006. Status: ready_to_implement (mục tiêu audit 10/10).*
