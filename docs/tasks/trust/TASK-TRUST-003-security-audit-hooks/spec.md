...
created: 2026-06-28
related_frs: [TASK-EXT-003, TASK-COMPLY-005, TASK-TRUST-001, TASK-TRUST-002, NFR-TRUST-001]
depends_on: [TASK-EXT-003, TASK-COMPLY-005]
blocks: []
source_pages:
...
source_decisions:
  ...
  - "DEC-TRUST-14: audit hook gọi verify-reproducible (TASK-TRUST-001) + payload_guard (TASK-COMPLY-005) như thành phần con; TASK-TRUST-003 là lớp tổng hợp 'bằng chứng đầu cuối'"
...
sub_tasks:
  ...
  - "1.0h: run-security-audit.sh - orchestrate egress + sbom + verify-reproducible (TASK-TRUST-001) + payload_guard (TASK-COMPLY-005); FAIL nếu hook nào đỏ"
...
4. Egress test **MUST** chỉ cho phép outbound tới endpoint backend SănDeal đã biết với payload đúng allowlist (`OutboundPayload` TASK-EXT-003); request tới host lạ -> FAIL (chống rò qua kênh bên).
...
7. `run-security-audit.sh` **MUST** orchestrate và gọi như thành phần con (DEC-TRUST-14): (a) egress test, (b) SBOM + vuln scan, (c) `verify-reproducible.sh` (TASK-TRUST-001), (d) `payload_guard` (TASK-COMPLY-005). Audit PASS chỉ khi MỌI hook con PASS (DEC-TRUST-15).
...
10. CI publish gate (TASK-TRUST-001) **MUST** gọi `run-security-audit.sh`; audit FAIL **MUST** chặn ship lên store.
...
**Vì sao kiểm tại biên mạng, không tin self-report (DEC-TRUST-12)?** Code có thể "nói" nó không gửi cookie, nhưng một bug, một thư viện bên thứ ba, hay một đường fetch quên lọc vẫn có thể mang cookie ra ngoài. Kiểm tĩnh (grep, TASK-COMPLY-005) bắt được mẫu đã biết, nhưng không thấy hành vi runtime. Egress test chặn tất cả rồi quan sát đúng những gì thật sự rời máy - đây là bằng chứng động, mạnh hơn mọi lời khai trong mã.
...
**Vì sao chỉ cho outbound tới endpoint đã biết với payload đúng allowlist (§1 #4)?** Rò dữ liệu không chỉ qua "gửi cookie tới backend" - còn có thể qua gửi dữ liệu tới host lạ (kênh bên). Whitelist endpoint + assert payload khớp allowlist TASK-EXT-003 đóng cả hai: đúng đích VÀ đúng nội dung. Bất kỳ outbound ngoài luật -> đỏ.

**Vì sao audit là lớp tổng hợp gọi các hook con (DEC-TRUST-14)?** TASK-TRUST-001 lo reproducible, TASK-COMPLY-005 lo kiểm tĩnh + payload guard. TASK-TRUST-003 không lặp lại chúng mà gọi chúng như mảnh ghép và thêm mảnh còn thiếu (egress động + SBOM), rồi tổng hợp thành "bằng chứng đầu cuối". Audit PASS chỉ khi cả chuỗi PASS - một mắt xích đỏ là cả audit đỏ.
...
test("đọc giỏ có cookie phiên sàn -> KHÔNG cookie/token rời máy", async () => {
  ...
  await loadExtensionAndReadCart(page);     // chạy luồng đọc giỏ thật (TASK-EXT-002/003)
...
5. Outbound mang payload ngoài allowlist (TASK-EXT-003) -> FAIL.
...
8. `run-security-audit.sh` gọi `verify-reproducible.sh` (TASK-TRUST-001) + `payload_guard` (TASK-COMPLY-005); một trong hai đỏ -> audit FAIL.
...
Xem §3. Thứ tự: `network-trap.ts` (bắt + assert outbound) -> `egress-guard.test.ts` (chạy luồng đọc giỏ có phiên qua Playwright) -> `generate-sbom.sh` + `scan-vulnerabilities.sh` (CycloneDX + CVE) -> `run-security-audit.sh` (orchestrate bốn hook, gọi `verify-reproducible.sh` của TASK-TRUST-001 + `payload_guard` của TASK-COMPLY-005) -> `build-report.ts` + template (JSON + markdown) -> `THIRD-PARTY-AUDIT-GUIDE.md` (hướng dẫn tái chạy) -> nối vào CI publish gate. Audit là lớp tổng hợp: nó không lặp lại reproducible/payload_guard mà gọi chúng và thêm egress động + SBOM. Mọi hook đỏ là audit đỏ; mọi lần chạy đủ bốn hook hoặc báo INCOMPLETE.

---

## §7 - Phụ thuộc

- **TASK-EXT-003** - allowlist `OutboundPayload` là chuẩn để egress test assert payload đúng (đúng nội dung rời máy).
- **TASK-COMPLY-005** - `payload_guard` (kiểm tĩnh + payload nhận từ extension không chứa cookie/token) là một hook con của audit; TASK-TRUST-003 thêm tầng động.
- **TASK-TRUST-001** - `verify-reproducible.sh` là một hook con; reproducible build + source công khai làm bằng chứng "mã chạy = mã công khai".
- **TASK-TRUST-002** - chính sách + data-flow là tài liệu đối chiếu cho audit (audit chứng minh hành vi khớp chính sách công bố).
...
- Egress test cho TikTok Shop/Lazada (P2) - cùng bộ test, thêm khi content script hai sàn đó ship (TASK-EXT-007/008).
...
| Mã chạy khác mã công khai | verify-reproducible (hook con) | audit trên mã sai | Gọi verify-reproducible của TASK-TRUST-001 (§1 #7) |
...
- TASK-TRUST-003 là lớp tổng hợp: nó gọi reproducible (TASK-TRUST-001) + payload_guard (TASK-COMPLY-005) như mảnh ghép và thêm egress động + SBOM để thành bằng chứng đầu cuối.
...
*Hết TASK-TRUST-003. Status: ready_to_implement (mục tiêu audit 10/10).*
