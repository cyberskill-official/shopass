...
created: 2026-06-28
related_frs: [TASK-AFFIL-001, TASK-AFFIL-005, TASK-TRUST-004, TASK-BILL-002, NFR-TRUST-001, NFR-AFFIL-001]
depends_on: [TASK-AFFIL-001, TASK-AFFIL-003, TASK-BILL-002, TASK-TRUST-004]
blocks: [TASK-AFFIL-005]
source_pages:
...
source_decisions:
  ...
  - "DEC-TRUST-23: tiêu thụ risk_score từ TASK-TRUST-004; conversion thuộc chủ thể risk cao -> kéo dài delay hoặc giữ chờ điều tra; KHÔNG tự từ chối payout mù"
  - "DEC-TRUST-24: trạng thái payout: eligible_at (thời điểm hết delay) + hold_reason; chỉ payout khi (a) qua delay, (b) không hold điều tra, (c) network đã confirm (TASK-AFFIL-003)"
...
disallowed_tools:
  ...
  - bỏ qua xác nhận network (postback TASK-AFFIL-003) khi giải ngân (vi phạm DEC-TRUST-24 - trả tiền chưa xác nhận)
...
sub_tasks:
  ...
  - "1.0h: delay.go - tính eligible_at = confirmed_at + delay_window; kéo dài nếu risk_score cao (TASK-TRUST-004)"
...
7. Self-referral check **MUST** dùng đồ thị quan hệ TASK-TRUST-004: nếu người mua và chủ link thuộc cùng cụm liên kết -> gắn `hold_reason='self_referral'`.
...
10. Khi điều tra kết luận `cleared` (TASK-TRUST-004), `payout_hold` **MUST** được giải phóng ở chu kỳ release kế tiếp (tiền thật không bị quỵt).
...
**Vì sao self-referral check qua đồ thị (DEC-TRUST-22b, §1 #7)?** Tự mua qua link của chính mình (hoặc tài khoản cùng cụm) là cách rút hoa hồng phổ biến. Một mình khó thấy; nhìn ở đồ thị quan hệ (TASK-TRUST-004), nếu người mua và chủ link cùng cụm liên kết thì lộ. Tái dùng đồ thị có sẵn thay vì dựng lại.
...
func (g *Guard) Inspect(ctx context.Context, conv Conversion) string {
    ...
    // (b) self-referral: người mua & chủ link cùng cụm (TASK-TRUST-004)
...
5. Self-referral (người mua + chủ link cùng cụm TASK-TRUST-004) -> `hold_reason='self_referral'`.
6. Click không user-initiated -> `hold_reason='cookie_stuffing_signal'` (nối NFR-AFFIL-001).
7. `risk_score` cao (TASK-TRUST-004) -> delay kéo dài / chuyển `under_investigation`; KHÔNG tự `denied`.
...
func TestGuard_SelfReferral_SameCluster(t *testing.T) {
    g := setupGuard(t)
    conv := convBuyerAndReferrerLinked(t)   // cùng cụm TASK-TRUST-004
...
func TestRelease_HighRisk_HoldsNotDenies(t *testing.T) {
    r := setup(t)
    h := seedHold(t, r, confirmedAt(time.Now().Add(-100*time.Hour)), delay(72*time.Hour))
    setRiskHigh(t, r, h)                 // TASK-TRUST-004
    r.ReleaseDue(ctx)
...
Xem §3. Thứ tự: migration 0003 (`payout_hold`) -> nối affil repo (conversion `confirmed` -> tạo `payout_hold`, không trả ngay) -> `attribution_guard.go` (gắn `hold_reason` cho last-click/self-referral/cookie-stuffing) -> `delay.go` (tính `eligible_at`, kéo dài khi risk cao) -> `release.go` (job giải ngân khi đủ ba điều kiện, idempotent qua lock + status flip). Self-referral check tái dùng `SameCluster` của TASK-TRUST-004. Job release chạy định kỳ; `denied` chỉ đến từ kết luận điều tra `confirmed_fraud`, không tự động. Cashback layering (TASK-AFFIL-005) trả qua cùng cơ chế hold-then-release.

---

## §7 - Phụ thuộc

- **TASK-AFFIL-001** - `affiliate_conversion` (status confirmed) là nguồn tạo `payout_hold`; `affiliate_click` (user_initiated) cho cookie-stuffing check.
- **TASK-TRUST-004** - `risk_score` + `SameCluster` (đồ thị quan hệ) cho kéo dài delay + self-referral check.
- **TASK-AFFIL-005 (downstream)** - cashback layering trả qua cơ chế hold-then-release của task này.
- **TASK-AFFIL-003** - postback network confirm là điều kiện (c) để giải ngân.
- **TASK-BILL-002** - cổng thanh toán thực hiện chi trả khi `release.go` quyết payout.
- NFR-AFFIL-001 - cookie-stuffing/user-initiated; task này thực thi tuyến phòng thủ payout cho cam kết đó.
...
Đã chốt. Hoãn:
- Độ dài `delay_window` tối ưu theo sàn/network - hiệu chỉnh theo dữ liệu thật (cân chống gian lận vs trải nghiệm chờ); task đặt cơ chế cấu hình.
...
- Thông báo cho user vì sao payout bị giữ (minh bạch hold_reason ở mức phù hợp) - thiết kế UX ở task web/mobile, task này cung cấp dữ liệu.
...
| Self-referral rút hoa hồng | guard test (same cluster) | tự cày tiền | Đồ thị quan hệ TASK-TRUST-004 (§1 #7) |
...
- Nguyên tắc đối xứng với TASK-TRUST-004: chỉ delay/giữ + điều tra, KHÔNG tự từ chối - mặc định nghiêng về trả tiền thật, chỉ giữ khi có hold_reason cụ thể.
- Ba dấu hiệu gaming (last-click thao túng, self-referral, cookie-stuffing) cùng nhắm đúng các trò rút hoa hồng phổ biến; self-referral tái dùng đồ thị TASK-TRUST-004.
...
- Cookie-stuffing check nối thẳng cam kết né-Honey (§4.2): đây đúng là trò Honey bị phơi bày, và SănDeal chặn nó ở cả tầng click (NFR-AFFIL-001) lẫn tầng payout (task này).

---

*Hết TASK-TRUST-005. Status: ready_to_implement (mục tiêu audit 10/10).*
