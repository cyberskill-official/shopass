...
created: 2026-06-28
related_frs: [TASK-EXT-003, TASK-EXT-006, TASK-TRUST-001, TASK-TRUST-003, TASK-COMPLY-001, TASK-COMPLY-005, NFR-TRUST-001]
depends_on: [TASK-EXT-003]
blocks: []
source_pages:
...
source_decisions:
  - "DEC-TRUST-06: chính sách tối thiểu hóa là VĂN BẢN có thể kiểm chứng (machine-checkable), KHÔNG phải tuyên bố marketing; nó tham chiếu allowlist TASK-EXT-003 làm nguồn sự thật"
  ...
  - "DEC-TRUST-09: chính sách phải gắn cơ sở pháp lý PDPL (TASK-COMPLY-001) cho từng mục đích xử lý; tối thiểu hóa dữ liệu là nguyên tắc PDPL, không chỉ là PR"
...
sub_tasks:
  ...
  - "0.5h: collected-fields.ts - hằng nguồn sự thật mô tả trường thu thập + mục đích (dùng cho policy + UI consent TASK-EXT-006)"
  ...
  - "0.5h: liên kết chính sách vào DISCLOSURE.md (TASK-TRUST-001) + UI consent (TASK-EXT-006) - một nguồn nội dung"

risk_if_skipped: "Tài liệu nguồn (§5.4) yêu cầu rõ 'chính sách dữ liệu minh bạch (không gửi cookie/mật khẩu)' và 'xử lý dữ liệu tối thiểu hóa, local-first' như hai trong năm trụ niềm tin. Nếu chính sách chỉ là văn bản marketing không nối với hành vi thật, nó sẽ trôi khỏi thực tế ngay khi pipeline đổi: người dùng đọc một đằng, extension làm một nẻo - đúng kiểu mất niềm tin mà SănDeal muốn tránh hậu-Honey. Tối thiểu hóa dữ liệu cũng là nguyên tắc PDPL (Luật 91/2025): thu thập vượt mục đích là vi phạm. Neo chính sách vào allowlist TASK-EXT-003 qua test biến 'minh bạch' thành ràng buộc máy kiểm: chính sách không thể nói nhiều/ít hơn thực tế. Bỏ task này, lời hứa tối thiểu hóa thành khẩu hiệu trống - phá nền tảng niềm tin và mời rủi ro PDPL."
---

## §1 - Mô tả (BCP-14 normative)

SănDeal **MUST** công bố một chính sách tối thiểu hóa dữ liệu kiểm chứng được: liệt kê đúng tập dữ liệu thu thập, mục đích từng trường, cam kết KHÔNG gửi cookie/mật khẩu/token, và neo vào allowlist pipeline (TASK-EXT-003) làm nguồn sự thật máy kiểm. Hợp đồng:

1. `DATA-MINIMIZATION-POLICY.md` **MUST** liệt kê CHÍNH XÁC tập dữ liệu extension thu thập và gửi đi - đúng tập tối thiểu `{platform, productId, price, qty}` (+ voucher hiển thị) của TASK-EXT-003 - không rộng hơn, không hẹp hơn (DEC-TRUST-08).
...
5. `data-flow.md` **MUST** trình bày sơ đồ luồng dữ liệu (client -> `minimize` -> queue -> backend) chỉ rõ ĐIỂM dữ liệu rời máy và khẳng định tại điểm đó payload không chứa dữ liệu nhạy cảm (nối TASK-EXT-003).
6. Chính sách **MUST** gắn cơ sở pháp lý PDPL (TASK-COMPLY-001) cho mỗi mục đích xử lý; nêu rõ tối thiểu hóa dữ liệu là nguyên tắc PDPL (Luật 91/2025), không chỉ là cam kết tự nguyện (DEC-TRUST-09).
7. **MUST** có `collected-fields.ts`: một hằng nguồn sự thật mô tả từng trường thu thập + mục đích, dùng chung cho chính sách, UI consent (TASK-EXT-006), và disclosure (TASK-TRUST-001) - để ba nơi không thể lệch nhau.
8. **MUST** có test `policy-allowlist-parity.test.ts` khẳng định: tập trường mô tả trong `collected-fields.ts` khớp đúng allowlist `OutboundPayload` của TASK-EXT-003. Thêm/bớt trường ở allowlist mà không cập nhật chính sách -> test fail.
9. Chính sách **MUST** nêu chính sách lưu trữ/xóa (retention) cho dữ liệu thu thập, nhất quán với quyền chủ thể dữ liệu (TASK-COMPLY-003, DSAR).
...
11. **MUST NOT** chứa tuyên bố không thể kiểm chứng hoặc rộng hơn hành vi thật (ví dụ "chúng tôi không bao giờ chia sẻ dữ liệu" nếu B2B aggregate TASK-B2B-001 có chia sẻ dạng ẩn danh - phải mô tả đúng, kèm điều kiện k-anonymity).
12. Chính sách **MUST** được tham chiếu từ DISCLOSURE.md (TASK-TRUST-001) và UI consent (TASK-EXT-006) - một nguồn nội dung duy nhất, tránh ba bản lệch nhau.

---

## §2 - Vì sao thiết kế này (rationale cho người đọc)

**Vì sao chính sách phải máy kiểm được (DEC-TRUST-06)?** Chính sách quyền riêng tư thông thường là văn bản pháp lý tách rời code - nó đúng ngày viết rồi trôi dần khỏi thực tế khi sản phẩm đổi. SănDeal đảo cách làm: chính sách neo vào allowlist thực thi (TASK-EXT-003) qua test, nên nếu pipeline đổi tập dữ liệu mà chính sách không cập nhật, CI đỏ. "Minh bạch" trở thành ràng buộc kỹ thuật, không phải thiện chí.

**Vì sao một nguồn sự thật chung `collected-fields.ts` (§1 #7)?** Cùng một thông tin xuất hiện ba nơi: chính sách, UI consent lúc cài (TASK-EXT-006), disclosure store (TASK-TRUST-001). Nếu mỗi nơi viết tay, chúng lệch nhau theo thời gian. Một hằng chung làm cả ba dẫn xuất từ một chỗ - sửa một lần, đồng bộ mọi nơi.
...
**Vì sao gắn cơ sở pháp lý PDPL (DEC-TRUST-09)?** Tối thiểu hóa dữ liệu không chỉ là điểm bán niềm tin - nó là nghĩa vụ pháp lý theo PDPL (Luật 91/2025): xử lý phải đúng mục đích, đơn mục đích, không thu thập thừa. Gắn mỗi mục đích với cơ sở pháp lý làm chính sách vừa minh bạch vừa phòng thủ pháp lý, nối với khung consent TASK-COMPLY-001.

**Vì sao cấm tuyên bố rộng hơn hành vi (§1 #11)?** Hứa "không bao giờ chia sẻ dữ liệu" rồi B2B (TASK-B2B-001) bán insight ẩn danh là mâu thuẫn phá niềm tin. Chính sách phải mô tả đúng - kể cả phần chia sẻ ẩn danh hóa với điều kiện k-anonymity - thay vì hứa tuyệt đối rồi vi phạm. Trung thực có điều kiện bền hơn tuyệt đối giả.
...
export interface CollectedField {
  field: string;        // tên trường khớp allowlist OutboundPayload (TASK-EXT-003)
  purpose: string;      // mục đích xử lý (ngôn ngữ người dùng)
  legalBasis: string;   // cơ sở pháp lý PDPL (TASK-COMPLY-001)
}
...
5. Chính sách gắn cơ sở pháp lý PDPL cho mỗi mục đích (nối TASK-COMPLY-001).
6. `collected-fields.ts` mô tả từng trường + purpose + legalBasis; được tham chiếu bởi chính sách, UI consent, disclosure.
7. `policy-allowlist-parity.test.ts`: mọi trường allowlist (TASK-EXT-003) được mô tả; KHÔNG mô tả trường pipeline không gửi.
8. Thêm trường mới vào allowlist mà không cập nhật `collected-fields.ts` -> test đỏ.
9. Chính sách nêu retention/xóa nhất quán TASK-COMPLY-003 (DSAR).
10. Chính sách dùng ngôn ngữ phổ thông (không chỉ thuật ngữ); rà soát đọc hiểu.
11. KHÔNG có tuyên bố tuyệt đối mâu thuẫn với B2B aggregate ẩn danh (TASK-B2B-001) - mô tả đúng kèm điều kiện.
12. Chính sách được tham chiếu từ DISCLOSURE.md (TASK-TRUST-001) và UI consent (TASK-EXT-006); `npm test` xanh.
...
Xem §3. Thứ tự: `collected-fields.ts` (định nghĩa nguồn sự thật trường + mục đích + cơ sở pháp lý) + export `ALLOWED_*` từ `allowlist.ts` -> `policy-allowlist-parity.test.ts` (khóa chính sách vào pipeline) -> `DATA-MINIMIZATION-POLICY.md` (dẫn xuất nội dung từ `collected-fields.ts`, thêm local-first + retention + cơ sở pháp lý PDPL) -> `data-flow.md` (sơ đồ điểm rời máy) -> liên kết vào DISCLOSURE.md (TASK-TRUST-001) + UI consent (TASK-EXT-006). Quy ước: mọi thay đổi tập dữ liệu phải sửa `collected-fields.ts` + allowlist cùng lúc; test parity là cổng giữ hai bên đồng bộ.

---

## §7 - Phụ thuộc

- **TASK-EXT-003** - allowlist `OutboundPayload` là nguồn sự thật cho tập dữ liệu; chính sách + test parity neo vào đây.
- **TASK-EXT-006** - UI consent lúc cài dùng `collected-fields.ts` cho nội dung minh bạch khi xin đồng thuận.
- **TASK-TRUST-001 (đồng cấp)** - DISCLOSURE.md trên store dẫn xuất cùng nguồn nội dung; reproducible build cho phép kiểm chứng chính sách khớp mã.
- **TASK-TRUST-003 (downstream)** - audit độc lập dùng chính sách + data-flow + parity test làm tài liệu tham chiếu khi kiểm KHÔNG gửi cookie/PII.
- **TASK-COMPLY-001** - khung consent PDPL cung cấp cơ sở pháp lý cho từng mục đích xử lý nêu trong chính sách.
- **TASK-COMPLY-003** - DSAR (quyền truy cập/xóa) khớp mục retention của chính sách.
- **TASK-COMPLY-005** - cưỡng chế no-cleartext/token; chính sách khẳng định đúng bất biến mà COMPLY-005 kiểm máy.
...
Đã chốt. Hoãn:
- Bản dịch chính sách sang tiếng Anh + ngôn ngữ SEA (ID/TH) - khi mở rộng per-country (TASK-COMPLY-007); cùng cơ chế parity.
- Mức chi tiết retention cụ thể (số ngày giữ price_snapshot quy cho user) - căn theo TASK-PRICE-002 (raw 18 tháng) + TASK-COMPLY-003; tinh ở giai đoạn DSAR.
- Mô tả B2B aggregate ẩn danh chi tiết - bổ sung khi TASK-B2B-001 chốt ngưỡng k-anonymity; task này chỉ cấm tuyên bố tuyệt đối mâu thuẫn.
- Chính sách cho dữ liệu mobile (TASK-MOBILE-001) - mở rộng cùng khung khi mobile ship (P3).
...
- TASK-TRUST-003 (audit độc lập) dùng chính sách này làm tài liệu đối chiếu khi chứng minh extension KHÔNG gửi cookie/mật khẩu.

---

*Hết TASK-TRUST-002. Status: ready_to_implement (mục tiêu audit 10/10).*
