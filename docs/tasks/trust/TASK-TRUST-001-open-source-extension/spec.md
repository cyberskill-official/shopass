...
created: 2026-06-28
related_frs: [TASK-EXT-001, TASK-EXT-003, TASK-EXT-006, TASK-TRUST-002, TASK-TRUST-003, TASK-COMPLY-005, NFR-TRUST-001]
...
source_decisions:
  ...
  - "DEC-TRUST-05: store listing data-use phải nhất quán với pipeline tối thiểu hóa (TASK-EXT-003) và chính sách local-first (TASK-TRUST-002) - không khai gì pipeline không làm, không làm gì không khai"
...
sub_tasks:
  ...
  - "1.0h: DISCLOSURE.md - liệt kê đúng dữ liệu thu thập (productId/price/qty) + khẳng định KHÔNG cookie/mật khẩu/token; trỏ TASK-EXT-003"
  - "0.5h: chrome-web-store-listing.md - mục data-use nhất quán DISCLOSURE.md + TASK-EXT-003"
...
6. `DISCLOSURE.md` **MUST** liệt kê CHÍNH XÁC tập dữ liệu extension thu thập và gửi đi - đúng tập tối thiểu `{platform, productId, price, qty}` (+ voucher hiển thị) của TASK-EXT-003 - và **MUST** khẳng định tường minh KHÔNG gửi cookie, mật khẩu, hay token phiên sàn về server (DEC-TRUST-03).
7. Mục data-use trên Chrome Web Store listing (`chrome-web-store-listing.md`) **MUST** nhất quán với `DISCLOSURE.md` và với allowlist thực tế của TASK-EXT-003: không khai gì pipeline không làm, không làm gì không khai (DEC-TRUST-05).
8. **MUST** có test `disclosure-consistency.test.ts` khẳng định danh sách trường khai trong DISCLOSURE/listing khớp đúng allowlist `OutboundPayload` của TASK-EXT-003. Nếu pipeline thêm/bớt trường mà disclosure không cập nhật -> test fail (chống "khai một đằng làm một nẻo").
...
10. Disclosure **MUST** nêu lý do từng quyền (`host_permissions` per-domain, `storage`, `alarms` của TASK-EXT-001) bằng ngôn ngữ người dùng hiểu - vì sao extension cần quyền đó, dùng làm gì.
...
**Vì sao cấm obfuscation (§1 #11)?** Mã ship bị làm rối cố ý thì "mở source" thành hình thức - người đọc không map được bundle về mã. Yêu cầu mã ship đọc được/map được là điều kiện để kiểm tra độc lập (TASK-TRUST-003) khả thi.
...
7. `disclosure-consistency.test.ts`: danh sách trường khai trong DISCLOSURE khớp allowlist `OutboundPayload` (TASK-EXT-003); thêm/bớt trường mà không cập nhật disclosure -> test đỏ.
...
Xem §3. Thứ tự: bật cờ deterministic trong `build.mjs` + `build-deterministic.mjs` (loại timestamp/path/thứ tự ngẫu nhiên) -> `verify-reproducible.sh` (build lại + so hash) -> `DISCLOSURE.md` + `chrome-web-store-listing.md` (khai data-use khớp TASK-EXT-003) -> `disclosure-consistency.test.ts` (nối disclosure với allowlist) -> `reproducible-publish-gate.yml` (CI chặn publish khi lệch) -> cập nhật `manifest.json` (homepage_url + version=tag). Quy trình release: tag commit -> CI build deterministic -> verify hash -> publish lên store -> ghi SHA-256 vào release notes. Mọi bản ship đi qua gate này.

---

## §7 - Phụ thuộc

- **TASK-EXT-001** - scaffold MV3 + build.mjs là nền để bật chế độ deterministic; manifest có sẵn để thêm homepage_url/version.
- **TASK-EXT-003** - allowlist `OutboundPayload` là nguồn sự thật cho danh sách trường khai trong DISCLOSURE; test consistency nối hai bên.
- **TASK-EXT-006** - UI consent + disclosure lúc cài đặt dùng cùng nội dung DISCLOSURE.md.
- **TASK-TRUST-002 (đồng cấp)** - chính sách tối thiểu hóa + local-first; disclosure phản ánh chính sách này.
- **TASK-TRUST-003 (downstream)** - audit độc lập dùng reproducible build + disclosure làm bằng chứng kiểm chứng được; TASK-TRUST-001 cung cấp nền hash + source công khai cho audit.
- **TASK-COMPLY-005** - cưỡng chế no-cleartext/token-not-on-server; disclosure khẳng định đúng bất biến mà COMPLY-005 kiểm máy.
...
- Bug bounty công khai cho extension - thuộc chương trình bảo mật rộng hơn, không thuộc task này.
...
| Disclosure khai sai/thiếu trường | disclosure-consistency test đỏ | "khai một đằng làm một nẻo" | Test nối disclosure với allowlist TASK-EXT-003 (§1 #8) |
| Listing store lệch DISCLOSURE.md | so trường-với-trường (§4 #8) | review từ chối / hiểu lầm | Một nguồn sự thật (TASK-EXT-003) cho cả hai |
...
- Disclosure không phải văn bản marketing - nó được nối với allowlist thực thi (TASK-EXT-003) qua test, nên không thể lệch khỏi hành vi thật của extension.
...
- TASK-TRUST-003 (audit độc lập) đứng trên nền này: hash + source công khai là đầu vào để bên thứ ba xác minh KHÔNG gửi cookie/mật khẩu.
...
*Hết TASK-TRUST-001. Status: ready_to_implement (mục tiêu audit 10/10).*
