...
created: 2026-06-27
related_frs: [TASK-AUTH-001, TASK-AUTH-002, TASK-INFRA-003, TASK-WEB-001]
...
source_decisions:
  ...
  - "DEC-AUTH-20: sau khi xác thực OAuth, phát hành cùng TokenPair của TASK-AUTH-002 (một đường token thống nhất)"
...
disallowed_tools:
  ...
  - lưu OAuth client secret trong code/env (phải qua TASK-INFRA-003)
...
sub_tasks:
  ...
  - "1.0h: oauth_callback.go - khớp/liên kết identity -> app_user; phát TokenPair (TASK-AUTH-002)"
...
Service AUTH **SHOULD** hỗ trợ đăng nhập qua OAuth/OIDC với Google, Facebook, Zalo, liên kết identity vào `app_user` an toàn, và phát hành cùng TokenPair của TASK-AUTH-002. Hợp đồng:
...
7. Nếu chưa có `app_user` khớp, AUTH **MUST** tạo `app_user` mới (qua đường TASK-AUTH-001, `pwd_hash` có thể NULL cho tài khoản chỉ-social) và gắn `social_identity`.
8. Sau xác thực thành công, AUTH **MUST** phát hành cùng `TokenPair` của TASK-AUTH-002 (DEC-AUTH-20) - một đường token thống nhất, không token riêng cho social.
9. OAuth client secret và cấu hình provider **MUST** lấy qua TASK-INFRA-003, KHÔNG nhúng trong code/env.
...
Vì sao một đường token thống nhất (DEC-AUTH-20)? Sau khi xác thực (mật khẩu hay social), phần còn lại của hệ thống chỉ nên thấy một loại phiên. Phát cùng `TokenPair` của TASK-AUTH-002 cho social tránh nhân đôi logic phiên/refresh và giữ gateway/route đồng nhất bất kể cách đăng nhập.
...
11. OAuth client secret đọc qua provider secret (TASK-INFRA-003), không có trong code/env (grep).
12. Sau xác thực social -> trả cùng cấu trúc `TokenPair` như đăng nhập mật khẩu (TASK-AUTH-002).
...
Xem §3. Adapter provider theo một interface chung (`AuthCodeURL`, `ExchangeAndVerify`) - Google triển khai trước, Facebook/Zalo stub cùng interface để mở rộng không sửa lõi. `state`/`code_verifier` lưu Redis TTL ngắn, dùng một lần. id_token Google verify qua JWKS của Google. Merge chỉ khi `email_verified`. Mọi đường kết thúc ở `IssueTokenPair` (TASK-AUTH-002). Client secret qua TASK-INFRA-003. Là task SHOULD: nếu cắt slice, không chặn release, nhưng nếu làm thì theo đúng chuẩn ở §1.

---

## §7 - Phụ thuộc

- TASK-AUTH-002 - phát hành `TokenPair` sau xác thực social (đường token thống nhất).
- TASK-AUTH-001 - tạo `app_user` mới cho tài khoản chỉ-social (pwd_hash NULL hợp lệ).
- TASK-INFRA-003 - OAuth client secret/cấu hình provider qua provider secret.
- TASK-WEB-001 (downstream) - UI nút "Đăng nhập với Google/Facebook/Zalo".
...
| Client secret trong env | grep CI | rò rỉ | Qua TASK-INFRA-003 (§1 #9) |
...
- Một đường token thống nhất (cùng TokenPair TASK-AUTH-002) giữ phần còn lại của hệ thống chỉ thấy một loại phiên.
- Là task SHOULD: giảm ma sát đăng ký cho persona VN (Zalo phổ biến) nhưng không chặn release; nếu ship thì ship đúng chuẩn.
- Adapter interface chung cho Google/Facebook/Zalo để thêm provider không sửa lõi.

---

*Hết TASK-AUTH-004. Status: ready_to_implement (mục tiêu audit 10/10).*
