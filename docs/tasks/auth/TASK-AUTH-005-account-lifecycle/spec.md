...
created: 2026-06-27
related_frs: [TASK-AUTH-001, TASK-AUTH-002, TASK-AUTH-003, TASK-COMPLY-003, TASK-NOTIF-001]
depends_on: [TASK-AUTH-001, TASK-COMPLY-003]
blocks: []
source_pages:
...
language: "PostgreSQL 16 + Go 1.22 (auth-svc); phối TASK-COMPLY-003 (DSAR), TASK-NOTIF-001 (gửi reset)"
...
2. Reset mật khẩu **MUST** dùng token một-lần, hết hạn ngắn (ví dụ 30 phút), gửi qua kênh đã verify (email/phone) qua TASK-NOTIF-001 (DEC-AUTH-22). Token lưu dạng hash trong bảng `password_reset`.
...
4. `ConfirmReset(token, newPassword)` **MUST** kiểm token hợp lệ, chưa dùng, chưa hết hạn; đặt mật khẩu mới (băm argon2id qua TASK-AUTH-001); đánh dấu token đã dùng; và **MUST** thu hồi mọi refresh token hiện hữu của user (đổi mật khẩu = đăng xuất mọi phiên).
...
func (s *Service) RequestReset(ctx context.Context, identifier string) error {
    ...
    if ok && u.Status == "active" {
        ...
        s.notif.SendReset(ctx, u, token) // gửi qua kênh verified (TASK-NOTIF-001)
    }
...
func (s *Service) DeleteAccount(ctx context.Context, userID int64) error {
    ...
    if err := s.repo.DeletePlatformAccounts(ctx, userID); err != nil { return err } // TASK-AUTH-003
...
Xem §3. Reset token lưu hash, một-lần, TTL ngắn; gửi qua TASK-NOTIF-001. `RequestReset` luôn trả nil để không lộ tồn tại. `ConfirmReset` và `SetStatus(suspended/deleted)` đều gọi `RevokeAllRefresh` - cùng access TTL ngắn của TASK-AUTH-002 làm khóa/đổi mật khẩu có hiệu lực thực. `DeleteAccount` ẩn danh hóa PII (tombstone) rồi gỡ liên kết, thu hồi token, đặt `deleted`; purge cứng chạy sau cửa sổ ân hạn bằng job riêng. Phối TASK-COMPLY-003 để dữ liệu liên quan ở module khác cũng theo chính sách lưu trữ. Audit log thao tác nhạy cảm không kèm token/PII thô.

---

## §7 - Phụ thuộc

- TASK-AUTH-001 - `Hash` mật khẩu mới (argon2id) khi ConfirmReset; `app_user.status` cột lõi.
- TASK-AUTH-002 - `RevokeAllRefresh` dùng cơ chế thu hồi refresh token; access TTL ngắn bổ trợ.
- TASK-AUTH-003 - `DeleteAccount` gỡ `platform_account`.
- TASK-COMPLY-003 - DSAR; task này là điểm thực thi quyền xóa.
- TASK-NOTIF-001 - gửi token reset qua kênh verified.
...
- Xuất dữ liệu (data portability) trước khi xóa - phối TASK-COMPLY-003 ở slice DSAR đầy đủ.
- Suspend tạm có thời hạn tự hết (ví dụ khóa 24h chống abuse) - gắn TASK-TRUST-004.
...
- Status là cờ DB nhưng access token tự chứng thực - phải thu hồi refresh khi suspend/delete để khóa có hiệu lực thực (cùng access TTL ngắn TASK-AUTH-002).
...
- Phối TASK-COMPLY-003 để xóa lan tới dữ liệu cá nhân ở module khác theo chính sách lưu trữ, không chỉ trong `app_user`.

---

*Hết TASK-AUTH-005. Status: ready_to_implement (mục tiêu audit 10/10).*
