...
Release ngay khi confirm là lỗ hổng farm vì đơn affiliate có thể bị hủy/hoàn sau đó. Giải: §1 #4 hold-then-release qua `available_at = confirmed_at + investigation_window` + hỏi TASK-TRUST-005; AC #5/#6/#7 + `TestRelease_HoldsBeforeWindow` + `TestRelease_FlaggedUserNotReleased`.
...
| #4 hold-then-release + TASK-TRUST-005 | #5,#6,#7 | `release.go::ReleaseDue` + idx + 2 release tests |
