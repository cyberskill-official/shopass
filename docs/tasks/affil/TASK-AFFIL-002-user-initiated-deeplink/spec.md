...
created: 2026-06-28
related_frs: [TASK-AFFIL-001, TASK-AFFIL-003, TASK-AFFIL-004, TASK-INFRA-001, TASK-EXT-003, NFR-AFFIL-001]
depends_on: [TASK-AFFIL-001]
blocks: [TASK-AFFIL-003, TASK-AFFIL-004]
source_pages:
...
source_decisions:
  ...
  - "DEC-AFFIL-07: mỗi lần tạo link sinh một sub_id mới + ghi affiliate_click (TASK-AFFIL-001) - link và sổ ghi click là một hành động nguyên tử"
  ...
  - "DEC-AFFIL-10: deep link build theo network template (TASK-AFFIL-003) với sub_id nhúng làm tham số tracking; thiếu mapping network -> 503, không bịa link"

language: "Go 1.22 (affil-svc); PostgreSQL 16; REST qua API Gateway (TASK-INFRA-001)"
...
2. **MUST** lấy `user_id` từ JWT do API Gateway (TASK-INFRA-001) gắn; handler KHÔNG tự parse token. Request thiếu auth bị gateway chặn trước.
...
4. **MUST** sinh một `sub_id` mới (TASK-AFFIL-001 `subid.go`) cho mỗi lần tạo link, và ghi `affiliate_click` qua `RecordClick` (TASK-AFFIL-001) như một hành động nguyên tử với việc trả link (DEC-AFFIL-07). Nếu `RecordClick` lỗi -> `500`, không trả link (không trả link mà không ghi, và không ghi mà không có chủ ý user).
5. **MUST** build deep link theo template của `network` cho sàn đó (TASK-AFFIL-003), nhúng `sub_id` làm tham số tracking (DEC-AFFIL-10). Thiếu mapping network cho sàn -> `503` `{"error":"affiliate network unavailable"}`, KHÔNG bịa link tự ghép tay.
...
func (h *Handler) HandleCreateLink(w http.ResponseWriter, req *http.Request) {
    ...
    network, tmpl, ok := h.networks.TemplateFor(tp.PlatformID) // TASK-AFFIL-003
    if !ok {
...
func BuildDeepLink(tmpl NetworkTemplate, p Product, subID string) string {
    ...
    q.Set(tmpl.SubIDParam, subID)   // sub_id để postback đối soát (TASK-AFFIL-001)
    u.RawQuery = q.Encode()
    return u.String()
}
```
...
Xem §3. Thứ tự: `deeplink.go` (build URL từ template + nhúng sub_id) -> `disclosure.go` (văn bản minh bạch) -> `link.go` (handler: chặn nếu thiếu cờ user_initiated, tra product, lấy network template, sinh sub_id, RecordClick nguyên tử, trả disclosure + target_url) -> đăng ký route trong `router.go` sau JWT middleware (TASK-INFRA-001) -> tests. Handler dùng `http.ServeMux` Go 1.22. Network template lấy từ TASK-AFFIL-003 (`networks.TemplateFor`). Không tạo HTTP client tới domain sàn - handler chỉ trả JSON, mọi điều hướng/cookie xảy ra phía client khi user mở `deep_link`.

---

## §7 - Phụ thuộc

- **TASK-AFFIL-001** - cung cấp `RecordClick`, `NewSubID`, bảng `affiliate_click`; là điều kiện cứng để ghi click khi tạo link.
- **TASK-AFFIL-003 (downstream + cấu hình)** - cung cấp network template (`TemplateFor`) để build deep link; postback dùng `sub_id` sinh ở đây.
- **TASK-AFFIL-004 (downstream)** - guardrails né Honey kiểm endpoint này không có đường tự động + có disclosure.
- **TASK-INFRA-001 (gateway)** - gắn JWT auth và `user_id` vào context trước handler.
- **TASK-EXT-003** - extension chỉ gửi dữ liệu sạch (productId); nút "Mua qua SănDeal" trong extension/web gọi endpoint này khi user bấm.
...
- Đa network cho cùng sàn (chọn network trả hoa hồng cao nhất tại thời điểm) - thêm logic chọn ở TASK-AFFIL-003; task này dùng template mặc định của sàn.
...
| Thiếu JWT | gateway chặn | - | Auth ở gateway (TASK-INFRA-001) |
| sub_id trùng (hi hữu) | UNIQUE ở TASK-AFFIL-001 | RecordClick lỗi -> 500 | Sinh lại sub_id; entropy đủ để cực hiếm |
...
- Đây là nền cho TASK-AFFIL-003 (postback dùng sub_id), TASK-AFFIL-004 (guardrails kiểm chính endpoint này) và TASK-AFFIL-005 (cashback).

---

*Hết TASK-AFFIL-002. Status: ready_to_implement (mục tiêu audit 10/10).*
