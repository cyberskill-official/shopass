...
created: 2026-06-28
related_frs: [TASK-AFFIL-001, TASK-AFFIL-002, TASK-AFFIL-004, TASK-AFFIL-005, TASK-INFRA-003, TASK-TRUST-005, NFR-AFFIL-001]
depends_on: [TASK-AFFIL-002]
blocks: [TASK-AFFIL-005, TASK-TRUST-005]
source_pages:
...
source_decisions:
  - "DEC-AFFIL-11: cấu hình network lưu ở bảng affiliate_network (code, base_url, target_param, sub_id_param, postback_secret_ref) - secret tham chiếu Vault (TASK-INFRA-003), KHÔNG lưu cleartext"
  ...
  - "DEC-AFFIL-14: postback ánh xạ sub_id -> click_id (TASK-AFFIL-001), ghi conversion 'pending'; status network 'approved' -> ConfirmConversion, 'rejected' -> RejectConversion"
  - "DEC-AFFIL-15: postback idempotent (network retry) + ghi raw payload vào affiliate_postback_log để truy vết tranh chấp"

language: "Go 1.22 (affil-svc); PostgreSQL 16; webhook nhận qua API Gateway (TASK-INFRA-001)"
...
sub_tasks:
  ...
  - "1.0h: network.go - TemplateFor(platform_id) -> NetworkTemplate (dùng bởi TASK-AFFIL-002); seed Involve Asia + Accesstrade"
  - "1.5h: postback_verify.go - xác thực chữ ký HMAC/secret từ Vault (TASK-INFRA-003); reject nếu sai/thiếu"
...
risk_if_skipped: "TASK-AFFIL-003 đóng vòng tiền của affiliate: nó biến một click (TASK-AFFIL-002) thành conversion được network xác nhận, rồi thành cashback (TASK-AFFIL-005). Không có nó thì deep link tạo ra không có template network đúng (TASK-AFFIL-002 trả 503) và conversion không bao giờ được ghi/đối soát - dòng doanh thu affiliate đứng. Nếu ghi conversion từ postback chưa xác thực chữ ký thì bất kỳ ai cũng giả mạo được conversion để rút cashback (mất tiền + gian lận). Nếu lưu postback_secret cleartext thì lộ secret là lộ khả năng giả mạo toàn bộ postback (vi phạm PDPL no-cleartext §5.5). Nếu xác nhận conversion confirmed khi network chưa approve thì trả cashback cho đơn có thể bị đảo - lỗ. Nếu không ghi raw postback thì khi network tranh chấp số liệu, SănDeal không có bằng chứng. Đây là mắt xích đối soát tiền của toàn hệ affiliate."
---

## §1 - Mô tả (BCP-14 normative)

Service AFFIL **MUST** tích hợp affiliate network (Involve Asia, Accesstrade): lưu cấu hình network (template build link + tham chiếu secret), cung cấp template cho deep link (TASK-AFFIL-002), và nhận webhook postback để ghi/đối soát conversion theo last-click, có xác thực chữ ký và idempotent. Hợp đồng:
...
2. `postback_secret_ref` **MUST** là một tham chiếu tới secret trong Vault (TASK-INFRA-003), KHÔNG phải secret cleartext (DEC-AFFIL-11). Cột này chứa key path/handle; secret thật đọc từ Vault lúc verify.
3. **MUST** expose `TemplateFor(platformID) (network string, tmpl NetworkTemplate, ok bool)` (dùng bởi TASK-AFFIL-002): trả `base_url`, `target_param`, `sub_id_param` của network `active` phục vụ sàn đó. Không có network active -> `ok=false` (TASK-AFFIL-002 trả 503).
...
7. Sau khi xác thực, **MUST** ánh xạ `sub_id` -> `click_id` (TASK-AFFIL-001) và ghi conversion qua `RecordConversion`. `sub_id` không khớp click nào -> `404`/`ErrUnknownSubID`, không tạo conversion mồ côi (đồng nhất TASK-AFFIL-001 #8).
...
9. **MUST** idempotent với postback lặp (network retry): cùng `sub_id` postback nhiều lần **MUST NOT** tạo conversion trùng (dựa UNIQUE click_id ở TASK-AFFIL-001); postback `approved` lặp trên conversion đã `confirmed` là no-op.
...
**Vì sao idempotent postback (§1 #9)?** Network gửi lại postback khi không nhận được 200 (mạng chập chờn). Nếu mỗi lần tạo một conversion, sổ cái phình và hoa hồng đếm trùng. Dựa UNIQUE click_id (TASK-AFFIL-001) làm postback idempotent: lần đầu tạo, lần sau cập nhật trạng thái hoặc no-op.
...
func VerifyPostback(ctx context.Context, secrets SecretReader, network string, body []byte, gotSig string) (bool, error) {
    secret, err := secrets.Get(ctx, secretRefFor(network)) // đọc từ Vault (TASK-INFRA-003)
    if err != nil {
        return false, err
    }
...
3. `TemplateFor` cho sàn không có network active -> `ok=false` (TASK-AFFIL-002 sẽ trả 503).
...
Xem §3. Thứ tự: migration `0003_affiliate_network.sql` -> `0004_affiliate_postback_log.sql` -> `network.go` (`TemplateFor` + seed Involve Asia/Accesstrade với `postback_secret_ref` trỏ Vault) -> `postback_verify.go` (HMAC-SHA256, `hmac.Equal` so sánh hằng thời gian, secret từ TASK-INFRA-003) -> `postback.go` (verify -> log raw -> RecordConversion -> map status -> Confirm/Reject, idempotent) -> đăng ký route `POST /v1/affiliate/postback/{network}` trong `router.go` (`http.ServeMux` Go 1.22 với `PathValue`) -> tests. Webhook KHÔNG đi qua JWT user (network không có JWT); thay vào đó xác thực bằng chữ ký HMAC. Secret đọc qua client Vault của TASK-INFRA-003.

---

## §7 - Phụ thuộc

- **TASK-AFFIL-002** - tạo deep link + sinh `sub_id` mà postback dùng để đối soát; `TemplateFor` của task này phục vụ TASK-AFFIL-002.
- **TASK-AFFIL-001** - cung cấp `RecordConversion`/`ConfirmConversion`/`RejectConversion` + bảng `affiliate_click`/`affiliate_conversion`.
- **TASK-INFRA-003 (Vault)** - đọc `postback_secret` để xác thực postback; secret không nằm trong DB.
- **TASK-INFRA-001 (gateway)** - định tuyến webhook; route postback bỏ qua JWT user, dùng chữ ký HMAC.
- **TASK-AFFIL-005 (downstream, P3)** - cashback đọc conversion `confirmed` (chỉ chi sau khi network approve).
- **TASK-TRUST-005 (downstream)** - phát hiện gaming attribution dùng log postback + tỷ lệ confirm/reject.
...
| Thiếu network template | TemplateFor ok=false | TASK-AFFIL-002 trả 503 | Seed network; lộ lỗi cấu hình (§1 #3) |
...
- TASK-AFFIL-003 đóng vòng tiền: click user-initiated (TASK-AFFIL-002) -> conversion (postback) -> confirm khi network approve -> cashback (TASK-AFFIL-005).
...
*Hết TASK-AFFIL-003. Status: ready_to_implement (mục tiêu audit 10/10).*
