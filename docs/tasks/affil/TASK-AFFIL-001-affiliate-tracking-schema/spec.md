...
created: 2026-06-28
related_frs: [TASK-INFRA-002, TASK-AFFIL-002, TASK-AFFIL-003, TASK-AFFIL-004, TASK-TRUST-004, TASK-TRUST-005, NFR-AFFIL-001]
depends_on: [TASK-INFRA-002]
blocks: [TASK-AFFIL-002, TASK-TRUST-004, TASK-TRUST-005]
source_pages:
...
source_decisions:
  ...
  - "DEC-AFFIL-05: conversion bắt đầu ở status 'pending'; chỉ chuyển 'confirmed' khi network xác nhận (postback TASK-AFFIL-003); cho phép delay payout chống gian lận (§5.3)"
...
risk_if_skipped: "affiliate_click và affiliate_conversion là sổ cái của dòng doanh thu affiliate - dòng tiền chính tài trợ free-tier (§4.1). Không có chúng thì deep link user-initiated (TASK-AFFIL-002) không có chỗ ghi nhận, postback network (TASK-AFFIL-003) không có bảng để đối soát, và anti-fraud (TASK-TRUST-004/005) không có dữ liệu để phát hiện gaming attribution. Nếu sub_id nhúng định danh người dùng sàn thật thì rò rỉ PII (vi phạm PDPL). Nếu tiền lưu float thì sai số tích lũy trên đối soát hoa hồng. Nếu conversion vào thẳng 'confirmed' không chờ postback thì SănDeal trả cashback (TASK-AFFIL-005) cho đơn chưa được network xác nhận - mở cửa cho gian lận và lỗ. Đây là nền dữ liệu cho toàn bộ kiếm tiền affiliate compliant hậu-Honey."
---
...
2. **MUST** đặt `sub_id TEXT NOT NULL UNIQUE`: mỗi click có một `sub_id` đối soát duy nhất, do SănDeal sinh (TASK-AFFIL-001 `subid.go`), dùng để network postback ánh xạ ngược conversion về click (DEC-AFFIL-01).
...
4. `affiliate_click` **MUST** chỉ được ghi qua hàm `RecordClick` mà caller (TASK-AFFIL-002) khẳng định là user-initiated; KHÔNG có đường ghi click tự động nền (NFR-AFFIL-001). `network` ghi nguồn (`'involve_asia'`, `'accesstrade'`, ...) để đối soát đúng kênh.
...
Xem §3. Thứ tự: migration `0001_affiliate_click.sql` -> `0002_affiliate_conversion.sql` -> `subid.go` (sinh token ngẫu nhiên qua `crypto/rand`, tiền tố `sd_` cho dễ nhận diện trong postback) -> `types.go` + `repo.go` (`RecordClick`, `RecordConversion` tra last-click, `ConfirmConversion`/`RejectConversion`) -> tests. `RecordClick` được TASK-AFFIL-002 gọi từ handler `POST /v1/affiliate/link` (luồng user bấm). `RecordConversion`/`ConfirmConversion` được TASK-AFFIL-003 gọi từ webhook postback của network. Driver `pgx`.

---

## §7 - Phụ thuộc

- **TASK-INFRA-002** - bảng `app_user`, `platform`, `tracked_product` phải tồn tại trước (mọi FK của `affiliate_click`).
- **TASK-AFFIL-002 (downstream)** - deep link user-initiated gọi `RecordClick` khi user bấm "Mua qua SănDeal".
- **TASK-AFFIL-003 (downstream)** - tích hợp network + webhook postback gọi `RecordConversion`/`ConfirmConversion`.
- **TASK-AFFIL-005 (downstream, P3)** - cashback layering đọc conversion `confirmed` để chia % cho user (hold tới khi confirm).
- **TASK-TRUST-004 / TASK-TRUST-005 (downstream)** - anti-fraud đọc `affiliate_click(user_id, clicked_at)` + conversion để phát hiện velocity/gaming attribution.
- Lib: `pgx`, `crypto/rand`.
...
### Ghi click (nội bộ, do TASK-AFFIL-002 gọi sau khi user bấm)
...
### Ghi conversion từ postback (nội bộ, do TASK-AFFIL-003 gọi)
...
Đã chốt. Hoãn:
- Cookie window per-sàn/per-network (7 ngày vs 30/7 desktop/mobile - cần xác minh §10 tài liệu nguồn) - lưu ở bảng cấu hình network của TASK-AFFIL-003, không đổi schema này.
...
- Lưu raw payload postback để truy vết tranh chấp - thêm bảng phụ `affiliate_postback_log` ở TASK-AFFIL-003.
...
| FK user/product không tồn tại | lỗi pgx | từ chối ghi | Tạo user/track trước (TASK-INFRA-002/TASK-PRICE-001) |
...
- Đây là nền cho TASK-AFFIL-002 (ghi click), TASK-AFFIL-003 (postback), TASK-AFFIL-005 (cashback) và TASK-TRUST-004/005 (anti-fraud).

---

*Hết TASK-AFFIL-001. Status: ready_to_implement (mục tiêu audit 10/10).*
