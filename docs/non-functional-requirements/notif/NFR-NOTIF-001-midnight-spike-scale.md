...
created: 2026-06-28
related_tasks: [TASK-NOTIF-002, TASK-NOTIF-003, TASK-NOTIF-004]
...
1. Lúc đỉnh 00:00 (và mọi mốc giờ), sau khi scheduler (TASK-NOTIF-004) san phẳng, KHÔNG bucket phút nào **MUST** vượt `FCM_RATE_LIMIT_PER_MIN = 600.000` tin/phút/project. Bất biến `notif_scheduler_max_bucket_size <= 600.000` là điều kiện sống còn của module.
...
3. Khi traffic tăng >2 lần trong 30s-2 phút đầu mỗi giờ (đo được ở §3.6), hệ **MUST** hấp thụ đột biến đó bằng jitter `[-90s, +180s]` + spread overflow (TASK-NOTIF-004) mà không drop alert nào và không đẩy bucket nào vượt trần.
4. Dưới tải đỉnh, hệ **MUST** ưu tiên kênh theo cost model push > email > sms (TASK-NOTIF-001 routing): chọn kênh rẻ nhất còn khả dụng cho mỗi alert, để chi phí gửi không bùng nổ khi lượng tin tăng vọt. SMS chỉ dùng khi push và email đều không khả dụng cho alert tới hạn.
5. Fan-out (TASK-NOTIF-003) **MUST** sustain throughput phát đủ để rút hết lô đỉnh đã san phẳng đúng theo `scheduled_at`: ở 1.500.000 alert/đỉnh rải qua ~3 phút (600k+600k+300k), pipeline phát kịp từng bucket trong phút của nó, không tồn dư cuốn sang phút sau.
6. FCM dispatcher (TASK-NOTIF-002) **MUST** xử lý 429 có kỷ luật nếu trần bị chạm sát: backoff theo `Retry-After`, đẩy phần dư vào DLQ thay vì retry mù, sao cho một sự cố sát trần không thành bão lỗi dây chuyền quanh nửa đêm.
...
- Gauge `notif_scheduler_max_bucket_size` - bucket phút lớn nhất sau spread; **MUST** `<= 600.000` mọi thời điểm (do TASK-NOTIF-004 phát).
...
- Load test (T): seed một lô midnight-burst >2x baseline (vd 1.500.000 alert cùng `event_time = 00:00:00`), chạy qua `ScheduleAlerts` (TASK-NOTIF-004) rồi mô phỏng phát; assert KHÔNG bucket phút nào > 600.000 và `fcm_429_total = 0` suốt cửa sổ.
...
- Throughput test (T): fan-out (TASK-NOTIF-003) rút lô đỉnh đã san phẳng; đo `notification_dispatch_latency_ms` p95 và xác nhận mỗi bucket được phát trong phút của nó, không tồn dư cuốn sang phút kế.
...
- 429 handling test (T): ép FCM trả 429 sát trần; xác nhận TASK-NOTIF-002 backoff theo `Retry-After` và đẩy DLQ, không retry mù làm vỡ phút kế.

## §5 - Xử lý khi vi phạm

- Một bucket phút > 600.000 HOẶC quan sát thấy 429 ở đỉnh -> sev-2; kiểm scheduler spread có lặp tới hội tụ (TASK-NOTIF-004 #4,#5) và quota headroom còn đủ; đây là van chính, vỡ là alert giờ vàng hỏng.
- `notification_dispatch_latency_ms` ở đỉnh quá cao (fan-out không rút kịp bucket trong phút của nó) -> sev-3; scale worker fan-out (TASK-NOTIF-003) + kiểm DLQ/backpressure.
- Cost spike vì channel mix sai (`notification_routing_total{chosen_channel="sms"}` tăng vọt khi push/email đáng lẽ khả dụng) -> sev-3; kiểm routing push > email > sms (TASK-NOTIF-001) và token/health của push.
