...
created: 2026-06-28
related_frs: [FR-COMPLY-001, FR-COMPLY-002, FR-COMPLY-003, FR-COMPLY-004, FR-COMPLY-005, FR-COMPLY-006, FR-COMPLY-007, FR-COMPLY-008]
...
1. Mọi bề mặt xử lý dữ liệu cá nhân **MUST** có cơ sở pháp lý là một bản ghi consent hợp lệ (FR-COMPLY-001): coverage consent = 100%; xử lý không có consent tương ứng là vi phạm cứng (cổng tiến phase §7 BACKLOG).
...
3. Mỗi hoạt động xử lý **MUST** có DPIA nộp trong vòng 60 ngày kể từ khi bắt đầu xử lý và được cập nhật tối thiểu mỗi 6 tháng (FR-COMPLY-002); số DPIA quá hạn nộp hoặc quá hạn rà soát phải bằng 0.
4. Hoạt động chuyển dữ liệu xuyên biên giới **MUST** có TIA gắn kèm (nước nhận + cơ chế bảo vệ) trước khi bật.
5. Vi phạm dữ liệu **MUST** được thông báo cơ quan trong vòng 72 giờ kể từ khi tổ chức nhận biết (FR-COMPLY-004); vi phạm nghiêm trọng còn phải thông báo chủ thể dữ liệu. Số sự cố quá hạn 72 giờ chưa thông báo phải bằng 0.
6. Hệ thống **MUST** giữ no-cleartext: mật khẩu chỉ tồn tại dạng argon2id, secrets chỉ trong Vault, và KHÔNG có token phiên sàn nào trên server (FR-COMPLY-005); số phát hiện cleartext/token-on-server qua audit gate phải bằng 0.
7. Quyền chủ thể dữ liệu (truy cập/sửa/xóa/di chuyển) **MUST** thực thi được qua quy trình DSAR (FR-COMPLY-003) trong thời hạn phản hồi đã định; rò dữ liệu chéo user = 0.
8. Khung tuân thủ **MUST** điều chỉnh theo nước khi mở SEA (FR-COMPLY-006/007) và theo luật TMĐT VN (FR-COMPLY-008); nước chưa cấu hình giữ deny-by-default.
...
- Gauge `dpia_overdue_total`, `dpia_review_due_soon_total` (FR-COMPLY-002): mục tiêu overdue = 0.
- Gauge `breach_overdue_total` (FR-COMPLY-004): mục tiêu = 0; thời gian từ `acknowledged_at` tới `notified_authority_at` < 72 giờ.
- Audit gate `no_cleartext_gate.sh` (FR-COMPLY-005): số vi phạm theo rule = 0 ở mọi PR.
- Property test rò cross-user trong DSAR (FR-COMPLY-003): mục tiêu 0 rò rỉ.
- Gauge `gating_denied_total{country,gate}` (FR-COMPLY-006): theo dõi nước/tính năng bị chặn.
...
- DPIA deadline test (T): seed hoạt động bắt đầu >60 ngày chưa nộp -> `Status=overdue`; >6 tháng chưa review -> `review_overdue` (FR-COMPLY-002).
- Breach clock test (T): sự cố `acknowledged_at` >72h chưa thông báo -> `breach_overdue`; high/critical không `closed` được nếu chưa `notified_subjects` (FR-COMPLY-004).
- No-cleartext gate (T): CI chạy `Scan` trên toàn repo; fixture token/cleartext bị bắt, mã sạch pass (FR-COMPLY-005).
- DSAR cross-user (T): property test sinh nhiều user, xuất từng user, khẳng định bundle chỉ chứa dữ liệu đúng chủ (FR-COMPLY-003).
...
- Audit gate phát hiện cleartext/token-on-server -> sev-1; chặn merge; gỡ ngay; nếu đã lên production thì coi là sự cố bảo mật, kích hoạt FR-COMPLY-004.
- Rò dữ liệu chéo user trong DSAR -> sev-1; rút tính năng export, vá truy vấn buộc `user_id`, coi là breach nếu đã lộ.
- Vi phạm luật nước SEA do adapter sai/thiếu -> sev-2; tắt tính năng cho nước đó (deny-by-default), sửa adapter FR-COMPLY-007.
