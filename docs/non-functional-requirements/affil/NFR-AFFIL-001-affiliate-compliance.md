...
created: 2026-06-28
related_frs: [FR-AFFIL-001, FR-AFFIL-002, FR-AFFIL-003, FR-AFFIL-004, FR-EXT-003, FR-TRUST-001]
...
1. Mô hình affiliate hợp lệ DUY NHẤT của SănDeal **MUST** là: người dùng chủ động bấm "Mua qua SănDeal" -> hệ tạo deep link affiliate hiển thị rõ ràng (URL đích thấy được) kèm disclosure (SănDeal có thể nhận hoa hồng). Mọi affiliate link/click **MUST** bắt nguồn từ một hành động người dùng tại thời điểm đó (FR-AFFIL-002).
...
5. Mọi response tạo affiliate link **MUST** kèm disclosure không rỗng + URL đích hiển thị (FR-AFFIL-002 #6/#10); che giấu đích hoặc thiếu disclosure là vi phạm.
6. Hệ **MUST** có guardrail tự động (CI gate) khẳng định không có hành vi bị cấm ở §1 #2-#4 (FR-AFFIL-004): vi phạm làm đỏ build, không merge được.
7. Cookie affiliate **MUST** tuân điều kiện ToS Shopee: chỉ hợp lệ khi link hiển thị và user click "voluntarily and consciously" - đúng những gì FR-AFFIL-002 thực thi (cờ user_initiated + hiển thị target_url).
8. Hệ **MUST** tuân Chrome Web Store policy (cập nhật 3/2025, thực thi 10/06/2025): cấm chèn affiliate link/code/cookie khi không mang lại lợi ích trực tiếp (giảm giá/cashback) + bắt buộc disclosure + bắt buộc hành động người dùng. Extension open-source (FR-TRUST-001) là bằng chứng kiểm chứng được.
...
- Counter `affiliate_link_created_total` vs `affiliate_link_rejected_total{reason="not_user_initiated"}` (FR-AFFIL-002): tỷ lệ link có cờ user-initiated phải là 100% (mọi link đã tạo đều qua cờ).
- Khẳng định "mọi response link kèm disclosure": assertion `AssertSingleAffiliatePath` (FR-AFFIL-004) + đếm response thiếu disclosure = 0.
- Guardrail CI: số kiểm tra hành vi cấm chạy + số vi phạm chặn (FR-AFFIL-004 #12) - vi phạm bị chặn trước merge.
- Đếm `affiliate_click` không gắn user_id hợp lệ = 0 (mọi click có chủ thể user-initiated).
- Kiểm manifest extension (FR-AFFIL-004 #4): 0 quyền `cookies` cho host sàn, 0 `webRequestBlocking` sửa redirect.
- Đối chiếu checklist Chrome Web Store policy (FR-AFFIL-004 #8): mỗi mục policy có một test xanh.

## §4 - Verification

- Compliance gate test (T): bộ guardrail FR-AFFIL-004 chạy như CI gate - bơm từng hành vi cấm (cookie-stuffing, pop-under, auto-redirect, link không user gesture, link thiếu disclosure, cửa affiliate thứ hai) -> mỗi cái làm đỏ build.
- User-initiated test (T): `POST /v1/affiliate/link` thiếu cờ `user_initiated` -> 400 và 0 `affiliate_click` ghi (FR-AFFIL-002).
...
- ToS-mapping review: ánh xạ từng điều cấm của Shopee ToS (robots/automated query, scraping, cookie dropping, pop-under, auto-redirect, forced install) tới một guardrail/FR thực thi.
- Open-source proof (FR-TRUST-001): mã extension công khai + reproducible build cho phép bên thứ ba kiểm chứng không cookie-stuffing.
...
- Network cảnh báo/đình chỉ do nghi hành vi cấm -> sev-1; làm việc với network, dùng open-source + log postback (FR-AFFIL-003) làm bằng chứng tuân thủ.
