...
created: 2026-06-27
related_tasks: [TASK-SCRAPE-001, TASK-SCRAPE-003, TASK-SCRAPE-004, TASK-SCRAPE-005, TASK-PRICE-002]
...
3. Hệ **MUST** dùng các đòn bẩy chi phí đã thiết kế để đạt ngưỡng: scan-frequency tiering (TASK-SCRAPE-001, không quét mọi SKU mỗi phút), delta-only writes (TASK-PRICE-002, không ghi khi giá không đổi), proxy tiering + cost-guard (TASK-SCRAPE-004, dùng budget/mid khi đủ, enterprise chỉ khi cần), pacing tránh request thừa (TASK-SCRAPE-005).
4. Chi phí proxy theo GB **MUST** được đo và quy về số nguyên (micro-USD) per provider/ngày (`proxy_usage` của TASK-SCRAPE-004) để báo cáo unit economics đáng tin.
5. Cost-guard **MUST** đặt trần ngân sách/ngày và hạ tier / dừng tier cold khi gần trần (TASK-SCRAPE-004 #7) - không để một sự cố (retry-loop, đợt sàn siết) đẩy chi phí vượt §4.1.
6. Hệ **SHOULD** ưu tiên đường rẻ trước đường đắt: gọi internal JSON (TASK-SCRAPE-002, rẻ) trước khi dùng headless farm (TASK-SCRAPE-003, đắt hơn nhiều lần); farm chỉ khi buộc.
...
- Counter `proxy_cost_usd_total{provider}` + `proxy_gb_used_total{provider, tier}` (TASK-SCRAPE-004) -> chi phí proxy thực.
...
- Counter `snapshot_skipped_total` vs `snapshot_written_total` (TASK-PRICE-002/TASK-SCRAPE-005) - hiệu quả delta-only phía storage/ghi.
...
- Tiering test (T): xác nhận phần lớn GB đi qua budget/mid; enterprise chỉ cho TikTok/Lazada (TASK-SCRAPE-004) - không enterprise tràn lan.
- Cost-guard test (T): vượt 80% ngân sách -> hạ tier; chạm trần -> dừng cold (TASK-SCRAPE-004 #7) - chi phí không vượt trần dù có sự cố.
...
- `sku_shared_ratio` thấp bất thường (quét trùng per-user) -> sev-3; kiểm orchestrator có dedup theo SKU (một SKU một job, TASK-SCRAPE-001 PK `product_id`).
...
- Farm (đắt) gọi quá thường (fallback rate cao) -> sev-3; kiểm vì sao JSON path hụt nhiều (sàn siết? adapter drift TASK-SCRAPE-006?), vì farm đắt hơn JSON nhiều lần.
