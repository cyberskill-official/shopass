...
created: 2026-06-27
related_tasks: [TASK-SCRAPE-001, TASK-SCRAPE-002, TASK-SCRAPE-003, TASK-SCRAPE-004, TASK-SCRAPE-005, TASK-SCRAPE-006, TASK-SCRAPE-007, TASK-SCRAPE-008]
...
2. Với scraping read-only, hệ **MUST** dùng đủ lớp chống ban tương ứng mức rủi ro: residential proxy bắt buộc (TASK-SCRAPE-004), anti-fingerprint + TLS match (TASK-SCRAPE-003), pacing/jitter (TASK-SCRAPE-005), concurrency cap per-platform (TASK-SCRAPE-001). TikTok (High) và Lazada/Akamai (Medium-High) **MUST** dùng proxy tier enterprise.
3. Tỷ lệ request bị ban/challenge per-sàn **MUST** giữ dưới ngưỡng vận hành cấu hình; vượt ngưỡng kéo dài **MUST** kích hoạt hạ tải (TASK-SCRAPE-006) thay vì tiếp tục đốt proxy vào request bị chặn.
...
6. Khi một sàn đổi DOM/API (rủi ro existential §5.2), hệ **MUST** phát hiện qua giám sát drift (TASK-SCRAPE-006) và phục hồi (người sửa adapter + tự bỏ throttle) trong thời gian giới hạn, không để dữ liệu sàn đó chết âm thầm.
...
- Counter `proxy_ip_banned_total{provider, platform}` (TASK-SCRAPE-004) - tốc độ đốt IP.
- Gauge `adapter_health_state{platform, version}` (TASK-SCRAPE-006) - healthy/degraded/broken.
...
- Tiering test (T): xác nhận TikTok/Lazada được cấp proxy enterprise (`SelectTier` đúng), Shopee-JSON dùng budget/mid (TASK-SCRAPE-004).
- DOM-drift recovery test (T): mô phỏng sàn đổi selector -> monitor (TASK-SCRAPE-006) chuyển broken + alert + throttle; sau khi "sửa adapter", tự về healthy và bỏ throttle; đo MTTR dưới ngưỡng.
...
- `scrape_ban_rate{platform}` vượt ngưỡng kéo dài -> sev-2; kiểm proxy tier (đúng enterprise cho TikTok/Lazada?), pacing/jitter, concurrency cap; hạ tải tạm qua TASK-SCRAPE-006.
